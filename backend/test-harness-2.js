 ...
 *   node test-harness.js --delay 3000       # Custom delay between requests (ms)
 *   node test-harness.js --no-delay         # No delay (use with raised rate limits)
 * 
...
const DEFAULT_DELAY_MS = 5200; // ~11.5 requests/min, stays under 12/min rate limit
...
class TestHarness {
  ...
  async run() {
    ...
    console.log(`Delay: ${this.delay > 0 ? `${this.delay}ms between requests (rate limit safe)` : 'NONE (make sure rate limits are raised!)'}`);
    if (this.toolFilter) console.log(`Filter: ${this.toolFilter}`);

    // Estimate time
    const endpointCount = this.toolFilter
      ? Object.keys(ENDPOINTS).filter(n => n.includes(this.toolFilter)).length
      : Object.keys(ENDPOINTS).length;
    const avgTestsPerEndpoint = 3.5;
    const estimatedRequests = Math.round(endpointCount * avgTestsPerEndpoint);
    if (this.delay > 0) {
      const etaMin = Math.ceil((estimatedRequests * this.delay) / 60000);
      console.log(`ETA: ~${etaMin} minutes (${estimatedRequests} requests × ${this.delay}ms)`);
    }
    console.log('');
