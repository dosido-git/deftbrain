...
router.post('/awkward-silence-filler', async (req, res) => {
  try {
    ...
    // Reject completely empty requests (no action and no context)
    if (!action && !req.body.context && !req.body.scenario && !req.body.relationship) {
      return res.status(400).json({ error: 'Please provide a situation context or select a mode' });
    }
    ...
    switch (action) {
      ...
      case 'panic': {
        ...
        const message = await anthropic.messages.create({
          ...
          messages: [{
            role: 'user',
            content: `I'm in an awkward silence RIGHT NOW.
