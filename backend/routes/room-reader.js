...
Generate 6-8 conversation starters with a mix of energies. Generate 2-4 people in the people_map. Generate 3-4 exit strategies and 2-3 worst case saves.`, userLanguage);
    ...
    console.log(`[RoomReaderPreGame] ${eventType || 'custom'} | ${parsed.conversation_starters?.length || 0} starters`);
    res.json(parsed);
  } catch (error) {
...
Return ONLY valid JSON:
{
  ...
  "body_tip": "One body language move for this exact scenario."
    ...
    console.log(`[RoomReaderQuick] ${scenario} + ${relationship || '?'}`);
    res.json(parsed);
  } catch (error) {
...
Return ONLY valid JSON:
{
  ...
  "reframe": "A warm, grounding perspective. The thing a wise friend would say."
    ...
    console.log(`[RoomReaderDecode] ${parsed.most_likely?.confidence || '?'}`);
    res.json(parsed);
  } catch (error) {
...
Return ONLY valid JSON:
{
  ...
  "next_challenge": {
    ...
    console.log(`[RoomReaderDebrief] ${parsed.wins?.length || 0} wins, ${parsed.awkward_reframes?.length || 0} reframes`);
    res.json(parsed);
  } catch (error) {
...
Generate 3 message options with different styles/risk levels.`, userLanguage);
    ...
    console.log(`[RoomReaderFollowUp] ${parsed.messages?.length || 0} options`);
    res.json(parsed);
  } catch (error) {
...
Generate 4-5 openers and 3-4 working topics.`, userLanguage);
    ...
    console.log(`[RoomReaderPerson] ${personName || relationship || '?'} | ${parsed.openers?.length || 0} openers`);
    res.json(parsed);
  } catch (error) {
...
Generate 3-4 entry techniques, 3-4 contribution methods, 2-3 traps, and 2-3 power moves.`, userLanguage);
    ...
    console.log(`[RoomReaderGroup] ${situation?.substring(0, 30) || challenge?.substring(0, 30) || '?'}`);
    res.json(parsed);
  } catch (error) {
...
Generate 3 recovery options with different strategies.`, userLanguage);
    ...
    console.log(`[RoomReaderRecover] damage: ${parsed.damage_check?.how_bad_really || '?'}/10`);
    res.json(parsed);
  } catch (error) {
...
Generate 4-5 'do this' items and 3-4 'avoid this' items.`, userLanguage);
    ...
    console.log(`[RoomReaderCulture] ${culture} + ${situation?.substring(0, 30) || '?'}`);
    res.json(parsed);
  } catch (error) {
...
Generate 3-4 fresh openers.`, userLanguage);
    ...
    console.log(`[RoomReaderPersonRefresh] ${personName || '?'} | ${notes.length} interactions`);
    res.json(parsed);
  } catch (error) {
...
Return ONLY valid JSON:
{
  ...
  "reframe": "The warm truth: energy mismatches feel bigger from inside than they look from outside."
    ...
    console.log(`[RoomReaderEnergy] ${myEnergy} vs ${roomEnergy}`);
    res.json(parsed);
  } catch (error) {
...
Generate a 5-level ladder from Surface to Genuine Connection.`, userLanguage);
    ...
    console.log(`[RoomReaderLadder] ${relationship || '?'} | ${parsed.ladder?.length || 0} levels`);
    res.json(parsed);
  } catch (error) {
...
Return ONLY valid JSON:
{
  ...
  "compassion_note": "The thing they need to hear. Warm, honest, human. 'You're being too hard on yourself' or 'That was a tough room' or 'Actually, you handled it better than you think.'"
    ...
    console.log(`[RoomReaderAutopsy] turning_point: ${parsed.turning_point?.was_it_you || '?'}`);
    res.json(parsed);
  } catch (error) {
