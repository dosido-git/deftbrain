...
3c-iv-b. The catalog holds two kinds of entry, and they are not equally trustworthy. An entry with FOR / PROBLEMS / DOES lines has been checked against what the tool actually does. An entry with only a blurb has not — it is a sales line, written to sound like the answer to as many problems as possible, and it is broader than the tool behind it every time.
    This creates an asymmetry you must correct for. A checked entry can be ruled out by its own NOT FOR line; a blurb-only entry can never be ruled out, because it never says what it cannot do. Left alone, that means the least verified tools survive every comparison. So: when a checked entry lists this person's problem — in its PROBLEMS lines, or squarely inside its FOR line — it wins, and a blurb-only tool does not displace it by sounding broad enough to cover it too. Take a blurb-only tool over a checked one only when no checked entry addresses the problem at all.
