...
router.outputGuard = {
  prohibit: [
    ...
    'overall_verdict_summary_or_swiss_cheese_note_claims_more_than_the_findings_actual_verdicts_support',
    'unexplained_mechanism_or_gap_upgraded_into_a_stated_rule_conflict_without_naming_both_established_rules',
    'hardest_question_asserts_the_storys_rules_genuinely_conflict_before_any_finding_has_been_adjudicated_a_real_hole',
  ],
  require: [
...
DO NOT UPGRADE AN UNEXPLAINED MECHANISM INTO A RULE CONFLICT
A story can leave its mechanics unexplained without contradicting itself. For any candidate along these lines, ask: "What two established rules cannot both be true?" If you cannot name both established rules specifically, this is not a genuine internal contradiction — classify it as UNEXPLAINED GAP, UNRESOLVED MYSTERY, or NOT ACTUALLY A HOLE instead. Do not write "the rules genuinely rub against each other," "the film's own rules conflict," or similar phrasing unless an actual contradiction between two specific, nameable established rules has been established and survives adjudication.
...
OVERALL SUMMARY MUST MATCH THE FINDING VERDICTS
overall_verdict.summary, swiss_cheese_note, strongest_case, and hardest_question must all tell the same story as the individual finding verdicts — never a stronger one. Reserve language like "genuine logic problem," "real contradiction," or "confirmed hole" for findings whose own verdict is YES — REAL HOLE. If a finding's verdict is MAYBE — THE STORY LEAVES A GAP, describe it in the summary as an unresolved question or gap ("two unresolved logic questions survive scrutiny"), never as a confirmed problem ("two genuine logic problems survive full scrutiny"). Before returning, verify that overall_verdict.summary, swiss_cheese_note, each finding's own verdict, and strongest_case/hardest_question all agree on how many findings are actually confirmed holes versus open questions versus explained/not-a-hole.
...
HARDEST QUESTION MUST STAY EPISTEMICALLY NEUTRAL
A useful unresolved question may be highlighted (hardest_question, show: true) even when no finding survives as a real hole — that is exactly the case it exists for. But describe why the question remains unanswered without first asserting that the story has contradicted itself. Prefer neutral framing: "the film leaves the relationship between these two situations unclear," or "it isn't clear whether X and Y can both be true." Avoid asserting "this is where the film's own rules genuinely conflict" or similar contradiction-language in hardest_question.why unless that specific conflict has actually survived adjudication as a finding with verdict YES — REAL HOLE (in which case it belongs in strongest_case, not hardest_question).
