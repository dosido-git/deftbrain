...
// ═══════════════════════════════════════════════════
router.post('/paper-digest', async (req, res) => {
  try {
...
Return ONLY valid JSON:
{
  ...
  "field_context": "Where does this fit in the bigger picture? Is this confirming what scientists already thought, or is it surprising? Is there an ongoing debate?"
    ...
    const parsed = await callClaudeWithRetry(prompt, { label: 'PaperDigest', max_tokens: 3000,
    ...
    console.log(`[PaperDigest] ${parsed.what_they_did?.study_type?.substring(0, 40) || '?'} | ${parsed.jargon_decoded?.length || 0} terms`);
    res.json(parsed);
  } catch (error) {
    console.error('[PaperDigest]', error);
...
// ═══════════════════════════════════════════════════
router.post('/paper-digest-media', async (req, res) => {
  try {
...
Return ONLY valid JSON:
{
  ...
  "should_you_worry": "Direct answer: based on the ACTUAL paper, should you change your behavior? Usually the answer is 'not yet' or 'this is one study.'"
    ...
    const parsed = await callClaudeWithRetry(prompt, { label: 'PaperDigestMedia', max_tokens: 2000,
    ...
    console.log(`[PaperDigestMedia] ${parsed.accuracy_rating?.score || '?'}`);
    res.json(parsed);
  } catch (error) {
    console.error('[PaperDigestMedia]', error);
...
// ═══════════════════════════════════════════════════
router.post('/paper-digest-jargon', async (req, res) => {
  try {
...
Return ONLY valid JSON:
{
  "terms": [
    ...
    const parsed = await callClaudeWithRetry(prompt, { label: 'PaperDigestJargon', max_tokens: 1500,
    ...
    console.log(`[PaperDigestJargon] ${termList.length} terms`);
    res.json(parsed);
  } catch (error) {
    console.error('[PaperDigestJargon]', error);
...
// ═══════════════════════════════════════════════════
router.post('/paper-digest-compare', async (req, res) => {
  try {
...
Return ONLY valid JSON:
{
  ...
  "what_we_still_dont_know": "What questions remain unanswered even with both papers?"
    ...
    const parsed = await callClaudeWithRetry(prompt, { label: 'PaperDigestCompare', max_tokens: 2500,
    ...
    console.error('[PaperDigestCompare]', error);
...
// ═══════════════════════════════════════════════════
router.post('/paper-digest-relevance', async (req, res) => {
  try {
...
Return ONLY valid JSON:
{
  ...
  "the_bottom_line": "One warm, honest, direct sentence. The thing a smart, caring friend would say."
    ...
    const parsed = await callClaudeWithRetry(prompt, { label: 'PaperDigestRelevance', max_tokens: 1500,
    ...
    console.log(`[PaperDigestRelevance] ${parsed.applies_to_you?.verdict || '?'}`);
    res.json(parsed);
  } catch (error) {
    console.error('[PaperDigestRelevance]', error);
