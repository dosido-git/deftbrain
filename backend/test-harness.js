...
const ENDPOINTS = {
  ...
  'awkward-silence-filler': {
    valid: { action: 'panic', context: 'networking event', relationship: 'stranger' },
    missing: [],
    schema: [],
    skipAI: true,
  },
  ...
  'conflict-text-coach': {
    valid: { receivedMessage: 'I dont think its fair that you got the promotion instead of me and everyone agrees', relationship: 'coworker', emotionalState: ['frustrated', 'defensive'], goals: ['de_escalate', 'maintain_relationship'] },
    missing: [{ relationship: 'coworker' }],
    schema: [],
    skipAI: true,
  },
  ...
  'email-urgency-triager': {
    valid: { emailContent: 'Subject: Partnership Opportunity\nHi, loved your pitch. Free for a call this week?', userRole: 'Employee', userTimezone: 'EST' },
    missing: [{}],
    schema: [],
    skipAI: true,
  },
  ...
  'meeting-hijack-preventer': {
    valid: { meetingGoal: 'Weekly team standup - review blockers and plan the week', duration: 30, participantCount: 8, meetingType: 'standup', decisionFramework: 'Leader Decides', challenges: { offTopic: true, schedule: true } },
    missing: [{}],
    schema: [],
    skipAI: true,
  },
  ...
  'routine-rupture-manager': {
    valid: { disruption: 'travel', customDisruption: 'Three-day business trip starting tomorrow, different timezone', energy: 5, normalRoutine: 'Morning workout, work 9-5, evening reading' },
    missing: [{}],
    schema: [],
    skipAI: true,
  },
  ...
  // ── Food Tools ──
  'fridge-alchemy': {
    valid: { action: 'generate', ingredients: ['chicken thighs', 'coconut milk', 'sweet potatoes', 'garlic', 'rice'], staples: ['salt', 'pepper', 'olive oil'], constraints: 'dairy-free' },
    missing: [{ action: 'generate', ingredients: ['one'] }],  // too few ingredients
    schema: [],
    skipAI: true,
  },
  'leftover-roulette': {
  ...
  'recipe-chaos-solver': {
    valid: { problemDescription: 'My bread dough wont rise no matter what I do', problemCategory: 'baking', cookingSkill: 'intermediate' },
    missing: [{}],
    schema: [],
    skipAI: true,
  },
  ...
  'micro-adventure-mapper': {
    valid: { action: 'generate', location: 'Portland, OR', timeAvailable: 3, budget: 50, interests: ['food', 'nature'] },
    missing: [{ action: 'generate', timeAvailable: 3 }],
    schema: [],
    skipAI: true,
  },
  ...
  'wardrobe-chaos-resolver': {
    valid: { wardrobeInventory: { tops: [{ name: 'Blue button-down', color: 'blue', comfortLevel: 7 }, { name: 'White t-shirt', color: 'white', comfortLevel: 9 }, { name: 'Grey sweater', color: 'grey', comfortLevel: 8 }], bottoms: [{ name: 'Dark jeans', color: 'dark blue', comfortLevel: 7 }, { name: 'Khaki chinos', color: 'khaki', comfortLevel: 6 }], shoes: [{ name: 'Sneakers', color: 'white', comfortLevel: 9 }] }, weather: 'cold and rainy', activities: ['work', 'casual'], mood: 'put-together', comfortPriority: 7, sensoryNeeds: {} },
    missing: [{}],
    schema: [],
    skipAI: true,
  },
  ...
  'safe-walk': {
    valid: { action: 'assess', route: 'Downtown Boston to Back Bay', timeOfDay: 'night', areaDescription: 'Urban area with mix of well-lit and darker streets', walkDuration: '20 minutes', concerns: ['poor lighting', 'isolated areas'] },
    missing: [],
    schema: [],
    skipAI: true,
  },
  'roommate-court': {
    valid: { action: 'assign', roommates: ['Alex', 'Jordan', 'Sam'], chores: ['dishes', 'vacuuming', 'bathroom', 'kitchen', 'trash'] },
    missing: [],
    schema: [],
    skipAI: true,
  },
  ...
  'fake-review-detective': {
    valid: { action: 'score', reviews: [{ index: 0, rawText: 'Best product ever! Changed my life!', starRating: 5, isVerified: false, wordCount: 6, exclamationCount: 2, capsRatio: 0, hasSpecificDetails: false, hasGenericPraise: true, mentionsCompetitor: false }, { index: 1, rawText: 'Decent quality for the price, battery lasts about 4 hours with heavy use', starRating: 3, isVerified: true, wordCount: 12, exclamationCount: 0, capsRatio: 0, hasSpecificDetails: true, hasGenericPraise: false, mentionsCompetitor: false }], stats: { totalReviews: 2, averageRating: 4.0, starDeviation: 1.4, starDistribution: { 5: 1, 4: 0, 3: 1, 2: 0, 1: 0 }, verifiedCount: 1, verifiedPercent: 50, avgWordCount: 9, shortReviewCount: 1, highCapsCount: 0, highExclamationCount: 1, genericPraiseCount: 1, specificDetailCount: 1 }, category: 'electronics' },
    missing: [],
    schema: [],
    skipAI: true,
  },
  ...
  'sensory-minefield-mapper': {
    valid: { location: 'Target store', visitDateTime: '2025-03-01T14:00', placeType: 'retail', sensitivities: ['noise', 'fluorescent lights', 'crowds'] },
    missing: [{}],
    schema: [],
    skipAI: true,
  },
  ...
  'anti-gift-panic': {
    valid: { age: 30, interests: 'coffee, hiking, true crime podcasts, cats', relationship: 'friend', budget: 50, occasion: 'birthday' },
    missing: [{ age: 30 }],  // missing interests
    schema: [],
    skipAI: true,
  },
  'name-anxiety-destroyer': {
    valid: { nameToLearn: 'Saoirse', context: 'Just forgot my new coworkers name after being introduced 3 times' },
    missing: [{}],
    schema: [],
    skipAI: true,
  },
  ...
  'laundro-mat': {
    valid: { action: 'advise', loadDescription: 'Mix of cotton t-shirts, one wool sweater, and jeans', machineType: 'standard' },
    missing: [],
    schema: [],
    skipAI: true,
  },
  'believable-excuse-generator': {
    valid: { eventType: 'party', relationship: 'friend', noticeTime: 'last-minute' },
    missing: [
      { relationship: 'friend' },  // missing eventType
    ],
    schema: [],
    skipAI: true,
  },
...
class TestHarness {
  constructor(options = {}) {
    ...
    this.delayMs = options.delay || 5200; // 5200ms = ~11.5 req/min, under 12/min limit
    this.results = [];
  ...
  async run() {
    ...
    if (!this.noDelay) {
      ...
      const totalRequests = endpoints.reduce((sum, [, cfg]) => {
        let tests = 2 + (cfg.missing?.length || 0); // empty + extra + missing
        if (!this.quick || !cfg.skipAI) tests += 4; // valid, response time, json, schema
        return sum + tests;
      }, 0);
      const eta = Math.ceil((totalRequests * this.delayMs) / 60000);
      console.log(`Delay: ${this.delayMs}ms between requests (rate limit safe)`);
      console.log(`ETA: ~${eta} minutes (${totalRequests} requests × ${this.delayMs}ms)`);
    } else {
      console.log('Delay: NONE (--no-delay mode, requires raised rate limits)');
    }
...
async function main() {
  ...
  if (delayIdx !== -1 && args[delayIdx + 1]) {
    options.delay = parseInt(args[delayIdx + 1]);
  }
