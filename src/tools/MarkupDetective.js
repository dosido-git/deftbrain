...
// Someone questioning a hospital bill and someone wondering about cinema
...
const MarkupDetective = ({ tool }) => {
  ...
  const loadExample = () => {
    const ex = pickExample('MarkupDetective', EXAMPLES);
    setProduct(ex.text);
  };
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    setResults(null);
    try {
      const data = await callToolEndpoint('markup-detective', { product: product.trim(), motive: motive || null, userLocale, userCurrency, userRegion });
      setResults(data);
      ...
      <div className={`${c.card} border ${c.border} rounded-xl shadow-sm`}>
        ...
        <div className="px-5 pb-5 pt-4 space-y-4">
          ...
          <button title={t('cmd_enter')}
            ...
            {loading
              ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '🏷️'}</span> {t('mkd_investigating')}</>
              : <><span className="me-1 text-xl">{tool?.icon ?? '🏷️'}</span> {t('mkd_investigate')}</>}
          {!loading && (
              ...
              <ul className="space-y-1.5">
                {results?.psychological_tactics?.map((tac, i) => (
                  <li key={i} className="text-sm flex items-start gap-2">
                    ...
                    {typeof tac === 'string' ? tac : (
                      <span>
                        <span className="font-bold">{tac.name}</span>
                        {tac.how_it_works && <span className="block mt-0.5 opacity-90">{tac.how_it_works}</span>}
                      </span>
                    )}
                  </li>
                ))}
