...
const DoctorVisitTranslator = ({ tool }) => {
  ...
  const readPdfAsBase64 = (file) => new Promise((resolve, reject) => {
    ...
    if (file.size > 10 * 1024 * 1024) { reject(new Error('PDF must be under 10 MB')); return; }
  ...
  const handleTranslate = async () => {
    if (!doctorNotes.trim() && !pdfFile) { setError('Please enter notes or upload a PDF'); return; }
  ...
  const buildFullExport = useCallback(() => {
    if (!results) return '';
    const r = results;
    const lines = ['DOCTOR VISIT TRANSLATION', '═'.repeat(50), `Date: ${new Date().toISOString().split('T')[0]}`, `Visit: ${visitType}`, doctorName ? `Doctor: ${doctorName}` : '', ''];
        ...
        <div className={`${c.card} border rounded-xl p-6 space-y-5`}>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button onClick={loadExample} disabled={loading} style={{ backgroundColor: (tool?.headerColor ?? '#888888') + '80' }} className={`px-2.5 py-0.5 rounded-full text-xs font-medium border disabled:opacity-40 ${isDark ? 'text-white border-white/40' : 'text-gray-800 border-transparent'}`} title="Fill in a sample visit to see how this works">Try example</button>
            </div>
          ...
          <div>
            ...
            {pdfFile ? (
              ...
              <div
                ...
                <p className={`text-xs ${c.textMuted}`}>📄 Drop a PDF here or <span className={`text-cyan-500 underline`}>browse</span></p>
                <p className={`text-[10px] ${c.textMuted} mt-0.5`}>Lab reports, prescriptions, discharge summaries, bills — max 10 MB</p>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div><label className={`text-sm font-semibold ${c.textSecondary} block mb-1.5`}>Visit Type</label>
            ...
            <div><label className={`text-sm font-semibold ${c.textSecondary} block mb-1.5`}>Doctor</label>
          ...
          <div>
            <label className={`text-sm font-semibold ${c.textSecondary} block mb-1.5`}>Concerns <span className={`text-[10px] ${c.textMuted}`}>(optional)</span></label>
          ...
          <div>
            <label className={`text-sm font-semibold ${c.textSecondary} block mb-1.5`}>Medications <span className={`text-[10px] ${c.textMuted}`}>(for interaction check)</span></label>
            {activeMeds.length > 0 && (
        ...
        <div ref={resultsRef} className="space-y-4">
                ...
                {comparisonEntry.results?.plain_english_summary?.diagnosis && results.plain_english_summary?.diagnosis && (
                  <div className={`${c.cardAlt} border rounded-lg p-4`}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div><p className={`text-[9px] ${c.textSecondary}`}>Today</p><p className={`text-xs ${c.text} font-semibold`}>{results.plain_english_summary.diagnosis?.split('|||')[0]?.slice(0, 6)}</p></div>
                    </div>
                  </div>
                )}
            ...
            <Sec icon="☑️" title="Actions" badge={`${results.action_checklist.length}`} open={secs.actions} onToggle={() => tog('actions')} c={c}>
              <div className="space-y-3">{results.action_checklist.map((a, i) => (
                <div key={i} className={`${c.cardAlt} border rounded-lg p-4`}>
                  ...
                  <div className="space-y-1 ml-7"><p className={`text-xs ${c.textMuted}`}>Why: {a.why}</p><p className={`text-xs ${c.textMuted}`}>When: {a.when}</p><p className={`text-sm ${c.textSecondary}`}>How: <BiText text={a.how} c={c} /></p>{a.what_if_you_dont && <p className={`text-xs ${c.textMuted} italic`}>If skipped: {a.what_if_you_dont}</p>}</div>
                </div>
              ))}</div>
            </Sec>
          )}
              ...
              <div className="space-y-4">{results.medications.map((m, i) => (
                <div key={i} className={`${c.cardAlt} border rounded-lg p-4`}>
                  ...
                  <div className="space-y-2">
                    <div><p className={`text-[10px] font-bold ${c.textMuted}`}>PURPOSE</p><p className={`text-sm ${c.textSecondary}`}><BiText text={m.purpose} c={c} /></p></div>
                    <div><p className={`text-[10px] font-bold ${c.textMuted}`}>HOW</p><p className={`text-sm ${c.textSecondary}`}><BiText text={m.how_to_take} c={c} /></p></div>
          ...
          {results.medication_safety && (
            <Sec icon="🛡️" title="Med Safety" open={secs.medSafety} onToggle={() => tog('medSafety')} c={c}>
              <div className="space-y-3">
                ...
                {results.medication_safety.interactions_to_check?.length > 0 && <div className={`${c.highlight} border rounded-lg p-4`}><h4 className="text-sm font-bold mb-1">❓ Ask your pharmacist</h4>{results.medication_safety.interactions_to_check.map((q, i) => <p key={i} className="text-xs mb-0.5">• {q}</p>)}</div>}
                {results.medication_safety.food_interactions && <div className={`${c.cardAlt} border rounded-lg p-4`}><h4 className={`text-sm font-bold ${c.text} mb-1`}>🍽️ Food</h4><p className={`text-xs ${c.textSecondary}`}>{results.medication_safety.food_interactions}</p></div>}
            ...
            <Sec icon="🔬" title="Tests" badge={`${results.test_results_explained.length}`} open={secs.tests} onToggle={() => tog('tests')} c={c}>
              <div className="space-y-3">{results.test_results_explained.map((t, i) => (
                <div key={i} className={`${c.cardAlt} border rounded-lg p-4`}>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-2"><div><p className={`text-[10px] ${c.textMuted}`}>Yours</p><p className={`text-sm font-semibold ${c.text}`}>{t.your_result}</p></div><div><p className={`text-[10px] ${c.textMuted}`}>Normal</p><p className={`text-sm ${c.textSecondary}`}>{t.normal_range}</p></div></div>
          ...
          {results.follow_up_requirements && (
            <Sec icon="📅" title="Follow-up" open={secs.followUp} onToggle={() => tog('followUp')} c={c}>
              <div className="space-y-3">
                {results.follow_up_requirements.next_appointment && <div className={`${c.cardAlt} border rounded-lg p-4`}><h4 className={`font-semibold text-sm ${c.text} mb-1`}>📅 Next</h4><p className={`text-sm ${c.textSecondary}`}><BiText text={results.follow_up_requirements.next_appointment} c={c} /></p></div>}
                {results.follow_up_requirements.what_to_monitor?.length > 0 && <div className={`${c.cardAlt} border rounded-lg p-4`}><h4 className={`font-semibold text-sm ${c.text} mb-1`}>👁️ Monitor</h4>{results.follow_up_requirements.what_to_monitor.map((w, i) => <p key={i} className={`text-xs ${c.textSecondary}`}>• {w}</p>)}</div>}
                ...
                {results.follow_up_requirements.what_to_bring_next_time?.length > 0 && <div className={`${c.cardAlt} border rounded-lg p-4`}><h4 className={`font-semibold text-sm ${c.text} mb-1`}>🎒 Bring</h4>{results.follow_up_requirements.what_to_bring_next_time.map((w, i) => <p key={i} className={`text-xs ${c.textSecondary}`}>• {w}</p>)}</div>}
              </div>
            </Sec>
          )}
          ...
          {results.second_opinion_guidance && (
            <Sec icon="🤝" title="Second Opinion" open={secs.secondOpinion} onToggle={() => tog('secondOpinion')} c={c}>
              <div className="space-y-3">
                {results.second_opinion_guidance.when_appropriate && <div className={`${c.cardAlt} border rounded-lg p-4`}><h4 className={`font-semibold text-sm ${c.text} mb-1`}>When</h4><p className={`text-xs ${c.textSecondary}`}>{results.second_opinion_guidance.when_appropriate}</p></div>}
                ...
                {results.second_opinion_guidance.how_to_request_records && <div className={`${c.cardAlt} border rounded-lg p-4`}><h4 className={`font-semibold text-sm ${c.text} mb-1`}>📄 Records</h4><p className={`text-xs ${c.textSecondary}`}>{results.second_opinion_guidance.how_to_request_records}</p></div>}
          ...
          {results.insurance_navigation && (
            <Sec icon="🏥" title="Insurance & Cost" open={secs.insurance} onToggle={() => tog('insurance')} c={c}>
              <div className="space-y-3">
                {results.insurance_navigation.likely_coverage && <div className={`${c.cardAlt} border rounded-lg p-4`}><h4 className={`font-semibold text-sm ${c.text} mb-1`}>📋 Coverage</h4><p className={`text-xs ${c.textSecondary}`}>{results.insurance_navigation.likely_coverage}</p></div>}
      ...
      {mode === 'journal' && (
        <div className="space-y-4">
          <div className={`${c.card} border rounded-xl p-5 space-y-4`}>
            ...
            <div>
              <label className={`text-xs font-semibold ${c.textSecondary} block mb-1`}>Severity: {journalEntry.severity}/10</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className={`text-xs font-semibold ${c.textSecondary} block mb-1`}>Triggers</label><input value={journalEntry.triggers} onChange={e => setJournalEntry(p => ({ ...p, triggers: e.target.value }))} placeholder="Stress, food..." className={`w-full p-2 border rounded-lg ${c.input} outline-none text-sm`} /></div>
              <div><label className={`text-xs font-semibold ${c.textSecondary} block mb-1`}>Notes</label><input value={journalEntry.notes} onChange={e => setJournalEntry(p => ({ ...p, notes: e.target.value }))} placeholder="Details..." className={`w-full p-2 border rounded-lg ${c.input} outline-none text-sm`} /></div>
            </div>
            ...
            <div className={`${c.card} border rounded-xl p-5`}>
              ...
              <div className="space-y-2">{symptomTrends.map(t => (
                <div key={t.name} className={`${c.cardAlt} border rounded-lg p-3`}>
                  ...
                  <div className="flex items-center gap-2"><div className={`flex-1 h-3 rounded-full overflow-hidden ${isDark ? 'bg-zinc-800' : 'bg-gray-200'}`}><div className={`h-full rounded-full ${t.recentAvg >= 7 ? 'bg-red-500' : t.recentAvg >= 4 ? 'bg-amber-500' : 'bg-emerald-500'}`} style={{ width: `${t.recentAvg * 10}%` }} /></div><span className={`text-xs ${c.textMuted} w-14 text-right`}>{t.recentAvg}/10</span></div>
          ...
          <div className={`${c.card} border rounded-xl p-5`}>
            <div className="flex items-center justify-between mb-3"><h3 className={`text-sm font-bold ${c.text}`}>📋 Recent ({journal.length})</h3>{journal.length > 0 && <button onClick={() => { if (window.confirm('Clear journal?')) setJournal([]); }} className={`text-xs ${c.textMuted} ${c.deleteHover}`}>Clear</button>}</div>
            {journal.length === 0 ? <p className={`text-sm ${c.textSecondary} text-center py-4`}>Log symptoms to track patterns.</p>
      ...
      {mode === 'health' && (
        <div className="space-y-4">
          ...
          <div className={`${c.card} border rounded-xl p-5`}>
            ...
            {reminders.filter(r => r.status === 'pending').length === 0 ? <p className={`text-sm ${c.textSecondary} text-center py-3`}>Save visits to auto-generate tasks.</p>
