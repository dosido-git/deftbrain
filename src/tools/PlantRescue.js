...
const EXAMPLES = [
  ...
  {
  mode: 'rescue',
  plantDescription: "Fiddle leaf fig, about four feet tall. Dropped six leaves in the three weeks since we moved flats. The ones left are still green but the lowest few have brown patches in the middle, not at the edges. No new growth since spring.",
  lightLevel: 'low-light',
  wateringFreq: 'rarely',
  location: 'indoor',
  ageOfOwnership: '3 years',
  hasPets: false,
  hasChildren: true,
},
];;
...
const PlantRescue = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    ...
    setWateringFreq(ex.wateringFreq);
    setLocation(ex.location);
    setAgeOfOwnership(ex.ageOfOwnership);
    ...
    <div className={`space-y-4 ${c.text}`}>
      ...
      <div className={`${c.card} border ${c.border} rounded-xl shadow-sm overflow-hidden`}>
        ...
        <div className="p-5 space-y-5">
          ...
          className={`relative w-full ${((mode === 'identify' ? !imageBase64 : (!imageBase64 && !plantDescription.trim() && selectedSymptoms.length === 0))) ? c.btnIdle : c.btnPrimary} font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
          {loading
