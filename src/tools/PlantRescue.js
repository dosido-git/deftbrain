...
const PlantRescue = ({ tool }) => {
  ...
  const linkStyle = isDark ? 'text-cyan-400 hover:text-cyan-300 underline underline-offset-2'
    ...
    <div className="space-y-6">
      ...
      {showCollection && (
        <div className={`${c.card} border rounded-xl p-5`}>
          <div className="flex items-center justify-between mb-3">
            ...
            {plantCollection.length >= 2 && <button onClick={handleGetCompanions} disabled={companionLoading} className={`${c.btnSecondary} px-3 py-1.5 rounded text-xs disabled:opacity-50`}>{companionLoading ? (tool?.icon ?? '⚙️') : '🤝 Companion Check'}</button>}
          </div>
        ...
        <div className="space-y-6">
          ...
          {mode !== 'identify' && <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label className={`block text-xs font-medium ${c.label} mb-1`}>⏱️ How long?</label><input type="text" value={ageOfOwnership} onChange={e => setAgeOfOwnership(e.target.value)} placeholder="e.g., 2 months" className={`w-full p-2.5 border rounded-lg text-sm ${c.input}`} /></div>
      ...
      {results && (
        <div className="space-y-6">
          ...
          <ActionBar content={''} title="Results" />
