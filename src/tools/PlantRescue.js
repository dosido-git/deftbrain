...
// Five, rotated: a slow yellowing pothos, a fiddle-leaf fig dropping
// leaves after a move, an overwatered succulent's mushy stem, an outdoor
// rose with powdery mildew, and a brand-new monstera needing a care plan
...
const EXAMPLES = [
  ...
  {
    mode: 'rescue',
    plantDescription: "Succulent (jade plant), been on the windowsill for two years, always fine. Stem has gone soft and mushy near the base and a couple of lower leaves fell off just by touching them.",
    symptoms: ['mushy_stem', 'leaf_drop'],
    symptomDuration: 'week_or_two',
    recentChanges: ['watering_changed'],
    lightLevel: 'full-sun',
    wateringMethod: 'schedule',
    wateringFreqText: 'Every 3 days since a house sitter took over',
    hasDrainage: 'no',
    location: 'indoor',
    hasPets: true,
    hasChildren: false,
  ...
  {
    mode: 'rescue',
    plantDescription: "Outdoor rose bush, three years established. White powdery coating on most leaves and some buds are not opening properly. Neighbor's roses look fine.",
    symptoms: ['white_fuzz', 'no_growth'],
    symptomDuration: 'few_months',
...
const PlantRescue = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    ...
    if (mode === 'care') {
      ...
      setCareHelpWith(ex.careHelpWith || '');
      return;
    }
    ...
    <div className={`space-y-4 ${c.text}`}>
      ...
      <div ref={formTopRef} className={`${c.card} border ${c.border} rounded-xl shadow-sm overflow-hidden`}>
        ...
        <div className="p-5 space-y-5">
          ...
          {loading
            ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '🪴'}</span> {t('pr_analyzing')}</>
            : <><span className="text-xl">{tool?.icon ?? '🪴'}</span> {mode === 'rescue' ? t('pr_btn_diagnose') : mode === 'identify' ? t('pr_btn_identify') : t('pr_btn_care')}</>}
          {!loading && (
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {plantCollection.length >= 2 && (
              <button onClick={handleGetCompanions} disabled={companionLoading}
                ...
                {companionLoading ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '🪴'}</span></> : t('pr_companion_check')}
              </button>
            )}
          </div>
          ...
          <div className={`${c.card} border ${c.border} rounded-xl p-5`}>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleFollowUp} disabled={followUpLoading || !followUpQuestion.trim()}
                ...
                {followUpLoading ? <span className="inline-block animate-spin text-xl">{tool?.icon ?? '🪴'}</span> : '❓'}
              </button>
            </div>
            {followUpAnswer && (
