...
const PlantRescue = ({ tool }) => {
  ...
  c.critical     = isDark ? 'bg-red-900/20 border-red-700 text-red-200'       : 'bg-red-50 border-red-300 text-red-800';
  c.concerning   = isDark ? 'bg-amber-900/20 border-amber-700 text-amber-200' : 'bg-amber-50 border-amber-300 text-amber-800';
  c.minor        = isDark ? 'bg-sky-900/20 border-sky-700 text-sky-200'       : 'bg-sky-50 border-sky-200 text-sky-800';
  c.propagation  = isDark ? 'bg-emerald-900/20 border-emerald-700'            : 'bg-emerald-50 border-emerald-200';
  c.btnDanger    = isDark ? 'bg-red-700 hover:bg-red-600 text-white'           : 'bg-red-500 hover:bg-red-600 text-white';
          ...
          <div>
            ...
            {!imagePreview ? (
              <div onDragOver={handleDragOver} onDrop={handleDrop} onPaste={handlePaste}
                ...
                {uploading
                  ? <><span className="inline-block animate-spin">{tool?.icon ?? '⚙️'}</span><p className={`text-sm ${c.text}`}>Compressing...</p></>
          ...
          {mode !== 'identify' && (
            <div>
              <label className={`block text-sm font-medium ${c.label} mb-1`}>
                📝 {mode === 'rescue' ? 'Additional details' : 'Describe your plant'} <span className={`font-normal ${c.textMuted}`}>(if no photo)</span>
              </label>
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {plantCollection.length >= 2 && (
              <button onClick={handleGetCompanions} disabled={companionLoading}
                ...
                {companionLoading ? <><span className="inline-block animate-spin">{tool?.icon ?? '⚙️'}</span></> : '🤝 Companion Check'}
              </button>
            )}
          </div>
          ...
          <div className={`${c.card} border ${c.border} rounded-xl p-5`}>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleFollowUp} disabled={followUpLoading || !followUpQuestion.trim()}
                ...
                {followUpLoading ? <span className="inline-block animate-spin">{tool?.icon ?? '⚙️'}</span> : '❓'}
              </button>
            </div>
            {followUpAnswer && (
