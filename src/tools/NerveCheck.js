                ...
                {liveResults.last_words && (
                  <div className={`${c.card} border-2 rounded-2xl p-5 ${isDark ? 'border-cyan-700/50' : 'border-cyan-300'}`}>
                    ...
                    <div className="space-y-2.5">
                      ...
                      <div className={`p-3 rounded-xl ${isDark ? 'bg-emerald-900/15' : 'bg-emerald-50'}`}><p className={`text-[10px] font-bold ${isDark ? 'text-emerald-400' : 'text-emerald-700'}`}>FIRST THING TO SAY</p><p className={`text-sm font-bold ${c.text}`}>"{liveResults.last_words.first_thing_to_say}"</p><div className="mt-1"></div></div>
          ...
          <div className="space-y-5">
            ...
            {specResults && (
              <div className="space-y-4">
                ...
                {specResults.cheat_sheet?.length > 0 && (<div className={`p-4 rounded-xl ${isDark ? 'bg-cyan-900/15 border border-cyan-800/40' : 'bg-cyan-50 border border-cyan-200'}`}><div className="flex items-center justify-between mb-2"><p className={`text-xs font-bold ${isDark ? 'text-cyan-400' : 'text-cyan-700'}`}>📝 Cheat Sheet</p></div>{specResults.cheat_sheet.map((item, i) => <p key={i} className={`text-xs ${c.text} mb-0.5`}>• {item}</p>)}</div>)}
              </div>
            )}
          </div>
        )}
