...
const TheAlibi = ({ tool }) => {
  ...
  const c = {
    ...
    btnGhost:     isDark ? 'text-zinc-400 hover:text-zinc-100' : 'text-gray-400 hover:text-gray-900',
    ...
    warnText:     isDark ? 'text-red-300' : 'text-red-700',
      ...
      <div ref={resultsRef} className="space-y-4 mt-4">
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className={'p-3 rounded-lg ' + c.successBox}>
                    <p className={'text-[10px] font-bold ' + c.successTxt + ' uppercase mb-1'}>Best for</p>
                    <p className={'text-xs ' + c.successTxt}>{v.when_to_use}</p>
                  </div>
            ...
            <div className="space-y-3 mt-4">
              {results.follow_up_prep.map((f, idx) => (
                <div key={idx} className={'p-4 rounded-xl border ' + c.cardAlt}>
                  ...
                  {f.trap_to_avoid && (
                    <div className={'p-2 rounded-lg ' + c.warningBox}>
          ...
          <Section title="Common Mistakes" emoji="🚫" open={showMistakes} onToggle={() => setShowMistakes(!showMistakes)}>
            <div className="space-y-2 mt-4">
              {results.common_mistakes.map((m, idx) => (
                <div key={idx} className={'p-3 rounded-lg ' + c.warningBox}>
        ...
        {results.confidence_note && (
          <div className={'p-4 rounded-xl border ' + c.successBox}>
            <p className={'text-xs font-bold ' + c.successTxt + ' mb-1'}>💚 Perspective check</p>
            <p className={'text-sm ' + c.successTxt}>{results.confidence_note}</p>
          </div>
        )}
    ...
    <div>
      ...
      {error && (
        <div className={'mt-4 p-4 ' + c.warningBox + ' border rounded-xl flex items-start gap-3'}>
          <span className={'text-base ' + c.warnText}>⚠️</span>
          <p className={'text-sm ' + c.warnText}>{error}</p>
        </div>
      )}
      {renderHistory()}
    </div>
