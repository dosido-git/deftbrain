...
// The dream text is a locale key, not a literal — a visitor reading the tool in
// Spanish was being handed an English dream to "try". Resolved through t() at
// click time in loadExample.
// Five, rotated: an unpreparedness/exam dream, a symbolic house dream, an
// anxious pursuit dream, a body-won't-move chase, and a grief dream — the
// range of dream logic the tool has to read without flattening into one
// interpretation template.
const EXAMPLES = [
  { mode: 'single', descriptionKey: 'dps_example1' },
  { mode: 'single', descriptionKey: 'dps_example2' },
  { mode: 'single', descriptionKey: 'dps_example3' },
  { mode: 'single', descriptionKey: 'dps_example4' },
...
const DreamPatternSpotter = ({ tool }) => {
  ...
  const handlePatternAnalyze = async () => {
    ...
    try {
      ...
      setResults(data); setTimeout(() => revealSection(resultsRef.current), 100);
    } catch (err) {
