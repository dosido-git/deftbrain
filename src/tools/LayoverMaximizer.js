...
const LayoverMaximizer = () => {
  ...
  const c = {
    ...
    btnPrimary: isDark ? 'bg-sky-600 hover:bg-sky-500 text-white' : 'bg-sky-600 hover:bg-sky-700 text-white',
    ...
    return (
      <div className="space-y-4">
        <div className={`${c.card} border rounded-xl p-5`}>
          <h3 className={`text-sm font-bold ${c.text} mb-1`}>✈️ Layover Planner</h3>
          <p className={`text-xs ${c.muted} mb-4`}>Should you leave the airport? And what should you do either way?</p>
          ...
          <div className="space-y-3">
            ...
            <div className={`flex items-center gap-2 p-2.5 rounded-lg border ${isLiveMode ? (isDark ? 'border-sky-600 bg-sky-900/20' : 'border-sky-300 bg-sky-50') : c.card}`}>
              ...
              {isLiveMode && (
                <div className="flex items-center gap-1 ml-auto">
                  <span className={`text-[10px] ${c.muted}`}>Departure:</span>
                  <input type="time" value={departureTime} onChange={e => setDepartureTime(e.target.value)}
                    className={`px-2 py-1 border rounded text-xs ${c.input} outline-none`} />
                  {liveHoursRemaining !== null && (
                    <span className={`text-xs font-black ${liveHoursRemaining < 2 ? c.dangerText : c.accent}`}>
            ...
            {/* Airport */}
            <div>
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Airport *</label>
              ...
              <div className="flex flex-wrap gap-1 mt-1.5">
                {POPULAR_AIRPORTS.slice(0, 12).map(ap => (
            ...
            {!isLiveMode && (
              <div>
                <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Layover duration (hours) *</label>
            ...
            <div className="flex flex-wrap gap-2">
              <div className="flex-1 min-w-[140px]">
                <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Nationality / Passport</label>
              ...
              <div className="flex-1 min-w-[100px]">
                <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Arrival terminal</label>
                <input value={arrivalTerminal} onChange={e => setArrivalTerminal(e.target.value)}
                  placeholder="e.g. 1, B"
                  className={`w-full px-2 py-1.5 border rounded-lg text-xs ${c.input} outline-none`} />
              </div>
              <div className="flex-1 min-w-[100px]">
                <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Departure terminal</label>
            ...
            {!isLiveMode && (
              <div>
                <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Landing time (optional — for return-by calculation)</label>
            ...
            {/* Travel style */}
            <div>
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Travel style</label>
            ...
            <button onClick={runAnalysis} disabled={loading || !airport.trim()}
              ...
              {loading ? <><span className="animate-spin inline-block">⏳</span> Analyzing...</> : <><span>✈️</span> Analyze My Layover</>}
            </button>
          </div>
        </div>
            ...
            <div className="space-y-4">
                ...
                <Section icon="⏱️" title="Time Math" defaultOpen c={c}>
                  ...
                  <div className="space-y-1.5">
                    {[
                      ...
                      <div key={i} className="flex items-center justify-between text-xs">
                        <span className={c.muted}>{row.minus ? '−' : ''} {row.label}</span>
                        <span className={`font-bold ${row.minus ? c.dangerText : c.text}`}>{row.val}</span>
                      </div>
                    ))}
                    <div className={`flex items-center justify-between text-sm font-black border-t ${c.divider} pt-2 mt-2`}>
                      <span className={c.text}>= Available time</span>
                      <span className={r.time_math.available_city_minutes > 60 ? c.successText : c.dangerText}>
                  ...
                  {r.time_math.return_by_time && (
                    <div className={`${c.warning} border rounded-lg p-3 mt-3 text-center`}>
                      <p className={`text-xs font-bold ${c.warningText}`}>⏰ Be back at the airport by: <span className="text-sm">{r.time_math.return_by_time}</span></p>
                    </div>
                  )}
                </Section>
              )}
              ...
              {r.terminal_change_warning && (
                <div className={`${c.warning} border rounded-xl p-4 flex items-start gap-2`}>
                  ...
                  <div>
                    <p className={`text-xs font-bold ${c.warningText}`}>Terminal Change</p>
                  ...
                  <div className="space-y-4">
                    ...
                    {r.leave_the_airport.visa_info && (
                      <div className={`${c.info} border rounded-lg p-3`}>
                        <p className={`text-[10px] font-bold ${c.label} uppercase mb-1`}>🛂 Visa / Transit</p>
                    ...
                    {r.leave_the_airport.transit_options?.length > 0 && (
                      <div>
                        <p className={`text-[10px] font-bold ${c.label} uppercase mb-1.5`}>🚇 Getting to the city</p>
                        {r.leave_the_airport.transit_options.map((t, i) => (
                          <div key={i} className={`${c.quoteBg} rounded-lg p-3 mb-1.5`}>
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`text-xs ${c.muted}`}>{t.time_minutes}min • {t.cost_estimate}</span>
                            </div>
                            {t.notes && <p className={`text-[10px] ${c.muted} mt-0.5`}>{t.notes}</p>}
                          </div>
                        ))}
                      </div>
                    )}
                        ...
                        {r.leave_the_airport.explore_itinerary.stops.map((stop, i) => (
                          <div key={i} className={`flex gap-3 ${c.quoteBg} rounded-lg p-3`}>
                            ...
                            <div>
                              ...
                              <p className={`text-xs ${c.muted}`}>{stop.what}</p>
                              <div className="flex gap-3 mt-0.5">
                                <span className={`text-[10px] ${c.muted}`}>⏱ {stop.time_needed}</span>
                                {stop.distance_from_previous && <span className={`text-[10px] ${c.muted}`}>🚶 {stop.distance_from_previous}</span>}
                              </div>
                              {stop.tip && <p className={`text-[10px] italic ${c.accent} mt-0.5`}>💡 {stop.tip}</p>}
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {r.leave_the_airport.warnings?.length > 0 && (
                      <div className={`${c.warning} border rounded-lg p-3`}>
                        <p className={`text-[10px] font-bold ${c.warningText} uppercase mb-1`}>⚠️ Warnings</p>
                ...
                <Section icon="🏢" title="Stay & Thrive" defaultOpen={r.verdict !== 'YES'} c={c}>
                  <div className="space-y-4">
                    {r.stay_in_airport.terminal_info && (
                      <p className={`text-xs ${c.muted}`}>📍 {r.stay_in_airport.terminal_info}</p>
                    )}

                    {/* Food */}
                    {r.stay_in_airport.food?.length > 0 && (
                      <div>
                        <p className={`text-[10px] font-bold ${c.label} uppercase mb-1.5`}>🍽️ Food</p>
                        <div className="space-y-1.5">
                          {r.stay_in_airport.food.map((f, i) => (
                            <div key={i} className={`${c.quoteBg} rounded-lg p-3`}>
                              <div className="flex items-center justify-between">
                                <span className="text-xs font-bold">{f.name}</span>
                                <span className={`text-[10px] ${c.muted}`}>{f.terminal} • {f.price_range}</span>
                              </div>
                              <p className={`text-xs ${c.muted}`}>{f.type}</p>
                              {f.tip && <p className={`text-[10px] italic ${c.accent}`}>💡 {f.tip}</p>}
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {r.stay_in_airport.lounges?.length > 0 && (
                      <div>
                        <p className={`text-[10px] font-bold ${c.label} uppercase mb-1.5`}>🛋️ Lounges</p>
                        {r.stay_in_airport.lounges.map((l, i) => (
                          <div key={i} className={`${c.quoteBg} rounded-lg p-3 mb-1.5`}>
                            <div className="flex items-center justify-between">
                              <span className="text-xs font-bold">{l.name}</span>
                              <span className={`text-[10px] ${l.worth_it ? c.successText : c.muted}`}>
                                {l.worth_it ? '✓ Worth it' : '? Maybe'}
                              </span>
                            </div>
                            <p className={`text-xs ${c.muted}`}>{l.access} • {l.terminal}</p>
                            {l.highlights && <p className={`text-[10px] ${c.text}`}>{l.highlights}</p>}
                          </div>
                        ))}
                        <button onClick={() => { setLoungeAirport(r.airport_code || airport); setView('lounge'); }}
                          className={`text-xs font-bold ${c.accent} underline min-h-[28px]`}>See full lounge guide →</button>
                      </div>
                    )}
                    ...
                    {r.stay_in_airport.sleep_spots && (
                      <div className={`${c.quoteBg} rounded-lg p-3`}>
                        <p className={`text-[10px] font-bold ${c.label} uppercase mb-1`}>😴 Rest spots</p>
                        <p className="text-xs">{r.stay_in_airport.sleep_spots}</p>
                      </div>
                    )}

                    {r.stay_in_airport.hidden_gems?.length > 0 && (
                      <div className={`${isDark ? 'bg-purple-900/20 border-purple-800' : 'bg-purple-50 border-purple-200'} border rounded-lg p-3`}>
                        <p className={`text-[10px] font-bold ${c.label} uppercase mb-1`}>✨ Hidden gems</p>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {r.stay_in_airport.practical.wifi && (
                          <div className={`${c.quoteBg} rounded-lg p-2`}>
                            <p className={`text-[10px] font-bold ${c.label}`}>📶 WiFi</p>
                            <p className={`text-[10px] ${c.muted}`}>{r.stay_in_airport.practical.wifi}</p>
                          </div>
                        )}
                        {r.stay_in_airport.practical.charging && (
                          <div className={`${c.quoteBg} rounded-lg p-2`}>
                            <p className={`text-[10px] font-bold ${c.label}`}>🔌 Charging</p>
                            <p className={`text-[10px] ${c.muted}`}>{r.stay_in_airport.practical.charging}</p>
                          </div>
                        )}
                        {r.stay_in_airport.practical.showers && (
                          <div className={`${c.quoteBg} rounded-lg p-2`}>
                            <p className={`text-[10px] font-bold ${c.label}`}>🚿 Showers</p>
                            <p className={`text-[10px] ${c.muted}`}>{r.stay_in_airport.practical.showers}</p>
                          </div>
                        )}
                        {r.stay_in_airport.practical.walking_path && (
                          <div className={`${c.quoteBg} rounded-lg p-2`}>
                            <p className={`text-[10px] font-bold ${c.label}`}>🚶 Walking</p>
                            <p className={`text-[10px] ${c.muted}`}>{r.stay_in_airport.practical.walking_path}</p>
                          </div>
                        )}
                      </div>
                    )}
  ...
  const renderLounge = () => (
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <p className={`text-xs ${c.muted} mb-4`}>Every lounge at your airport. Which ones you can access and whether they're worth it.</p>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Terminal</label>
              <input value={loungeTerminal} onChange={e => setLoungeTerminal(e.target.value)}
                placeholder="Optional"
                className={`w-full px-2 py-1.5 border rounded-lg text-xs ${c.input} outline-none`} />
            </div>
            <div className="flex-1">
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Airline</label>
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Credit cards (comma-separated)</label>
              <input value={loungeCards} onChange={e => setLoungeCards(e.target.value)}
                placeholder="e.g. Amex Platinum, Chase Sapphire"
                className={`w-full px-2 py-1.5 border rounded-lg text-xs ${c.input} outline-none`} />
            </div>
            <div className="flex-1">
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Airline status</label>
          ...
          <button onClick={runLounge} disabled={loading || !loungeAirport.trim()}
            className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
            {loading ? <><span className="animate-spin inline-block">⏳</span> Searching...</> : <><span>🛋️</span> Find Lounges</>}
          </button>
        </div>
      </div>
      ...
      {loungeResults?.lounges?.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {loungeResults.best_overall && (
              <div className={`${c.success} border rounded-lg p-3 text-center`}>
                <p className={`text-[10px] font-bold ${c.successText} uppercase`}>🏆 Best Overall</p>
            ...
            {loungeResults.best_value && (
              <div className={`${c.info} border rounded-lg p-3 text-center`}>
                <p className={`text-[10px] font-bold ${c.label} uppercase`}>💰 Best Value</p>
            ...
            {loungeResults.best_for_sleep && (
              <div className={`${c.card} border rounded-lg p-3 text-center`}>
                <p className={`text-[10px] font-bold ${c.label} uppercase`}>😴 Best for Sleep</p>
            ...
            <Section key={i} icon="🛋️" title={lounge.name}
              ...
              <div className="space-y-3">
                ...
                {lounge.access_methods?.length > 0 && (
                  <div>
                    <p className={`text-[10px] font-bold ${c.label} uppercase mb-1`}>Access</p>
                    {lounge.access_methods.map((am, ai) => (
                      <div key={ai} className="flex items-center gap-2 text-xs mb-0.5">
                        <span>{am.eligible ? '✅' : '❌'}</span>
                        <span className={am.eligible ? `font-bold ${c.text}` : c.muted}>{am.method}</span>
                        <span className={c.muted}>{am.cost}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {lounge.crowding && <p className={`text-xs ${c.muted}`}>👥 {lounge.crowding}</p>}
                {lounge.best_feature && <p className={`text-xs ${c.successText}`}>✓ Best: {lounge.best_feature}</p>}
                {lounge.worst_feature && <p className={`text-xs ${c.dangerText}`}>✕ Worst: {lounge.worst_feature}</p>}
                {lounge.tip && <p className={`text-xs italic ${c.accent}`}>💡 {lounge.tip}</p>}
          ...
          {loungeResults.no_lounge_alternative && (
            <div className={`${c.info} border rounded-lg p-3`}>
              <p className={`text-[10px] font-bold ${c.label} uppercase mb-1`}>No lounge? Try this instead</p>
    ...
    return (
      <div className="space-y-4">
        <div className={`${c.card} border rounded-xl p-5`}>
          ...
          <p className={`text-xs ${c.muted} mb-4`}>What happens if things go wrong? Delays, missed connections, worst-case scenarios.</p>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Layover hours</label>
                <input type="number" step="0.5" value={riskHours} onChange={e => setRiskHours(e.target.value)}
                  className={`w-full px-2 py-1.5 border rounded-lg text-xs ${c.input} outline-none`} />
              </div>
              <div className="flex-1">
                <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Current delay (min)</label>
            ...
            <div>
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>What are you worried about?</label>
            ...
            <button onClick={runRisk} disabled={loading || !riskAirport.trim()}
              className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
              {loading ? <><span className="animate-spin inline-block">⏳</span> Calculating...</> : <><span>⚠️</span> Assess Risk</>}
            </button>
          </div>
        </div>
            ...
            <div className="space-y-4">
                ...
                <Section icon="😰" title="If You Miss Your Flight" defaultOpen c={c}>
                  <div className="space-y-2">
                    {r.if_you_miss_it.next_flight_likely && (
                      <div className={`${c.quoteBg} rounded-lg p-3`}>
                        <p className={`text-[10px] font-bold ${c.label} uppercase mb-1`}>✈️ Next flight</p>
                    ...
                    {r.if_you_miss_it.estimated_cost && (
                      <div className={`${c.danger} border rounded-lg p-3 text-center`}>
                        <p className={`text-xs font-bold ${c.dangerText}`}>Potential cost: {r.if_you_miss_it.estimated_cost}</p>
                      </div>
                    )}
                  </div>
                </Section>
              ...
              {r.delay_cascade && r.delay_cascade.adjusted_plan && (
                <Section icon="⏰" title="Delay Impact" c={c}>
                  ...
                  <p className={`text-xs ${r.delay_cascade.still_feasible ? c.successText : c.dangerText} font-bold`}>
                  ...
                  {r.delay_cascade.when_to_worry && (
                    <p className={`text-xs ${c.warningText} mt-1`}>⚠️ Worry point: {r.delay_cascade.when_to_worry}</p>
                  )}
                </Section>
              )}
                  ...
                  <div className="space-y-2">
                    {r.mitigation.map((m, i) => (
                      <div key={i} className={`${c.quoteBg} rounded-lg p-3`}>
                        ...
                        <p className={`text-[10px] ${c.muted}`}>{m.why}</p>
                        {m.when && <p className={`text-[10px] ${c.accent}`}>⏰ {m.when}</p>}
                      </div>
                    ))}
                  </div>
                </Section>
              ...
              {r.worst_case_timeline && (
                <div className={`${c.danger} border rounded-xl p-4`}>
                  <p className={`text-[10px] font-bold ${c.dangerText} uppercase mb-1`}>☠️ Worst case scenario</p>
              ...
              {/* Gamble verdict */}
              {r.gamble_verdict && (
                <div className={`${c.card} border-2 rounded-xl p-4`}>
                  <p className={`text-[10px] font-bold ${c.label} uppercase mb-1`}>🎲 The verdict</p>
  ...
  const renderSaved = () => (
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <p className={`text-xs ${c.muted} mb-4`}>Your layover history. Tap any to reload that airport.</p>

        {savedLayovers.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-2xl mb-2">📌</p>
            <p className={`text-xs ${c.muted}`}>No saved layovers yet. Analyze a layover and hit Save.</p>
          ...
          <div className="space-y-2">
            {savedLayovers.map(lay => {
              ...
              return (
                <div key={lay.id} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border ${c.card}`}>
                  ...
                  <button onClick={() => { setAirport(lay.airport); setLayoverHours(String(lay.hours)); setView('plan'); }}
                    ...
                    <span className={`text-xs ${c.muted} ml-2`}>{lay.hours}h</span>
                    {lay.city && <span className={`text-xs ${c.muted} ml-1`}>• {lay.city}</span>}
                  </button>
                  <span className={`text-[10px] ${c.muted}`}>{new Date(lay.date).toLocaleDateString()}</span>
                  <button onClick={() => removeSaved(lay.id)} className={`text-xs ${c.dangerText} min-h-[24px]`}>✕</button>
                </div>
              );
            })}
            <button onClick={() => {
    ...
    return (
      <div className="space-y-4">
        <div className={`${c.card} border rounded-xl p-5`}>
          ...
          <p className={`text-xs ${c.muted} mb-4`}>How to get between terminals/gates. Step-by-step, with time estimates.</p>
          <div className="space-y-3">
            <div>
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Airport *</label>
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Arriving at (gate/terminal)</label>
                <input value={g2gArrival} onChange={e => setG2gArrival(e.target.value)} placeholder="e.g. Gate B22, Terminal 1"
                  className={`w-full px-2 py-1.5 border rounded-lg text-xs ${c.input} outline-none`} />
              </div>
              <div className="flex-1">
                <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Departing from (gate/terminal)</label>
                <input value={g2gDeparture} onChange={e => setG2gDeparture(e.target.value)} placeholder="e.g. Gate E15, Terminal 4"
                  className={`w-full px-2 py-1.5 border rounded-lg text-xs ${c.input} outline-none`} />
              </div>
            </div>
            <div>
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Minutes available</label>
            ...
            <button onClick={runGateToGate} disabled={loading || !g2gAirport.trim()}
              className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
              {loading ? <><span className="animate-spin inline-block">⏳</span> Routing...</> : <><span>🚶</span> Get Directions</>}
            </button>
          </div>
        </div>
          ...
          return (
            <div className="space-y-4">
              <div className={`border-2 rounded-xl p-5 text-center ${DIFF_COLORS[r.difficulty] || c.card}`}>
                ...
                {r.requires_security_recheck && <p className={`text-xs font-bold ${c.dangerText} mt-1`}>⚠️ Requires security re-screening</p>}
                {r.feasibility && <p className="text-sm mt-2">{r.feasibility}</p>}
              </div>

              {r.fastest_route && (
                <div className={`${c.info} border rounded-lg p-3`}>
                  <p className={`text-[10px] font-bold ${c.label} uppercase mb-1`}>🏃 Fastest route</p>
              ...
              {r.steps?.length > 0 && (
                <div className={`${c.card} border rounded-xl p-4`}>
                  <p className={`text-[10px] font-bold ${c.label} uppercase mb-3`}>Step by step</p>
                  <div className="space-y-2">
                    {r.steps.map((step, i) => (
                      <div key={i} className="flex gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-2 mt-0.5">
                            <span className={`text-[10px] ${c.muted}`}>⏱ {step.time_minutes}min</span>
                            {step.tip && <span className={`text-[10px] italic ${c.accent}`}>💡 {step.tip}</span>}
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              {r.shuttle_or_train?.available && (
                <div className={`${c.success} border rounded-lg p-3`}>
                  ...
                  {r.shuttle_or_train.where_to_catch && <p className={`text-[10px] ${c.muted}`}>📍 {r.shuttle_or_train.where_to_catch}</p>}
                </div>
              )}

              {r.security_info && <p className={`text-xs ${c.muted}`}>🛡️ {r.security_info}</p>}

              {r.tight_connection_tips?.length > 0 && (
                <div className={`${c.warning} border rounded-lg p-3`}>
                  <p className={`text-[10px] font-bold ${c.warningText} uppercase mb-1`}>⚡ Tight connection tips</p>
    ...
    return (
      <div className="space-y-4">
        <div className={`${c.card} border rounded-xl p-5`}>
          ...
          <p className={`text-xs ${c.muted} mb-4`}>Your flight is delayed. Does your layover plan still work?</p>

          {!results ? (
            <div className="text-center py-8">
              <p className="text-2xl mb-2">⏰</p>
              <p className={`text-xs ${c.muted}`}>Run a layover analysis first to enable the delay tracker.</p>
              <button onClick={() => setView('plan')} className={`${c.btnPrimary} px-4 py-2 rounded-lg text-xs font-bold mt-3 min-h-[36px]`}>✈️ Plan First</button>
            </div>
          ) : (
            <div className="space-y-4">
              <div className={`${c.quoteBg} rounded-lg p-3`}>
                <p className={`text-xs ${c.muted}`}>Current plan: <strong>{results.airport_name}</strong> — {results.verdict_emoji} {results.verdict}</p>
                <p className={`text-xs ${c.muted}`}>Available time: <strong>{Math.floor(results.time_math.available_city_minutes / 60)}h {results.time_math.available_city_minutes % 60}m</strong></p>
              </div>

              <div>
                <label className={`text-xs font-bold ${c.label} block mb-1.5`}>How many minutes delayed?</label>
                ...
                <div className={`border-2 rounded-xl p-5 text-center ${
                  ...
                  {delayImpact.changed ? (
                    ...
                    <>
                      ...
                      <p className={`text-xs ${c.muted} mt-1`}>Lost {delayImpact.lost}min but plan still works</p>
                    </>
                  )}
                </div>
              )}
              ...
              {thresholds.length > 0 && (
                <div className={`${c.card} border rounded-xl p-4`}>
                  <p className={`text-[10px] font-bold ${c.label} uppercase mb-3`}>Delay impact scale</p>
                  <div className="space-y-2">
                    {thresholds.map((t, i) => {
                      ...
                      return (
                        <div key={i} className={`flex items-center gap-3 px-3 py-2 rounded-lg ${active ? (isDark ? 'bg-sky-900/30 border border-sky-700' : 'bg-sky-50 border border-sky-200') : ''}`}>
                          <span className={`text-xs font-bold w-14 ${c.muted}`}>{t.delay}min</span>
                          ...
                          <span className={`text-xs font-bold ${verdict === 'YES' ? c.successText : verdict === 'RISKY' ? c.warningText : c.dangerText}`}>
                  ...
                  <p className={`text-[10px] ${c.muted} mt-2`}>
  ...
  const renderCompare = () => (
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <p className={`text-xs ${c.muted} mb-4`}>Choosing between connections? Compare layover quality side-by-side.</p>
        <div className="space-y-3">
          {cmpOptions.map((opt, i) => (
            <div key={i} className={`flex gap-2 p-3 rounded-lg border ${c.card}`}>
          ...
          {cmpOptions.length < 4 && (
            <button onClick={() => setCmpOptions([...cmpOptions, { airport: '', hours: '', notes: '' }])}
              className={`${c.btnSec} font-bold py-1.5 px-3 rounded-lg text-xs min-h-[32px]`}>➕ Add option</button>
          )}
          <button onClick={runCompare} disabled={loading || cmpOptions.filter(o => o.airport.trim() && o.hours).length < 2}
            className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
            {loading ? <><span className="animate-spin inline-block">⏳</span> Comparing...</> : <><span>⚖️</span> Compare Layovers</>}
          </button>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          {cmpResults.winner && (
            <div className={`${c.success} border-2 rounded-xl p-5 text-center`}>
              ...
              <p className={`text-lg font-black ${c.successText}`}>{cmpResults.winner} wins</p>
              <p className="text-sm mt-1">{cmpResults.winner_reason}</p>
              {cmpResults.runner_up_case && <p className={`text-xs ${c.muted} mt-2 italic`}>But: {cmpResults.runner_up_case}</p>}
            </div>
          )}
          ...
          {cmpResults.options?.length > 0 && (
            <div className={`grid grid-cols-1 ${cmpResults.options.length === 2 ? 'sm:grid-cols-2' : cmpResults.options.length === 3 ? 'sm:grid-cols-3' : ''} gap-3`}>
              {cmpResults.options.map((opt, i) => {
                ...
                return (
                  <div key={i} className={`${c.card} border rounded-xl p-4 ${isWinner ? (isDark ? 'border-emerald-600' : 'border-emerald-400') : ''}`}>
                    <div className="flex items-center gap-2 mb-3">
                      {isWinner && <span className="text-sm">🏆</span>}
                      <span className={`text-sm font-black ${c.text}`}>{opt.airport_name || opt.airport}</span>
                      <span className={`text-xs ${c.muted}`}>{opt.hours}h</span>
                    </div>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-xs">
                        <span className={c.muted}>Can leave?</span>
                        <span className={`font-bold ${opt.can_leave ? c.successText : c.dangerText}`}>{opt.leave_verdict}</span>
                      </div>
                      {opt.city_time_minutes > 0 && (
                        <div className="flex items-center justify-between text-xs">
                          <span className={c.muted}>City time</span>
                          <span className="font-bold">{Math.floor(opt.city_time_minutes / 60)}h{opt.city_time_minutes % 60}m</span>
                        </div>
                      )}
                      <div className="flex items-center justify-between text-xs">
                        <span className={c.muted}>Airport</span>
                        <span>{'⭐'.repeat(Math.min(Number(opt.airport_rating) || 0, 5))}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className={c.muted}>Food</span>
                        <span>{'⭐'.repeat(Math.min(Number(opt.food_rating) || 0, 5))}</span>
                      </div>
                      {opt.best_thing && <p className={`text-[10px] ${c.successText}`}>✓ {opt.best_thing}</p>}
                      {opt.worst_thing && <p className={`text-[10px] ${c.dangerText}`}>✕ {opt.worst_thing}</p>}
                      {opt.explore_highlight && <p className={`text-[10px] ${c.muted}`}>🌍 {opt.explore_highlight}</p>}
                      <div className="text-center pt-2">
                        <span className={`text-lg font-black ${c.accent}`}>{opt.overall_score}</span>
                        <span className={`text-[10px] ${c.muted}`}>/100</span>
                      </div>
                    </div>
                  </div>
                );
    ...
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        {!results ? (
          <div className="text-center py-6">
            <button onClick={() => setView('plan')} className={`${c.btnPrimary} px-4 py-2 rounded-lg text-xs font-bold min-h-[36px]`}>✈️ Plan a Layover First</button>
          </div>
        ) : (
          <button onClick={runPacking} disabled={loading}
            className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
            {loading ? <><span className="animate-spin inline-block">⏳</span> Building list...</> : <><span>🎒</span> Generate Packing List</>}
          </button>
        )}
      </div>
      ...
      {packResults && (
        <div className="space-y-4">
          {/* Grab list */}
          <div className={`${c.card} border rounded-xl p-5`}>
            <p className={`text-[10px] font-bold ${c.label} uppercase mb-3`}>🎒 Grab before deplaning</p>
            <div className="space-y-2">
              {(packResults.grab_before_deplaning || []).map((item, i) => (
                <div key={i} className={`flex items-start gap-2 text-xs px-3 py-2 rounded-lg ${
                  ...
                  <div>
                    ...
                    <p className={c.muted}>{item.why}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {packResults.weather_note && (
              <div className={`${c.quoteBg} rounded-lg p-3`}>
                <p className={`text-[10px] font-bold ${c.label}`}>🌤️ Weather</p>
                <p className={`text-xs ${c.muted}`}>{packResults.weather_note}</p>
              </div>
            )}
            {packResults.currency_tip && (
              <div className={`${c.quoteBg} rounded-lg p-3`}>
                <p className={`text-[10px] font-bold ${c.label}`}>💰 Currency</p>
                <p className={`text-xs ${c.muted}`}>{packResults.currency_tip}</p>
              </div>
            )}
            {packResults.phone_tip && (
              <div className={`${c.quoteBg} rounded-lg p-3`}>
                <p className={`text-[10px] font-bold ${c.label}`}>📱 Phone</p>
                <p className={`text-xs ${c.muted}`}>{packResults.phone_tip}</p>
              </div>
            )}
            {packResults.cultural_note && (
              <div className={`${c.quoteBg} rounded-lg p-3`}>
                <p className={`text-[10px] font-bold ${c.label}`}>🌍 Culture</p>
                <p className={`text-xs ${c.muted}`}>{packResults.cultural_note}</p>
              </div>
            )}
          </div>
  ...
  const renderSurvivalKit = () => (
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <p className={`text-xs ${c.muted} mb-4`}>Everything you need when you land — screenshot this before you lose WiFi.</p>
        <div className="space-y-3">
          <div className="flex gap-2">
            <div className="flex-1">
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Airport *</label>
            ...
            <div className="flex-1">
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>Hours</label>
          ...
          <button onClick={runSurvivalKit} disabled={loading || !(kitAirport.trim() || results?.airport_code || airport.trim())}
            className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
            {loading ? <><span className="animate-spin inline-block">⏳</span> Building kit...</> : <><span>🧰</span> Generate Survival Kit</>}
          </button>
        </div>
      </div>
            ...
            <div className={`${c.card} border rounded-xl p-5`}>
              ...
              {r.wifi && (
                <div className={`${c.info} border rounded-lg p-3 mb-3`}>
                  <p className={`text-[10px] font-bold ${c.label} uppercase mb-1`}>📶 WiFi</p>
                  <p className="text-xs font-bold">{r.wifi.network_name}</p>
                  {r.wifi.password && <p className={`text-xs ${c.muted}`}>Password: {r.wifi.password}</p>}
                  {r.wifi.how_to_connect && <p className={`text-[10px] ${c.muted}`}>{r.wifi.how_to_connect}</p>}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                {r.time_zone && (
                  <div className={`${c.quoteBg} rounded-lg p-2`}>
                    <p className={`text-[10px] font-bold ${c.label}`}>🕐 Time Zone</p>
                ...
                {r.currency && (
                  <div className={`${c.quoteBg} rounded-lg p-2`}>
                    ...
                    {r.currency.exchange_rate_approx && <p className={`text-[10px] ${c.muted}`}>{r.currency.exchange_rate_approx}</p>}
                  </div>
                )}
                {r.power_outlets && (
                  <div className={`${c.quoteBg} rounded-lg p-2`}>
                    <p className={`text-[10px] font-bold ${c.label}`}>🔌 Outlets</p>
                    <p className="text-xs">{r.power_outlets}</p>
                  </div>
                )}
                {r.emergency_numbers && (
                  <div className={`${c.quoteBg} rounded-lg p-2`}>
                    <p className={`text-[10px] font-bold ${c.label}`}>🆘 Emergency</p>
              ...
              {r.airline_desk && (
                <div className={`${c.quoteBg} rounded-lg p-3 mb-3`}>
                  <p className={`text-[10px] font-bold ${c.label}`}>✈️ Airline Desk</p>
              ...
              {r.transport_from_airport && (
                <div className={`${c.quoteBg} rounded-lg p-3 mb-3`}>
                  <p className={`text-[10px] font-bold ${c.label}`}>🚇 Getting to the City</p>
                  <p className="text-xs">{r.transport_from_airport.to_city}</p>
                  {r.transport_from_airport.taxi_tip && <p className={`text-[10px] ${c.warningText}`}>⚠️ {r.transport_from_airport.taxi_tip}</p>}
                </div>
              )}
              ...
              {r.one_thing_to_know && (
                <div className={`${isDark ? 'bg-purple-900/20 border-purple-800' : 'bg-purple-50 border-purple-200'} border rounded-lg p-3`}>
                  <p className={`text-[10px] font-bold ${c.label} uppercase mb-1`}>☝️ One thing to know</p>
