...
const LayoverMaximizer = ({ tool }) => {
  ...
  useRegisterActions(buildPlanText(), tool?.title || 'Layover Maximizer');
      ...
      <div className="space-y-4">
        <div className={`${c.card} ${c.border} border rounded-xl p-5`}>
          <div className="space-y-3">
            ...
            {!isLiveMode && (
              <div>
                <label className={`text-xs font-bold ${c.textSecondary} block mb-1.5`}>Layover duration (hours) <span className={c.required}>*</span></label>
          ...
          return (
            <div ref={resultsRef} className="space-y-4">
              <button onClick={() => {
                ...
                <span>🔄</span> Analyze a Different Layover
              </button>
              {/* Verdict banner */}
        ...
        <div className={`${c.card} ${c.border} border rounded-xl p-5`}>
          ...
          <div className="space-y-3">
            ...
            <button onClick={runRisk} disabled={loading || !riskAirport.trim()}
              className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
              {loading ? <><span className="animate-spin inline-block">{tool?.icon ?? '✈️'}</span> Calculating...</> : <><span className="mr-1">{tool?.icon ?? '✈️'}</span> Assess Risk</>}
            </button>
          </div>
        </div>
        ...
        <div className={`${c.card} ${c.border} border rounded-xl p-5`}>
          ...
          <div className="space-y-3">
            ...
            <button onClick={runGateToGate} disabled={loading || !g2gAirport.trim()}
              className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
              {loading ? <><span className="animate-spin inline-block">{tool?.icon ?? '✈️'}</span> Routing...</> : <><span className="mr-1">{tool?.icon ?? '✈️'}</span> Get Directions</>}
            </button>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {cmpOptions.length < 4 && (
            <button onClick={() => setCmpOptions([...cmpOptions, { airport: '', hours: '', notes: '' }])}
              className={`${c.btnSecondary} font-bold py-1.5 px-3 rounded-lg text-xs min-h-[32px]`}>➕ Add option</button>
          )}
    ...
    <div className={`space-y-4 ${c.text}`}>
      ...
      <div className={`${c.cardAlt} border ${c.border} rounded-xl p-4`}>
        ...
        <div className="flex flex-wrap gap-3">
          <a href="/DateNight" className={`text-xs ${linkStyle}`}>🌙 Date Night</a>
      ...
      {results && (
        <p className={`text-xs ${c.textMuted} mt-3 text-center`}>
          Got more than a layover? <a href="/MicroAdventureMapper" className={linkStyle}>🗺️ Micro Adventure Mapper</a> plans tiny trips for any window of free time.
        </p>
      )}
    </div>
  );
