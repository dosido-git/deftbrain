...
// Five, rotated across every travel style and layover length: a tight
// Schengen-transfer efficient layover, checked bags with a short connection,
// a long explorer layover in Seoul, a foodie's Singapore stopover, and a
// late-night relaxer layover in Dubai with bags to reclaim.
const EXAMPLES = [
  {
  view: 'plan',
  ...
  connectionTerminal: 'Terminal 5',
  arrivalTime: '17:45',
  travelStyle: 'efficient',
},
  {
  view: 'plan',
  airport: 'ICN — Seoul Incheon',
  layoverHours: '9',
  nationality: 'CA',
  hasCheckedBags: false,
  hasPreCheck: true,
  arrivalTerminal: 'Terminal 2',
  connectionTerminal: 'Terminal 2',
  arrivalTime: '11:00',
...
const LayoverMaximizer = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    const ex = pickExample('LayoverMaximizer', EXAMPLES);
    setView(ex.view);
    setAirport(ex.airport);
    setLayoverHours(ex.layoverHours);
    setNationality(ex.nationality);
    setHasCheckedBags(ex.hasCheckedBags);
    setHasPreCheck(ex.hasPreCheck);
    setArrivalTerminal(ex.arrivalTerminal);
    setConnectionTerminal(ex.connectionTerminal);
    setArrivalTime(ex.arrivalTime);
    setTravelStyle(ex.travelStyle);
    setShowDetails(true);
    setResults(null);
              ...
              {Array.isArray(r.need_to_know) && r.need_to_know.length > 0 && (
                <div className={`${c.quoteBg} border ${c.border} rounded-xl p-4`}>
                  ...
                  <button
                    onClick={() => {
                      setShowDetails(true);
                      setTimeout(() => revealSection(detailsRef.current, { block: 'center' }), 60);
                    }}
