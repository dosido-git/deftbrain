...
const CrisisPrioritizer = ({ tool }) => {
  ...
  const buildText = () => {
    ...
    if (results.reality_check) {
      ...
      L.push(`Tasks: ${results.tasks_analyzed} | Urgent: ${results.actual_crisis_tasks} | Can wait: ${results.can_wait}`);
      L.push('');
    }
    ...
    if (results.todays_actual_must_dos?.length) {
      ...
      if (results.estimated_time) L.push(`  ⏱ Estimated time: ${results.estimated_time}`);
      L.push('');
    }

    if (results.objective_priorities?.length) {
      L.push('📋 PRIORITY LIST');
      results.objective_priorities.forEach(p => {
        ...
        if (p.why_urgent) L.push(`     Why: ${p.why_urgent}`);
      });
      L.push('');
    }
    ...
    if (results.multi_week_plan?.length) {
      L.push('🗓️ MULTI-WEEK PLAN');
      results.multi_week_plan.forEach(week => {
        L.push(`\n  ${week.week_label}${week.focus ? ` — ${week.focus}` : ''}`);
        if (week.must_dos?.length) { L.push('  🔴 Must-dos:'); week.must_dos.forEach(t => L.push(`    [ ] ${t}`)); }
        if (week.delegate?.length) { L.push('  🤝 Delegate:'); week.delegate.forEach(t => L.push(`    → ${t}`)); }
        if (week.delete?.length) { L.push('  🗑️ Drop:'); week.delete.forEach(t => L.push(`    ✕ ${t}`)); }
    ...
    if (oneThingResult) {
      L.push('🎯 JUST ONE THING');
      L.push(oneThingResult.the_one_thing);
      if (oneThingResult.first_physical_action) L.push(`First action: ${oneThingResult.first_physical_action}`);
      if (oneThingResult.time_estimate) L.push(`Time: ${oneThingResult.time_estimate}`);
      if (oneThingResult.why_this_one) L.push(oneThingResult.why_this_one);
      if (oneThingResult.after_this) L.push(`After this → ${oneThingResult.after_this}`);
      L.push('');
    }
    ...
    if (retriageResult) {
      ...
      if (retriageResult.can_stop_now) {
        L.push(`✅ You can stop now. ${retriageResult.stop_reasoning}`);
      } else {
        if (retriageResult.still_must_do?.length) {
          L.push('Still need to do:');
          retriageResult.still_must_do.forEach(t => L.push(`  • ${t}`));
        }
        if (retriageResult.next_action) L.push(`→ Next: ${retriageResult.next_action}`);
      }
    ...
    if (rollingResult) {
      ...
      if (rollingResult.next_check_in) L.push(`Next check-in: ${rollingResult.next_check_in}`);
      L.push('');
    }

    if (fuResult) {
      L.push('📊 FOLLOW-UP REVIEW');
      if (fuResult.hindsight_summary) L.push(fuResult.hindsight_summary);
      if (fuResult.calibration_insight) L.push(`🎯 Calibration: ${fuResult.calibration_insight}`);
      if (fuResult.deferral_note) L.push(`✅ Deferrals: ${fuResult.deferral_note}`);
    ...
    if (patternResult) {
      L.push('📊 CRISIS PATTERNS');
      if (patternResult.pattern_summary) L.push(patternResult.pattern_summary);
      if (patternResult.frequency_insight) L.push(`📅 Frequency: ${patternResult.frequency_insight}`);
      if (patternResult.urgency_calibration) L.push(`🎯 Calibration: ${patternResult.urgency_calibration}`);
  ...
  const buildFullText = useCallback(() => {
    if (!results) return '';
    return buildText();
  }, [results, timeBlockResult, oneThingResult, retriageResult, acctResult, rollingResult, fuResult, patternResult]);
  ...
  return (
    <div className={`space-y-4 ${c.text}`}>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {journal.length >= 3 && <button onClick={handlePattern} disabled={patternLoading} className={`text-xs ${c.navPatternBtn} disabled:opacity-40`}>{patternLoading ? <span className="animate-spin">{tool?.icon ?? '🚨'}</span> : '📊 Patterns'}</button>}
      ...
      {showDashboard && <div className={`${c.dash} border rounded-xl p-5 space-y-4`}>
        ...
        {!dashInsights && journal.length >= 3 && <button onClick={handleDashInsights} disabled={dashLoading} className={`w-full py-2 rounded-lg text-xs font-bold ${c.btnSecondary} border ${c.border} disabled:opacity-40`}>{dashLoading ? <><span className="animate-spin">{tool?.icon ?? '🚨'}</span> Analyzing...</> : '🤖 Get AI Insights'}</button>}
            ...
            <div className="space-y-3 mb-4">
              {tasks.map((task, i) => <div key={i}>
                <div className="flex gap-2">
                  ...
                  {task.text.trim().length > 15 && <button onClick={() => handleSplitTask(task.text.trim())} disabled={splitLoading && splitTask === task.text.trim()} className={`px-2.5 rounded-lg border text-xs ${c.chip(false)} disabled:opacity-40`} title="Split this task">{splitLoading && splitTask === task.text.trim() ? <span className="animate-spin">{tool?.icon ?? '🚨'}</span> : '🧩'}</button>}
      ...
      {results && !showBreather && <div ref={resultsRef} className="space-y-5">
        ...
        {results.objective_priorities?.length > 0 && <Section id="priorities" title="Full Priority List" emoji="📋" defaultOpen>
          <div className="space-y-3">{results.objective_priorities.map((item, idx) => <div key={idx} className={`border-2 rounded-lg p-4 ${urgStyle(item.actual_urgency)}`}>
            ...
            <div className="flex gap-2 mt-2 flex-wrap">
              ...
              <button onClick={() => handleSplitTask(item.task)} disabled={splitLoading && splitTask === item.task} className={`text-xs font-bold ${c.textMuted} disabled:opacity-40`}>{splitLoading && splitTask === item.task ? <span className="animate-spin">{tool?.icon ?? '🚨'}</span> : '🧩 Split'}</button>
            </div>
          </div>)}</div>
        </Section>}
        ...
        <div className={`${c.cardAlt} border ${c.border} rounded-xl p-4`}>
          ...
          <div className="flex flex-wrap gap-3">
            <a href="/CrashPredictor" className={`text-xs ${linkStyle}`}>⚠️ Crash Predictor</a>
            <a href="/BrainStateDeejay" className={`text-xs ${linkStyle}`}>🎧 Brain State Deejay</a>
          </div>
        </div>
      </div>}
        {history.length > 0 && (
