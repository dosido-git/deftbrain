...
const EXAMPLE = {
  location: 'Brooklyn, NY — Park Slope area',
  dateType: 'anniversary',
  budget: 200,
  yearsTogether: 5,
  duration: 'standard',
  startTime: '6:30 PM',
  lastTime: 'Last anniversary we did the same Italian place we always do — felt sweet but predictable. Want something with a story this year.',
  restrictions: 'Nothing too loud — we both want to actually talk',
};
// ═══════════════════════════════════════════
// CONSTANTS
// ═══════════════════════════════════════════
...
const DateNight = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    ...
    setLastTime(EXAMPLE.lastTime);
    setRestrictions(EXAMPLE.restrictions);
    setResults(null);
    setShowInputs(true);
  }, [setLocation, setDateType, setBudget, setYearsTogether, setDuration, setStartTime, setLastTime, setRestrictions, setResults, setShowInputs]);
            ...
            <div key={idx} className="flex gap-4 relative mb-4">
              ...
              <div className={`flex-1 ${c.stopCard} border rounded-xl p-4 ${isSwap ? 'opacity-50' : ''}`}>
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  ...
                  <button onClick={() => handleSimilar(stop)} disabled={similarLoading} className={`text-xs font-bold ${c.roseText} disabled:opacity-40`}>💜 More like this</button>
  ...
  return (
    <div className={`space-y-4 ${c.text}`}>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {journal.length >= 3 && <button onClick={handleRutDetect} disabled={rutLoading} className={`text-xs font-bold ${c.rutText} disabled:opacity-40`}>{rutLoading ? <><span className="animate-spin inline-block">{tool?.icon ?? '💘'}</span></> : '🔍 Rut check'}</button>}
