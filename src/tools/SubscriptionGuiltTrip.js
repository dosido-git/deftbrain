...
const SubscriptionGuiltTrip = ({ tool }) => {
  ...
  const c = {
    pageBgColor: isDark ? 'bg-zinc-900' : 'bg-gradient-to-br from-cyan-50 via-slate-50 to-emerald-50',
    card: isDark ? 'bg-zinc-800' : 'bg-white',
    cardAlt: isDark ? 'bg-zinc-700/50' : 'bg-slate-50',
    cardBorderClass: isDark ? 'border-zinc-700' : 'border-slate-200',
  ...
  return (
    <div className={`min-h-screen ${c.pageBgColor} py-8 px-4`}>
      <div className="max-w-5xl mx-auto">
        ...
        {/* ── Input Mode Tabs ── */}
        <div className={`${c.card} border ${c.cardBorderClass} rounded-2xl shadow-sm p-6 mb-6`}>
          ...
          {inputMode === 'upload' && (
            <>
              <div
                ...
                className={`border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-all mb-4 ${isDragging ? (isDark ? 'border-cyan-400 bg-cyan-900/20' : 'border-cyan-400 bg-cyan-50') : c.dropzone}`}
              >
                <input
                  ref={fileInputRef}
                  type="file"
          ...
          {error && (
            <div className={`mt-4 p-4 rounded-xl flex items-start gap-3 ${isDark ? 'bg-red-900/20 border border-red-800' : 'bg-red-50 border border-red-200'}`}>
              <span className={`flex-shrink-0 mt-0.5 ${isDark ? 'text-red-400' : 'text-red-600'}`}>⚠️</span>
        ...
        {results && (
          <div ref={resultsRef} className="space-y-6">
            ...
            {(results.total_monthly_cost || results.annual_cost) && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={idx} className={`${c.card} border ${c.cardBorderClass} rounded-xl p-4 text-center`}>
            ...
            {results.subscriptions_analyzed && results.subscriptions_analyzed.length > 0 && (
              <div className={`${c.card} border ${c.cardBorderClass} rounded-2xl shadow-sm p-6`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {results.subscriptions_analyzed.map((sub, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className={`border-2 rounded-xl p-4 transition-all cursor-pointer ${c.cardHover} ${sub.verdict === 'cancel' ? c.cancelCard : sub.verdict === 'keep' ? c.keepCard : `${c.card} border ${c.cardBorderClass}`}`}
            ...
            {results.recommended_cancellations && results.recommended_cancellations.length > 0 && (
              <div className={`${c.card} border ${c.cardBorderClass} rounded-2xl shadow-sm p-6`}>
                ...
                {Object.values(selectedForCancel).some(v => v) && (
                  <div className={`mb-4 px-4 py-3 rounded-xl ${isDark ? 'bg-cyan-900/30 border border-cyan-700' : 'bg-cyan-50 border border-cyan-200'}`}>
            ...
            {results.permission_statements && results.permission_statements.length > 0 && (
              <div className={`border-2 rounded-2xl p-6 ${c.permissionBox}`}>
                <h3 className={`text-lg font-bold mb-3 ${isDark ? 'text-sky-300' : 'text-sky-800'}`}>
                  💙 Permission to Cancel
                </h3>
                <div className="space-y-2">
                  {results.permission_statements.map((statement, idx) => (
                    <p key={idx} className={`text-sm ${isDark ? 'text-sky-200' : 'text-sky-700'}`}>
                      • {statement}
                    </p>
                  ))}
                </div>
