...
// CONSTANTS
// ════════════════════════════════════════════════════════════
// Config arrays carry a stable English `minutes`/`id` identity (sent to backend or
// used as active-pill comparison) plus `labelKey`/`subKey`/`descKey` resolved via t().
const PRESETS = [
  { labelKey: 'fpc_preset_25_label', minutes: 25, subKey: 'fpc_preset_25_sub' },
  { labelKey: 'fpc_preset_45_label', minutes: 45, subKey: 'fpc_preset_45_sub' },
  { labelKey: 'fpc_preset_60_label', minutes: 60, subKey: 'fpc_preset_60_sub' },
  { labelKey: 'fpc_preset_90_label', minutes: 90, subKey: 'fpc_preset_90_sub' },
];

const MAX_SNOOZES = 3;

const BODY_EMOJIS = { hydration: '💧', hunger: '☕', posture: '🪑', eyes: '👁️', movement: '🚶' };
// Localized body-check labels — keyed by stable field id from the API response.
const BODY_LABEL_KEYS = { hydration: 'fpc_body_hydration', hunger: 'fpc_body_hunger', posture: 'fpc_body_posture', eyes: 'fpc_body_eyes', movement: 'fpc_body_movement' };

const POMODORO_CHAINS = [
  { labelKey: 'fpc_chain_2x25_label', sessions: [25, 25], breakMin: 5, longBreakMin: 15 },
  { labelKey: 'fpc_chain_3x25_label', sessions: [25, 25, 25], breakMin: 5, longBreakMin: 15 },
  { labelKey: 'fpc_chain_4x25_label', sessions: [25, 25, 25, 25], breakMin: 5, longBreakMin: 20 },
  { labelKey: 'fpc_chain_2x45_label', sessions: [45, 45], breakMin: 10, longBreakMin: 20 },
];

const DISTRACTION_TYPES = [
  { id: 'phone', emoji: '📱', labelKey: 'fpc_distract_phone' },
  { id: 'person', emoji: '🗣️', labelKey: 'fpc_distract_person' },
  { id: 'thought', emoji: '💭', labelKey: 'fpc_distract_thought' },
  { id: 'noise', emoji: '🔊', labelKey: 'fpc_distract_noise' },
  { id: 'other', emoji: '❓', labelKey: 'fpc_distract_other' },
];
...
const EXAMPLES = [
  { sessionMode: 'single', activityKey: 'fpc_ex_activity', sessionDurationMin: 45,
    obligationsKey: 'fpc_ex_obligations', missedKey: 'fpc_ex_missed' },
  { sessionMode: 'single', activityKey: 'fpc_ex2_activity', sessionDurationMin: 25,
    obligationsKey: 'fpc_ex2_obligations', missedKey: 'fpc_ex2_missed' },
];
// Milestones / challenges carry a stable `id` plus labelKey/descKey resolved via t() at
// render. `metric` and challenge `target` are computed values, not user-facing labels.
const MILESTONES = [
  { id: 'first', emoji: '🎉', labelKey: 'fpc_ms_first', check: (h) => h.length >= 1 },
  { id: 'ten', emoji: '🔟', labelKey: 'fpc_ms_ten', check: (h) => h.length >= 10 },
  { id: 'streak3', emoji: '🔥', labelKey: 'fpc_ms_streak3', descKey: 'fpc_ms_streak3_desc', check: (_, s) => s.longestStreak >= 3 },
  { id: 'streak7', emoji: '🏆', labelKey: 'fpc_ms_streak7', descKey: 'fpc_ms_streak7_desc', check: (_, s) => s.longestStreak >= 7 },
  { id: 'tenHours', emoji: '⏰', labelKey: 'fpc_ms_tenHours', check: (h) => h.reduce((s, e) => s + e.actualMin, 0) >= 600 },
  { id: 'perfect5', emoji: '💎', labelKey: 'fpc_ms_perfect5', descKey: 'fpc_ms_perfect5_desc', check: (h) => h.filter(e => e.score >= 85).length >= 5 },
  { id: 'zenMaster', emoji: '🧘', labelKey: 'fpc_ms_zenMaster', descKey: 'fpc_ms_zenMaster_desc', check: (h) => h.some(e => e.actualMin >= 25 && (e.distractions || 0) === 0) },
  { id: 'centurion', emoji: '💯', labelKey: 'fpc_ms_centurion', descKey: 'fpc_ms_centurion_desc', check: (h) => h.length >= 50 },
];

const SOUND_PRESETS = [
  { id: 'rain', emoji: '🌧️', labelKey: 'fpc_sound_rain', type: 'brown' },
  { id: 'white', emoji: '📻', labelKey: 'fpc_sound_white', type: 'white' },
  { id: 'ocean', emoji: '🌊', labelKey: 'fpc_sound_ocean', type: 'pink' },
  { id: 'focus', emoji: '🧠', labelKey: 'fpc_sound_focus', type: 'binaural' },
];

// Feature hints — shown contextually at the right moment (resolved via t() at render).
const FEATURE_HINT_KEYS = {
  chains: 'fpc_hint_chains',
  score: 'fpc_hint_score',
  accomplishment: 'fpc_hint_accomplishment',
};

const DAY_NAME_KEYS = ['fpc_day_sun', 'fpc_day_mon', 'fpc_day_tue', 'fpc_day_wed', 'fpc_day_thu', 'fpc_day_fri', 'fpc_day_sat'];

// Challenge templates: `label`/`desc` come from labelKey/descKey via t() at render time.
// `descArgs` carries the interpolation vars for the desc key; `target`/`metric` are data.
const CHALLENGE_TEMPLATES = [
  { id: 'beat-avg', emoji: '📈', labelKey: 'fpc_ch_beat_avg_label', descKey: 'fpc_ch_beat_avg_desc', gen: (h) => { const avg = Math.round(h.reduce((s, e) => s + (e.score || 0), 0) / h.length); return { target: avg + 5, descArgs: { target: avg + 5 }, metric: 'score' }; }},
  { id: 'zero-distract', emoji: '🧘', labelKey: 'fpc_ch_zero_distract_label', descKey: 'fpc_ch_zero_distract_desc', gen: () => ({ target: 0, descArgs: {}, metric: 'distractions' }) },
  { id: 'streak-3', emoji: '🔥', labelKey: 'fpc_ch_streak3_label', descKey: 'fpc_ch_streak3_desc', gen: () => ({ target: 3, descArgs: {}, metric: 'streak' }) },
  { id: 'long-session', emoji: '⏱️', labelKey: 'fpc_ch_long_session_label', descKey: 'fpc_ch_long_session_desc', gen: (h) => { const avg = Math.round(h.reduce((s, e) => s + e.actualMin, 0) / h.length); return { target: Math.max(avg + 15, 45), descArgs: { target: Math.max(avg + 15, 45) }, metric: 'duration' }; }},
  { id: 'perfect-week', emoji: '💎', labelKey: 'fpc_ch_perfect_week_label', descKey: 'fpc_ch_perfect_week_desc', gen: () => ({ target: 5, descArgs: {}, metric: 'weekly_quality' }) },
  { id: 'early-bird', emoji: '🌅', labelKey: 'fpc_ch_early_bird_label', descKey: 'fpc_ch_early_bird_desc', gen: () => ({ target: 10, descArgs: {}, metric: 'time' }) },
  { id: 'variety', emoji: '🎨', labelKey: 'fpc_ch_variety_label', descKey: 'fpc_ch_variety_desc', gen: () => ({ target: 3, descArgs: {}, metric: 'variety' }) },
];
...
// Nudge message keys, indexed by urgency level (0 = none/empty).
const NUDGE_KEYS = ['', 'fpc_nudge_1', 'fpc_nudge_2', 'fpc_nudge_3', 'fpc_nudge_4'];

// Quit rebuttal keys — interpolate {{duration}} / {{elapsed}} at render via t().
const QUIT_KEYS = ['fpc_quit_1', 'fpc_quit_2', 'fpc_quit_3', 'fpc_quit_4', 'fpc_quit_5'];
...
const scoreLabelKey = (s) => s >= 85 ? 'fpc_score_excellent' : s >= 70 ? 'fpc_score_good' : s >= 50 ? 'fpc_score_fair' : 'fpc_score_rough';
...
const FocusPocus = ({ tool }) => {
  ...
  const handleAiPatterns = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || t('fpc_err_analyze_patterns'));
    } finally {
      setAiPatternsLoading(false);
    }
  }, [sessionHistory, multiDayStreak, callToolEndpoint, setAiPatterns, t]);
  ...
  const handleBreakCoach = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || t('fpc_err_break_coaching'));
    } finally {
      setBreakCoachLoading(false);
    }
  }, [sessionNote, sessionActivity, elapsedMin, callToolEndpoint, t]);
  ...
  const deleteTemplate = useCallback((id) => {
    setSessionTemplates(prev => prev.filter(tpl => tpl.id !== id));
  }, [setSessionTemplates]);
  ...
  const generateChallenges = useCallback(() => {
    ...
    const available = CHALLENGE_TEMPLATES.filter(ct => {
      try { ct.gen(sessionHistory); return true; } catch { return false; }
    });
    const shuffled = [...available].sort(() => Math.random() - 0.5);
    const picked = shuffled.slice(0, 3).map(ct => ({
      ...ct,
      ...ct.gen(sessionHistory),
    }));
  ...
  const proactiveInsight = useMemo(() => {
    ...
    if (aiPatterns.peak_performance?.worst_time) {
      ...
      if ((worst.includes('afternoon') && currentHour >= 12 && currentHour <= 17) ||
        ...
        insights.push(t('fpc_insight_low_window'));
      }
    }
    if (aiPatterns.peak_performance?.best_time) {
      ...
      if ((best.includes('morning') && currentHour >= 6 && currentHour <= 11) ||
        ...
        insights.push(t('fpc_insight_peak_window'));
      }
    }

    // Distraction-based insight (strategy text is AI-generated, already localized server-side)
    if (aiPatterns.distraction_analysis?.strategy) {
      insights.push(`💡 ${aiPatterns.distraction_analysis.strategy}`);
    }

    return insights.length > 0 ? insights[0] : null;
  }, [aiPatterns, phase, sessionActivity, t]);
  ...
  const handleWantToQuit = () => {
    const idx = Math.floor(Math.random() * QUIT_KEYS.length);
    const msg = t(QUIT_KEYS[idx], {
      duration: t('fpc_minutes_val', { n: sessionDurationMin }),
      elapsed: t('fpc_minutes_val', { n: elapsedMin }),
    });
    setQuitRebuttal(msg);
  ...
  const recordSession = useCallback((aiResults, frozenElapsedMin, frozenOvertimeMin) => {
    const entry = {
      ...
      chain: chainConfig ? { labelKey: chainConfig.labelKey, index: chainIndex, total: chainConfig.sessions.length } : null,
  ...
  }, [sessionActivity, originalDurationMin, elapsedMin, overtimeFromOriginalMin, pauseCount, distractions, snoozeCount, accomplishment, sessionNote, chainConfig, chainIndex, setSessionHistory, setDailyStreak, setBodyInsights, updateMultiDayStreak, t]);
  ...
  const handleTakeBreak = async () => {
    ...
    try {
      ...
      setError(err.message || t('fpc_err_break_plan'));
    }
  };
  ...
  const loadExample = useCallback(() => {
    ...
    setSessionActivity(t(ex.activityKey));
    setSessionDurationMin(ex.sessionDurationMin);
    setUpcomingObligations(t(ex.obligationsKey));
    setMissedNeeds(t(ex.missedKey));
  }, [setSessionMode, setSessionActivity, setSessionDurationMin, setUpcomingObligations, setMissedNeeds, t]);
  ...
  const breakPlanText = results ? [
    ...
    t('fpc_copy_actions'), ...(results?.mandatory_actions || []).map((a, i) => `${i + 1}. ${a}`),
    '', t('fpc_copy_re_entry'), results?.re_entry, '', t('fpc_copy_next_session'), results?.next_session,
  ].join('\n') : '';
  const breakPlanTextWithBrand = breakPlanText + BRAND;

  // ── AI Patterns copy text ──
  const aiPatternText = aiPatterns ? [
    `${aiPatterns.focus_profile?.emoji || '🎯'} ${aiPatterns.focus_profile?.title || t('fpc_copy_focus_profile')}`,
    aiPatterns.focus_profile?.description || '',
    '', t('fpc_copy_peak', { time: aiPatterns.peak_performance?.best_time || '—' }),
    t('fpc_copy_growth', { trajectory: aiPatterns.growth?.trajectory || '—', early: aiPatterns.growth?.early_avg_score || '?', recent: aiPatterns.growth?.recent_avg_score || '?' }),
    '', t('fpc_copy_core_blocker', { blocker: aiPatterns.core_blocker || '—' }),
    t('fpc_copy_prescription', { prescription: aiPatterns.prescription || '—' }),
  ...
  const getPatterns = useCallback(() => {
    ...
    // Day distribution — key by day index (0–6); resolve to localized name at render.
    const dayCounts = {};
    recent.forEach(h => {
    ...
    return {
      ...
      peakDayKey: peakDay ? DAY_NAME_KEYS[peakDay[0]] : null,
  ...
  const urgencyColors = (level) => {
    const maps = [
      { bg: isDark ? 'bg-cyan-900/30' : 'bg-cyan-50', border: isDark ? 'border-cyan-700' : 'border-cyan-200', text: isDark ? 'text-cyan-300' : 'text-cyan-700', timerText: isDark ? 'text-cyan-300' : 'text-cyan-600', ring: isDark ? 'stroke-cyan-400' : 'stroke-cyan-500', ringBg: isDark ? 'stroke-zinc-700' : 'stroke-zinc-200' },
      { bg: isDark ? 'bg-amber-900/20' : 'bg-amber-50', border: isDark ? 'border-amber-700' : 'border-amber-200', text: isDark ? 'text-amber-300' : 'text-amber-700', timerText: isDark ? 'text-amber-300' : 'text-amber-600', ring: isDark ? 'stroke-amber-400' : 'stroke-amber-500', ringBg: isDark ? 'stroke-zinc-700' : 'stroke-zinc-200' },
      { bg: isDark ? 'bg-orange-900/20' : 'bg-orange-50', border: isDark ? 'border-orange-700' : 'border-orange-200', text: isDark ? 'text-orange-300' : 'text-orange-700', timerText: isDark ? 'text-orange-300' : 'text-orange-600', ring: isDark ? 'stroke-orange-400' : 'stroke-orange-500', ringBg: isDark ? 'stroke-zinc-700' : 'stroke-zinc-200' },
      { bg: isDark ? 'bg-red-900/20' : 'bg-red-50', border: isDark ? 'border-red-700' : 'border-red-200', text: isDark ? 'text-red-300' : 'text-red-700', timerText: isDark ? 'text-red-300' : 'text-red-600', ring: isDark ? 'stroke-red-400' : 'stroke-red-500', ringBg: isDark ? 'stroke-zinc-700' : 'stroke-zinc-200' },
      { bg: isDark ? 'bg-red-900/40' : 'bg-red-100', border: isDark ? 'border-red-600' : 'border-red-300', text: isDark ? 'text-red-200' : 'text-red-800', timerText: isDark ? 'text-red-200' : 'text-red-700', ring: isDark ? 'stroke-red-400' : 'stroke-red-600', ringBg: isDark ? 'stroke-zinc-700' : 'stroke-zinc-200' },
    ];
  ...
  useRegisterActions(buildFullText(), tool?.title || t('fpc_title'));
    ...
    return (
      <div className="relative w-56 h-56 mx-auto">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          {isOvertime && <span className={`text-xs font-bold uppercase tracking-wider mb-1 ${uc.text}`}>{t('fpc_overtime')}</span>}
          <span className={`text-4xl font-mono font-bold tracking-tight ${uc.timerText}`}>{fmt(seconds)}</span>
          {!isOvertime && <span className={`text-xs ${c.textMuteded} mt-1`}>{t('fpc_remaining')}</span>}
        </div>
      </div>
    );
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-base ${c.textSecondary}`}>
                <span className="me-2 text-lg">{tool?.icon ?? '🎩'}</span>{tool?.tagline ?? t('fpc_tagline')}
              </p>
                ...
                <>
                  {multiDayStreak.currentStreak >= 2 && (
                    <div className={`flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-bold ${isDark ? 'bg-amber-900/30 text-amber-300' : 'bg-amber-50 text-amber-700'}`}>
                      {t('fpc_streak_days', { n: multiDayStreak.currentStreak })}
                    </div>
                  )}
                  <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold ${isDark ? 'bg-cyan-900/30 text-cyan-300' : 'bg-cyan-50 text-cyan-700'}`}>
                    {t('fpc_today_summary', { count: todaySessions, min: todayMinutes })}
                  </div>
                </>
              )}
            </div>
        ...
        <div className="space-y-4">
          ...
          {!hasVisited && (
            <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
              <h3 className={`text-lg font-bold ${c.text} mb-2`}>{t('fpc_welcome_title')}</h3>
              <p className={`text-sm ${c.textSecondary} mb-4`}>
                {t('fpc_welcome_desc')}
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
                {[
                  { emoji: '⏱️', titleKey: 'fpc_welcome_f1_title', descKey: 'fpc_welcome_f1_desc' },
                  { emoji: '📊', titleKey: 'fpc_welcome_f2_title', descKey: 'fpc_welcome_f2_desc' },
                  { emoji: '📱', titleKey: 'fpc_welcome_f3_title', descKey: 'fpc_welcome_f3_desc' },
                  { emoji: '🧠', titleKey: 'fpc_welcome_f4_title', descKey: 'fpc_welcome_f4_desc' },
                ].map((f, i) => (
                  <div key={i} className={`flex items-start gap-2 p-2.5 rounded-xl ${isDark ? 'bg-zinc-700/50' : 'bg-zinc-50'}`}>
                    <span className="text-lg">{f.emoji}</span>
                    <div>
                      <p className={`text-sm font-bold ${c.text}`}>{t(f.titleKey)}</p>
                      <p className={`text-xs ${c.textMuteded}`}>{t(f.descKey)}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Activity */}
          <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
            <label className={`block text-lg font-bold ${c.text} mb-3`}>{t('fpc_activity_label')} <span className={c.required}>*</span></label>
            <input
              type="text" value={sessionActivity}
              onChange={e => setSessionActivity(e.target.value)}
              placeholder={t('fpc_activity_ph')}
              className={`w-full p-3 border rounded-xl outline-none focus:ring-2 transition-colors text-sm ${c.input}`}
            />
            {sessionActivity.trim() && <p className={`text-xs ${c.textMuteded} mt-2`}>{t('fpc_press_enter')}</p>}

            {/* Favorite activities */}
            {favoriteActivities.length > 0 && !sessionActivity.trim() && (
              <div className="mt-3">
                <p className={`text-xs font-bold ${c.textMuteded} mb-1.5`}>{t('fpc_favorites_label')}</p>
            ...
            {recentActivities.length > 0 && !sessionActivity.trim() && favoriteActivities.length === 0 && (
              <div className="mt-3">
                <p className={`text-xs font-bold ${c.textMuteded} mb-1.5`}>{t('fpc_recent_label')}</p>
                <div className="flex flex-wrap gap-1.5">
            ...
            {sessionActivity.trim() && (
              <button onClick={() => toggleFavorite(sessionActivity.trim())}
                ...
                {favoriteActivities.includes(sessionActivity.trim()) ? t('fpc_favorited') : t('fpc_add_favorite')}
              </button>
            )}
            ...
            {lastNoteForActivity && (
              <div className={`mt-3 p-3 rounded-xl flex items-start gap-2 ${isDark ? 'bg-cyan-900/20 border border-cyan-800/40' : 'bg-cyan-50 border border-cyan-100'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-[10px] font-bold ${isDark ? 'text-cyan-400' : 'text-cyan-600'} mb-0.5`}>{t('fpc_last_left_off')}</p>
          ...
          <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
            ...
            <div className="flex gap-1 mb-4">
              {[
                { id: 'single', labelKey: 'fpc_mode_single' },
                { id: 'chain', labelKey: 'fpc_mode_chain' },
              ].map(m => (
            ...
            {sessionMode === 'single' && (
              <>
                <label className={`block text-sm font-bold ${c.text} mb-3`}>{t('fpc_session_length')}</label>
                    ...
                    <button key={p.minutes}
                      ...
                      className={`px-4 py-2.5 rounded-xl text-sm font-bold border transition-all ${
                        sessionDurationMin === p.minutes && !customMinutes
                          ? isDark ? 'border-cyan-500 bg-cyan-900/40 text-cyan-300 ring-1 ring-cyan-500/30' : 'border-cyan-400 bg-cyan-50 text-cyan-700 ring-1 ring-cyan-200'
            ...
            {sessionMode === 'chain' && (
              <>
                <label className={`block text-sm font-bold ${c.text} mb-2`}>{t('fpc_choose_chain')}</label>
                <Hint id="chains">{t(FEATURE_HINT_KEYS.chains)}</Hint>
                  ...
                  {POMODORO_CHAINS.map((ch, i) => (
                    <button key={i}
                      ...
                      <div className={`text-xs ${c.textMuteded} mt-0.5`}>
                        {t('fpc_chain_meta', { count: ch.sessions.length, min: ch.breakMin })}
                      </div>
                    </button>
                  ))}
                </div>
                {!sessionActivity.trim() && (
                  <p className={`text-xs ${c.textMuteded} mt-2`}>{t('fpc_chain_enter_activity')}</p>
                )}
              </>
            )}
          </div>
          ...
          {(sessionTemplates.length > 0 || sessionActivity.trim()) && (
            <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
              <div className="flex items-center justify-between mb-3">
                <label className={`text-sm font-bold ${c.text}`}>{t('fpc_templates_title')}</label>
                {sessionActivity.trim() && (
                  <button onClick={() => setShowTemplateForm(!showTemplateForm)}
                    className={`text-xs font-bold ${isDark ? 'text-cyan-400' : 'text-cyan-600'}`}>
                    {showTemplateForm ? t('fpc_templates_cancel') : t('fpc_templates_save_current')}
                  </button>
                )}
              </div>
              ...
              {showTemplateForm && sessionActivity.trim() && (
                <div className={`mb-3 p-3 rounded-xl border ${isDark ? 'bg-zinc-700/50 border-zinc-600' : 'bg-zinc-50 border-zinc-200'}`}>
                  <p className={`text-xs ${c.textSecondary} mb-2`}>{t('fpc_templates_save_prompt', { activity: sessionActivity.trim(), min: sessionDurationMin })}</p>
              ...
              {sessionTemplates.length > 0 ? (
                    ...
                    <div key={tpl.id} className={`flex items-center gap-2 p-2.5 rounded-xl transition-colors cursor-pointer group ${isDark ? 'hover:bg-zinc-700/50' : 'hover:bg-zinc-50'}`}>
                      <button onClick={() => { loadTemplate(tpl); }}
                        ...
                        <div className="min-w-0">
                          ...
                          <p className={`text-[10px] ${c.textMuteded}`}>{tpl.context ? t('fpc_template_meta_ctx', { min: tpl.duration, context: tpl.context }) : t('fpc_template_meta', { min: tpl.duration })}</p>
                        </div>
                      </button>
                ...
                <p className={`text-xs ${c.textMuteded}`}>{t('fpc_templates_empty')}</p>
              )}
            </div>
          )}
          ...
          <div className={`${c.card} ${c.border} border rounded-xl p-4 flex items-center gap-3`}>
            ...
            <div className="flex-1">
              <p className={`text-sm font-bold ${c.text}`}>{t('fpc_need_sounds_title')}</p>
              <p className={`text-xs ${c.textMuteded}`}>{t('fpc_need_sounds_desc')}</p>
            </div>
            <a href="/FocusSoundArchitect" className={`text-xs ${linkStyle}`}>{t('fpc_sound_architect_link')}</a>
          </div>
          ...
          {sessionHistory.length > 0 && (
            <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
              <div className="flex items-center justify-between mb-3">
                <h4 className={`text-sm font-bold ${c.text}`}>{t('fpc_focus_data')}</h4>
              ...
              <div className="flex gap-3 mb-3">
                <div className={`text-center px-3 py-2 rounded-lg ${isDark ? 'bg-zinc-700/50' : 'bg-zinc-50'}`}>
                  ...
                  <div className={`text-[10px] ${c.textMuteded}`}>{t('fpc_stat_sessions')}</div>
                </div>
                <div className={`text-center px-3 py-2 rounded-lg ${isDark ? 'bg-zinc-700/50' : 'bg-zinc-50'}`}>
                  <div className={`text-lg font-bold ${c.text}`}>{Math.round(sessionHistory.reduce((s, h) => s + h.actualMin, 0) / 60)}h</div>
                  <div className={`text-[10px] ${c.textMuteded}`}>{t('fpc_stat_total')}</div>
                </div>
                <div className={`text-center px-3 py-2 rounded-lg ${isDark ? 'bg-zinc-700/50' : 'bg-zinc-50'}`}>
                  ...
                  <div className={`text-[10px] ${c.textMuteded}`}>{t('fpc_stat_avg_score')}</div>
                </div>
              </div>
              {/* Recent sessions */}
                    ...
                    <div key={h.id} className={`flex items-center gap-3 p-3 rounded-xl ${isDark ? 'bg-zinc-700/50' : 'bg-zinc-50'}`}>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs ${c.textMuteded}`}>
                          ...
                          {h.chain && ` · ${t('fpc_hist_chain', { index: h.chain.index + 1, total: h.chain.total })}`}
                          {h.distractions > 0 && ` · ${t('fpc_hist_distractions', { count: h.distractions })}`}
                          {h.note && ` · 🔖`}
                        </p>
                      </div>
              ...
              {sessionHistory.length < 5 && (
                <p className={`text-xs ${c.textMuteded} mt-2`}>
                  {t(5 - sessionHistory.length > 1 ? 'fpc_unlock_patterns_other' : 'fpc_unlock_patterns_one', { n: 5 - sessionHistory.length })}
                </p>
              )}
            </div>
          )}
        ...
        return (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              <h3 className={`text-xl font-bold ${c.text}`}>{t('fpc_patterns_title')}</h3>
              <button onClick={() => setPhase('setup')} className={`text-sm font-bold ${c.textMuteded} hover:${c.text}`}>{t('fpc_back')}</button>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <Pill emoji="🎯" label={t('fpc_pill_sessions')} value={p.totalSessions} />
              <Pill emoji="⏱️" label={t('fpc_pill_total_time')} value={t('fpc_pill_total_time_val', { hours: Math.round(p.totalMin / 60), min: p.totalMin % 60 })} />
              <Pill emoji="📊" label={t('fpc_pill_avg_score')} value={t('fpc_pill_avg_score_val', { score: p.avgScore, label: t(scoreLabelKey(p.avgScore)) })} color={scoreColor(p.avgScore, isDark)} />
              <Pill emoji="⏰" label={t('fpc_pill_avg_duration')} value={t('fpc_pill_avg_duration_val', { min: p.avgDuration })} />
              <Pill emoji="✅" label={t('fpc_pill_completion')} value={`${p.completionRate}%`} color={p.completionRate >= 70 ? (isDark ? 'text-emerald-400' : 'text-emerald-600') : (isDark ? 'text-amber-400' : 'text-amber-600')} />
              {p.peakHour && <Pill emoji="🕐" label={t('fpc_pill_peak_hour')} value={p.peakHour} />}
              {p.peakDayKey && <Pill emoji="📅" label={t('fpc_pill_peak_day')} value={t(p.peakDayKey)} />}
              {p.topDistraction && <Pill emoji="📱" label={t('fpc_pill_top_distractor')} value={t('fpc_pill_top_distractor_val', { label: t(`fpc_distract_${p.topDistraction}`), count: p.topDistractionCount })} />}
              {p.accomplishmentRate !== null && <Pill emoji="🎉" label={t('fpc_pill_goals_met')} value={`${p.accomplishmentRate}%`} />}
            </div>
            ...
            {chartData && (
              <>
                {/* Score Trend Line */}
                <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                  <h4 className={`text-sm font-bold ${c.text} mb-3`}>{t('fpc_chart_score_trend')}</h4>
                  <div className="h-48">
                ...
                {chartData.durationVsScore.length >= 5 && (
                  <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                    <h4 className={`text-sm font-bold ${c.text} mb-1`}>{t('fpc_chart_duration_score')}</h4>
                    <p className={`text-[10px] ${c.textMuteded} mb-3`}>{t('fpc_chart_duration_score_sub')}</p>
                    <div className="h-48">
                      <ResponsiveContainer width="100%" height="100%">
                        <ScatterChart margin={{ top: 5, right: 10, bottom: 5, left: -20 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke={isDark ? 'rgb(63,63,70)' : 'rgb(231,229,228)'} />
                          <XAxis dataKey="duration" name={t('fpc_chart_axis_duration')} unit="m" tick={{ fill: isDark ? 'rgb(161,161,170)' : 'rgb(120,113,108)', fontSize: 10 }} />
                          <YAxis dataKey="score" name={t('fpc_chart_axis_score')} domain={[0, 100]} tick={{ fill: isDark ? 'rgb(161,161,170)' : 'rgb(120,113,108)', fontSize: 10 }} />
                          <Tooltip
                            contentStyle={{ background: isDark ? isDark ? 'bg-zinc-800' : 'rgb(248,250,252)' : 'rgb(255,255,255)', border: `1px solid ${isDark ? 'rgb(63,63,70)' : 'rgb(231,229,228)'}`, borderRadius: 8, fontSize: 12 }}
                            formatter={(val, name) => [val, name === 'duration' ? t('fpc_chart_tooltip_duration') : t('fpc_chart_axis_score')]}
                          />
                ...
                {/* Activity Heatmap */}
                <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                  <h4 className={`text-sm font-bold ${c.text} mb-1`}>{t('fpc_heatmap_activity')}</h4>
                  <p className={`text-[10px] ${c.textMuteded} mb-3`}>{t('fpc_heatmap_activity_sub')}</p>
                  <div className="overflow-x-auto">
                    <div className="min-w-[400px]">
                      ...
                      {DAY_NAME_KEYS.map((dayKey, dayIdx) => {
                        const day = t(dayKey);
                              ...
                              {chartData.activityHeatmap[dayIdx].map((count, hourIdx) => {
                                ...
                                return (
                                  <div key={hourIdx}
                                    ...
                                    title={t(count !== 1 ? 'fpc_heatmap_cell_session_other' : 'fpc_heatmap_cell_session_one', { day, hour: hourIdx, count })}
                                    style={{
                ...
                {sessionHistory.some(h => (h.distractions || 0) > 0) && (
                  <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                    <h4 className={`text-sm font-bold ${c.text} mb-1`}>{t('fpc_heatmap_distraction')}</h4>
                            ...
                            <div key={dayKey} className="flex items-center gap-1 mb-0.5">
                              ...
                              <div className="flex-1 flex gap-[1px]">
                                {chartData.heatmap[dayIdx].map((count, hourIdx) => (
                                  <div key={hourIdx}
                                    className="flex-1 h-4 rounded-[2px] transition-colors"
                                    title={t(count !== 1 ? 'fpc_heatmap_cell_distraction_other' : 'fpc_heatmap_cell_distraction_one', { day, hour: hourIdx, count })}
                                    style={{
                                      backgroundColor: count === 0
                                        ? (isDark ? isDark ? 'bg-zinc-800' : 'rgb(248,250,252)' : '#f5f5f4')
                                        : isDark
                ...
                {chartData.chainStats && (
                  <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                    <h4 className={`text-sm font-bold ${c.text} mb-3`}>{t('fpc_chain_analytics')}</h4>
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div className={`text-center p-3 rounded-xl ${isDark ? 'bg-zinc-700/50' : 'bg-zinc-50'}`}>
                        ...
                        <div className={`text-[10px] ${c.textMuteded}`}>{t('fpc_chain_sessions_label')}</div>
                      </div>
                      <div className={`text-center p-3 rounded-xl ${isDark ? 'bg-zinc-700/50' : 'bg-zinc-50'}`}>
                        ...
                        <div className={`text-[10px] ${c.textMuteded}`}>{t('fpc_chain_completion_label')}</div>
                      </div>
                    </div>
                    <p className={`text-xs font-bold ${c.textSecondary} mb-2`}>{t('fpc_perf_by_position')}</p>
                    <div className="space-y-1.5">
                      {chartData.chainStats.positionData.map(pos => (
                        <div key={pos.position} className={`flex items-center gap-3 p-2.5 rounded-lg ${isDark ? 'bg-zinc-700/30' : 'bg-zinc-50/80'}`}>
                          <span className={`text-xs font-bold ${c.text} w-20`}>{t('fpc_position_label', { n: pos.position })}</span>
                          <div className="flex-1 flex items-center gap-3">
                            <div className="flex items-center gap-1">
                              <span className={`text-xs ${c.textMuteded}`}>{t('fpc_score_colon')}</span>
                    ...
                    {chartData.chainStats.positionData.length > 1 && (() => {
                      ...
                      if (first && last && last.avgScore < first.avgScore - 5) {
                        return <p className={`text-xs ${isDark ? 'text-amber-400' : 'text-amber-600'} mt-2`}>{t('fpc_chain_perf_drop', { points: first.avgScore - last.avgScore })}</p>;
                      }
                      if (first && last && last.avgScore > first.avgScore + 3) {
                        return <p className={`text-xs ${isDark ? 'text-emerald-400' : 'text-emerald-600'} mt-2`}>{t('fpc_chain_perf_warmup')}</p>;
                      }
                      return null;
                    })()}
                  </div>
            ...
            {Object.keys(bodyInsights).length > 0 && (
              <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                <h4 className={`text-sm font-bold ${c.text} mb-3`}>{t('fpc_body_patterns')}</h4>
                <div className="space-y-2">
                  {Object.entries(bodyInsights).map(([key, vals]) => (
                    <div key={key} className={`flex items-center gap-2 text-sm ${c.textSecondary}`}>
                      ...
                      <span className={`font-bold ${c.text}`}>{BODY_LABEL_KEYS[key] ? t(BODY_LABEL_KEYS[key]) : key}:</span>
            ...
            {/* Insights */}
            <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
              <h4 className={`text-sm font-bold ${c.text} mb-3`}>{t('fpc_insights')}</h4>
              <div className="space-y-2">
                {p.avgDuration < 30 && <p className={`text-sm ${c.textSecondary}`}>{t('fpc_insight_short_avg', { min: p.avgDuration })}</p>}
                {p.avgDuration > 60 && <p className={`text-sm ${c.textSecondary}`}>{t('fpc_insight_long_avg', { min: p.avgDuration })}</p>}
                {p.topDistraction === 'phone' && <p className={`text-sm ${c.textSecondary}`}>{t('fpc_insight_phone')}</p>}
                {p.completionRate < 60 && <p className={`text-sm ${c.textSecondary}`}>{t('fpc_insight_low_completion', { pct: p.completionRate })}</p>}
                {p.completionRate >= 90 && <p className={`text-sm ${c.textSecondary}`}>{t('fpc_insight_high_completion', { pct: p.completionRate })}</p>}
                {p.peakHour && <p className={`text-sm ${c.textSecondary}`}>{t('fpc_insight_peak_hour', { hour: p.peakHour })}</p>}
              </div>
            </div>
            ...
            {earnedMilestones.length > 0 && (
              <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                <h4 className={`text-sm font-bold ${c.text} mb-3`}>{t('fpc_milestones')}</h4>
                  ...
                  {MILESTONES.map(m => {
                    ...
                    return (
                      <div key={m.id} className={`flex flex-col items-center p-3 rounded-xl border text-center transition-colors ${
                        ...
                        <span className={`text-xs font-bold ${earned ? c.text : c.textMuteded}`}>{t(m.labelKey)}</span>
                        {m.descKey && <span className={`text-[10px] ${c.textMuteded}`}>{t(m.descKey)}</span>}
                      </div>
                    );
                  })}
                </div>
            ...
            {sessionHistory.length >= 5 && (
              <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                <div className="flex items-center justify-between mb-3">
                  <h4 className={`text-sm font-bold ${c.text}`}>{t('fpc_weekly_challenges')}</h4>
                  {weeklyChallenges.challenges.length > 0 && (
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${isDark ? 'bg-cyan-900/30 text-cyan-300' : 'bg-cyan-50 text-cyan-700'}`}>
                      {t('fpc_challenges_done', { done: weeklyChallenges.completed.length, total: weeklyChallenges.challenges.length })}
                    </span>
                  )}
                </div>
                {weeklyChallenges.challenges.length === 0 ? (
                  <div className="text-center py-2">
                    <p className={`text-xs ${c.textSecondary} mb-3`}>{t('fpc_challenges_intro')}</p>
                    ...
                    {weeklyChallenges.challenges.map(ch => {
                      ...
                      return (
                        <div key={ch.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`text-sm font-bold ${done ? (isDark ? 'text-emerald-300 line-through' : 'text-emerald-700 line-through') : c.text}`}>{t(ch.labelKey)}</p>
                            <p className={`text-xs ${c.textMuteded}`}>{t(ch.descKey, ch.descArgs || {})}</p>
                          </div>
                        </div>
                      );
                    })}
            ...
            <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
              <div className="flex items-center justify-between mb-3">
                <h4 className={`text-sm font-bold ${c.text}`}>{t('fpc_ai_patterns')}</h4>

              </div>
              {!aiPatterns ? (
                <div className="text-center py-2">
                  <p className={`text-xs ${c.textSecondary} mb-3`}>{t('fpc_ai_intro')}</p>
                ...
                <div className="space-y-3">
                  ...
                  {aiPatterns.peak_performance && (
                    <div className={`p-3 rounded-xl ${isDark ? 'bg-emerald-900/20 border border-emerald-800/40' : 'bg-emerald-50 border border-emerald-100'}`}>
                      <p className={`text-xs font-bold ${isDark ? 'text-emerald-300' : 'text-emerald-700'} mb-2`}>{t('fpc_ai_peak_performance')}</p>
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div><span className={c.textMuteded}>{t('fpc_ai_best_time')}</span> <span className={c.text}>{aiPatterns.peak_performance.best_time}</span></div>
                        <div><span className={c.textMuteded}>{t('fpc_ai_worst_time')}</span> <span className={c.text}>{aiPatterns.peak_performance.worst_time}</span></div>
                        <div className="col-span-2"><span className={c.textMuteded}>{t('fpc_ai_sweet_spot')}</span> <span className={c.text}>{aiPatterns.peak_performance.optimal_duration}</span></div>
                      </div>
                  ...
                  {aiPatterns.distraction_analysis && (
                    <div className={`p-3 rounded-xl ${isDark ? 'bg-amber-900/20 border border-amber-800/40' : 'bg-amber-50 border border-amber-100'}`}>
                      <p className={`text-xs font-bold ${isDark ? 'text-amber-300' : 'text-amber-700'} mb-2`}>{t('fpc_ai_distraction_analysis')}</p>
                      <p className={`text-xs ${c.textSecondary}`}><strong className={c.text}>{t('fpc_ai_trigger')}</strong> {aiPatterns.distraction_analysis.primary_trigger}</p>
                      <p className={`text-xs ${c.textSecondary}`}><strong className={c.text}>{t('fpc_ai_pattern')}</strong> {aiPatterns.distraction_analysis.pattern}</p>
                  ...
                  {aiPatterns.growth && (
                    <div className={`p-3 rounded-xl ${isDark ? 'bg-cyan-900/20 border border-cyan-800/40' : 'bg-cyan-50 border border-cyan-100'}`}>
                      <p className={`text-xs font-bold ${isDark ? 'text-cyan-300' : 'text-cyan-700'} mb-2`}>{t('fpc_ai_growth')}</p>
                  ...
                  {aiPatterns.core_blocker && (
                    <div className={`p-3 rounded-xl ${isDark ? 'bg-red-900/20 border border-red-800/40' : 'bg-red-50 border border-red-100'}`}>
                      <p className={`text-xs font-bold ${isDark ? 'text-red-300' : 'text-red-700'} mb-1`}>{t('fpc_ai_core_blocker')}</p>
                      <p className={`text-xs ${c.textSecondary}`}>{aiPatterns.core_blocker}</p>
                    </div>
                  )}
                  {aiPatterns.prescription && (
                    <div className={`p-3 rounded-xl ${isDark ? 'bg-emerald-900/20 border border-emerald-800/40' : 'bg-emerald-50 border border-emerald-100'}`}>
                      <p className={`text-xs font-bold ${isDark ? 'text-emerald-300' : 'text-emerald-700'} mb-1`}>{t('fpc_ai_prescription')}</p>
                      <p className={`text-xs ${c.textSecondary}`}>{aiPatterns.prescription}</p>
                    </div>
                  )}
                  {aiPatterns.weekly_strategy && (
                    <div className={`p-3 rounded-xl ${isDark ? 'bg-cyan-900/20 border border-cyan-800/40' : 'bg-cyan-50 border border-cyan-100'}`}>
                      <p className={`text-xs font-bold ${isDark ? 'text-cyan-300' : 'text-cyan-700'} mb-1`}>{t('fpc_ai_weekly_strategy')}</p>
                  ...
                  <button onClick={() => { setAiPatterns(null); handleAiPatterns(); }}
                    ...
                    className={`w-full text-xs font-bold py-2 rounded-lg transition-colors ${isDark ? 'text-zinc-400 hover:text-zinc-200' : 'text-zinc-400 hover:text-zinc-600'} disabled:opacity-40`}>
                    {t('fpc_ai_reanalyze')}
                  </button>
                </div>
              )}
      ...
      {(phase === 'active' || phase === 'paused') && (
        <div className="space-y-4">
          <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-6 sm:p-8 text-center`}>
            ...
            <div className={`mt-4 p-3 rounded-xl ${isDark ? 'bg-zinc-700/30' : 'bg-zinc-50'}`}>
              <div className="flex items-center justify-between mb-1">
                <span className={`text-xs font-bold ${c.textSecondary}`}>{t('fpc_distraction_log')}</span>
                {distractions.length > 0 && (
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${isDark ? 'bg-amber-900/30 text-amber-300' : 'bg-amber-50 text-amber-700'}`}>
                    {t('fpc_logged', { count: distractions.length })}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-1.5">
                {DISTRACTION_TYPES.map(d => (
                  <button key={d.id} onClick={() => logDistraction(d.id)}
                    ...
                    <span className={c.textMuteded}>{t(d.labelKey)}</span>
                  </button>
                ))}
              </div>
              {distractions.length === 0 && !dismissedHints.distractions && (
                <p className={`text-xs ${c.textMuteded} mt-1.5`}>{t('fpc_distract_hint')}</p>
              )}
            </div>
              ...
              <div className="flex items-center gap-1.5">
                {SOUND_PRESETS.map(p => (
                  <button key={p.id} onClick={() => toggleAmbient(p.id)}
                    ...
                    <span className={activeAmbient === p.id ? '' : c.textMuteded}>{t(p.labelKey)}</span>
                  </button>
                ))}
              </div>
              {activeAmbient && (
          ...
          <div className={`flex items-center justify-between text-xs ${c.textMuteded} px-1`}>
            <span>{t('fpc_min_elapsed', { min: elapsedMin })}</span>
            <span>{distractions.length > 0 ? t('fpc_min_session_distractions', { min: sessionDurationMin, count: distractions.length }) : t('fpc_min_session', { min: sessionDurationMin })}</span>
          </div>
        </div>
      )}
          ...
          <div className="space-y-4">
            <div className={`${uc.cardAlt} border-2 ${uc.border} rounded-xl shadow-sm p-6 sm:p-8 text-center transition-colors duration-500`}>
              ...
              <p className={`text-lg font-bold mb-4 ${uc.text}`}>
                {(() => { const nk = NUDGE_KEYS[Math.min(currentUrgency, NUDGE_KEYS.length - 1)]; return nk ? t(nk) : ''; })()}
              </p>
              <p className={`text-sm mb-4 ${c.textSecondary}`}>
                {t('fpc_overtime_working', { activity: sessionActivity, min: elapsedMin })}
                {overtimeFromOriginalMin > 0 && <> {t('fpc_overtime_past', { min: overtimeFromOriginalMin })}</>}
              </p>
              <RingTimer seconds={-Math.floor(overtimeMs / 1000)} progressVal={1} urgency={currentUrgency} />

              {/* Distraction count during overtime */}
              {distractions.length > 0 && (
                <p className={`text-xs mt-2 ${c.textMuteded}`}>{t(distractions.length > 1 ? 'fpc_overtime_distractions_other' : 'fpc_overtime_distractions_one', { count: distractions.length })}</p>
              )}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-6">
                ...
                {snoozeCount < MAX_SNOOZES && (
                  <button onClick={handleSnooze}
                    ...
                    {t('fpc_five_more', { n: MAX_SNOOZES - snoozeCount })}
                  </button>
                )}
                {snoozeCount >= MAX_SNOOZES && (
                  <p className={`text-xs font-bold ${uc.text}`}>{t('fpc_no_extensions')}</p>
                )}
              </div>
              {error && (
        ...
        <div className="space-y-4">
          <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-6 text-center`}>
            ...
            <h3 className={`text-xl font-bold ${c.text} mb-2`}>
              {t('fpc_break_time_complete', { index: chainIndex + 1, total: chainConfig?.sessions.length })}
            </h3>
            <p className={`text-sm ${c.textSecondary} mb-4`}>
              {t('fpc_next_starts_in', { time: fmt(breakRemainingSec) })}
            </p>
        ...
        <div className="space-y-4">
          ...
          <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-xs ${c.textMuteded}`}>
                  {t('fpc_session_min_total', { min: elapsedMin })}{overtimeFromOriginalMin > 0 && ` ${t('fpc_session_overtime_paren', { min: overtimeFromOriginalMin })}`}
                  {snoozeCount > 0 && ` · ${t(snoozeCount > 1 ? 'fpc_snooze_count_other' : 'fpc_snooze_count_one', { count: snoozeCount })}`}
                  {distractions.length > 0 && ` · ${t(distractions.length > 1 ? 'fpc_distraction_count_other' : 'fpc_distraction_count_one', { count: distractions.length })}`}
                </p>
              </div>
              {sessionHistory.length > 0 && (() => {
                ...
                return s != null ? (
                  <div className="text-center">
                    ...
                    <div className={`text-[10px] font-bold ${scoreColor(s, isDark)}`}>{t(scoreLabelKey(s))}</div>
                  </div>
                ) : null;
              })()}
            </div>
            ...
            <div className={`mt-3 pt-3 border-t ${isDark ? 'border-zinc-700' : 'border-zinc-200'}`}>
              <p className={`text-xs font-bold ${c.textMuteded} mb-2`}>{t('fpc_accomplish_q')}</p>
              {[{ v: 'yes', labelKey: 'fpc_accomplish_yes' }, { v: 'partial', labelKey: 'fpc_accomplish_partial' }, { v: 'no', labelKey: 'fpc_accomplish_no' }].map(opt => (
                <button key={opt.v}
                  onClick={() => {
              ...
              {sessionHistory.length <= 3 && (
                <Hint id="accomplishment">
                  {t(FEATURE_HINT_KEYS.accomplishment)}
                </Hint>
              )}
            </div>
          </div>
          ...
          {results ? (
            <>
              ...
              {results?.mandatory_actions?.length > 0 && (
                <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                  <h4 className={`text-sm font-bold ${c.text} mb-3 flex items-center gap-2`}>
                    <span className={isDark ? 'text-amber-400' : 'text-amber-600'}>⚠️</span> {t('fpc_do_before_continue')}
                  </h4>
                  <div className="space-y-2">
              ...
              {results?.body_check && (
                <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                  <h4 className={`text-sm font-bold ${c.text} mb-3`}>{t('fpc_body_check')}</h4>
                    ...
                    {Object.entries(results?.body_check).map(([key, val]) => (
                      <div key={key} className={`p-3 rounded-xl border ${isDark ? 'bg-zinc-700/50 border-zinc-600' : 'bg-zinc-50 border-zinc-200'}`}>
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <span className={`text-xs font-bold ${c.text}`}>{BODY_LABEL_KEYS[key] ? t(BODY_LABEL_KEYS[key]) : key}</span>
                        </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {results?.re_entry && (
                  <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                    <h4 className={`text-sm font-bold ${c.text} mb-2 flex items-center gap-2`}>
                      <span className={isDark ? 'text-cyan-400' : 'text-cyan-600'}>🔖</span> {t('fpc_re_entry_plan')}
                    </h4>
                ...
                {results?.next_session && (
                  <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
                    <h4 className={`text-sm font-bold ${c.text} mb-2 flex items-center gap-2`}>
                      <span className={isDark ? 'text-emerald-400' : 'text-emerald-600'}>→</span> {t('fpc_next_session')}
                    </h4>
            ...
            <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-6 text-center`}>
              <h3 className={`text-xl font-bold ${c.text} mb-2`}>{t('fpc_session_ended')}</h3>
              <p className={`text-sm ${c.textSecondary} mb-4`}>
                {t('fpc_session_ended_desc')}
              </p>
            </div>
          )}
          ...
          <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
            <div className="flex items-center justify-between mb-3">
              <h4 className={`text-sm font-bold ${c.text} flex items-center gap-1.5`}>{t('fpc_break_timer')}</h4>
            ...
            {!breakEndTime || breakRemainingMs <= 0 ? (
              ...
              <p className={`text-sm font-bold ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>{t('fpc_break_complete')}</p>
            ) : (
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setBreakEndTime(null)}
                  className={`text-xs font-bold ${c.textMuteded}`}>{t('fpc_cancel')}</button>
              </div>
            )}
          </div>
          ...
          <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
            <div className="flex items-center justify-between mb-3">
              <h4 className={`text-sm font-bold ${c.text} flex items-center gap-1.5`}>{t('fpc_break_coach')}</h4>
            </div>
            {!breakCoachResult ? (
              <div className="text-center">
              ...
              <div className="space-y-3">
                {breakCoachResult.acknowledgment && (
                  <div className={`p-3 rounded-xl ${isDark ? 'bg-cyan-900/20 border border-cyan-800/40' : 'bg-cyan-50 border border-cyan-100'}`}>
                    <p className={`text-xs font-bold ${isDark ? 'text-cyan-300' : 'text-cyan-700'} mb-1`}>{t('fpc_coach_acknowledgment')}</p>
                    <p className={`text-xs ${c.textSecondary}`}>{breakCoachResult.acknowledgment}</p>
                  </div>
                )}
                {breakCoachResult.physical_reset && (
                  <div className={`p-3 rounded-xl ${isDark ? 'bg-emerald-900/20 border border-emerald-800/40' : 'bg-emerald-50 border border-emerald-100'}`}>
                    <p className={`text-xs font-bold ${isDark ? 'text-emerald-300' : 'text-emerald-700'} mb-1`}>{t('fpc_coach_physical_reset')}</p>
                    <p className={`text-xs ${c.textSecondary}`}>{breakCoachResult.physical_reset}</p>
                  </div>
                )}
                {breakCoachResult.mental_transition && (
                  <div className={`p-3 rounded-xl ${isDark ? 'bg-cyan-900/20 border border-cyan-800/40' : 'bg-cyan-50 border border-cyan-100'}`}>
                    <p className={`text-xs font-bold ${isDark ? 'text-cyan-300' : 'text-cyan-700'} mb-1`}>{t('fpc_coach_mental_transition')}</p>
                    <p className={`text-xs ${c.textSecondary}`}>{breakCoachResult.mental_transition}</p>
                  </div>
                )}
                {breakCoachResult.resume_tip && (
                  <div className={`p-3 rounded-xl ${isDark ? 'bg-amber-900/20 border border-amber-800/40' : 'bg-amber-50 border border-amber-100'}`}>
                    <p className={`text-xs font-bold ${isDark ? 'text-amber-300' : 'text-amber-700'} mb-1`}>{t('fpc_coach_resume_tip')}</p>
          ...
          <div className={`${c.cardAlt} border ${c.border} rounded-xl p-4`}>
            <p className={`text-[10px] font-bold ${c.textMuted} uppercase mb-2`}>{t('fpc_related_tools')}</p>
            <div className="flex flex-wrap gap-3">
              <a href="/FocusSoundArchitect" className={`text-xs ${linkStyle}`}>{t('fpc_xref_sound_architect')}</a>
              <a href="/TaskAvalancheBreaker" className={`text-xs ${linkStyle}`}>{t('fpc_xref_task_avalanche')}</a>
            </div>
          </div>
        </div>
      )}
