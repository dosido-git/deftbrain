...
const STORE_KEYS = {
  audit: 'deftbrain_apology_audit',
  repairs: 'deftbrain_apology_repairs',
};
...
const ApologyCalibrator = () => {
  ...
  // Audit
  const [auditSituations, setAuditSituations] = useState(() => loadStore(STORE_KEYS.audit));
  ...
  // Repairs
  const [repairs, setRepairs] = useState(() => loadStore(STORE_KEYS.repairs));
  const [repairForm, setRepairForm] = useState({ who: '', what: '', apologyGiven: '', commitments: '', date: new Date().toISOString().split('T')[0] });
  const [showRepairForm, setShowRepairForm] = useState(false);

  // Persist
  useEffect(() => { saveStore(STORE_KEYS.audit, auditSituations); }, [auditSituations]);
  useEffect(() => { saveStore(STORE_KEYS.repairs, repairs); }, [repairs]);
