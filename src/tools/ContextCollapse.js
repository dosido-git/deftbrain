...
const EXAMPLE = {
  platform: 'email',
  message: "Hey team — I need to push the launch back by two weeks. Beta feedback surfaced three critical UX issues we shouldn't ship without addressing. I know this is frustrating, especially with how hard everyone has worked on the timeline.",
  audiences: [
    { label: 'Engineering team', relationship: 'Direct collaborators — they built it, they know exactly what slipped' },
    { label: 'CEO', relationship: 'Cares about market timing and external commitments — board update coming up' },
    { label: 'Customer Success', relationship: 'Already told beta users about original launch date — needs to manage that gracefully' },
  ],
  intent: "Land the delay without losing trust. Acknowledge the cost honestly without apologizing for the right call.",
  concerns: "I don't want to sound defensive, but I also don't want to undersell the impact. The CEO version especially has to be confident.",
};
// ════════════════════════════════════════════════════════════
...
const ContextCollapse = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    ...
    setAudiences(EXAMPLE.audiences);
    setIntent(EXAMPLE.intent);
    setConcerns(EXAMPLE.concerns);
    setResults(null);
  }, [setPlatform, setMessage, setAudiences, setIntent, setConcerns, setResults]);
