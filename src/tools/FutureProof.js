...
const EXAMPLE = {
  subject: 'Software engineering as a career path',
  subjectType: 'career',
  context: "Mid-career — 8 years experience, currently at a mid-size SaaS company. Wondering whether to deepen specialization in backend systems or pivot toward AI/ML or management.",
  timeframe: '5 years',
};
// ─── Constants ───
const SUBJECT_TYPES = [
...
const FutureProof = ({ tool }) => {
  ...
  const trajStyle = (traj) => {
    const map = {
      ...
      context_dependent: { bg: isDark ? 'bg-cyan-900/30 border-cyan-600'   : 'bg-cyan-50 border-cyan-300',   txt: isDark ? 'text-cyan-300'  : 'text-cyan-800',  dot: '🟣', label: 'CONTEXT-DEPENDENT ◇' },
    };
  ...
  const loadExample = useCallback(() => {
    ...
    setResults(null);
  }, [setSubject, setSubjectType, setContext, setTimeframe, setResults]);
