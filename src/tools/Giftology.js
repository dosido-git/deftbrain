...
const Giftology = ({ tool }) => {
  ...
  const loadExample = () => {
    const ex = pickExample('Giftology', EXAMPLES);
    setRecipient(ex);
  };
    ...
    <div className={`space-y-4 ${c.text}`}>
      ...
      <div className={`${c.card} border ${c.border} rounded-xl shadow-sm px-5 pt-2.5 pb-5`}>
        ...
        <button title={t('cmd_enter')}
          ...
          className={`relative w-full ${(!recipient.trim()) ? c.btnIdle : c.btnPrimary} font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}
        >
          {loading
            ? <><span className="animate-spin inline-block text-xl">{tool?.icon ?? '🎁'}</span> {t('gft_finding')}</>
            : <><span className="text-xl">{tool?.icon ?? '🎁'}</span> {t('gft_find_ideas')}</>}
        {!loading && (
