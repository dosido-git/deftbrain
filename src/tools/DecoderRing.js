      ...
      <div ref={resultsRef} className="space-y-4 mt-4">
        ...
        <div className={`p-4 rounded-2xl border ${c.border} ${isDark ? 'bg-zinc-800/60' : 'bg-slate-50'}`}>
          <p className={`text-xs ${c.textMuted}`}>
            💡 Need a tough reply?{' '}<a href="/VelvetHammer" className={linkStyle}>Velvet Hammer</a>{' '}
            turns angry drafts into assertive messages. Stuck in a bigger decision?{' '}<a href="/ContextCollapse" className={linkStyle}>Context Collapse</a>{' '}
  ...
  return (
    <div className={`space-y-4 ${c.text}`}>
      ...
      <p className={`text-sm ${c.textMuted} text-center`}>
        Missing the bigger picture?{' '}<a href="/ContextCollapse" className={linkStyle}>Context Collapse</a>{' '}
        helps you map the full conversation before decoding a single message.
      </p>
      {renderResults()}
      {error && (
