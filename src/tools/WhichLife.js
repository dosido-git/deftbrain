...
const WhichLife = ({ tool }) => {
  ...
  const handleExample = useCallback(() => {
    // Keeps the existing 'never re-offer the pair already on screen' rule,
    // but steps through the rest in order instead of re-rolling.
    const filtered = EXAMPLE_DECISIONS.filter(e => e.a !== pathA);
    const ex = pickExample('WhichLife', filtered);
    setPathA(ex.a);
    setPathB(ex.b);
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className={`text-base ${c.textSecondary}`}>
                <span className="me-2 text-lg">{tool?.icon ?? '🔮'}</span>{tool?.tagline ?? t('cr_tagline')}
              </p>
