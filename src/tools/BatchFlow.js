...
const BatchFlow = ({ tool }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => revealSection(sharePanelRef.current, { block: 'center' }), 50);
    return () => clearTimeout(timer);
  }, [showShare]);
  useEffect(() => {
    if ((!progressResult && !progressLoading) || !progressPanelRef.current) return;
    const timer = setTimeout(() => revealSection(progressPanelRef.current, { block: 'center' }), 50);
    return () => clearTimeout(timer);
  }, [progressResult, progressLoading]);
  useEffect(() => {
    if (!showSaveTemplate || !savePanelRef.current) return;
    const timer = setTimeout(() => revealSection(savePanelRef.current, { block: 'center' }), 50);
    return () => clearTimeout(timer);
  }, [showSaveTemplate]);
  useEffect(() => {
    if (!showWeekly || !weeklyPanelRef.current) return;
    const timer = setTimeout(() => revealSection(weeklyPanelRef.current, { block: 'center' }), 50);
    return () => clearTimeout(timer);
  }, [showWeekly]);
