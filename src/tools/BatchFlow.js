...
import { Spin } from '../components/PendingAction';
...
const BatchFlow = ({ tool }) => {
  ...
  // ─── Shared components ───
  // Spin now lives in ../components/PendingAction (this file's local copy was the
  // original; it was promoted so the other tools could stop reinventing it).
  ...
  return (
    <div className={`space-y-4 ${c.text}`}>
      <div className={`${c.card} rounded-xl shadow-sm p-5`}>
        <div className="flex items-start justify-between gap-3 mb-4 pb-3 border-b border-zinc-500">
          <div className="flex-1 min-w-0">
            ...
            {/* The one idea the whole tool rests on, said once, under the
                subtitle — before any of the machinery. */}
            <p className={`text-sm ${c.textMuteded} mt-1.5`}>{t('bf_lede')}</p>
      ...
      {!results && !abResult && !locationResult && <div className="space-y-5">
        {/* Tasks — the first question, because it is the one the
            visitor already has an answer to. */}
        {!dumpMode && <>
      ...
      {results && <div ref={resultsRef} className="space-y-5">
        {/* ── FIRST, THIS MATTERS ─────────────────────────────────────────
            ...
            usually isn't — switching context has a real cost. Not generated:
        ...
        <div className={`${c.card} border-2 ${isDark ? 'border-emerald-700' : 'border-emerald-300'} rounded-xl p-5 space-y-2`}>
          <p className={`text-sm font-bold ${c.text}`}>💚 {t('bf_first_matters')}</p>
          <p className={`text-sm ${c.textSecondary}`}>{t('bf_fm_1')}</p>
          <p className={`text-sm ${c.textSecondary}`}>{t('bf_fm_2')}</p>
          <p className={`text-sm ${c.textSecondary}`}>{t('bf_fm_3')}</p>
        </div>

        <div className={`${c.cardAlt} border ${c.border} rounded-xl p-5 space-y-1`}>
          <p className={`text-sm font-bold ${c.text}`}>🎯 {t('bf_todays_job')}</p>
          <p className={`text-sm ${c.textSecondary}`}>{t('bf_job_1')}</p>
        </div>
        ...
        {/* Every outcome ends with a next step — and with permission to stop */}
        <div className={`${c.success} border rounded-xl p-5 space-y-2`}>
          <p className="text-sm font-bold">💚 {t('bf_youre_set')}</p>
          <p className="text-sm">{t('bf_ys_1')}</p>
          <p className="text-sm">{t('bf_ys_2')}</p>
        </div>
      </div>}
    </div>
  );
