...
const MagicMouth = ({ tool }) => {
  ...
  const EXAMPLE_ASKS = [
    { want: t('mm_ex1_want'), situation: t('mm_ex1_situation') },
    { want: t('mm_ex2_want'), situation: t('mm_ex2_situation') },
    { want: t('mm_ex3_want'), situation: t('mm_ex3_situation') },
    { want: t('mm_ex4_want'), situation: t('mm_ex4_situation') },
    { want: t('mm_ex5_want'), situation: t('mm_ex5_situation') },
    { want: t('mm_ex6_want'), situation: t('mm_ex6_situation') },
    { want: t('mm_ex7_want'), situation: t('mm_ex7_situation') },
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err.message || t('mm_err_angle'));
    }
  };
  ...
  const handlePhoneTree = async () => {
    if (!phoneCompany.trim()) { setError(t('mm_err_company')); return; }
    if (!phoneIssue.trim()) { setError(t('mm_err_issue')); return; }
    ...
    try {
      ...
      setError(err.message || t('mm_err_phone'));
    }
  };
  ...
  const handleNuclear = async () => {
    ...
    if (!nuclearProblem.trim()) { setError(t('mm_err_problem')); return; }
    ...
    try {
      ...
      setError(err.message || t('mm_err_nuclear'));
    }
  };
  ...
  const buildFullText = useCallback(() => {
    ...
    out += BRAND;
    return out;
  }, [results, whatYouWant, situation, t, DIFFICULTY_MAP]);
    ...
    <div className={`space-y-4 ${c.text}`}>
      ...
      <div className={`${c.card} border ${c.border} rounded-xl shadow-sm`}>
        ...
        <div className={`flex gap-1 p-1 rounded-xl ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
          {[
            { id: 'ask',     icon: '🎯', label: t('mm_tab_ask') },
            { id: 'phone',   icon: '📞', label: t('mm_tab_phone') },
            { id: 'nuclear', icon: '💣', label: t('mm_tab_nuclear') },
          ].map(tab => (
          ...
          <div ref={resultsRef} className="space-y-5">
            ...
            {results?.the_script && (
              <div className="space-y-3">
                ...
                <ScriptLine label={t('mm_the_ask')} icon="🎯" text={results?.the_script?.the_ask} />
                <ScriptLine label={t('mm_if_hesitate')} icon="🤔" text={results?.the_script?.if_they_hesitate} />
                <ScriptLine label={t('mm_graceful_exit')} icon="🚪" text={results?.the_script?.graceful_exit} />
              </div>
            )}
            ...
            <div className={`${c.cardAlt} ${c.border} border rounded-xl p-4 space-y-2`}>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  { id: 'VelvetHammer', icon: '🔨', label: t('mm_ref_velvethammer') },
                  { id: 'BuyWise', icon: '🛒', label: t('mm_ref_buywise') },
                  { id: 'DecoderRing', icon: '🔓', label: t('mm_ref_decoderring') },
                ].map(ref => (
                  <a
                    key={ref.id}
                    href={`/${ref.id}`}
        ...
        {mode === 'nuclear' && nuclearResults && (
          <div className="space-y-4">
            ...
            <div className={`${c.cardAlt} ${c.border} border rounded-xl p-4 space-y-2`}>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  { id: 'RulebookBreaker', icon: '🏴‍☠️', label: t('mm_ref_rulebookbreaker') },
