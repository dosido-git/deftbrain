...
const MagicMouth = ({ tool }) => {
  ...
  handleSubmitRef.current = mode === 'ask' ? handleSubmit
                          : mode === 'phone' ? handlePhoneTree
                          : handleNuclear;
  canSubmitRef.current = mode === 'ask' ? !!whatYouWant.trim()
                       : mode === 'phone' ? !!(phoneCompany.trim() && phoneIssue.trim())
                       : !!(nuclearCompany.trim() && nuclearProblem.trim());
    ...
    <div className={`space-y-4 ${c.text}`}>
      ...
      {/* ─── Phone Tree Mode ─── */}
          ...
          <div className={`${c.card} ${c.border} border rounded-2xl p-5 shadow-sm space-y-4`}>
            ...
            <div className="space-y-2">
              <label className={`text-sm font-semibold ${c.text}`}>
                <span className="mr-1.5">🎯</span> What do you want? <span className={c.required}>*</span>
              </label>
              <textarea
                value={whatYouWant}
        ...
        {mode === 'nuclear' && !nuclearResults && (
          <div className={`${c.card} ${c.border} border rounded-2xl p-5 shadow-sm space-y-4`}>
            ...
            <div className="space-y-2">
              <label className={`text-sm font-semibold ${c.text}`}><span className="mr-1.5">⚠️</span> What's the problem that won't get resolved? <span className={c.required}>*</span></label>
      ...
      <div className={`${c.cardAlt} border ${c.border} rounded-xl p-3 text-center`}>
        <p className={`text-xs ${c.textMuted}`}>
          AI-generated scripts — adapt to your situation. Not legal advice.
        </p>
      </div>
