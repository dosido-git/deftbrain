      ...
      {/* ── HEADER ── */} <div className={`${c.card} border ${c.border} rounded-xl p-6`}>
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className={`text-base ${c.textSecondary}`}>
                <span className="me-2 text-lg">{tool?.icon ?? '🥂'}</span>{tool?.tagline ?? t('tst_tagline')}
              </p>
        ...
        <div className="flex gap-3">
          <button title={t('cmd_enter')}
            onClick={generate} disabled={loading || !person.trim() || !occasion} className={`relative flex-1 ${c.btnPrimary} disabled:opacity-40 disabled:cursor-not-allowed font-bold py-3 px-6 rounded-xl flex items-center justify-center gap-2 min-h-[48px] shadow-lg`} >
            {loading
              ? <><span className="inline-block animate-spin">{tool?.icon ?? '🥂'}</span> {t('tst_writing')}</>
              : <><span>{tool?.icon ?? '🥂'}</span> {t('tst_write_btn')}</>} {!loading && (
