      ...
      {mode === 'results' && results && (
        <div className="space-y-4">
          ...
          <div className={`${c.card} border rounded-xl p-5`}>
            ...
            {results?.decision_making_structure && <div className={`${c.infoBox} border-2 rounded-lg p-4 mt-4`}><p className="text-sm font-bold mb-1">🗳️ {decisionFramework}</p>{results?.decision_making_structure?.process && <p className="text-xs">{results?.decision_making_structure?.process}</p>}{results?.decision_making_structure?.when_to_use && <p className={`text-xs ${c.textSecondary} mt-1`}>When: {results?.decision_making_structure?.when_to_use}</p>}{results?.decision_making_structure?.voting_mechanism && <p className={`text-xs ${c.textSecondary} mt-1`}>🗳️ {results?.decision_making_structure?.voting_mechanism}</p>}{results?.decision_making_structure?.conflict_resolution && <p className={`text-xs ${c.textSecondary} mt-1`}>⚡ If stuck: {results?.decision_making_structure?.conflict_resolution}</p>}</div>}
            {results?.meeting_minutes_template && <div className={`${c.cardAlt} border-2 rounded-lg p-4 mt-4`}><p className="text-sm font-bold mb-2">📋 Meeting Minutes Template</p><pre className={`text-[10px] ${c.textSecondary} whitespace-pre-wrap font-mono ${isDark ? 'bg-zinc-900' : 'bg-white'} p-2 rounded border ${c.border} max-h-40 overflow-y-auto`}>{results?.meeting_minutes_template}</pre></div>}
          </div>
