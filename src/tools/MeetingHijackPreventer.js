...
const MeetingHijackPreventer = ({ tool }) => {
  ...
  // RESULTS
  const [results, setResults] = usePersistentState('meetinghijackpreventer-result', null);
      ...
      {mode === 'setup' && (
        <div className={`${c.card} border rounded-xl p-6 space-y-5`}>
          <div className={`${c.infoBox} border-l-4 rounded-r-lg p-4 flex items-start gap-2`}>
            <span>🎯</span>
            <div><h4 className="font-bold text-sm mb-0.5">Built for Everyone</h4><p className={`text-xs ${c.textSecondary}`}>Neurodivergent-friendly: clear time boxes, explicit roles, and redirect scripts.</p></div>
          </div>
          ...
          <div>
            <label className={`text-sm font-semibold ${c.labelText} block mb-1.5`}>Goal {!useTemplate && '*'}</label>
          ...
          <div><label className={`text-sm font-semibold ${c.labelText} block mb-1.5`}>👥 Participants <span className={`text-[10px] ${c.textMuted}`}>(optional)</span></label>
            <div className="flex gap-2 mb-2"><input value={newParticipant} onChange={e => setNewParticipant(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addParticipant(); } }} placeholder="Name..." className={`flex-1 p-2 border rounded-lg ${c.input} outline-none text-sm`} />
              <button onClick={addParticipant} disabled={!newParticipant.trim()} className={`${c.btnSecondaryondary} px-3 py-2 rounded-lg text-sm disabled:opacity-40`}>➕</button></div>
          ...
          <button onClick={handleGenerate} disabled={loading || (!meetingGoal.trim() && !useTemplate)} className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2`}>
            {loading ? <span className="inline-block animate-spin">{tool?.icon ?? '⚙️'}</span> : <>🛡️ Generate Structure</>}</button>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex flex-wrap gap-2">
              ...
              <button onClick={() => handleCapture(null)} className={`${c.btnSecondaryondary} py-2 px-3 rounded-lg text-sm`}>📝 Capture</button>
              <button onClick={() => saveToHistory()} className={`${c.btnSecondaryondary} py-2 px-3 rounded-lg text-sm`}>💾 Save</button>
            </div>
          ...
          <button onClick={handleReset} className={`${c.btnSecondaryondary} w-full py-2 rounded-lg text-sm`}>✨ New Meeting</button>
        </div>
      )}
      ...
      {mode === 'capture' && (
        <div className="space-y-4">
          ...
          <div className={`${c.card} border rounded-xl p-5 space-y-4`}>
            ...
            <div><div className="flex items-center justify-between mb-1.5"><label className={`text-sm font-semibold ${c.labelText}`}>🅿️ Parking Lot</label><button onClick={() => addCaptureItem('parkingLot')} className={`text-xs ${c.accentTxt}`}>➕</button></div>
          ...
          <div className="flex gap-3"><button onClick={finishCapture} className={`flex-1 ${c.btnPrimary} py-3 rounded-lg font-semibold`}>💾 Save & Finish</button><button onClick={() => setMode('results')} className={`${c.btnSecondaryondary} py-3 px-6 rounded-lg`}>Skip</button></div>
        </div>
      )}
            ...
            : <div className="space-y-2 max-h-96 overflow-y-auto">{history.map(entry => (
              <div key={entry.id} className={`${c.cardAlt} border rounded-lg p-4`}>
                ...
                <div className="flex gap-2 mt-2">
                  <button onClick={() => reuseEntry(entry)} className={`${c.btnSecondaryondary} text-xs px-3 py-1 rounded-lg`}>♻️ Reuse</button>
