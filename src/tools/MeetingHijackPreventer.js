...
const EXAMPLES = [
  ...
  {
  meetingGoal: 'Weekly team sync — in theory a status update, in practice 45 minutes of one person narrating their week while everyone else waits.',
  ...
  decisionFramework: 'Disagree & Commit (commit even if you objected)',
  challenges: { dominates: true, offTopic: true, talkOver: false, schedule: false, quietVoices: true },
  hourlyRate: 70,
},
];;
...
const MeetingHijackPreventer = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    ...
    setIsVirtual(ex.isVirtual);
    setVirtualPlatform(ex.virtualPlatform);
    setDecisionFramework(ex.decisionFramework);
    setChallenges(ex.challenges);
    setHourlyRate(ex.hourlyRate);
  }, []);
  ...
  // RENDER
  // ════════════════════════════════════════════════════════════
  // A screen change here swaps the whole tool body, so this reveals the
  // tool: frame mode shows the header and the controls, and focus lands on
  // the body that just changed.
    ...
    <div ref={stageRef} className={`scroll-mt-24 space-y-4 ${c.text}`}>
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className={`text-base ${c.textSecondary}`}>
                  <span className="me-2 text-lg">{tool?.icon ?? '🛡️'}</span>{tool?.tagline ?? t('mhp_tagline')}
                </p>
      ...
      {mode === 'setup' && (
        <div className={`${c.card} border rounded-xl p-6 space-y-5`}>
          ...
          <button title={t('cmd_enter')} onClick={handleGenerate} disabled={loading || (!meetingGoal.trim() && !useTemplate)} className={`relative w-full ${((!meetingGoal.trim() && !useTemplate)) ? c.btnIdle : c.btnPrimary} font-bold py-3 rounded-lg flex items-center justify-center gap-2`}>
            {loading
