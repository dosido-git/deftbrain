...
// Five, rotated by pickExample. All three depths, audiences from a child to a
// professional room, science / engineering / money / statistics / AI, and one
// where the interests are thin — so the rotation shows the whole range of what
// an analogy here can look like.
const EXAMPLES = [
  {
  ...
  {
    concept: 'Why our database needs sharding before Black Friday',
    audience: 'our CEO, who is not technical',
    audienceInterests: 'ran a restaurant before this, follows Formula 1 closely, thinks in unit economics',
    depth: 'solid_understanding',
  },
  {
    concept: 'Compound interest, and why starting at 22 beats starting at 32',
    audience: 'my 17-year-old, who just got her first paycheck',
    audienceInterests: 'runs cross-country, saving for a used car, allergic to anything that sounds like a lecture',
    depth: 'quick_grasp',
  },
  {
    concept: 'What a p-value actually tells you — and what it does not',
    audience: 'a room of nurses in a quality-improvement workshop',
    audienceInterests: 'practical and time-poor; have seen research misused to justify staffing cuts',
    depth: 'deep_understanding',
  },
  {
    concept: 'How a large language model predicts the next word',
    audience: 'my 74-year-old dad',
    audienceInterests: 'retired plumber, does the crossword daily',
    depth: 'quick_grasp',
  },
];
...
const AnalogyEngine = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    const ex = pickExample('AnalogyEngine', EXAMPLES);
    setConcept(ex.concept);
    setAudience(ex.audience);
    setAudienceInterests(ex.audienceInterests);
    setDepth(ex.depth);
    setResults(null);
  ...
  return (
    <div className={`space-y-4 ${c.text}`}>
      ...
      <div className={`${c.card} border ${c.border} rounded-2xl px-5 pt-2.5 pb-5 shadow-sm space-y-4`}>
        ...
        <div className="flex items-start justify-between gap-3 pb-4 border-b border-zinc-500">
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-base ${c.textSecondary}`}>
              <span className="me-2 text-lg">{tool?.icon ?? '💡'}</span>{t('ae_tagline')}
            </p>
        ...
        {/* PF-31 — the ⌘↵ handler has been here since the tool shipped */}
        <button
          onClick={handleSubmit}
          disabled={loading || !concept.trim()}
          title={t('ae_cmd_enter')}
