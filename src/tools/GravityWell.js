...
const GravityWell = ({ tool }) => {
  ...
  const [results, setResults] = usePersistentState('gw-results', null);
  ...
  return (
    <div className={`min-h-screen py-8 px-4 ${c.card} ${c.border}`}>
      <div className="max-w-2xl mx-auto">

        <div className="text-center mb-8">
          <div className="text-5xl mb-3">🌀</div>
          <h1 className={`text-3xl font-black tracking-tight mb-2 ${c.text}`}>Gravity Well</h1>
          <p className={`text-sm ${c.textMuteded}`}>A 90-day plan to naturally enter someone's orbit — before you ever reach out.</p>
        </div>
                ...
                {TARGET_TYPES.map(t => (
                  <button key={t.id} onClick={() => setTargetType(targetType === t.id ? '' : t.id)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${
                      targetType === t.id ? 'bg-cyan-700 text-white border-cyan-700' : c.btnSecondaryondary
                    }`}>
                    {t.label}
                  </button>
                ))}
