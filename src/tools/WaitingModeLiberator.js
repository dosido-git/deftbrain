...
const WaitingModeLiberator = () => {
  ...
  const c = {
    bg: isDark ? 'bg-zinc-900' : 'bg-gradient-to-br from-lime-50 via-emerald-50 to-teal-50',
    ...
    heroAccent: isDark ? 'text-emerald-300' : 'text-emerald-700',
  ...
  const intensityBadge = (intensity) => {
    ...
    if (intensity === 'high') return { bg: isDark ? 'bg-violet-800/50' : 'bg-violet-100', text: isDark ? 'text-violet-300' : 'text-violet-700', label: '🟣 Deep' };
      ...
      <div className={`min-h-screen ${c.bg} py-6 px-4`}>
        <div className="max-w-xl mx-auto space-y-5">
          <div className="text-center mb-2">
            ...
            <p className={`${c.textSecondary} text-sm mt-1`}>Reclaim the hours around your appointments</p>
          </div>
              ...
              <div>
                {sessionLog.length > 0 && (
                  <span className={`text-sm font-medium ${c.text}`}>
                    ...
                    <span className={`text-xs ${c.textMuted}`}>~{Math.round(sessionLog.reduce((sum, s) => sum + (s.freeMinutes || 0), 0) / 60 * 10) / 10}h reclaimed</span>
                  </span>
                )}
              </div>
            ...
            {events.map((ev, idx) => (
              <div key={ev.id} className={`p-4 rounded-xl border ${isDark ? 'border-zinc-600 bg-zinc-700/30' : 'border-gray-200 bg-gray-50'} space-y-3`}>
                <div className="flex items-center justify-between">
                  <span className={`text-xs font-bold ${c.textMuted} uppercase`}>Event {idx + 1}</span>
                  {events.length > 1 && <button onClick={() => removeEvent(ev.id)} className={`text-xs ${c.textMuted} px-2 py-1 rounded hover:opacity-70`}>✕</button>}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <span className={`text-[10px] ${c.textMuted} uppercase`}>Prep</span>
                  ...
                  <div>
                    <span className={`text-[10px] ${c.textMuted} uppercase`}>Travel</span>
                    <div className="flex gap-1 mt-1">
          ...
          <div className={`${c.card} border rounded-xl p-5 space-y-3`}>
            ...
            <div className="flex justify-between">
              <span className={`text-[10px] ${c.textMuted}`}>1 — Totally chill</span>
              <span className={`text-sm font-bold ${c.text}`}>{anxietyBefore}/10</span>
              <span className={`text-[10px] ${c.textMuted}`}>10 — Dreading it</span>
            </div>
          ...
          <div className={`${c.card} border rounded-xl p-5`}>
            ...
            <p className={`text-xs ${c.textMuted} mb-3`}>We'll map your tasks to your free windows</p>
          ...
          <button onClick={handleLiberate} disabled={loading || !hasAnyTime}
            ...
            {loading ? <span><span className="animate-spin inline-block">⏳</span> Calculating free time...</span> : <span><span>🔓</span> Liberate My Time</span>}
          </button>
          ...
          <div className={`${c.launch} border-2 rounded-2xl p-6 text-center shadow-lg`}>
            {isGuiding ? (
              ...
              <>
                ...
                <button onClick={() => {}} className={`mt-3 text-xs ${c.launchAccent}`}>Starting...</button>
              </>
            )}
          </div>
        ...
        <div className="max-w-xl mx-auto space-y-5">
          ...
          {reframes && (
            <div className={`${c.card} border rounded-xl p-5 space-y-3`}>
              ...
              {reframes.truth_bomb && <p className={`text-xs ${c.textMuted} italic border-t ${c.divider} pt-2 mt-2`}>🎯 {reframes.truth_bomb}</p>}
            </div>
          )}
                    ...
                    <div key={i} className={`p-4 rounded-xl border-2 transition-all ${isCompleted ? (isDark ? 'bg-emerald-900/30 border-emerald-700 opacity-60' : 'bg-emerald-50 border-emerald-200 opacity-60') : `${c.blockBg} ${isDark ? 'border-zinc-600' : 'border-gray-200'}`}`}>
                      ...
                      {!isCompleted && (
                        <button onClick={() => handleStartWithMe(i)} disabled={loading}
                          ...
                          {loading ? <span className="animate-spin inline-block">⏳</span> : <span>🚀</span>} Start this block with me
                        </button>
                      )}
                    </div>
                  );
          ...
          {!oneThingData && (
            <button onClick={handleOneThing} disabled={loading}
              className={`w-full py-3 rounded-xl text-sm font-medium ${c.warning} border ${c.warningText} transition-all`}>
              {loading ? <span className="animate-spin inline-block">⏳</span> : <span>🎯</span>} Can't do the whole plan? Just pick one thing
            </button>
          )}
          {oneThingData && (
            <div className={`${c.warning} border-2 rounded-xl p-5 space-y-3`}>
              <h3 className={`text-sm font-bold ${c.warningText}`}><span>🎯</span> Just This One Thing</h3>
              <p className={`text-base font-medium ${c.warningText}`}>{oneThingData.the_one_thing}</p>
              <div className={`p-3 rounded-lg ${isDark ? 'bg-amber-900/30' : 'bg-amber-100/50'}`}>
                <p className={`text-xs font-bold ${c.warningText} mb-1`}>First physical action:</p>
                <p className={`text-sm ${c.warningText}`}>{oneThingData.first_physical_action}</p>
              </div>
              <div className="flex items-center justify-between">
                <span className={`text-xs ${c.warningText}`}>⏱️ {oneThingData.time_needed}</span>
                <span className={`text-xs ${c.warningText}`}>{oneThingData.why_this_one}</span>
              </div>
              {oneThingData.escape_hatch && <p className={`text-xs ${c.warningText} italic`}>🚪 {oneThingData.escape_hatch}</p>}
              {oneThingData.momentum_hook && <p className={`text-xs ${c.warningText} opacity-70`}>→ {oneThingData.momentum_hook}</p>}
            </div>
          )}
        ...
        <div className="max-w-xl mx-auto space-y-5">
          ...
          {!debriefData && (
            <button onClick={handleDebrief} disabled={loading || (!debriefUsedTime && !debriefReality)}
              className={`w-full py-4 rounded-xl font-bold text-lg ${c.accent} ${c.accentHover} ${c.accentText} disabled:opacity-40 transition-all`}>
              {loading ? <span><span className="animate-spin inline-block">⏳</span> Reflecting...</span> : <span><span>🔍</span> Get My Debrief</span>}
            </button>
          )}
            ...
            <div className="space-y-4">
                ...
                <div className={`${c.card} border rounded-xl p-5 space-y-3`}>
                  ...
                  {debriefData.anxiety_check.trend && (
                    <div className={`${c.warning} border rounded-lg p-3`}>
                      <p className={`text-xs font-medium ${c.warningText}`}>📊 Trend: {debriefData.anxiety_check.trend}</p>
                    </div>
                  )}
              ...
              {debriefData.encouragement && (
                <div className={`${c.success} border rounded-xl p-4`}>
                  <p className={`text-sm ${c.successText}`}><span>💚</span> {debriefData.encouragement}</p>
                </div>
              )}

              {/* Save */}
              <button onClick={saveSession} className={`w-full py-4 rounded-xl font-bold text-lg ${c.accent} ${c.accentHover} ${c.accentText}`}>
          ...
          {reviewData ? (
            <>
              ...
              {reviewData.anxiety_trends && (
                <div className={`${isDark ? 'bg-purple-900/30 border-purple-700' : 'bg-purple-50 border-purple-200'} border rounded-xl p-5 space-y-2`}>
                  <h3 className={`text-sm font-bold ${isDark ? 'text-purple-300' : 'text-purple-800'} mb-2`}><span>🧠</span> Anxiety vs. Reality</h3>
                  {reviewData.anxiety_trends.gap && <p className={`text-sm ${isDark ? 'text-purple-300' : 'text-purple-800'}`}>Gap: {reviewData.anxiety_trends.gap}</p>}
                  {reviewData.anxiety_trends.insight && <p className={`text-sm font-medium ${isDark ? 'text-purple-200' : 'text-purple-900'}`}>💡 {reviewData.anxiety_trends.insight}</p>}
                </div>
              )}
              ...
              {reviewData.encouragement && <div className={`${c.success} border rounded-xl p-4`}><p className={`text-sm ${c.successText}`}><span>💚</span> {reviewData.encouragement}</p></div>}
            </>
          ) : (
            <div className={`${c.card} border rounded-xl p-8 text-center`}><span className="animate-spin inline-block text-2xl">⏳</span><p className={`text-sm ${c.textMuted} mt-2`}>Analyzing patterns...</p></div>
          )}
        </div>
      </div>
    );
