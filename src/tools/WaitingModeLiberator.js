...
const EXAMPLES = [
  ...
  anxietyBefore: 8,
  events: [
    { id: 1, name: 'Dentist — first visit in four years', time: '11:15 AM', dayOffset: 3, type: 'medical', prepMinutes: 10, travelMinutes: 25 },
  ],
...
const WaitingModeLiberator = ({ tool }) => {
  ...
  const commitDraft = () => {
    ...
    setTimeout(() => {
      revealSection(newEventRef.current, { block: 'nearest' });
  ...
  const loadExample = useCallback(() => {
    const ex = pickExample('WaitingModeLiberator', EXAMPLES);
    setView(ex.view);
    setEnergy(ex.energy);
    setAnxietyBefore(ex.anxietyBefore);
    setEvents(ex.events);
    setUserTasks(ex.userTasks);
  ...
  if (view === 'active' && results) {
    return (<div ref={viewTopRef} className="pt-1 pb-6 px-4">
        <div className="max-w-xl mx-auto space-y-5">
          ...
          {/* Results anchor */} <div data-copy-results ref={resultsRef} data-results-anchor className="scroll-mt-24" />
