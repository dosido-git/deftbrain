        ...
        <div className="max-w-xl mx-auto space-y-4">
          ...
          {/* Cross-refs */} <div className={`${c.card} border rounded-xl p-4`}>
            ...
            <div className="flex flex-wrap gap-2">
              {[{ id: 'VirtualBodyDouble', icon: '👥', label: 'Virtual Body Double' }, { id: 'CrisisPrioritizer', icon: '🚨', label: 'Crisis Prioritizer' }, { id: 'DopamineMenuBuilder', icon: '🎯', label: 'Dopamine Menu' }].map(t => (<a key={t.id} href={`/${t.id}`}
    ...
    return (<div className="py-6 px-4">
        <div className="max-w-xl mx-auto space-y-5">
          ...
          {/* Event timeline */} {results.events_summary?.length > 0 && (<div className={`${c.card} border rounded-xl p-4`}>
              ...
              <button onClick={handleReframes} disabled={loading} className={`text-xs ${c.textMuted} mt-2 underline disabled:opacity-40`}>More reframes →</button>
            </div>
    ...
    return (<div className="py-6 px-4">
        <div className="max-w-xl mx-auto space-y-5">
          ...
          {reviewData ? (<>
              ...
              {reviewData.trigger_patterns && (<div className={`${c.card} border rounded-xl p-5 space-y-2`}>
                  ...
                  {reviewData.time_insights.avg_free_time && <p className={`text-sm ${c.accentLightText}`}>Avg free time: {reviewData.time_insights.avg_free_time}</p>} {reviewData.time_insights.utilization && <p className={`text-sm ${c.accentLightText}`}>Utilization: {reviewData.time_insights.utilization}</p>} {reviewData.time_insights.best_block_length && <p className={`text-sm ${c.accentLightText}`}>Best block: {reviewData.time_insights.best_block_length}</p>} </div>
      ...
      {results && (
        <p className={`text-xs ${c.textMuted} mt-3 text-center`}>
          Got things to do while waiting? <a href="/TaskAvalancheBreaker" className={linkStyle}>⛏️ Task Avalanche Breaker</a> finds micro-tasks that fit.
        </p>
      )}
      </div>
    );
