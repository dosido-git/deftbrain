...
const PetWeirdnessDecoder = ({ tool }) => {
  ...
  const getSeverityTrend = () => {
    ...
    if (diff > 0.5) return { direction: 'worsening', icon: '📈', colorKey: 'trendWorse' };
    if (diff < -0.5) return { direction: 'improving', icon: '📉', colorKey: 'trendBetter' };
    return { direction: 'stable', icon: '➡️', colorKey: 'trendStable' };
  };
          ...
          <div className="pb-3 border-b border-zinc-500 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              {(results || behavior.trim()) && (
                <button onClick={handleReset} className={`${c.btnSecondary} px-3 py-1.5 rounded-lg text-xs font-bold`}>↺ Start Over</button>
              )}
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className={`${c.btnSecondary} disabled:opacity-40 px-3 py-2 rounded text-sm border ${isDark ? 'border-zinc-600' : 'border-amber-300'}`}>
                {uploading ? <span className="animate-spin inline-block">{tool?.icon ?? '🐾'}</span> : '📷'} {imagePreview ? 'Change' : 'Photo'}
              </button>
              {!isRecording ? (
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 mt-4">
              ...
              <button onClick={handleFindEmergencyVet} disabled={locatingVet} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 disabled:opacity-40">
                {locatingVet ? <span className="animate-spin inline-block">{tool?.icon ?? '🐾'}</span> : <span>📍</span>} Find Emergency Vet
              </button>
            </div>
          </div>
        </div>
        ...
        <div ref={resultsRef} className="space-y-6">
          ...
          {showTracker && (
            <div className={`${c.card} border rounded-xl p-4`}>
              ...
              {(() => { const t = getSeverityTrend(); return t ? <p className={`text-sm mb-2 font-semibold ${c[t.colorKey]}`}>{t.icon} {t.direction}</p> : null; })()}
          ...
          <div className={`${c.card} border rounded-xl p-5`}>
            ...
            <div className="flex gap-2" ref={followupRef}>
              <label htmlFor="pwd-followup-q" className="sr-only">Follow-up question about your pet's behavior</label>
              <input id="pwd-followup-q" type="text" value={followupQuestion} onChange={e => setFollowupQuestion(e.target.value)} placeholder="What if she also starts limping?" className={`flex-1 p-3 border rounded-lg ${c.input}`} onKeyDown={e => { if (e.key === 'Enter') handleFollowup(); }} />
