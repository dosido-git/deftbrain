...
const RECENT_KEY = 'say-what-recent-log';
const MAX_RECENT = 20;

const CLASSIFICATION_META = {
  IDIOM: ['🧩', 'syw_class_idiom', 'Idiom'],
  SLANG: ['🗯️', 'syw_class_slang', 'Slang'],
  JARGON: ['🧠', 'syw_class_jargon', 'Jargon'],
  METAPHOR: ['🎭', 'syw_class_metaphor', 'Metaphor'],
  EUPHEMISM: ['🌫️', 'syw_class_euphemism', 'Euphemism'],
  REGIONAL_EXPRESSION: ['🗺️', 'syw_class_regional', 'Regional expression'],
  WORKPLACE_PHRASE: ['💼', 'syw_class_workplace', 'Workplace phrase'],
  CULTURAL_REFERENCE: ['📚', 'syw_class_cultural', 'Cultural reference'],
  PROVERB: ['📜', 'syw_class_proverb', 'Proverb'],
  LITERAL: ['🔤', 'syw_class_literal', 'Literal'],
};
...
const SayWhat = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    const ex = pickExample('SayWhat', EXAMPLES);
    setPhrase(ex.phrase);
  ...
  const runDecode = useCallback(async (overrideContext = null) => {
    if (!phrase.trim()) {
      setError(tr('syw_err_phrase', 'Enter the phrase you want to understand.'));
      return;
    }
    setError('');
    setEquivResult(null);
  ...
  const runEquivalent = useCallback(async () => {
    ...
    try {
      ...
      setError(e?.message || tr('syw_err_equivalent', 'Could not find an equivalent right now.'));
    } finally {
        ...
        <div className={`mt-5 pt-5 border-t ${c.border} space-y-4`}>
          <label className="block">
            ...
            <textarea
              ...
              rows={2}
              placeholder={tr('syw_phrase_placeholder', 'e.g. “We don’t want to boil the ocean.”')}
          ...
          <button
            ...
            title={tr('syw_cmd_enter', 'Press ⌘↵ (Ctrl+Enter) to decode')}
        ...
        <div ref={resultsRef} className="space-y-4 scroll-mt-24">
              ...
              <div className="mt-3 space-y-3">
                {(decoded.ambiguity?.possible_readings || []).map((r, i) => (
                  <div key={i} className={`${c.cardAlt} rounded-lg p-3`}>
                    <div className="font-semibold text-sm">{r.classification || tr('syw_possible', 'Possible meaning')}</div>
                    <div className={`text-sm mt-1 ${c.textSecondary}`}>{r.meaning}</div>
                  </div>
                ))}
              </div>
              {decoded.ambiguity?.what_would_help && (
                <div className={`mt-3 text-sm ${c.textSecondary}`}><strong>{tr('syw_what_settle', 'What would settle it:')}</strong> {decoded.ambiguity.what_would_help}</div>
              )}
            </div>
          )}
          ...
          <div className={`${c.card} border ${c.border} rounded-xl overflow-hidden`}>
            <button onClick={() => setShowMore(v => !v)} className="w-full p-4 flex justify-between items-center text-start font-bold">
              <span>➕ {tr('syw_more', 'More about this phrase')}</span><span>{showMore ? '▲' : '▼'}</span>
            </button>
            {showMore && (
              <div className={`border-t ${c.border} p-4 space-y-4`}>
                {decoded.background?.available && decoded.background?.explanation && (
                  <div>
                    <div className="font-semibold">📚 {firstNonEmpty(decoded.background.heading, tr('syw_background', 'Background'))}</div>
                    <div className={`mt-1 text-sm ${c.textSecondary}`}>{decoded.background.explanation}</div>
                  </div>
                )}
                {decoded.regional_note?.available && decoded.regional_note?.explanation && (
                  <div>
                    <div className="font-semibold">🗺️ {tr('syw_regional_note', 'Regional note')}</div>
                ...
                {!decoded.background?.available && !decoded.regional_note?.available && (
                  <div className={`text-sm ${c.textMuted}`}>{tr('syw_no_extra', 'There is no extra background you need in order to understand this phrase.')}</div>
                )}

                <div className={`pt-3 border-t ${c.border}`}>
                  <div className="font-semibold">🌍 {tr('syw_equiv_title', 'Something similar in another language')}</div>
                  <div className="flex flex-col sm:flex-row gap-2 mt-2">
                    ...
                    <button onClick={runEquivalent} disabled={equivLoading} className={`${c.btnSecondary} border rounded-lg px-4 py-2 text-sm font-bold disabled:opacity-40`}>
                      {equivLoading ? tr('syw_working', 'Working…') : tr('syw_find_equiv', 'Find an equivalent')}
                    </button>
                  </div>
                  {equivResult && (
          ...
          {decoded.response_help?.available && responseOptions.length > 0 && (
            <div className={`${c.card} border ${c.border} rounded-xl overflow-hidden`}>
              <button onClick={() => setShowResponses(v => !v)} className="w-full p-4 flex justify-between items-center text-start font-bold">
                <span>💬 {tr('syw_respond', 'How could I respond?')}</span><span>{showResponses ? '▲' : '▼'}</span>
              </button>
              {showResponses && (
