...
const HobbyMatch = ({ tool }) => {
  ...
  const scrollToResults = React.useCallback(() => {
    const go = () => revealSection(resultsRef.current);
    go();
    setTimeout(go, 0);
  }, []);
  ...
  return (
    <div className={`space-y-4 ${c.text}`}>
      ...
      {/* Error */}
      {error && (
        <div data-copy-results ref={resultsRef} className={`scroll-mt-24 ${c.danger} border rounded-xl p-4 flex items-start gap-3`}>
