...
const RentersDepositSaver = ({ tool }) => {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      setResults(data);
      setHistory(prev => [{ id: Date.now(), date: new Date().toISOString(), preview: (address || region || '').slice(0, 40) }, ...prev].slice(0, 6));
      setStep(3);
    } catch (err) {
...
<pre>${escaped}</pre>
  ...
  canSubmitRef.current = step === 2 && !loading && !!step2Valid && completionStats.done > 0;
                    ...
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex gap-1.5">
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold ${c.btnPrimary} transition-colors`}
                        >
          ...
          <div className={`${c.card} border rounded-2xl shadow-lg p-3 flex flex-wrap items-center gap-1.5`}>
            ...
            <div className="ml-auto flex gap-1.5">
              <button
                onClick={printAll}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-colors ${
                  isDark ? 'bg-zinc-700 text-zinc-300 hover:bg-zinc-600' : 'bg-zinc-100 text-gray-600 hover:bg-zinc-200'
                }`}
              >
          ...
          {expandedSection === 'report' && results.condition_report && (
            <div className={`${c.card} border rounded-2xl shadow-lg p-5 sm:p-6`}>
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex gap-1.5">
                  <CopyBtn content={results.condition_report} label="Copy" />
                  <button
                    onClick={() => printSection('Move-In Condition Report', results.condition_report)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold ${c.btnPrimary} transition-colors`}
                  >
          ...
          {expandedSection === 'letter' && results.landlord_letter && (
            <div className={`${c.card} border rounded-2xl shadow-lg p-5 sm:p-6`}>
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex gap-1.5">
                  <CopyBtn content={results.landlord_letter} label="Copy" />
                  <button
          ...
          {expandedSection === 'photos' && results.photo_shot_list && (
            <div className={`${c.card} border rounded-2xl shadow-lg p-5 sm:p-6`}>
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex gap-1.5">
                  <CopyBtn content={results.photo_shot_list} label="Copy" />
                  <button
          ...
          {expandedSection === 'rights' && results.deposit_rights && (
            <div className={`${c.card} border rounded-2xl shadow-lg p-5 sm:p-6`}>
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex gap-1.5">
                  <CopyBtn content={results.deposit_rights} label="Copy" />
                  <button
