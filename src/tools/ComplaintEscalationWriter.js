...
// Five, rotated: an airline, a contractor holding a deposit hostage, a
// phone carrier's broken quote, a healthcare billing-code error, and a
// subscription that kept charging after cancellation.
const EXAMPLES = [
  {
  ...
  {
  company: 'Brightline Home Renovations',
  industry: 'contractor',
  issue: "Paid a 4,000 deposit in March for a bathroom remodel that was quoted at six weeks. Demo happened in April and then nothing. The bathroom has been unusable since. Three scheduled start dates have come and gone with a text the morning of each one. Last contact was five weeks ago.",
  previousAttempts: 'Texted the owner directly eight or nine times — sometimes a reply the same day, usually nothing. Called twice, went to voicemail. Left one voicemail asking for a written schedule; no response.',
},
  {
  company: 'Horizon Mobile',
  industry: 'telecom',
  issue: "Switched plans in June and was quoted a $45/month rate in writing over chat. Every bill since has been $71. Support says the chat quote 'does not reflect current promotions' and has refused to honor it. This is the fourth billing cycle at the wrong rate.",
  previousAttempts: 'Three chat sessions (transcripts saved) where different agents gave three different explanations. One phone call where I was transferred four times and then disconnected. No callback despite a promise of one.',
},
  {
  company: 'Riverside Health Clinic',
  industry: 'healthcare',
  issue: "Billed $1,240 for a lab panel my insurance says should have been billed at the in-network rate — the clinic used an out-of-network billing code for an in-network provider. Insurance has confirmed the error in writing but the clinic's billing department has not corrected the claim in ten weeks.",
  previousAttempts: 'Insurance filed a corrected-claim request on my behalf twice. I have called the clinic billing office five times; twice was told "it is in process," three times got voicemail. One email to the practice manager, no reply after three weeks.',
},
  {
  company: 'StreamBox Plus',
  industry: 'subscription',
  issue: "Cancelled my subscription in the app in March. Charged monthly every month since — six charges totaling $71.94. The app shows my account as 'cancelled' but billing continued regardless.",
  previousAttempts: 'Two support tickets through the app, both closed by an agent with a canned "please try cancelling again" reply and no refund offered. One live chat where the agent confirmed the account showed cancelled but said billing was "a separate system" they could not access.',
},
];
const ComplaintEscalationWriter = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    const ex = pickExample('ComplaintEscalationWriter', EXAMPLES);
