  ...
  useEffect(() => {
    const handler = (e) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
        e.preventDefault();
        if (!loading && company.trim() && issue.trim()) handleSubmit();
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setComplaintHistory(prev => [{
        id, company: company.trim(), industry, issue: issue.trim().slice(0, 120),
        severity: data.situation_assessment?.severity || 'medium', date: new Date().toISOString(), tone,
      }, ...prev].slice(0, 6));
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      {showHistory && complaintHistory.length > 0 && (
        <div className={`${c.card} border ${c.border} rounded-xl shadow-lg p-5`}>
          <h3 className={`text-sm font-bold ${c.text} mb-3`}>🕐 Past Complaints</h3>
      ...
      {results && (
        <div ref={resultsRef} className="space-y-4">
          ...
          {/* Controls bar — ActionBar at top of results */}
          <div className={`${c.card} border ${c.border} rounded-xl shadow-lg p-4 flex items-center justify-between flex-wrap gap-3`}>
            ...
            <div className="flex items-center gap-2">
              <ActionBar content={buildFullText()} title="Complaint Escalation Writer" />
              <button onClick={reset} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium ${c.resetBtn}`}>
          ...
          <div className={`${c.cardAlt} border ${c.border} rounded-xl p-4`}>
            ...
            <p className={`text-sm ${c.textSecondary}`}>
              ...
              {' '}Unsure whether to escalate or accept?{' '}
              <a href="/DecisionCoach" className={linkStyle}>Decision Coach</a>{' '}
              helps you think it through.
              {amountAtStake && (
                <>{' '}Dealing with billing issues?{' '}<a href="/BillRescue" className={linkStyle}>Bill Rescue</a>{' '}analyzes unfair charges.</>
              )}
            </p>
          </div>
