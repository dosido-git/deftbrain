      ...
      return (
        <div className={`rounded-xl border ${c.border} ${c.bgCard} overflow-hidden mb-3`}>
          <button onClick={() => toggleSection(key)} className="w-full flex items-center gap-2 p-4 text-left">
            ...
            {isOpen ? <span className={c.textMut}>▲</span> : <span className={c.textMut}>▼</span>}
          </button>
    ...
    return (
      <div>
        ...
        <button onClick={submitAssessment} disabled={loading || !route.trim()}
          ...
          {loading ? <><span className="inline-block animate-spin">{tool?.icon ?? "🚶"}</span> Assessing your route...</>
            : <><span>🛡️</span> Assess My Walk</>}
        </button>
      </div>
    );
  };
          ...
          {!walkTimer && !timerExpired ? (
            ...
            <div className="text-center">
              <div className="animate-pulse mb-4">
                <span className="text-amber-400 text-5xl block mx-auto mb-2">⚠️</span>
    ...
    <div className={c.text}>
      ...
      {error && (
        <div className={`mt-4 p-4 ${c.errBg} border rounded-xl flex items-start gap-3`}>
          <span className={`${c.errText} flex-shrink-0 mt-0.5`}>⚠️</span>
