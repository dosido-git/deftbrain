...
function SleepArchitect({ tool }) {
  ...
  const EXAMPLES = [
    { goals: ['fall_asleep', 'wake_rested'], bedtime: '12:00 AM', wakeTime: '7:00 AM', hoursActual: '5',
  ...
  const loadExample = useCallback(() => {
    const ex = pickExample('SleepArchitect', EXAMPLES);
    setGoals(ex.goals);
    setBedtime(ex.bedtime);
    setWakeTime(ex.wakeTime);
    setHoursActual(ex.hoursActual);
    setDisruptors(ex.disruptors);
  ...
  const renderInput = () => (
    <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5 space-y-5`}>
      ...
      <button title={t('cmd_enter')} onClick={handleBuild} disabled={!canSubmit || loading}
        ...
        {loading
          ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '😴'}</span> {t('sa_building')}</>
          : <><span className="me-1 text-xl">{tool?.icon ?? '😴'}</span> {t('sa_build_btn')}</>}
      {!loading && (
  ...
  const hasInput = !!(results || bedtime.trim() || disruptors.length || freeform.trim() || goals.length || hoursActual.trim() || wakeTime.trim());
