...
export default function BeforeTheCrash() {
  ...
  // PF-2 — small_experiment was one field doing two jobs; accept the old
  // shape so a response in flight during deploy still renders.
  const nextStep = analysis?.next_step || analysis?.small_experiment || null;
  ...
  // The storage key keeps the old name on purpose. It holds real check-ins
  // people have recorded over weeks; renaming the tool must not empty them.
  ...
  useRegisterActions(buildFullText(), tool?.title || 'Before the Crash');
                  ...
                  <div className="space-y-4">
                    ...
                    {nextStep?.try && (
                      <div className="rounded-lg border p-3">
                        ...
                        <p className="text-sm mt-2"><strong>{t('cpv2_watch')}</strong> {nextStep.watch}</p>
                        <p className={`text-xs mt-1 ${c.textMuted}`}>{nextStep.why_this_one}</p>
                      </div>
                    )}
                  </div>
                ) : (
...
BeforeTheCrash.displayName = 'BeforeTheCrash';
