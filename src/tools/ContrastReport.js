...
// Derive a meaningful stage message from the accumulating raw JSON text
const getStreamingStage = (text) => {
  if (text.includes('"what_i_noticed"'))  return { label: 'Finding what I noticed…',  pct: 85 };
  if (text.includes('"path_b"'))          return { label: 'Living Path B…',            pct: 55 };
  if (text.includes('"path_a"'))          return { label: 'Living Path A…',            pct: 25 };
  return                                         { label: 'Imagining both futures…',   pct: 8  };
};

const ContrastReport = () => {
  const { callToolEndpoint, callToolEndpointStreaming, loading } = useClaudeAPI();
  ...
  const c = {
    ...
    progressBg:    isDark ? 'bg-[#3d3630]'     : 'bg-[#e8e1d5]',
    progressFill:  isDark ? 'bg-[#4a6a8a]'     : 'bg-[#2c4a6e]',
  };
  ...
  const [streamStage, setStreamStage] = useState({ label: 'Imagining both futures…', pct: 8 });
  ...
  const handleSubmit = async () => {
    ...
    setResults(null);
    setStreamingText('');
    setStreamStage({ label: 'Imagining both futures…', pct: 8 });
    setActiveNarrative('a');

    callToolEndpointStreaming(
      'contrast-report/stream',
      {
        pathA: pathA.trim(),
        pathB: pathB.trim(),
      ...
      },
      {
        onChunk: (accumulated) => {
          ...
          setStreamStage(getStreamingStage(accumulated));
        },
        onDone: (parsed) => {
          setStreamingText('');
          setResults(parsed);
        ...
        onError: (msg) => {
          setStreamingText('');
          setError(msg || 'Failed to generate contrast report.');
        },
      }
    );
  };
  ...
  const StreamingProgress = () => (
    <div className={`${c.card} ${c.cardBorder} border rounded-2xl p-8 shadow-sm text-center space-y-6`}>
      {/* Animated orb */}
      <div className="flex justify-center">
        <div className="relative w-16 h-16">
          <div className="absolute inset-0 rounded-full opacity-20 animate-ping"
            style={{ background: isDark ? '#4a6a8a' : '#2c4a6e', animationDuration: '2.5s' }} />
          <div className="absolute inset-2 rounded-full flex items-center justify-center text-2xl"
            style={{ background: isDark ? '#2a2623' : '#edf1f7' }}>
            🔮
          </div>
        </div>
      </div>
      ...
      <div className="space-y-1">
        <p className={`text-sm font-semibold ${c.heading}`}>{streamStage.label}</p>
        <p className={`text-xs ${c.textMuted}`}>Writing your futures in real time</p>
      </div>

      {/* Progress bar */}
      <div className={`w-full h-1.5 rounded-full ${c.progressBg} overflow-hidden`}>
        <div
          className={`h-full rounded-full ${c.progressFill} transition-all duration-700 ease-out`}
          style={{ width: `${streamStage.pct}%` }}
        />
      </div>
      ...
      <div className="flex items-center justify-center gap-6">
        {[
          { label: 'Path A', threshold: '"path_a"' },
          { label: 'Path B', threshold: '"path_b"' },
          { label: 'Reflection', threshold: '"what_i_noticed"' },
        ].map(({ label, threshold }) => {
          const done = streamingText.includes(threshold);
          return (
            <div key={label} className="flex flex-col items-center gap-1.5">
              <div className={`w-2 h-2 rounded-full transition-all duration-500 ${
                done
                  ? (isDark ? 'bg-[#4a6a8a]' : 'bg-[#2c4a6e]')
                  : (isDark ? 'bg-[#3d3630]' : 'bg-[#e8e1d5]')
              }`} />
              <span className={`text-xs ${done ? c.textSecondary : c.textMuted}`}>{label}</span>
            </div>
          );
        })}
      </div>
        ...
        {!results && !loading && (
          <div className={`${c.card} ${c.cardBorder} border rounded-2xl p-6 shadow-sm space-y-5`}>
            ...
            <button
              ...
              disabled={!pathA.trim() || !pathB.trim()}
