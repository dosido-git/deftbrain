...
const DifficultTalkCoach = ({ tool }) => {
  ...
  const resistanceLabels = [
    { min: 0, max: 20, label: 'Very receptive', colorKey: 'resReceptive' },
    { min: 21, max: 40, label: 'Somewhat open', colorKey: 'resOpen' },
    { min: 41, max: 60, label: 'Defensive', colorKey: 'resDefensive' },
    { min: 61, max: 80, label: 'Very resistant', colorKey: 'resResistant' },
    { min: 81, max: 100, label: 'Hostile / Shutdown', colorKey: 'resHostile' },
  ];
        ...
        <div className="space-y-5">
          ...
          <div className={`p-4 rounded-2xl border ${c.border} ${isDark ? 'bg-zinc-800/60' : 'bg-slate-50'}`}>
            <p className={`text-xs ${c.textMuted}`}>
              💡 Ongoing friction with this person?{' '}<a href="/ConflictCoach" className={linkStyle}>Conflict Coach</a>{' '}
              maps the dynamic before you plan the talk.
            </p>
          </div>
        </div>
      )}
          ...
          {activeTab === 'prepare' && (
            <div className="space-y-5">
              ...
              <div className={`p-4 rounded-2xl border ${c.border} ${isDark ? 'bg-zinc-800/60' : 'bg-slate-50'} mt-2`}>
                <p className={`text-xs ${c.textMuted}`}>
                  💡 Need diplomatic words?{' '}<a href="/VelvetHammer" className={linkStyle}>Velvet Hammer</a>{' '}
                  crafts the message. Conversation calls for an apology?{' '}<a href="/ApologyCalibrator" className={linkStyle}>Apology Calibrator</a>{' '}
                  gets the tone right.
                </p>
              </div>
            </div>
              ...
              <div className={`${c.card} rounded-xl shadow-lg p-6`}>
                ...
                {debriefMode === 'real' ? (
                  <>
                    ...
                    <label htmlFor="dtc-how-it-went" className={`block text-sm font-semibold ${c.textSecondary} mb-1`}>What happened? <span className={c.required}>*</span></label>
                    <textarea
                      id="dtc-how-it-went"
                      value={howItWent}
