...
const SkillGapMap = ({ tool }) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setResults(data);
      setSessionHistory(prev => [{ id: Date.now(), date: new Date().toISOString(), preview: (targetRole || currentRole || '').slice(0, 40) }, ...prev].slice(0, 6)); // PF-25 exception: slice(0,40) is preview truncation; outer history cap is 6
              ...
              <div className="p-5 space-y-3">
                ...
                {resumeData && (
                  <div className="space-y-4">
                    ...
                    {resumeData.summary_suggestion && <div className="flex items-start gap-2"><p className={`text-xs ${c.text} flex-1`}>📝 Summary: {resumeData.summary_suggestion}</p></div>}
              ...
              <div className="p-5 space-y-3">
                ...
                {reframeData && (
                  <div className="space-y-3">
                    ...
                    {reframeData.elevator_pitch && <div className={`${c.warningBox} border rounded-lg p-3`}><p className={`text-[10px] font-bold ${c.accentTxt}`}>🎤 ELEVATOR PITCH</p><p className={`text-sm ${c.text} italic`}>"{reframeData.elevator_pitch}"</p></div>}
