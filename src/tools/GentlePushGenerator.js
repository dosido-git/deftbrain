...
const GentlePushGenerator = ({ tool }) => {
  ...
  const buildFullText = useCallback(() => {
    ...
    if (celebrationVault.length > 0) {
      return [
        'CELEBRATION VAULT — My Brave Moments', '═'.repeat(35), '',
        ...celebrationVault.map(p => `• ${p.challenge} (${new Date(p.date).toLocaleDateString()})${p.scariness ? ` — scariness ${p.scariness}/5` : ''}${p.outcome ? ` — "${p.outcome}"` : ''}`),
        BRAND,
      ].join('\n');
    }
    return '';
  }, [activePush, celebrationVault]);

  useRegisterActions(buildFullText(), tool?.title || 'Gentle Push Generator');
  ...
  if (view === 'setup') {
    return (
      <div className={`space-y-4 ${c.text}`}>
          ...
          <div className={`${c.card} ${c.border} border rounded-xl p-5 space-y-3`}>
            <label className={`block text-sm font-semibold ${c.text}`}>
              Where do you want to grow? <span className={c.required}>*</span>
            </label>
          ...
          {!growthArea.trim() && !loading && (
            <div className="flex justify-center">
              <button
                onClick={() => {
                  setDomain('social');
                  setComfortZone('I keep conversations surface-level with new people and never share what I actually think.');
                  setGrowthArea('Be a little more honest in conversations — share an opinion or feeling I usually keep to myself.');
                  setCapacity('medium');
                }}
          ...
          <p className={`text-xs text-center ${c.textMuted}`}>
            If your push involves a hard conversation,{' '}
            <a href="/DifficultTalkCoach" className={linkStyle}>🗣️ Difficult Talk Coach</a>{' '}
            lets you rehearse it before the moment comes.
          </p>
      ...
      <div className={`space-y-4 ${c.text}`}>
          ...
          {!showFeedback ? (
            ...
            <div className={`${c.card} ${c.border} border rounded-xl p-5 space-y-3`}>
              ...
              <label htmlFor="gpg-feedback-custom" className="sr-only">Custom feedback for regeneration</label>
              <input id="gpg-feedback-custom" type="text" value={feedbackCustom} onChange={e => setFeedbackCustom(e.target.value)}
                ...
                placeholder="Or describe what you need... (Cmd/Ctrl+Enter to submit)"
                className={`w-full p-2.5 rounded-lg border ${c.input} text-sm`} />
              <div className="flex gap-2">
    ...
    return (
      <div className={`space-y-4 ${c.text}`}>
          ...
          <div className={`${c.card} ${c.border} border rounded-xl p-4 space-y-1`}>
            ...
            <p className={`text-[10px] ${c.textMuted}`}>Use the copy button at the top to grab this push, then paste into your reminders app, calendar, or notes.</p>
          </div>
  ...
  if (view === 'vault') {
    return (
      <div className={`space-y-4 ${c.text}`}>
          ...
          <p className={`text-xs text-center ${c.textMuted}`}>
            Stuck on what to push toward next? <a href="/DifficultTalkCoach" className={linkStyle}>🗣️ Difficult Talk Coach</a> turns hard conversations into rehearsable scenes.
          </p>
