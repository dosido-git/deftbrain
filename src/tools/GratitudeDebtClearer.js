...
const EXAMPLES = [
  {
    recipientName: 'Sarah',
    gratitudePoints: "She went to bat for me during a reorg and kept my role intact. Then coached me through a rough performance period without making me feel judged. Never made a big deal of it.",
    context: 'Work kindness',
...
const GratitudeDebtClearer = ({ tool }) => {
  ...
  const c = {
    ...
    // tool-specific
    accentCard:    isDark ? 'bg-cyan-900/20 border-cyan-700' : 'bg-cyan-50 border-cyan-200',
    btnDisabled:   isDark ? 'bg-zinc-700 text-zinc-500' : 'bg-gray-200 text-gray-400',
    giftInput:     isDark ? 'bg-zinc-900 border-zinc-600 text-zinc-100 placeholder-zinc-400 focus:border-amber-500' : 'bg-white border-gray-300 text-gray-900 placeholder-gray-400 focus:border-amber-500',
    giftSection:   isDark ? 'bg-amber-900/20 border-amber-700 border rounded-xl' : 'bg-amber-50 border-amber-200 border rounded-xl',
    handwritingCheck: isDark ? 'bg-cyan-900/20 border-cyan-700 hover:border-cyan-500' : 'bg-cyan-50/50 border-cyan-200 hover:border-cyan-400',
    highlight:     isDark ? 'bg-cyan-900/20 border-cyan-700 text-cyan-200' : 'bg-cyan-50 border-cyan-200 text-cyan-800',
    messageBody:   isDark ? 'bg-zinc-700/50 border-zinc-600' : 'bg-slate-50 border-slate-200',
    messageCard:   isDark ? 'bg-zinc-800 border-zinc-600' : 'bg-white border-slate-200',
    select:        isDark ? 'bg-zinc-900 border-zinc-600 text-zinc-100 focus:border-cyan-500 focus:ring-cyan-500/20' : 'bg-white border-gray-300 text-gray-900 focus:border-cyan-500 focus:ring-cyan-100',
    toneBadge:     isDark ? 'bg-emerald-900/30 text-emerald-300' : 'bg-emerald-50 text-emerald-700',
  };
  ...
  const checkToneCalibration = (selectedTone, rel, ctx) => {
    ...
    if (selectedTone === 'Heartfelt & emotional' && rel === 'Professional') {
      mismatches.push({ suggested: 'Professional', reason: t('gdc_tone_reason_pro') });
    }
    if (selectedTone === 'Professional' && rel === 'Family' && (ctx === 'Emotional support' || ctx === 'General kindness' || ctx === 'Condolence support')) {
      mismatches.push({ suggested: 'Heartfelt & emotional', reason: t('gdc_tone_reason_warm') });
    }
    if (selectedTone === 'Brief' && (ctx === 'Condolence support' || ctx === 'Mentorship')) {
      mismatches.push({ suggested: 'Heartfelt & emotional', reason: t('gdc_tone_reason_depth') });
    }
  ...
  const handleFollowUp = async () => {
    ...
    try {
      ...
      setError(err.message || t('gdc_err_followup'));
    }
  ...
  // Map a stored (English) tone label to its localized display string.
  const toneLabelDisplay = (label) => {
    const found = toneOptions.find(o => o.label === label);
    return found ? t(`gdc_tone_${found.id}`) : label;
  };
  ...
  const buildFullText = useCallback(() => {
    ...
    const combined = results.thank_you_messages
      .map((m, i) => `── ${m.version} (${m.tone}, ${t('gdc_words', { count: m.length })}) ──\n\n${getMessageText(i, m.message_text)}\n\n${t('gdc_copy_why_works')} ${m.why_this_works}\n${t('gdc_copy_best_for')} ${m.best_for}`)
      .join('\n\n────────────────────────────────\n\n');
    const extras = [];
    if (results.delivery_suggestions) {
      extras.push(`\n── ${t('gdc_copy_delivery')} ──\n${t('gdc_copy_method')} ${results.delivery_suggestions.method}\n${t('gdc_copy_timing')} ${results.delivery_suggestions.timing}`);
    }
    if (results.personalization_tips?.length) {
      extras.push(`\n── ${t('gdc_copy_tips')} ──\n${results.personalization_tips.map(tip => '• ' + tip).join('\n')}`);
    }
    return `${t('gdc_copy_header', { name: recipientName })}\n${'═'.repeat(40)}\n\n${combined}${extras.join('\n')}${BRAND}`;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [results, recipientName, editedMessages, t]);
  ...
  const handleGenerate = async () => {
    if (!recipientName.trim() || !gratitudePoints.trim()) {
      ...
      validationTimerRef.current = setTimeout(() => setValidationFailed(false), 2500);
      return;
    }
  ...
  useRegisterActions(buildFullText(), tool?.title || 'Gratitude Debt Clearer');
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tone, relationship, context, t]);

  const handleGenerateRef = useRef(null);
  const validationTimerRef = useRef(null);
  const canSubmitRef = useRef(false);
  handleGenerateRef.current = handleGenerate;
  canSubmitRef.current = !!recipientName.trim() && !!gratitudePoints.trim();
    ...
    <div className={`space-y-4 ${c.text}`}>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-base ${c.textSecondary}`}>
                <span className="me-2 text-lg">{tool?.icon ?? '💝'}</span>{tool?.tagline ?? t('gdc_tagline')}
              </p>
          ...
          <div className="flex items-center gap-2">
            ...
            {messageHistory.length > 0 && (
              <button
                ...
                title={t('gdc_sent_title')}
              >
                <span>📜</span>
                <span className="hidden sm:inline">{t('gdc_sent')}</span>
      ...
      {showLedger && (
        <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5 transition-colors duration-200`}>
          <div className="flex items-center gap-2 mb-3">
            <span>📖</span>
            <h3 className={`text-lg font-bold ${c.text}`}>{t('gdc_debt_ledger_title')}</h3>
            <span className={`text-xs ${c.textMuteded}`}>
              {t('gdc_ledger_count', { pending: ledger.filter(i => !i.done).length, cleared: ledger.filter(i => i.done).length })}
            </span>
          </div>
          ...
          {batchMode ? (
            <div className="space-y-3">
              <p className={`text-xs ${c.textSecondary}`}>{t('gdc_batch_intro')}</p>
              ...
              <div className="flex gap-2">
                <input type="text" placeholder={t('gdc_ph_name')} value={newBatchName}
                  onChange={e => setNewBatchName(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && addBatchEntry()}
                  className={`flex-1 px-3 py-2 border rounded-lg text-sm outline-none ${c.input}`} />
                <input type="text" placeholder={t('gdc_ph_gift_reason')} value={newBatchGift}
              ...
              {batchEntries.length > 0 && (
                <div className="space-y-1.5">
                    ...
                    <div key={entry.id} className={`flex items-center justify-between px-3 py-2 rounded-lg border ${c.cardAlt} ${c.border}`}>
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => {
                          ...
                          setGratitudePoints(entry.gift || t('gdc_thank_you_for', { context: batchContext.toLowerCase() }));
                          setContext(batchContext);
                          setRelationship(batchRelationship);
                          setResults(null);
                          setBatchMode(false);
                          setShowLedger(false);
                        }} className={`px-2 py-0.5 rounded text-[10px] font-bold ${c.btnPrimary}`}>{t('gdc_generate')}</button>
                  ...
                  <button onClick={() => {
                    ...
                    📖 {t('gdc_add_all', { count: batchEntries.length })}
                  </button>
                </div>
              )}
            </div>
          ...
          <div className="flex gap-2 mb-4">
            <input
              type="text"
              placeholder={t('gdc_ph_owe_thanks')}
              value={newLedgerName}
              ...
              {ledger.map(item => (
                ...
                >
                  <button
                    ...
                    title={item.done ? t('gdc_mark_not_sent') : t('gdc_mark_sent_short')}
                  >
      ...
      {showHistory && messageHistory.length > 0 && (
        <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5 transition-colors duration-200`}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <span>📜</span>
              <h3 className={`text-lg font-bold ${c.text}`}>{t('gdc_sent_title')}</h3>
              <span className={`text-xs ${c.textMuteded}`}>({messageHistory.length})</span>
            </div>
            <button onClick={() => { if (window.confirm(t('gdc_clear_history_confirm'))) setMessageHistory([]); }}
              className={`text-xs ${c.textMuteded} hover:${isDark ? 'text-red-400' : 'text-red-500'}`}>{t('gdc_clear_all')}</button>
          </div>
          ...
          {(() => {
            ...
            return nudges.length > 0 ? (
              <div className={`mb-4 p-3 rounded-lg border ${c.accentCard}`}>
                <p className={`text-xs font-bold mb-2`}>🔔 {t('gdc_followup_opportunities')}</p>
                {nudges.map(n => (
                  <div key={n.id} className="flex items-center justify-between mb-1.5">
                    <span className={`text-xs`}>
                      {t('gdc_thanked_for_on', { name: n.recipientName, context: n.context.toLowerCase(), date: new Date(n.sentAt).toLocaleDateString() })}
                    </span>
                  ...
                  {h.reaction ? (
                    ...
                    <div className="flex items-center gap-1">
                      <div className="flex gap-1">
                        {[t('gdc_reaction_loved'), t('gdc_reaction_surprised'), t('gdc_reaction_touched'), t('gdc_reaction_no_response')].map(r => (
                      ...
                      <input type="text" value={reactionText} onChange={e => setReactionText(e.target.value)}
                        ...
                        placeholder={t('gdc_ph_custom')} className={`w-20 px-1.5 py-0.5 text-[10px] border rounded ${c.input}`} />
        ...
        <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-5`}>
          <div className="flex items-center justify-between mb-3">
            <h3 className={`text-lg font-bold ${c.text} flex items-center gap-2`}>
              <span>🔁</span> {t('gdc_follow_up_with', { name: followUpTarget.recipientName })}
            </h3>
          ...
          <p className={`text-xs ${c.textSecondary} mb-3`}>
            {t('gdc_thanked_detail', { name: followUpTarget.recipientName, context: followUpTarget.context.toLowerCase(), date: new Date(followUpTarget.sentAt).toLocaleDateString() })}
            {followUpTarget.reaction ? t('gdc_they_reacted', { reaction: followUpTarget.reaction }) : ''}
          </p>
          <div className={`p-3 rounded-lg border mb-3 ${c.messageBody}`}>
            <p className={`text-xs ${c.textMuteded} mb-1`}>{t('gdc_original_message')}</p>
            <p className={`text-xs ${c.textSecondary} italic`}>{followUpTarget.messageText.slice(0, 200)}…</p>
          </div>
          <label className={`block text-sm font-medium ${c.textSecondary} mb-2`}>{t('gdc_whats_update')} <span className={c.required}>*</span></label>
          <textarea value={followUpOutcome} onChange={e => setFollowUpOutcome(e.target.value)}
            placeholder={t('gdc_ph_outcome')}
          ...
          <button onClick={handleFollowUp} disabled={followUpLoading || !followUpOutcome.trim()}
            ...
            {followUpLoading ? (<><span className="animate-spin inline-block">{tool?.icon ?? '💝'}</span> {t('gdc_generating')}</>) : (<><span>{tool?.icon ?? '💝'}</span> {t('gdc_generate_followup')}</>)}
          </button>
      ...
      <div className={`${c.card} ${c.border} border rounded-xl shadow-sm p-6`} onKeyDown={handleFormKeyDown}>
          ...
          <div className="mb-6">
            ...
            >
              {t('gdc_who_thanking')} <span className={c.required}>*</span>
            </label>
          ...
          <div className="mb-6">
            <label 
              ...
              className={`block text-lg font-semibold ${c.text} mb-3`}
            >
              {t('gdc_what_grateful')} <span className={c.required}>*</span>
            </label>
            <textarea
              id="gratitude-points"
            ...
            {gratitudePoints.trim().length > 10 && !specificityData && !specificityDismissed && !results && (
              <button onClick={checkSpecificity} disabled={specificityLoading}
                ...
                {specificityLoading ? (<><span className="animate-spin inline-block">{tool?.icon ?? '💝'}</span> {t('gdc_checking')}</>) : (<><span>{tool?.icon ?? '💝'}</span> {t('gdc_check_specific')}</>)}
              </button>
            )}
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs font-bold ${c.text}`}>
                      {specificityData.specificity_level === 'vague' ? t('gdc_vague_title') : t('gdc_good_start_title')}
                    </span>
                  </div>
                  <button onClick={() => setSpecificityDismissed(true)} className={`text-xs ${c.textMuteded}`}>{t('gdc_skip')}</button>
                </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder={t('gdc_ph_gift_details')}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
              <label 
                htmlFor="context"
                className={`block text-sm font-medium ${c.textSecondary} mb-2`}
              >
                {t('gdc_context_occasion')}
              </label>
              <select
                id="context"
                value={context}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {toneOptions.map(option => (
                ...
                >
                  ...
                  <div className="text-sm font-medium">{t(`gdc_tone_${option.id}`)}</div>
                </button>
              ))}
            </div>
            ...
            {toneWarning && (
              <div className={`mt-3 p-3 rounded-lg border ${c.warning}`}>
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className={`text-xs font-bold`}>{t('gdc_consider_tone', { tone: toneLabelDisplay(toneWarning.suggested) })}</p>
                    ...
                    <button onClick={() => setTone(toneWarning.suggested)}
                      ...
                      {t('gdc_switch_to', { tone: toneLabelDisplay(toneWarning.suggested) })}
                    </button>
                  </div>
                </div>
              </div>
          ...
          <div className="mb-6">
            <label className={`flex items-center gap-3 p-4 border rounded-lg cursor-pointer transition-colors ${c.handwritingCheck}`}>
              ...
              <div className="flex-1">
                <div className={`font-medium ${isDark ? 'text-cyan-200' : 'text-cyan-900'}`}>{t('gdc_gen_handwritten')}</div>
                <div className={`text-sm ${isDark ? 'text-cyan-300' : 'text-cyan-700'}`}>
                  {t('gdc_handwritten_desc')}
                </div>
              </div>
            </label>
          </div>
          ...
          <div className="mb-6">
            <label 
              ...
              className={`block text-sm font-medium ${c.textSecondary} mb-3`}
            >
              {t('gdc_msg_length')} <span className={`font-semibold ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>
                {length <= 3 ? t('gdc_len_concise') : length <= 7 ? t('gdc_len_moderate') : t('gdc_len_detailed')}
              </span>
            </label>
            <input
              id="length-slider"
              ...
              aria-label={t('gdc_msg_length')}
            />
            <div className={`flex justify-between text-xs ${c.textMuteded} mt-1`}>
              <span>{t('gdc_short_sweet')}</span>
              <span>{t('gdc_detailed_thorough')}</span>
            </div>
          </div>
          ...
          <p className={`text-xs text-center ${c.textMuteded} mt-2`}>
            <kbd className={`px-1.5 py-0.5 rounded text-xs font-mono ${isDark ? 'bg-zinc-700' : 'bg-slate-100'}`}>{t('gdc_ctrl')}</kbd>+<kbd className={`px-1.5 py-0.5 rounded text-xs font-mono ${isDark ? 'bg-zinc-700' : 'bg-slate-100'}`}>{t('gdc_enter')}</kbd> {t('gdc_shortcut_hint')}
          </p>
        ...
        {results && (
          <div data-copy-results ref={resultsRef} className="space-y-6">
            ...
            <div>
                ...
                <div className="flex items-center gap-2">
                  {results.thank_you_messages?.length > 1 && (
                    <button onClick={() => setShowComparison(!showComparison)}
                      ...
                      <span>{showComparison ? '📋' : '⚖️'}</span> {showComparison ? t('gdc_full_view') : t('gdc_compare')}
                    </button>
                  )}
                </div>
              </div>
              ...
              {results.tone_calibration && (
                <div className={`mb-4 p-3 rounded-lg border ${c.warning}`}>
                  {results.tone_calibration.suggested_tone && <p className={`text-xs font-bold mb-0.5`}>{t('gdc_suggested', { tone: results.tone_calibration.suggested_tone })}</p>}
                  <p className="text-xs"><strong>💡 {t('gdc_tone_suggestion')}</strong> {results.tone_calibration.reason}</p>
                </div>
              )}
                  ...
                  <div className={`grid grid-cols-${Math.min(results.thank_you_messages.length, 3)} divide-x rtl:divide-x-reverse ${isDark ? 'divide-zinc-700' : 'divide-slate-200'}`}>
                    {results.thank_you_messages.map((msg, i) => (
                      <div key={i} className="p-4">
                        ...
                        <p className={`text-[10px] ${c.textMuteded} mb-2`}>{t('gdc_words', { count: msg.length })} · {msg.best_for}</p>
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {isEdited && <span className={`text-xs ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>✏️ {t('gdc_edited')}</span>}
                        </div>
                        <div className={`flex items-center gap-3 text-sm ${c.textSecondary}`}>
                          ...
                          <span>{t('gdc_words', { count: message.length })}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className={`border-s-4 rounded-e-lg p-3 mb-4 ${c.highlight}`}>
                      ...
                      <p className={`text-sm mt-1 ${isDark ? 'text-cyan-300' : 'text-cyan-800'}`}>
                        <strong>{t('gdc_best_for')}</strong> {message.best_for}
                      </p>
                    </div>
                    ...
                    {isEditing ? (
                      <div className="flex gap-2 mb-3">
                        <button onClick={() => setEditingIndex(null)}
                          className={`px-3 py-1.5 rounded-lg text-xs font-bold ${c.btnPrimary}`}>{t('gdc_done_editing')}</button>
                        <button onClick={() => cancelEditing(index, message.message_text)}
                          className={`text-xs ${c.textMuteded}`}>{t('gdc_reset_original')}</button>
                      </div>
                    ) : null}
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        aria-label={t('gdc_too_mushy_aria')}
                      >
                        {adjustingIndex === index ? (
                          <><span className="animate-spin inline-block">{tool?.icon ?? '💝'}</span> {t('gdc_adjusting')}</>
                        ) : (
                          <><span>➖</span> {t('gdc_too_mushy')}</>
                        )}
                      </button>
                      ...
                      <button
                        ...
                        aria-label={t('gdc_more_specific_aria')}
                      >
                        {adjustingIndex === index ? (
                          <><span className="animate-spin inline-block">{tool?.icon ?? '💝'}</span> {t('gdc_adjusting')}</>
                        ) : (
                          <><span>➕</span> {t('gdc_more_specific')}</>
                        )}
                      </button>
                    </div>
                    ...
                    <div className={`flex flex-wrap gap-1.5 mt-2 pt-2 border-t ${isDark ? 'border-zinc-700' : 'border-slate-200'}`}>
                      ...
                      <a href={buildSmsLink(messageText)}
                        className={`flex items-center gap-1 px-2 py-1 rounded text-[10px] font-medium ${c.btnSecondary}`}>
                        <span>📱</span> {t('gdc_text')}
                      </a>
                      <a href={buildWhatsAppLink(messageText)}
                        className={`flex items-center gap-1 px-2 py-1 rounded text-[10px] font-medium ${c.btnSecondary}`}>
                        <span>💬</span> {t('gdc_whatsapp')}
                      </a>
                    </div>
                  </div>
                  );
            ...
            {results.delivery_suggestions && (
              <div className={`border-2 rounded-xl p-5 ${c.success}`}>
                ...
                <div className={`space-y-2 ms-8 ${isDark ? 'text-emerald-200' : 'text-emerald-800'}`}>
                  ...
                  <p>
                    <strong>{t('gdc_timing')}</strong> {results.delivery_suggestions.timing}
                  </p>
                  {results.delivery_suggestions.timing_cultural_note && (
                    <p className={`p-2 rounded ${isDark ? 'bg-emerald-900/30' : 'bg-emerald-100'}`}>
                      <strong>{t('gdc_cultural_note_label')}</strong> {results.delivery_suggestions.timing_cultural_note}
                    </p>
                  )}
                  {results.delivery_suggestions.additional_gesture && (
                    <p>
                      <strong>{t('gdc_bonus_idea')}</strong> {results.delivery_suggestions.additional_gesture}
                    </p>
                  )}
                </div>
              </div>
            ...
            {context === 'Post-interview' && (
              <p className={`text-xs text-center ${c.textMuteded} mt-2`}>
                {t('gdc_xref_interview_q')}{' '}
                <a href="/DifficultTalkCoach" className={linkStyle}>🗣️ {t('gdc_difficult_talk_coach')}</a>{' '}
                {t('gdc_xref_interview_help')}
              </p>
            )}
            {(context === 'Condolence support' || context === 'Emotional support') && (
              <p className={`text-xs text-center ${c.textMuteded} mt-2`}>
                {t('gdc_xref_sensitive_q')}{' '}
                <a href="/VelvetHammer" className={linkStyle}>{t('gdc_velvet_hammer')}</a>{' '}
                {t('gdc_xref_sensitive_help')}
              </p>
            )}
            </div>
          </div>
          ...
          <div className={`border-2 rounded-xl p-6 ${c.success}`}>
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-bold mb-1">
                  {t('gdc_handwritten_template')}
                </h3>
                <p className={`text-sm ${c.textSecondary}`}>
                  {t('gdc_handwritten_subtitle')}
                </p>
              </div>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className={`rounded-lg p-4 border ${isDark ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-emerald-200'}`}>
                <h4 className={`font-semibold ${c.text} mb-2 text-sm`}>{t('gdc_font_suggestions')}</h4>
              ...
              <div className={`rounded-lg p-4 border ${isDark ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-emerald-200'}`}>
                <h4 className={`font-semibold ${c.text} mb-2 text-sm`}>{t('gdc_card_tips')}</h4>
                <ul className={`space-y-1 text-sm ${c.textSecondary}`}>
            ...
            {results.handwriting_template.length_guidance && (
              <div className={`mt-4 p-3 rounded-lg ${isDark ? 'bg-emerald-900/30' : 'bg-emerald-100'}`}>
                <p className={`text-sm ${c.text}`}>
                  <strong>{t('gdc_length_label')}</strong> {results.handwriting_template.length_guidance}
                </p>
              </div>
            )}
          </div>
