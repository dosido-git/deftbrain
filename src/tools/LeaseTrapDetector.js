...
import { trackVariant } from '../utils/analytics';
import { revealSection } from '../utils/revealSection';

const BRAND = '\n\n— Generated by DeftBrain · deftbrain.com';

// Five, rotated across every lease type: an apartment with an auto-renewal
// trap, a five-year commercial lease with a personal guarantee, a single-
// family house shifting maintenance onto the tenant, a room rental with a
// master key and 14-day notice, and a sublease with no landlord consent on
// record.
const EXAMPLES = [
  {
  inputMethod: 'text',
  ...
  concerns: "First shop lease. I mostly want to know what I am personally on the hook for if the business does not work out, and whether five years with no break is normal.",
  leaseText: "COMMERCIAL LEASE\n\n1. THE PREMISES. Ground floor retail unit, 41 Cross Street, Manchester.\n\n2. TERM. Five (5) years from the commencement date. There shall be no break clause.\n\n3. RENT. Payable quarterly in advance. Rent shall be reviewed at the end of year three on an upward-only basis.\n\n4. REPAIRS. The Tenant shall keep the Premises in good and substantial repair and condition, including the roof, structure and all service media, and shall yield up the Premises in such condition at the end of the Term regardless of their condition at the commencement of the Term.\n\n5. GUARANTEE. The obligations of the Tenant shall be personally guaranteed by the director(s) named in Schedule 2 for the full Term.\n\n6. ASSIGNMENT. The Tenant may not assign or sublet the whole or any part of the Premises without the Landlord's written consent, such consent to be granted at the Landlord's absolute discretion.\n\n7. INSURANCE. The Landlord shall insure the building and the Tenant shall reimburse the premium in full, together with any excess.",
},
  {
  inputMethod: 'text',
  location: 'Austin, TX',
  leaseType: 'house',
  concerns: "Single-family home, first time renting a house instead of an apartment. Want to know what's normal to be on the hook for versus what a landlord usually still covers.",
  leaseText: "SINGLE-FAMILY RESIDENCE LEASE\n\n1. TERM. Twelve (12) months, beginning June 1.\n\n2. RENT. $2,400/month.\n\n3. MAINTENANCE. Tenant is responsible for lawn care, gutter cleaning, HVAC filter replacement (monthly), and pest control. Tenant shall pay the first $250 of any repair, regardless of cause.\n\n4. APPLIANCES. Landlord makes no warranty as to the condition of any appliances left on the premises; Tenant accepts them 'as-is' and is responsible for all repair or replacement.\n\n5. POOL/SPA. If present, Tenant assumes full liability for the pool and shall maintain a service contract at Tenant's expense, proof of which shall be provided within 30 days.\n\n6. INSURANCE. Tenant shall carry renter's insurance with liability coverage of not less than $300,000 and name Landlord as additional insured.\n\n7. HOLDOVER. Any holdover after the Term shall be at 200% of the monthly rent, calculated daily.",
},
  {
  inputMethod: 'text',
  location: 'Toronto, ON',
  leaseType: 'room',
  concerns: "Renting a room in a shared house, not a whole unit. Want to know what rights I actually have compared to renting my own place, since the landlord keeps saying 'roommate agreements are different.'",
  leaseText: "ROOM RENTAL AGREEMENT\n\n1. This Agreement is for a single furnished room in a shared house. Common areas (kitchen, living room, one bathroom) are shared among all occupants.\n\n2. RENT. $850/month including utilities and Wi-Fi, due on the 1st.\n\n3. HOUSE RULES. Landlord may add or amend house rules at any time by posting notice in the kitchen; continued occupancy constitutes acceptance.\n\n4. GUESTS. Overnight guests permitted no more than 2 nights per week; Landlord may charge $25/night for additional guest nights.\n\n5. ACCESS. As this is a shared living arrangement, Landlord retains a key to all rooms including Tenant's, for safety and maintenance purposes, and may enter at Landlord's discretion.\n\n6. NOTICE TO VACATE. Landlord may terminate this Agreement with 14 days' notice for any reason, as this is a room rental and not a standard tenancy.\n\n7. DEPOSIT. $425 deposit, non-refundable cleaning portion of $100 deducted regardless of condition at move-out.",
},
  {
  inputMethod: 'text',
  location: 'Chicago, IL',
  leaseType: 'sublease',
  concerns: "Subletting from the original tenant, not the landlord directly. Trying to understand what happens to my deposit and my rights if the original tenant's lease with the actual landlord falls apart.",
  leaseText: "SUBLEASE AGREEMENT\n\n1. PARTIES. This Sublease is between the original Tenant ('Sublessor') and the undersigned ('Sublessee'), for the unit located at 900 W. Diversey Ave, Unit 2R.\n\n2. TERM. Sublease term is month-to-month, but shall automatically terminate immediately and without notice upon termination of the Master Lease between Sublessor and the building owner for any reason.\n\n3. RENT. $1,650/month paid to Sublessor, who remains solely responsible for payment to the building owner under the Master Lease.\n\n4. DEPOSIT. Sublessee pays a deposit of $1,650 directly to Sublessor. This deposit is separate from and in addition to any deposit Sublessor paid to the building owner, and its return is at Sublessor's sole discretion.\n\n5. LANDLORD CONSENT. Sublessor represents that the building owner has been notified of this arrangement, though written consent has not been obtained.\n\n6. SUBLESSEE OBLIGATIONS. Sublessee agrees to comply with all terms of the Master Lease, a copy of which has not been provided, and to indemnify Sublessor for any violation thereof.",
},
];
...
const LeaseTrapDetector = ({ tool }) => {
  ...
  const placeName = (v) => String(v || '').split(/[(,;—]/)[0].trim().slice(0, 40);
  ...
  const askFollowup = async () => {
    trackVariant('LeaseTrapDetector', 'followup');
  ...
  const generateEmail = async () => {
    trackVariant('LeaseTrapDetector', 'email');
  ...
  const generateAmendment = async () => {
    trackVariant('LeaseTrapDetector', 'amendment');
  ...
  const generateChecklist = async (type) => {
    trackVariant('LeaseTrapDetector', 'checklist');
  ...
  const loadExample = useCallback(() => {
    const ex = pickExample('LeaseTrapDetector', EXAMPLES);
    setInputMethod(ex.inputMethod);
    setLocation(ex.location);
    setLeaseType(ex.leaseType);
    setConcerns(ex.concerns);
    setLeaseText(ex.leaseText);
  ...
  // Re-runs the analysis pinned to one jurisdiction. The first report was
  // produced under the stated location; choosing the lease's own means the
  // whole thing has to be redone, not relabelled.
  const reanalyzeUnder = async (state) => {
    setGoverningLaw(state); setJurisdictionConfirmed(true);
    setResults(null); setError('');
    try {
      const data = await callToolEndpoint('lease-trap-detector', {
        leaseText: leaseText.trim() || undefined,
        pdfBase64: fileBase64 || undefined,
        location: state,
        leaseType, concerns: concerns.trim(), situation: situation || null,
        userLocale, userCurrency, userRegion,
      });
      setResults(data);
    } catch (err) { setError(err.message || t('ltd_err_analyze')); }
  ...
  useEffect(() => {
    ...
    revealSection(headerRef.current);
  }, [modeNonce]);
    ...
    <div className={`space-y-4 ${c.text}`}>
      ...
      {ltdMode === 'analyze' && !results && (
        <div className="space-y-5">
          ...
          {/* Public product demonstration. The content lives in tools.js and is
              also emitted by scripts/prerender.js, keeping the visitor and
            ...
            return (
              <details className={`${isDark ? 'bg-amber-950/20 border-amber-800/50' : 'bg-amber-50/60 border-amber-200'} border-2 border-dashed rounded-2xl overflow-hidden`}>
                ...
                <div className={`border-t border-dashed ${isDark ? 'border-amber-800/50' : 'border-amber-200'} p-5 space-y-4`}>
                  ...
                  {x.sections?.map((section, i) => (
                    <div key={i} className={`p-4 rounded-xl border ${toneClass[section.tone] || toneClass.neutral}`}>
                      ...
                      {section.items?.length > 0 && (
                        <ul className={`list-disc ps-5 space-y-1.5 text-sm ${c.textSecondary}`}>
                  ...
                  {x.nextStep && (
                    <div>
                      <p className={`text-sm font-black mb-1 ${c.text}`}>{x.nextStepLabel}</p>
                      <p className={`text-sm leading-relaxed ${c.textSecondary}`}>{x.nextStep}</p>
                    </div>
                  )}
        ...
        {/* ── Jurisdiction gate ──────────────────────────────────────────
            Tenant law is state law. When the lease points somewhere other than
            the location given, the report is withheld until the reader says
            which law governs — a report that blends two states reads as
            authoritative and cannot be told apart line by line. */}
        {ltdMode === 'analyze' && results?.jurisdiction_check?.mismatch
          && results.jurisdiction_check.lease_jurisdiction && !jurisdictionConfirmed && (
          <div className={`${c.card} border-2 rounded-2xl p-6 space-y-4 ${isDark ? 'border-amber-700 bg-amber-900/15' : 'border-amber-400 bg-amber-50'}`}>
            <h3 className={`text-lg font-black ${c.text}`}>🚨 {t('ltd_jur_title')}</h3>
            <p className={`text-sm leading-relaxed ${c.text}`}>
              {t('ltd_jur_body', { lease: placeName(results.jurisdiction_check.lease_jurisdiction), stated: location || results.jurisdiction_check.stated_jurisdiction })}
            </p>
            <p className={`text-sm leading-relaxed ${c.textSecondary}`}>{t('ltd_jur_why')}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
              <button onClick={() => reanalyzeUnder(placeName(results.jurisdiction_check.lease_jurisdiction))} disabled={loading}
                className={`w-full py-3 rounded-xl font-bold text-sm ${c.btnPrimary} disabled:opacity-40`}>
                {t('ltd_jur_use_lease', { state: placeName(results.jurisdiction_check.lease_jurisdiction) })}
              </button>
              <button onClick={() => setJurisdictionConfirmed(true)} disabled={loading}
                className={`w-full py-3 rounded-xl font-bold text-sm ${c.btnSecondary} disabled:opacity-40`}>
                {t('ltd_jur_use_stated', { state: location || results.jurisdiction_check.stated_jurisdiction })}
              </button>
            </div>
          </div>
        )}

        {ltdMode === 'analyze' && results && !(results.jurisdiction_check?.mismatch && results.jurisdiction_check?.lease_jurisdiction && !jurisdictionConfirmed) && (
          <div className="space-y-5">
            {/* ── The three things, always visible ────────────────────────
            ...
            {Array.isArray(results.top_fixes) && results.top_fixes.length > 0 && (
              <div className={`${c.card} border-2 rounded-2xl p-5 space-y-3 ${isDark ? 'border-cyan-700' : 'border-cyan-300'}`}>
                ...
                {governingLaw && (
                  <p className={`text-xs pt-1 ${c.textMuted}`}>⚖️ {t('ltd_jur_analyzed_under', { state: governingLaw })}</p>
                )}
                {results.financial_summary?.worst_case_penalties && (
                  <p className={`text-xs pt-1 ${c.textSecondary}`}>
                    <span className="font-bold">{t('ltd_exposure_label')}</span> {results.financial_summary.worst_case_penalties}
                  </p>
                )}
              </div>
            )}
