                    ...
                    <div key={idx} className="border-l-4 border-red-500 pl-4">
                      ...
                      {expandedSections[`red-${idx}`] && (
                        <div className="mt-2 space-y-2">
                          ...
                          {flag.negotiation_script && <div className={`p-2.5 rounded-lg ${isDark ? 'bg-emerald-900/20' : 'bg-emerald-50'}`}><p className={`text-[10px] font-bold ${isDark ? 'text-emerald-400' : 'text-emerald-700'}`}>💬 YOUR SCRIPT</p><p className={`text-xs ${c.textSecondary}`}>{flag.negotiation_script}</p><div className="mt-1"></div></div>}
                        </div>
                      )}
                    </div>
                  ))}
