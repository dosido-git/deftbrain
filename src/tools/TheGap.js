  ...
  const renderInput = () => (
    <div className="space-y-4">
      ...
      <button onClick={trace} disabled={loading || !concept.trim()}
        ...
        {loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '⚙️'}</span> Tracing prerequisites...</> : <><span>📝</span> Find the Gap</>}
      </button>
    </div>
  );
    ...
    return (
      <div ref={resultsRef} className="space-y-4 mt-4">
        ...
        <div className="flex gap-2">
          <div className="flex-1"><ActionBar content={buildCopy()} title="Study Plan" /></div>
          <button onClick={handleReset} className={'flex-1 py-3 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 ' + c.btnPrimary}><span>📝</span> New Concept</button>
        </div>
        ...
        <div className={'p-4 rounded-2xl border ' + c.card}>
          ...
          <div className={'space-y-1.5 text-xs ' + c.textSecondary}>
            <p>Have lecture notes to distill? <a href="/tool/recall" target="_blank" rel="noopener noreferrer" className={linkStyle}>Recall</a> extracts the key points.</p>
            <p>Need practice questions? <a href="/tool/recall" target="_blank" rel="noopener noreferrer" className={linkStyle}>Recall's Test Prep</a> mode generates them from your notes.</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-5">
        <div>
          <h2 className={'text-2xl font-bold ' + c.text}>The Gap <span className="text-xl">📝</span></h2>
          <p className={'text-sm ' + c.textMuted}>Stuck on a concept? We'll trace back to find where your understanding broke.</p>
        </div>
      </div>
      {!results && renderInput()}
      {renderResults()}
