...
const TruthBomb = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    ...
    setWhyNotSaying(t(ex.why));
    setRelationshipContext(t(ex.ctx));
    setResults(null);
    ...
    <div className={`space-y-4 ${c.text}`}>
      ...
      <div className={`${c.card} border ${c.border} rounded-xl p-6`}>
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className={`text-base ${c.textSecondary}`}>
                <span className="me-2 text-lg">{tool?.icon ?? '💣'}</span>{tool?.tagline ?? t('tb_tagline')}
              </p>
        ...
        <div className="space-y-4">
          ...
          className={`relative w-full py-3 rounded-xl font-bold min-h-[48px] flex items-center justify-center gap-2 ${(!theUnsaidThing.trim()) ? c.btnIdle : c.btnPrimary}`}>
          {loading
