...
const EXAMPLE = {
  whoItsAbout: 'My closest friend from college',
  theUnsaidThing: "I think their drinking has gotten worse over the last year, and the people around them are just... going along with it.",
  whyNotSaying: "Worried they'll get defensive and shut me out. Also worried I'm wrong, or it's not my place. Last time I said something even mildly critical, they didn't speak to me for weeks.",
  relationshipContext: "15+ year friendship, we live in different cities now but talk weekly. Their other close friends are all in their drinking circle.",
};
const WHO_OPTIONS = [
...
const TruthBomb = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    setWhoItsAbout(EXAMPLE.whoItsAbout);
    setTheUnsaidThing(EXAMPLE.theUnsaidThing);
    setWhyNotSaying(EXAMPLE.whyNotSaying);
    setRelationshipContext(EXAMPLE.relationshipContext);
    setResults(null);
  }, [setTheUnsaidThing, setResults]);
