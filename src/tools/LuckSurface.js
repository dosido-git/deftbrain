...
const MECHANISM_CONFIG = {
  broadcast:  { icon: '📢', labelKey: 'lks_mech_broadcast',  color: (d) => d ? 'bg-[rgb(44,74,110)]/20 text-[rgb(168,185,206)] border-[rgb(44,74,110)]/40'  : 'bg-[rgb(232,238,245)] text-[rgb(30,58,88)] border-[rgb(176,196,216)]' },
  infiltrate: { icon: '🚪', labelKey: 'lks_mech_infiltrate', color: (d) => d ? 'bg-[rgb(200,135,46)]/15 text-[rgb(217,160,78)] border-[rgb(200,135,46)]/40'  : 'bg-[rgb(253,243,228)] text-[rgb(147,84,31)] border-[rgb(224,196,154)]' },
  create:     { icon: '✍️', labelKey: 'lks_mech_create',     color: (d) => d ? 'bg-[rgb(90,138,92)]/15 text-[rgb(138,186,140)] border-[rgb(90,138,92)]/40' : 'bg-[rgb(232,240,232)] text-[rgb(61,110,63)] border-[rgb(168,196,168)]' },
  curate:     { icon: '🧲', labelKey: 'lks_mech_curate',     color: (d) => d ? 'bg-[rgb(181,74,63)]/15 text-[rgb(232,169,163)] border-[rgb(181,74,63)]/40' : 'bg-[rgb(252,234,232)] text-[rgb(138,48,40)] border-[rgb(212,144,138)]' },
  compound:   { icon: '⚡', labelKey: 'lks_mech_compound',   color: (d) => d ? 'bg-[rgb(61,54,48)]/60 text-[rgb(200,195,185)] border-[rgb(61,54,48)]'     : 'bg-[rgb(243,239,232)] text-[rgb(94,80,66)] border-[rgb(213,202,184)]' },
};
...
const LuckSurface = ({ tool }) => {
  ...
  const buildFullText = useCallback(() => {
    ...
    let out = `🧲 ${t('lks_copy_header')}\n\n${t('lks_copy_current')} ${a?.current_surface_area}\n${a?.the_diagnosis}\n\n`;
    out += `${t('lks_copy_wall')} ${a?.the_invisible_wall}\n\n`;
    out += `${t('lks_copy_moves')}\n`;
    results?.the_five_moves?.forEach((m, i) => {
      out += `${i + 1}. ${m.title}\n${m.the_move}\n${t('lks_copy_why')} ${m.why_asymmetric}\n\n`;
    });
    if (results?.the_one_to_start) out += `${t('lks_copy_start')} ${results?.the_one_to_start}\n`;
    return out + BRAND;
  }, [results, t]);
    ...
    <div className={`space-y-4 ${c.text}`}>
          ...
          <div className="space-y-4" ref={resultsRef}>
            ...
            {results?.audit && (
              <div className={`rounded-2xl border p-6 ${c.card} ${c.border}`}>
                <div className="flex items-center justify-between mb-4">
                  <p className={`text-xs font-black uppercase tracking-widest ${c.textMuteded}`}>{t('lks_current_area')}</p>
                ...
                {results?.audit?.the_invisible_wall && (
                  <div className={`p-3 rounded-xl border ${isDark ? 'bg-[rgb(181,74,63)]/10 border-[rgb(181,74,63)]/30' : 'bg-[rgb(252,234,232)] border-[rgb(212,144,138)]'}`}>
                    <p className={`text-xs font-bold uppercase tracking-wide mb-1 ${isDark ? 'text-[rgb(232,169,163)]' : 'text-[rgb(138,48,40)]'}`}>🧱 {t('lks_invisible_wall')}</p>
                    <p className={`text-sm ${c.textSecondary}`}>{results?.audit?.the_invisible_wall}</p>
                  </div>
                )}
                {results?.audit?.what_theyre_good_at && (
                  <p className={`text-xs mt-3 ${c.textMuteded}`}><span className="font-semibold">{t('lks_working')}</span> {results?.audit?.what_theyre_good_at}</p>
                )}
              </div>
            )}
            ...
            {results?.the_five_moves?.length > 0 && (
              <div className="space-y-3">
                <p className={`text-xs font-black uppercase tracking-widest px-1 ${c.textMuteded}`}>⚡ {t('lks_five_moves')}</p>
                    ...
                    <div key={i} className={`rounded-2xl border overflow-hidden ${c.card} ${c.border}`}>
                      ...
                      {isOpen && (
                        <div className={`px-5 pb-5 space-y-3 border-t ${c.border}`}>
                          <div className="pt-3">
                            <p className={`text-xs font-bold uppercase tracking-wide mb-1 ${c.textMuteded}`}>{t('lks_the_move')}</p>
                            <p className={`text-sm ${c.text}`}>{move.the_move}</p>
                          </div>
                          {move.why_asymmetric && (
                            <p className={`text-xs ${c.textMuteded}`}><span className="font-semibold">{t('lks_why_asymmetric')}</span> {move.why_asymmetric}</p>
                          )}
            ...
            {results?.the_target && (
              <div className={`rounded-2xl border p-5 ${isDark ? 'bg-[rgb(200,135,46)]/10 border-[rgb(200,135,46)]/40' : 'bg-[rgb(253,243,228)] border-[rgb(224,196,154)]'}`}>
                <div className="flex items-center justify-between mb-3">
                  <p className={`text-xs font-black uppercase tracking-widest ${c.goldText}`}>{t('lks_after_moves')}</p>
      ...
      {sessionHistory.length > 0 && (
        <div className={`${c.cardAlt} border ${c.border} rounded-xl p-4`}>
          <p className={`text-xs font-bold ${c.textMuted} mb-2`}>📋 {t('lks_recent')}</p>
