...
const LuckSurface = ({ tool }) => {
  ...
  const linkStyle = isDark
    ...
    input:        isDark ? 'bg-[rgb(26,24,22)] border-[rgb(61,54,48)] text-[rgb(240,238,234)] placeholder:text-[rgb(138,130,117)]'
                         : 'bg-[rgb(250,248,245)] border-[rgb(213,202,184)] text-[rgb(61,57,53)] placeholder:text-[rgb(138,130,117)]',
    ...
    goldBg:        isDark ? 'bg-[rgb(200,135,46)]/15 border-[rgb(200,135,46)]/40' : 'bg-[rgb(253,243,228)] border-[rgb(224,196,154)]',
  ...
  const [_auditHistory, _setAuditHistory] = usePersistentState('lucksurface-historylog', []); // history marker
    ...
    <div className={`min-h-screen py-8 px-4 ${c.cardAlt}`}>
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className={`text-3xl font-black tracking-tight mb-2 ${c.text}`}>Luck Surface</h1>
          <p className={`text-sm ${c.textMuteded}`}>Calculate your luck surface area. Expand it with 5 specific moves.</p>
        </div>
          ...
          <div className={`rounded-2xl border p-6 shadow-sm space-y-4 ${c.card} ${c.border}`}>
            <div>
              ...
              <textarea value={description} onChange={e => setDescription(e.target.value)}
                ...
                className={`w-full px-4 py-3 rounded-xl border text-sm resize-none ${c.input} ${c.input}`} />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" value={goals} onChange={e => setGoals(e.target.value)}
                  ...
                  className={`w-full px-4 py-3 rounded-xl border text-sm ${c.input} ${c.input}`} />
              </div>
              <div>
            ...
            <div className="flex gap-2">
              <button onClick={handleSubmit} disabled={loading || !description.trim()}
                className={`flex-1 py-3 rounded-xl font-bold disabled:opacity-40 ${c.btnPrimaryPrimary}`}>
                {loading ? <><span className="animate-spin inline-block mr-2">{tool?.icon ?? '⚙️'}</span>Calculating…</> : '🧲 Calculate My Luck Surface'}
              </button>
              <button onClick={handleExample} disabled={loading}
                className={`px-4 py-3 rounded-xl text-sm font-medium ${c.btnPrimarySecondaryondary}`}>
                Example
              </button>
            </div>
          </div>
