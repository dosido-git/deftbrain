  ...
  const renderInput = () => (
    <div className="space-y-4">
      ...
      <button title={t('cmd_enter')} onClick={frame} disabled={loading || !situation.trim()}
        ...
        {loading ? <><span className="relative inline-block animate-spin text-xl">{tool?.icon ?? '📖'}</span> {t('ta_framing')}</> : <><span className="me-1 text-xl">{tool?.icon ?? '📖'}</span> {t('ta_frame')}</>}
      {!loading && (
