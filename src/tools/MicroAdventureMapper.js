...
const MicroAdventureMapper = ({ tool }) => {
  ...
  // Every input that ships with a preset value, so "has the visitor touched
  // anything?" can be answered honestly. The old gate asked whether
  // timeAvailable was non-empty — it defaults to '2hrs', so it was true from
  // the first render and Start Over greeted people who had typed nothing.
  const UNTOUCHED = {
    location: '', timeAvailable: '2hrs', when: 'right_now', timeOfDay: 'afternoon',
    vibe: 'balanced', budget: 'low', transport: 'walking', companions: 'solo',
  };

  // ── Results ──
  const [results, setResults] = usePersistentState('micro-adventure-mapper-results-v2', null);
  ...
  // ── Persistent ──
  const [pastAdventures, setPastAdventures] = usePersistentState('micro-adventure-journal-v2', []);
  ...
  const saveToJournal = useCallback((adventureResults, loc) => {
    const entry = {
      ...
      between_stops: adventureResults.between_stops,
      what_to_bring: adventureResults.what_to_bring,
      backup_plan: adventureResults.backup_plan,
      keep_going: adventureResults.keep_going,
    };
  ...
  const loadFromJournal = useCallback((entry) => {
    setResults({
      ...
      stops: entry.stops,
      between_stops: entry.between_stops,
      what_to_bring: entry.what_to_bring,
      backup_plan: entry.backup_plan,
      keep_going: entry.keep_going,
    });
  ...
  // True once there is an answer on screen, or the visitor has changed
  // anything away from the value the form shipped with.
  const hasSomethingToClear = !!results
    || location.trim() !== UNTOUCHED.location
    || timeAvailable !== UNTOUCHED.timeAvailable
    || when !== UNTOUCHED.when
    || timeOfDay !== UNTOUCHED.timeOfDay
    || vibe !== UNTOUCHED.vibe
    || budget !== UNTOUCHED.budget
    || transport !== UNTOUCHED.transport
    || companions !== UNTOUCHED.companions
    || interests.length > 0
  ...
  const handleReset = useCallback(() => {
    ...
    setCheckedItems({});
    setShowInputs(true);
    // Back to the values the form ships with — otherwise the reset clears the
    // answer, leaves the choices behind, and the button stays on screen with
    // nothing left that a visitor would call unfinished.
    setLocation(UNTOUCHED.location);
    setTimeAvailable(UNTOUCHED.timeAvailable);
    setWhen(UNTOUCHED.when);
    setTimeOfDay(UNTOUCHED.timeOfDay);
    setVibe(UNTOUCHED.vibe);
    setBudget(UNTOUCHED.budget);
    setTransport(UNTOUCHED.transport);
    setCompanions(UNTOUCHED.companions);
    setInterests([]);
    setAccessibility([]);
  ...
  const swapStop = useCallback(async (stopNumber) => {
    ...
    try {
      ...
      // The route returns ONE replacement stop and ONE transition now, not
      // arrays of them — a single stop was always what this asked for.
      if (res.stop) {
        setResults(prev => {
          const newStops = [...(prev.stops || [])];
          const idx = newStops.findIndex(st => st.number === stopNumber);
          if (idx !== -1) newStops[idx] = { ...res.stop, number: stopNumber };
          let newBetween = prev.between_stops ? [...prev.between_stops] : [];
          if (res.transition && (res.transition.guidance || res.transition.estimated_time)) {
            const from = stopNumber > 1 ? stopNumber - 1 : stopNumber;
            const tIdx = newBetween.findIndex(nt => nt.from_stop === from && nt.to_stop === stopNumber);
            const merged = { from_stop: from, to_stop: stopNumber, ...res.transition };
            if (tIdx !== -1) newBetween[tIdx] = merged; else newBetween.push(merged);
          }
          return { ...prev, stops: newStops, between_stops: newBetween };
        });
      }
    } catch (err) {
  ...
  const buildFullText = useCallback(() => {
    ...
    const lines = [
      ...
      adv.tagline || '', '',
      `⏱️ ${adv.total_time || ''} · 💰 ${adv.total_cost || ''} · ${adv.activity_level || ''}`, '',
      adv.why_this_fits ? `${t('mam_copy_why')} ${adv.why_this_fits}` : '', '',
      `── ${t('mam_copy_itinerary')} ──`, '',
    ];
    (results.stops || []).forEach((s, i) => {
      lines.push(`${t('mam_copy_stop')} ${s.number || i + 1}: ${s.name}`);
      if (s.area) lines.push(`📍 ${s.area}`);
      if (s.duration_min) lines.push(`⏱️ ${s.duration_min} ${t('mam_min')}`);
      lines.push(s.description || '');
      if (s.small_twist) lines.push(`💡 ${s.small_twist}`);
      if (s.photo_op) lines.push(`📸 ${s.photo_op}`);
      if (s.verify) lines.push(`✅ ${t('mam_copy_verify')} ${s.verify}`);
      lines.push('');
      const transit = (results.between_stops || []).find(tr => tr.from_stop === (s.number || i + 1));
      if (transit?.guidance) lines.push(`→ ${transit.guidance}${transit.estimated_time ? ` (${transit.estimated_time})` : ''}`, '');
    });
    if (results.what_to_bring) lines.push(`🎒 ${t('mam_copy_bring')} ` + results.what_to_bring.join(', '), '');
    if (results.backup_plan?.description) lines.push(`☔ ${t('mam_copy_rain')} ${results.backup_plan.description}`, '');
    if (results.keep_going?.suggestion) lines.push(`⏱️ ${t('mam_copy_extend')} ${results.keep_going.suggestion}`, '');
    lines.push(BRAND);
  ...
  const renderResults = () => {
    ...
    const transits = results.between_stops || [];
    ...
    const renderAdventureHeader = () => (
      <div className={`p-5 rounded-2xl border-2 ${c.headerBg} mb-5`}>
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          {adv.activity_level && <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${c.tagBg}`}>{adv.activity_level}</span>}
        </div>
        {adv.why_this_fits && (
          <div>
            ...
            <div key={idx}>
              ...
              <div className="flex gap-4">
                ...
                <div className={`flex-1 p-4 rounded-xl border ${c.stopCard} mb-2 transition-all ${isSwapping ? 'opacity-50' : ''}`}>
                  ...
                  {stop.small_twist && (
                    <div className={`text-xs p-2.5 rounded-lg border mb-2 ${c.tipBg}`}>
                      <span className="font-bold">💡 {t('mam_small_twist')}</span> {stop.small_twist}
                    </div>
                  )}

                  {/* photo_op and verify are optional by design — the schema
                      returns an empty string when they do not apply, and an
                      empty block reads as a missing answer. */}
                  {stop.photo_op && (
              ...
              {transit && idx < stops.length - 1 && (
                <div className="flex gap-4 mb-2">
                  ...
                  <div className={`flex items-center gap-2 py-1.5 px-3 rounded-lg text-xs ${c.transitText}`}>
                    ...
                    <span>{transit.guidance}</span>
                    {transit.estimated_time && <span className="opacity-60">· {transit.estimated_time}</span>}
                  </div>
                </div>
              )}
            </div>
    ...
    const renderRainyBackup = () => {
      const bp = results.backup_plan;
      if (!bp?.description) return null;
      return (
        <div className={`p-4 rounded-xl border ${c.rainBg} mb-4`}>
          <h4 className="text-sm font-bold mb-1 flex items-center gap-2">
            ☔ {t('mam_rain_title')}
          </h4>
          {bp.when_to_use && <p className="text-xs opacity-70 mb-1">{bp.when_to_use}</p>}
          <p className="text-sm">{bp.description}</p>
        </div>
      );
    };
    ...
    // Extend it
    const renderExtend = () => {
      const kg = results.keep_going;
      if (!kg?.suggestion) return null;
      return (
        <div className={`p-4 rounded-xl border ${c.extendBg} mb-4`}>
          <h4 className="text-sm font-bold mb-1 flex items-center gap-2">
            ⏱️ {t('mam_extend_title')}
          </h4>
          {kg.extra_time && <p className="text-xs opacity-70 mb-1">{t('mam_add_time', { time: kg.extra_time })}</p>}
          <p className="text-sm">{kg.suggestion}</p>
        </div>
      );
    };
      ...
      <div className={`${c.card} border ${c.border} rounded-xl shadow-sm`}>
        <div className="px-5 pt-2.5">
          <div className="pb-3 border-b border-zinc-500 flex items-start justify-between gap-3">
            ...
            {/* PF-16: gated on first input, like every other tool — and a
                preset the visitor never chose is not input. */}
            {hasSomethingToClear ? (
