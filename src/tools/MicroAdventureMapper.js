...
const MicroAdventureMapper = ({ tool }) => {
  ...
  // ── Results ──
  const [results, setResults] = usePersistentState('microadventuremapper-result', null);
...
${results.extend_it ? `<div class="section extend"><strong>⏱️ Keep going:</strong> ${results.extend_it.suggestion}</div>` : ''}
</body></html>`;
  ...
  const renderHeader = () => (
    <div className="flex items-center gap-3 mb-5">
      <div>
        ...
        <p className={`text-sm ${c.textMuted}`}>Plan a local adventure in 30 seconds</p>
      </div>
    </div>
  );
  ...
  const renderInputForm = () => (
    <div className="space-y-4">
      ...
      <div className={`p-5 rounded-2xl border ${c.border} ${c.cardAlt}`}>
        <label className={`text-xs font-bold ${c.textSecondary} uppercase tracking-wide mb-2 block`}>📍 Where are you?</label>
      ...
      <button onClick={generate}
        ...
        className={`w-full py-4 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 shadow-lg transition-all
          ${location.trim().length >= 2 && !loading ? `${c.btnPrimary} hover:scale-[1.01] active:scale-[0.99]` : c.btnDis}`}>
        {loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '⚙️'}</span> Planning your adventure...</>
          : <> Map My Adventure</>}
      </button>
    </div>
  );
    ...
    const renderActions = () => (
      <div className="flex flex-wrap gap-2">
        <button onClick={regenerate} disabled={loading}
          ...
          <p className={`text-xs text-center ${c.textMuted}`}>AI-generated suggestions — use your own judgment.</p>
        <ActionBar content={copyItinerary ? copyItinerary() || '' : ''} title="Adventure Plan" />
        <button onClick={printItinerary}
          className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 ${c.btnSecondary}`}>
          🖨️ Print
        </button>
      </div>
    );
            ...
            <div className={`px-3 pb-3 border-t ${c.border}`}>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => {
                  ...
                  className={`px-3 py-2 rounded-lg text-xs font-semibold flex items-center gap-1.5 ${c.btnSecondary}`}>
                   Copy
                </button>
