        ...
        <>
          <div className={`${c.card} border rounded-xl p-5`}>
            <div className={`mb-4 pb-3 border-b ${isDark ? 'border-zinc-500' : 'border-zinc-500'}`}>
              ...
              <p className={`text-sm ${c.textSecondary}`}>Name a movie, show, book, or game — I'll find where the logic breaks</p>
            </div>
            ...
            <div className="mb-4">
              <label className={`text-xs font-bold ${c.label} block mb-1.5`}>
                Extra context <span className={`font-normal ${c.textMuteded}`}>(optional — specific scene or season)</span>
              </label>
            ...
            <button onClick={runAnalysis} disabled={!title.trim() || loading}
              ...
              {loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '⚙️'}</span> Analyzing...</> : <><span>🕳️</span> Find Plot Holes</>}
            </button>
          </div>
              ...
              <div className={`${c.warningBox} border-2 rounded-xl p-5 text-center`}>
                ...
                {results.swiss_cheese_rating && (
                  <div className="inline-block">
                    ...
                    <span className={`text-sm ${c.textMuteded}`}>/10 Swiss Cheese Rating</span>
                  </div>
                )}
              </div>
                    ...
                    {results.holes.map((h, i) => {
                      ...
                      return (
                        <div key={i} className={`${c.quoteBg} rounded-lg p-4`}>
                          ...
                          {h.why_it_matters && <p className={`text-[11px] ${c.textMuteded} mb-1`}>💥 {h.why_it_matters}</p>}
                          {h.best_defense && <p className={`text-[11px] ${isDark ? 'text-zinc-500' : 'text-zinc-500'} mb-1`}>🛡️ {h.best_defense}</p>}
                          {h.reddit_would_say && (
                            <p className={`text-[11px] ${c.textMuteded} italic mt-2`}>💬 r/plotholes: "{h.reddit_would_say}"</p>
                          )}
      ...
      {view === 'defend' && (
        <>
          <div className={`${c.card} border rounded-xl p-5`}>
            ...
            <p className={`text-sm ${c.textSecondary} mb-4`}>Describe the hole — I'll build the strongest possible defense</p>
            ...
            <button onClick={runDefend} disabled={!defendHole.trim() || loading}
              className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
              {loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '⚙️'}</span> Building defense...</> : <><span>⚖️</span> Defend It</>}
            </button>
          </div>
          ...
          {defendResults && (
            <div className="space-y-4">
              <div className={`${c.warningBox} border-2 rounded-xl p-5 text-center`}>
                ...
                <p className={`text-xs ${c.textMuteded} mb-1`}>{defendResults.hole_summary}</p>
                    ...
                    {defendResults.defense_arguments.map((a, i) => (
                      <div key={i} className={`${c.quoteBg} rounded-lg p-3`}>
                        <div className="flex items-start justify-between gap-2 mb-1">
                          ...
                          <span className={`text-[8px] font-black ${c.textMuteded} whitespace-nowrap`}>{a.strength}</span>
                        </div>
                        <p className={`text-[10px] ${c.textMuteded} mb-1`}>Type: {a.type}</p>
              ...
              {defendResults.honest_take && (
                <div className={`${c.quoteBg} border ${isDark ? 'border-zinc-500' : 'border-zinc-500'} rounded-xl p-4`}>
                  <p className={`text-[10px] font-bold ${c.textMuteded} uppercase mb-1`}>🤫 Off the Record</p>
