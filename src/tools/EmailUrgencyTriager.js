      ...
      <div key={key} className={`${c.card} border-2 ${TIER_CFG[tier].bc} rounded-lg p-4 transition-all ${status === 'done' ? 'opacity-50' : ''}`}>
        ...
        {isExp && (
          <div className={`mt-3 pt-3 ${c.border} border-t space-y-3`}>
            ...
            {email.draft_reply && (
              <div>
                <div className="flex items-center justify-between mb-1"><p className={`text-[10px] font-bold ${c.textMuted}`}>DRAFT REPLY</p></div>
        ...
        <div ref={resultsRef} className="space-y-4">
          ...
          {results.response_templates?.length > 0 && (
            <div className={`${c.card} border rounded-xl p-5`}><h3 className={`text-sm font-bold ${c.text} mb-3`}>⚡ Templates</h3><div className="space-y-2">{results.response_templates.map((t, i) => (
              <div key={i} className={`${c.cardAlt} border rounded-lg p-3`}><div className="flex items-center justify-between mb-1"><span className={`${t.for_urgency === 'now' ? c.pillRed : c.pillAmber} border text-[9px] font-bold px-1.5 py-0.5 rounded`}>{t.for_urgency}</span></div><p className={`text-xs ${c.textSecondary}`}>{t.template}</p></div>
            ))}</div></div>
          )}
