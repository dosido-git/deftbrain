...
const TIER_CFG = {
  now: { icon: '🔴', key: 'today', border: 'border-red-400' },
  this_week: { icon: '🟠', key: 'week', border: 'border-amber-400' },
  optional: { icon: '🟢', key: 'noReply', border: 'border-emerald-400' },
};
...
export default function EmailUrgencyTriager({ tool }) {
  ...
  const renderEmail = (email, tier, idx) => {
    ...
    return (
      <div key={key} className={`${c.card} border ${cfg.border} rounded-xl p-4 ${done ? 'opacity-50' : ''}`}>
