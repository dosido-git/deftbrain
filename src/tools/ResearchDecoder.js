...
// Five, rotated: a landmark ML paper, a nutrition epidemiology cohort
// study, a workplace-policy trial with selection bias worth flagging, and
// a phase 2 drug trial with real efficacy numbers and honest limitations.
const EXAMPLES = [
  {
  ...
  {
    paperTitle: 'Effects of a four-day work week on employee wellbeing and productivity: a six-month trial',
    paperText: "We report results from a six-month trial of a four-day, 32-hour work week with no reduction in pay across 61 companies (n=2,900 employees) in English-speaking countries. Employee self-reported stress, burnout, and fatigue declined significantly, while job satisfaction and self-reported work ability increased. Company revenue during the trial period was, on average, similar to the same period in the prior year, though most participating companies were self-selected volunteers with above-median existing revenue growth, and we did not have a randomized control group of companies retaining a five-day week over the same period.",
  },
  {
    paperTitle: 'A phase 2 randomized trial of Drug X for moderate-to-severe eczema',
    paperText: "In this 16-week, double-blind, placebo-controlled trial, 218 adults with moderate-to-severe atopic dermatitis were randomized 2:1 to receive Drug X or placebo. At week 16, 44% of the Drug X group achieved at least a 75% reduction in disease severity score (EASI-75), compared with 12% in the placebo group (p<0.001). The most common adverse events in the Drug X group were injection-site reactions (18%) and upper respiratory infection (9%), similar in frequency to placebo. The trial was not powered to detect rare adverse events, and long-term safety beyond 16 weeks has not yet been established.",
  },
  {
    paperTitle: 'Screen time and adolescent depression: a systematic review and meta-analysis',
    paperText: "We synthesized 47 longitudinal studies (total n=124,000) examining the relationship between adolescent screen time and subsequent depressive symptoms. The pooled effect size was small (r=0.08, 95% CI 0.05 to 0.11), and heterogeneity across studies was high (I²=81%). Effect sizes were larger for social media specifically than for screen time overall, and larger in studies relying on self-reported screen time than those using passive tracking. Nearly all included studies were observational; reverse causation (that depressed adolescents may seek out more screen time) could not be ruled out, and only 6 studies attempted to address it directly.",
  },
];
      ...
      {mode === 'decode' && <>
        <div className={`${c.card} border ${c.border} rounded-xl p-5 space-y-4`}>
          ...
          <button title={t('rd_cmd_enter')} onClick={handleDecode} disabled={(!paperText.trim() && !pdfBase64) || loading} className={`relative w-full ${(!paperText.trim() && !pdfBase64) ? c.btnIdle : c.btnPrimary} font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
            {loading
              ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '📄'}</span> {tr('rd_working', 'Working…')}</>
              : <><span className="me-1 text-xl">{tool?.icon ?? '📄'}</span> {tr('rd_decode_it', 'Decode It')} →</>}
            {!loading && (
            ...
            {showRelevance && (
              <div className={`${c.card} border ${c.border} rounded-xl p-5 space-y-3`}>
                ...
                <button onClick={handleRelevance} disabled={!relWonder.trim() || loading} className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
                  {loading ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '📄'}</span> {tr('rd_working', 'Working…')}</> : <>🎯 {tr('rd_check_relevance', 'Check Relevance to My Situation')}</>}
                </button>
      ...
      {mode === 'headline' && <>
        <div className={`${c.card} border ${c.border} rounded-xl p-5 space-y-4`}>
          ...
          <button onClick={handleHeadline} disabled={(!hlResearchText.trim() && !hlPdfBase64) || !hlHeadline.trim() || loading} title={tr('rd_cmd_enter', 'Press ⌘↵ (Ctrl+Enter) to decode')} className={`relative w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
            {loading ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '📄'}</span> {tr('rd_checking', 'Checking…')}</> : <>📰 {tr('rd_check_headline', 'Check the Headline')}</>}
            {!loading && (
              <kbd aria-hidden="true" className="hidden sm:flex items-center absolute end-3 top-1/2 -translate-y-1/2 px-1.5 py-0.5 rounded border border-white/30 bg-white/15 text-[10px] font-bold tracking-wide">⌘↵</kbd>
            )}
          </button>
      ...
      {mode === 'compare' && <>
        <div className={`${c.card} border ${c.border} rounded-xl p-5 space-y-4`}>
          ...
          <button onClick={handleCompare} disabled={!cmpPaper1.trim() || !cmpPaper2.trim() || loading} title={tr('rd_cmd_enter', 'Press ⌘↵ (Ctrl+Enter) to decode')} className={`relative w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
            {loading ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '📄'}</span> {tr('rd_comparing', 'Comparing…')}</> : <>⚖️ {tr('rd_compare_papers', 'Compare These Papers')}</>}
            {!loading && (
              <kbd aria-hidden="true" className="hidden sm:flex items-center absolute end-3 top-1/2 -translate-y-1/2 px-1.5 py-0.5 rounded border border-white/30 bg-white/15 text-[10px] font-bold tracking-wide">⌘↵</kbd>
            )}
          </button>
      ...
      {mode === 'term' && <>
        <div className={`${c.card} border ${c.border} rounded-xl p-5 space-y-4`}>
          ...
          <button onClick={handleTerm} disabled={!termText.trim() || loading} title={tr('rd_cmd_enter', 'Press ⌘↵ (Ctrl+Enter) to decode')} className={`relative w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
            {loading ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '📄'}</span> {tr('rd_decoding', 'Decoding…')}</> : <>🔤 {tr('rd_explain_it', 'Explain It')}</>}
            {!loading && (
              <kbd aria-hidden="true" className="hidden sm:flex items-center absolute end-3 top-1/2 -translate-y-1/2 px-1.5 py-0.5 rounded border border-white/30 bg-white/15 text-[10px] font-bold tracking-wide">⌘↵</kbd>
            )}
          </button>
