    ...
    <div ref={stageRef} className={`scroll-mt-24 space-y-4 ${c.text}`}>
      ...
      {mode === 'digest' && <>
        <div className={`${c.card} border ${c.border} rounded-xl p-5 space-y-4`}>
          ...
          <button title={t('cmd_enter')} onClick={handleDigest} disabled={!paperText.trim() || loading} className={`relative w-full ${(!paperText.trim()) ? c.btnIdle : c.btnPrimary} font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
          {loading
