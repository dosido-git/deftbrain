      ...
      <div className={`${c.card} border rounded-xl p-5`}>
        <div className={`mb-4 pb-3 border-b ${c.divider}`}>
          <h2 className={`text-xl font-bold ${c.text}`}>
            <span className="mr-2">⚡</span>Social Energy Audit
          </h2>
          <p className={`text-sm ${c.textSec}`}>Log your interactions — see where your energy actually goes</p>
        </div>

        {/* Week label */}
        <div className="mb-4">
          <label className={`text-sm font-bold ${c.label} block mb-1.5`}>
            Label this week <span className={`font-normal ${c.textMuted}`}>(optional)</span>
          </label>
          <input
            type="text"
            value={weekLabel}
        ...
        {editingEntryId && (
          <div className={`${c.warning} border rounded-lg p-3 mb-4 flex items-center justify-between`}>
            <p className={`text-xs font-bold ${c.warningText}`}>✏️ Editing a previous week — changes will update that entry</p>
            <button
              onClick={() => {
                setEditingEntryId(null);
              ...
              className={`text-xs font-bold ${c.accent} min-h-[28px] ml-2`}
            >
              Cancel
            </button>
          </div>
          ...
          {savedTemplate ? (
            <>
              <button onClick={loadFromTemplate}
                className={`${c.btnSec} px-3 py-1.5 rounded-lg text-xs font-bold min-h-[32px] flex items-center gap-1`}>
                <span>📋</span> Load my typical week ({savedTemplate.length} items)
              </button>
              <button onClick={saveCurrentAsTemplate}
                className={`text-xs font-bold ${c.accent} min-h-[32px]`}>
                💾 Update template
              </button>
            ...
            <button
              ...
              className={`text-xs font-bold ${c.accent} min-h-[32px] disabled:opacity-30`}
            >
          ...
          {interactions.map((int, i) => (
            <div key={i} className={`${c.quoteBg} rounded-xl p-4 space-y-3`}>
              <div className="flex items-start gap-2">
                <span className={`text-xs font-bold ${c.textMuted} mt-2.5 w-5 text-center flex-shrink-0`}>{i + 1}</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      <div className="flex justify-between mb-0.5">
                        ...
                        <span className={`text-[10px] font-bold ${int.performance >= 7 ? c.dangerText : int.performance <= 3 ? c.successText : c.warningText}`}>
                          {int.performance}/10
                        </span>
                      </div>
                      <input
                    ...
                    <div>
                      <div className="flex justify-between mb-0.5">
                        ...
                        <span className={`text-[10px] font-bold ${c.accent}`}>{int.energyBefore}/10</span>
                      </div>
                      <input
                    ...
                    <div>
                      <div className="flex justify-between mb-0.5">
                        ...
                        <span className={`text-[10px] font-bold ${int.energyAfter < int.energyBefore ? c.dangerText : c.successText}`}>{int.energyAfter}/10</span>
                      </div>
                      <input
                ...
                {interactions.length > 1 && (
                  <button onClick={() => removeInteraction(i)} className={`text-sm ${c.dangerText} min-h-[32px] px-1 mt-1.5`}>✕</button>
                )}
              </div>
            </div>
          ))}
        ...
        <button onClick={addInteraction} className={`text-xs font-bold ${c.accent} mb-4 min-h-[32px] flex items-center gap-1`}>
          ➕ Add interaction
        </button>
        ...
        >
          {loading
            ? <><span className="animate-spin inline-block">⏳</span> Auditing your energy...</>
            : editingEntryId
              ? <><span>✏️</span> Update &amp; Re-Audit</>
              : <><span>⚡</span> Run Energy Audit</>}
        </button>
      </div>
    </div>
  );
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <button onClick={() => setView('log')} className={`text-xs font-bold ${c.accent} min-h-[32px]`}>← Back to log</button>
          <ActionBar content={buildFullText()} title="Social Energy Audit" />
        </div>
        ...
        {r.energy_score && (
          <div className={`${c.verdict} border-2 rounded-xl p-5 text-center`}>
            ...
            <p className={`text-xs ${c.textMuted} mb-2`}>energy spent this week</p>
            ...
            {r.energy_score.one_liner && (
              <p className={`text-sm ${c.textSec} mt-3 max-w-md mx-auto`}>{r.energy_score.one_liner}</p>
            )}
            {r.energy_score.net_energy_change && (
              <p className={`text-xs font-bold ${r.energy_score.net_energy_change.startsWith('+') ? c.successText : c.dangerText} mt-2`}>
          ...
          <div className={`${c.card} border rounded-xl p-4`}>
            ...
            <div className="grid grid-cols-3 gap-2 mb-2">
              <div className={`${c.quoteBg} rounded-lg p-3 text-center`}>
                ...
                <p className={`text-[9px] ${c.textMuted}`}>Capacity</p>
              </div>
              <div className={`${c.quoteBg} rounded-lg p-3 text-center`}>
                <p className={`text-lg font-black ${c.warningText}`}>{r.weekly_budget.spent}</p>
                <p className={`text-[9px] ${c.textMuted}`}>Spent</p>
              </div>
              <div className={`${c.quoteBg} rounded-lg p-3 text-center`}>
                <p className={`text-lg font-black ${String(r.weekly_budget.remaining).startsWith('-') ? c.dangerText : c.successText}`}>
                  {r.weekly_budget.remaining}
                </p>
                <p className={`text-[9px] ${c.textMuted}`}>Remaining</p>
              </div>
            </div>
            {r.weekly_budget.verdict && <p className={`text-xs ${c.textSec} text-center`}>{r.weekly_budget.verdict}</p>}
          </div>
        )}
              ...
              {r.drains.map((d, i) => (
                <div key={i} className={`${c.danger} border rounded-lg p-3`}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-xs font-black ${c.dangerText}`}>{d.energy_cost}</span>
                  </div>
                  <div className={`flex gap-2 text-[10px] ${c.textMuted} mb-1`}>
                    <span>Performance: {d.performance_tax}</span>
                    {d.cost_per_hour && <span>· {d.cost_per_hour}/hr</span>}
                  </div>
                  {d.why_costly && <p className={`text-[11px] ${c.textSec}`}>{d.why_costly}</p>}
                </div>
              ))}
            </div>
          </Section>
              ...
              {r.rechargers.map((rc, i) => (
                <div key={i} className={`${c.success} border rounded-lg p-3`}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-xs font-black ${c.successText}`}>{rc.energy_effect}</span>
                  </div>
                  {rc.why_good && <p className={`text-[11px] ${c.textSec}`}>{rc.why_good}</p>}
                </div>
              ))}
            </div>
          </Section>
          ...
          <Section icon="💡" title="How to Restructure" defaultOpen={true} c={c}>
            <div className="space-y-2">
              {r.restructure_suggestions.map((s, i) => (
                <p key={i} className={`text-xs ${c.textSec}`}>→ {s}</p>
              ))}
            </div>
          </Section>
        )}
        ...
        {r.recovery_time && (
          <Section icon="🛋️" title="Recovery Needed" c={c}>
            {r.recovery_time.estimated_hours && <p className={`text-sm ${c.textSec}`}>⏱️ {r.recovery_time.estimated_hours}</p>}
            {r.recovery_time.best_recovery_day && <p className={`text-sm ${c.textSec}`}>📅 {r.recovery_time.best_recovery_day}</p>}
            {r.recovery_time.recovery_type && <p className={`text-sm ${c.textSec}`}>🧘 {r.recovery_time.recovery_type}</p>}
          </Section>
        )}
        ...
        <div className={`${c.card} border rounded-xl p-4`}>
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={() => setView('plan')} className={`${c.btnSec} px-4 py-2 rounded-lg text-xs font-bold min-h-[36px]`}>
              📅 Plan Next Week
            </button>
          </div>
        </div>
  ...
  const renderPlan = () => (
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <p className={`text-sm ${c.textSec} mb-4`}>Enter upcoming commitments — I'll predict energy costs and suggest optimizations</p>
        ...
        <div className="space-y-2 mb-4">
          {upcoming.map((u, i) => (
            <div key={i} className="flex gap-2 items-start">
              ...
              {upcoming.length > 1 && (
                <button onClick={() => removeUpcoming(i)} className={`text-sm ${c.dangerText} min-h-[32px] px-1`}>✕</button>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-3 items-center mb-4">
          <button onClick={addUpcoming} className={`text-xs font-bold ${c.accent} min-h-[32px]`}>➕ Add commitment</button>
        </div>

        <button onClick={runPlan} disabled={!upcoming.some(u => u.situation.trim()) || loading}
          className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
          {loading ? <><span className="animate-spin inline-block">⏳</span> Planning...</> : <><span>📅</span> Predict My Week</>}
        </button>
      </div>
      ...
      {planResults && (
        <div className="space-y-4">
          <div className={`${c.verdict} border-2 rounded-xl p-5 text-center`}>
            ...
            <p className={`text-xs ${c.textMuted} mb-2`}>predicted energy cost</p>
              ...
              <div className="space-y-2">
                {planResults.day_breakdown.map((day, i) => (
                  <div key={i} className={`${day.risk === 'HIGH' ? c.danger : day.risk === 'LOW' ? c.success : c.warning} border rounded-lg p-3`}>
                    ...
                    {day.note && <p className={`text-[11px] ${c.textSec}`}>{day.note}</p>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {planResults.danger_zones?.length > 0 && (
            <div className={`${c.danger} border rounded-xl p-4`}>
              <p className={`text-xs font-bold ${c.dangerText} mb-1`}>⚠️ Danger Zones</p>
  ...
  const renderRecharge = () => (
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <p className={`text-sm ${c.textSec} mb-4`}>Tell me how drained you are — I'll build you a recovery plan</p>

        <div className="mb-4">
          <div className="flex justify-between mb-1">
            <label className={`text-sm font-bold ${c.label}`}>Current energy level</label>
            <span className={`text-sm font-bold ${currentEnergy <= 3 ? c.dangerText : currentEnergy <= 6 ? c.warningText : c.successText}`}>
              {currentEnergy}/10
            </span>
          </div>
        ...
        <div className="mb-4">
          <label className={`text-sm font-bold ${c.label} block mb-1.5`}>
            What drained you most? <span className={`font-normal ${c.textMuted}`}>(optional)</span>
          </label>
        ...
        <div className="mb-5">
          <label className={`text-sm font-bold ${c.label} block mb-1.5`}>
            What recharges you? <span className={`font-normal ${c.textMuted}`}>(optional)</span>
          </label>
        ...
        <button onClick={runRecharge} disabled={loading}
          className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
          {loading ? <><span className="animate-spin inline-block">⏳</span> Building plan...</> : <><span>🔋</span> Get Recharge Plan</>}
        </button>
      </div>
        ...
        <div className="space-y-4">
          {rechargeResults.energy_assessment && (
            <div className={`${c.verdict} border-2 rounded-xl p-5`}>
              ...
              <p className={`text-sm ${c.textSec} text-center`}>{rechargeResults.energy_assessment}</p>
            </div>
          )}
            ...
            <div className={`${c.card} border rounded-xl p-4`}>
              ...
              {rechargeResults.immediate.avoid && (
                <div className={`${c.danger} border rounded-lg p-3`}>
                  <p className={`text-xs font-bold ${c.dangerText}`}>Avoid:</p>
          ...
          {rechargeResults.tonight && (
            <Section icon="🌙" title="Tonight" defaultOpen={true} c={c}>
              <p className={`text-sm ${c.textSec}`}>{rechargeResults.tonight.activity}</p>
              {rechargeResults.tonight.why && <p className={`text-xs ${c.textMuted}`}>Why: {rechargeResults.tonight.why}</p>}
              {rechargeResults.tonight.duration && <p className={`text-xs font-bold ${c.accent}`}>⏱️ {rechargeResults.tonight.duration}</p>}
            </Section>
          )}
          ...
          {rechargeResults.this_week?.length > 0 && (
            <Section icon="📋" title="This Week" defaultOpen={true} c={c}>
              {rechargeResults.this_week.map((a, i) => (
                <p key={i} className={`text-xs ${c.textSec}`}>→ {a}</p>
              ))}
            </Section>
          )}
          ...
          {rechargeResults.recharge_ratio && (
            <div className={`${c.accentBg} border rounded-xl p-4`}>
              <p className={`text-xs font-bold ${c.accent}`}>📐 {rechargeResults.recharge_ratio}</p>
            </div>
          )}

          {/* Share recharge plan */}
          <ActionBar content={buildRechargeText()} title="Recharge Plan" />
        </div>
      )}
    </div>
  );
  ...
  const renderQuickCheck = () => (
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <p className={`text-sm ${c.textSec} mb-4`}>Instant energy check before you commit</p>
        ...
        <div className="mb-4">
          <div className="flex justify-between mb-1">
            ...
            <span className={`text-sm font-bold ${qcEnergy <= 3 ? c.dangerText : qcEnergy <= 6 ? c.warningText : c.successText}`}>
              {qcEnergy}/10
            </span>
          </div>
        ...
        <div className="mb-5">
          <label className={`text-sm font-bold ${c.label} block mb-1.5`}>
            What's already on my plate? <span className={`font-normal ${c.textMuted}`}>(optional)</span>
          </label>
        ...
        <button onClick={runQuickCheck} disabled={!qcCommitment.trim() || loading}
          className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
          {loading ? <><span className="animate-spin inline-block">⏳</span> Checking...</> : <><span>🤔</span> Should I Say Yes?</>}
        </button>
      </div>
      ...
      {qcResults && (
        <div className="space-y-4">
          ...
          <div className={`${
            ...
            <p className={`text-sm ${c.textSec}`}>{qcResults.one_liner}</p>
            {qcResults.energy_impact && (
              <p className={`text-xs font-bold ${c.dangerText} mt-2`}>{qcResults.energy_impact}</p>
            )}
          </div>

          {qcResults.condition && (
            <div className={`${c.warning} border rounded-xl p-4`}>
              <p className={`text-xs font-bold ${c.warningText} mb-1`}>⚠️ Condition</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {qcResults.if_you_say_no && (
              <div className={`${c.card} border rounded-xl p-4`}>
                ...
                <p className={`text-xs ${c.textSec} mb-2`}>{qcResults.if_you_say_no}</p>
          ...
          <button onClick={() => { setQcCommitment(''); setQcResults(null); }}
            className={`text-xs font-bold ${c.accent} min-h-[32px]`}>🔄 Check another</button>
        </div>
      )}
    </div>
  );
    ...
    return (
      <div className="space-y-4">
        <div className={`${c.card} border rounded-xl p-5`}>
          ...
          <p className={`text-sm ${c.textSec} mb-4`}>30 seconds. How's your energy today?</p>

          {alreadyCheckedIn && (
            <div className={`${c.success} border rounded-lg p-3 mb-4`}>
              <p className={`text-xs font-bold ${c.successText}`}>✅ Already checked in today! You can still update it.</p>
            </div>
          )}
          ...
          <div className="mb-4">
            <div className="flex justify-between mb-1">
              ...
              <span className={`text-sm font-bold ${ciEnergy <= 3 ? c.dangerText : ciEnergy <= 6 ? c.warningText : c.successText}`}>
                {ciEnergy}/10
              </span>
            </div>
              ...
              {DAYS.map(day => {
                ...
                return (
                  <div key={day} className={`flex-1 min-w-[40px] ${checkin ? (
                    ...
                    <p className={`text-[8px] font-bold ${c.textMuted}`}>{day.slice(0, 3)}</p>
                    {checkin ? (
                      <p className={`text-sm font-black ${c.text}`}>{checkin.energy}</p>
                    ) : (
                      <p className={`text-sm ${c.textMuted}`}>—</p>
                    )}
                  </div>
                );
              })}
              ...
              {dailyCheckins.slice(0, 14).map((d, i) => (
                <div key={d.id || i} className={`flex items-center gap-2 p-2 rounded-lg ${c.quoteBg}`}>
                  <span className={`text-sm font-black w-6 text-center ${
                    d.energy <= 3 ? c.dangerText : d.energy <= 6 ? c.warningText : c.successText
                  }`}>{d.energy}</span>
                  <div className="flex-1 min-w-0">
                    <span className={`text-[10px] font-bold ${c.text}`}>{d.day}</span>
                    <span className={`text-[10px] ${c.textMuted} ml-1`}>{new Date(d.date).toLocaleDateString()}</span>
                  </div>
                  {d.biggestDrain && <span className={`text-[9px] ${c.dangerText} truncate max-w-[80px]`}>🔴 {d.biggestDrain}</span>}
                  {d.biggestRecharge && <span className={`text-[9px] ${c.successText} truncate max-w-[80px]`}>🟢 {d.biggestRecharge}</span>}
                </div>
              ))}
            </div>
            {dailyCheckins.length > 0 && (
  ...
  const renderForecast = () => (
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <p className={`text-sm ${c.textSec} mb-2`}>
        ...
        <div className={`flex gap-2 mb-4 text-[10px] ${c.textMuted}`}>
        ...
        <button onClick={runForecast} disabled={loading || (!savedTemplate && !dailyCheckins.length)}
          className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
          {loading ? <><span className="animate-spin inline-block">⏳</span> Forecasting...</> : <><span>🌤️</span> Generate Forecast</>}
        </button>
      </div>
      ...
      {forecastResults && (
        <div className="space-y-4">
          ...
          <div className={`${c.verdict} border-2 rounded-xl p-5 text-center`}>
            ...
            <p className={`text-xs ${c.textMuted}`}>predicted energy spend this week</p>
            {forecastResults.reality_check && (
              <p className={`text-xs ${c.textSec} mt-2`}>📊 {forecastResults.reality_check}</p>
            )}
          </div>
                ...
                {forecastResults.predicted_curve.map((day, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] font-bold w-6 ${
                      day.risk === 'HIGH' ? c.dangerText : day.risk === 'LOW' ? c.successText : c.warningText
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {forecastResults.worst_day && (
              <div className={`${c.danger} border rounded-lg p-3`}>
                <p className={`text-[10px] font-bold ${c.dangerText} uppercase`}>Hardest day</p>
          ...
          {forecastResults.danger_zones?.length > 0 && (
            <div className={`${c.warning} border rounded-xl p-4`}>
              <p className={`text-xs font-bold ${c.warningText} mb-1`}>⚠️ Watch Out</p>
          ...
          {forecastResults.strategic_advice?.length > 0 && (
            <Section icon="💡" title="Strategic Advice" defaultOpen={true} c={c}>
              {forecastResults.strategic_advice.map((s, i) => <p key={i} className={`text-xs ${c.textSec}`}>→ {s}</p>)}
            </Section>
          )}
    ...
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className={`text-sm ${c.textSec}`}>Your weekly energy patterns over time</p>
          </div>
          <div className="flex gap-2">
            {journal.length >= 2 && (
              <button
                onClick={() => { setCompareMode(p => !p); setCompareA(null); setCompareB(null); }}
                className={`text-xs font-bold ${compareMode ? c.dangerText : c.accent} min-h-[32px]`}
              >
        ...
        {journalStats && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
            <div className={`${c.quoteBg} rounded-lg p-3 text-center`}>
              ...
              <p className={`text-[9px] ${c.textMuted}`}>Weeks tracked</p>
            </div>
            <div className={`${c.quoteBg} rounded-lg p-3 text-center`}>
              <p className={`text-lg font-black ${c.successText}`}>{journalStats.sustainableWeeks}</p>
              <p className={`text-[9px] ${c.textMuted}`}>Sustainable</p>
            </div>
            <div className={`${c.quoteBg} rounded-lg p-3 text-center`}>
              <p className={`text-lg font-black ${c.dangerText}`}>{journalStats.burnoutWeeks}</p>
              <p className={`text-[9px] ${c.textMuted}`}>Burnout risk</p>
            </div>
            <div className={`${c.quoteBg} rounded-lg p-3 text-center`}>
              <p className={`text-lg font-black ${c.accent}`}>{journalStats.avgInteractions}</p>
              <p className={`text-[9px] ${c.textMuted}`}>Avg interactions</p>
            </div>
          </div>
        )}
        ...
        {journalStats?.trendLine && (
          <div className={`${c.quoteBg} rounded-lg p-3 mb-3`}>
            ...
            <div className={`flex justify-between text-[8px] ${c.textMuted} mt-1`}>
              ...
              {journal.slice(0, 8).reverse().map((entry, i) => {
                ...
                return (
                  <div key={entry.id} className="flex items-center gap-2">
                    ...
                    <span className={`text-[9px] font-bold ${c.textMuted} w-6`}>{score}</span>
                  </div>
                );
              })}
            </div>
      ...
      {peopleInsights && (
        <div className={`${c.card} border rounded-xl p-4`}>
          ...
          <p className={`text-[10px] ${c.textMuted} mb-3`}>Based on {journal.length} weeks of data — situations that appear 2+ times</p>

          {peopleInsights.drains.length > 0 && (
            <div className="mb-3">
              <p className={`text-[10px] font-bold ${c.dangerText} uppercase mb-1.5`}>🔴 Consistent drains</p>
              <div className="space-y-1">
                {peopleInsights.drains.map((d, i) => (
                  <div key={i} className={`flex items-center justify-between p-2 rounded-lg ${c.danger} border`}>
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-bold ${c.text}`}>{d.situation}</span>
                      <span className={`text-[9px] ${c.textMuted}`}>×{d.count}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`text-[9px] ${c.textMuted}`}>perf {d.avgPerf}</span>
                      <span className={`text-[10px] font-black ${c.dangerText}`}>-{d.avgDrain}/interaction</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {peopleInsights.rechargers.length > 0 && (
            <div className="mb-3">
              <p className={`text-[10px] font-bold ${c.successText} uppercase mb-1.5`}>🟢 Consistent rechargers</p>
              <div className="space-y-1">
                {peopleInsights.rechargers.map((r, i) => (
                  <div key={i} className={`flex items-center justify-between p-2 rounded-lg ${c.success} border`}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[9px] ${c.textMuted}`}>×{r.count}</span>
                    </div>
                    <span className={`text-[10px] font-black ${c.successText}`}>+{Math.abs(r.avgDrain)}/interaction</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {peopleInsights.neutral.length > 0 && (
            <div>
              <p className={`text-[10px] font-bold ${c.textMuted} uppercase mb-1.5`}>⚪ Neutral</p>
              <div className="flex flex-wrap gap-1.5">
                {peopleInsights.neutral.map((n, i) => (
                  <span key={i} className={`text-[10px] px-2 py-1 rounded-lg ${c.quoteBg} ${c.textSec}`}>
                    {n.situation} ×{n.count}
                  </span>
                ))}
              </div>
        ...
        <div className={`${c.card} border rounded-xl p-4`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-[10px] ${c.textMuted}`}>AI-designed schedule based on {journal.length} weeks of data</p>
            </div>
            ...
            <div className="mt-4 space-y-3">
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {idealWeekResults.rules.map((rule, i) => (
                      <p key={i} className={`text-xs ${c.textSec} p-2 rounded-lg ${c.quoteBg}`}>📌 {rule}</p>
                    ))}
                  </div>
                </div>
              )}
                    ...
                    {idealWeekResults.ideal_week.map((day, i) => (
                      <div key={i} className={`${c.quoteBg} rounded-lg p-3`}>
                        <div className="flex items-center justify-between mb-1">
                          <span className={`text-xs font-bold ${c.text}`}>{day.day}</span>
                          {day.energy_budget && (
                            <span className={`text-[9px] font-black ${c.accent}`}>{day.energy_budget}</span>
                          )}
                        </div>
                        {day.what_goes_here && <p className={`text-[10px] ${c.successText}`}>✅ {day.what_goes_here}</p>}
                        {day.avoid && <p className={`text-[10px] ${c.dangerText}`}>🚫 {day.avoid}</p>}
                        {day.recharge_window && <p className={`text-[10px] ${c.accent}`}>🔋 {day.recharge_window}</p>}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {idealWeekResults.golden_rule && (
                <div className={`${c.accentBg} border-2 rounded-xl p-4 text-center`}>
                  ...
                  <p className={`text-sm font-bold ${c.accent}`}>⭐ {idealWeekResults.golden_rule}</p>
                </div>
              )}
              ...
              {idealWeekResults.warning_pattern && (
                <div className={`${c.danger} border rounded-lg p-3`}>
                  <p className={`text-xs font-bold ${c.dangerText} mb-0.5`}>⚠️ Watch for this pattern</p>
        ...
        <div className={`${c.card} border rounded-xl p-4`}>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => { setCompareMode(false); setCompareA(null); setCompareB(null); }}
              className={`text-xs ${c.dangerText} min-h-[28px]`}>✕ Close</button>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {[compareEntries.a, compareEntries.b].map((entry, idx) => (
              <div key={idx} className={`${c.quoteBg} rounded-lg p-3`}>
                ...
                <p className={`text-[10px] ${c.textMuted} mb-2`}>{new Date(entry.date).toLocaleDateString()}</p>
                <div className="space-y-1.5">
                  <div className="flex justify-between">
                    <span className={`text-[10px] ${c.textMuted}`}>Energy spent</span>
                    <span className={`text-xs font-black ${c.accent}`}>{entry.energyScore || '—'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={`text-[10px] ${c.textMuted}`}>Interactions</span>
                    <span className={`text-xs font-bold ${c.text}`}>{entry.interactionCount}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={`text-[10px] ${c.textMuted}`}>Verdict</span>
                    <span className={`text-[9px] font-black px-1.5 py-0.5 rounded ${verdictColor(entry.verdict)}`}>
                      {entry.verdict || '—'}
                    </span>
                  </div>
                </div>
                {entry.oneLiner && <p className={`text-[10px] ${c.textSec} mt-2`}>{entry.oneLiner}</p>}
                  ...
                  <div className="mt-2 space-y-0.5">
                    ...
                    {entry.interactions.map((int, ii) => (
                      <div key={ii} className={`flex justify-between text-[9px]`}>
                        <span className={c.textSec}>{int.situation}</span>
                        <span className={`font-bold ${(int.energyBefore - int.energyAfter) > 3 ? c.dangerText : c.textMuted}`}>
      ...
      {journal.length === 0 ? (
        <div className={`${c.card} border rounded-xl p-8 text-center`}>
          ...
          <p className={`text-sm ${c.textMuted}`}>No weeks logged yet. Run your first energy audit!</p>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      {compareMode && (
                        <span className={`text-xs font-bold ${isSelected ? c.accent : c.textMuted}`}>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <div className="text-right">
                      {entry.energyScore && <p className={`text-sm font-black ${c.accent}`}>{entry.energyScore}</p>}
                      {entry.verdict && (
                ...
                {entry.oneLiner && <p className={`text-xs ${c.textSec} mt-1`}>{entry.oneLiner}</p>}
              </div>
            );
          })}
        </div>
