    ...
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        <div className={`mb-4 pb-3 border-b ${c.border}`}>
          ...
          <p className={`text-sm ${c.textSecondary}`}>Log your interactions — see where your energy actually goes</p>
        </div>
    ...
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <button onClick={runPlan} disabled={!upcoming.some(u => u.situation.trim()) || loading}
          className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
          {loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '⚙️'}</span> Planning...</> : <><span>📅</span> Predict My Week</>}
        </button>
      </div>
    ...
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <button onClick={runRecharge} disabled={loading}
          className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
          {loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '⚙️'}</span> Building plan...</> : <><span>🔋</span> Get Recharge Plan</>}
        </button>
      </div>
    ...
    <div className="space-y-4">
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <button onClick={runForecast} disabled={loading || (!savedTemplate && !dailyCheckins.length)}
          className={`w-full ${c.btnPrimary} disabled:opacity-40 font-bold py-3 rounded-lg flex items-center justify-center gap-2 min-h-[48px]`}>
          {loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '⚙️'}</span> Forecasting...</> : <><span>🌤️</span> Generate Forecast</>}
        </button>
      </div>
