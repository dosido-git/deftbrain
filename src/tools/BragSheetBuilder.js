...
const EXAMPLES = [
  ...
  level: 'mid',
  accomplishments: [
    'Rewrote the intake form after noticing that families were dropping out between the phone call and the first appointment. Completion went from about half to nearly all of them, though nobody was tracking it before so I only have our own numbers.',
    'I run the volunteer rota. It has not fallen over in two years, which sounds like nothing until you remember what happened the year before I took it on.',
    'Sat with a funder for two hours explaining why our numbers dipped in Q2 rather than hiding it in an appendix. They renewed, and mentioned the honesty specifically.',
    'Trained four new caseworkers. I do not have a formal training role, it just kept being me.',
    'Noticed we were paying for two overlapping database licences and cancelled one. Saves about 3,000 a year.',
  ],
  currentEntry: '',
},
];;
const BragSheetBuilder = ({ tool }) => {
  ...
  // Which bullet's softer/stronger tweak is running. tweakLoading only spins
  // inside the custom-reword row, so the softer/stronger buttons showed nothing.
  const [pendingTweak, runTweak] = usePendingKey();
  ...
  const loadExample = useCallback(() => {
    const ex = pickExample('BragSheetBuilder', EXAMPLES);
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-base ${c.textSecondary}`}>
                <span className="me-2 text-lg">{tool?.icon ?? '🏆'}</span>{tool?.tagline ?? t('bsb_tagline')}
              </p>
