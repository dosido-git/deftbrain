...
const BragSheetBuilder = () => {
  ...
  const toolData = getToolById('BragSheetBuilder');
  ...
  const c = {
    ...
    tab: (active) => active
      ? (isDark ? 'border-amber-500 text-amber-300' : 'border-amber-600 text-amber-700')
    ...
    accentBg: isDark ? 'bg-amber-900/30 border-amber-700' : 'bg-amber-50 border-amber-200',
  };
  ...
  const handleBuild = async () => {
    ...
    try {
      ...
      setHistory(prev => [{ id: Date.now().toString(), count: all.length, industry, date: new Date().toISOString() }, ...prev].slice(0, 10));
  ...
  const ScoreBar = ({ score, label, size = 'md' }) => {
    const color = score >= 75 ? (isDark ? 'bg-green-500' : 'bg-green-500') : score >= 50 ? (isDark ? 'bg-amber-500' : 'bg-amber-500') : (isDark ? 'bg-red-500' : 'bg-red-500');
    const h = size === 'sm' ? 'h-1.5' : 'h-2.5';
    return (
      <div className="flex items-center gap-2 w-full">
        {label && <span className={`text-[10px] font-bold ${c.textMuted} w-28 text-right flex-shrink-0`}>{label}</span>}
        <div className={`flex-1 ${isDark ? 'bg-zinc-700' : 'bg-gray-200'} rounded-full ${h} overflow-hidden`}>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className={`text-2xl font-bold ${c.text}`}>{toolData?.title || 'Brag Sheet Builder'} {toolData?.icon || '✨'}</h2>
          <p className={`text-sm ${c.textMuted}`}>{toolData?.tagline || 'Turn humble descriptions into powerful achievement statements'}</p>
        </div>
        {!results && (
      ...
      {/* ═══════════════ JOURNAL PANEL ═══════════════ */}
      {showJournal && !results && (
        <div className={`${c.card} rounded-xl shadow-lg p-5 mb-5 border-l-4 ${isDark ? 'border-cyan-500' : 'border-cyan-400'}`}>
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto mb-3">
                {journal.map((entry, i) => (
                  <div key={entry.id} className={`flex items-start gap-2 p-2 rounded-lg ${c.cardAlt} text-xs`}>
                    <input type="checkbox" id={`j-${i}`} className="mt-0.5 accent-amber-500" />
                    ...
                    <button onClick={() => setJournal(prev => prev.filter((_, idx) => idx !== i))} className={`${c.textMuted} hover:text-red-500`}>✕</button>
                  </div>
                ))}
              </div>
              <div className="flex items-center gap-2">
        ...
        <div className="space-y-5">
          ...
          <div className={`${c.card} rounded-xl shadow-lg p-6`}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className={`block text-sm font-semibold ${c.label} mb-1.5`}>Your role <span className={`font-normal ${c.textMuted}`}>(optional)</span></label>
                <input type="text" value={roleTitle} onChange={e => setRoleTitle(e.target.value)} placeholder="e.g., Product Manager, Nurse, Teacher..."
                  className={`w-full p-3 border rounded-xl outline-none text-sm focus:ring-2 focus:ring-amber-300 ${c.input}`} />
              </div>
              <div>
                <label className={`block text-sm font-semibold ${c.label} mb-1.5`}>Years of experience <span className={`font-normal ${c.textMuted}`}>(optional)</span></label>
          ...
          <div className={`${c.card} rounded-xl shadow-lg p-6`}>
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={handleExcavate} disabled={excavatorLoading}
                className={`flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-lg ${isDark ? 'bg-cyan-900/30 text-cyan-300 hover:bg-cyan-800/40' : 'bg-cyan-50 text-cyan-700 hover:bg-cyan-100'}`}>
                {excavatorLoading ? <span className="inline-block animate-spin">⏳</span> : <span>🧠</span>} Can't remember? Let me help
              </button>
            </div>

            {/* Excavator results */}
            {showExcavator && excavatorData?.categories && (
              <div className={`mb-4 p-4 rounded-xl border-2 ${isDark ? 'border-cyan-700 bg-cyan-900/10' : 'border-cyan-200 bg-cyan-50/50'}`}>
                <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="space-y-1.5 ml-5">
                          {cat.questions.map((q, qi) => (
                            <button key={qi} onClick={() => { setCurrentEntry(q.exampleAccomplishment || ''); entryRef.current?.focus(); }}
                              className={`w-full text-left p-2 rounded-lg text-xs ${isDark ? 'hover:bg-zinc-700' : 'hover:bg-gray-100'} transition-colors`}>
              ...
              <div className="space-y-2 mb-3">
                {accomplishments.map((acc, idx) => (
                  <div key={idx} className={`flex items-start gap-2 p-3 rounded-lg ${c.cardAlt}`}>
                    <span className={`text-xs font-bold ${isDark ? 'text-amber-400' : 'text-amber-600'} mt-0.5`}>{idx + 1}.</span>
                    <p className={`flex-1 text-sm ${c.text}`}>{acc}</p>
                    <button onClick={() => removeAccomplishment(idx)} className={`${c.textMuted} hover:text-red-500 transition-colors p-1 text-sm`}>✕</button>
                  </div>
                ))}
              </div>
            )}
            <div className="flex gap-2">
              <input ref={entryRef} type="text" value={currentEntry} onChange={e => setCurrentEntry(e.target.value)}
                ...
                className={`flex-1 p-3 border rounded-xl outline-none text-sm focus:ring-2 focus:ring-amber-300 ${c.input}`} />
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              {accomplishments.length > 0 && (
                <button onClick={() => setSavedAccomplishments([...accomplishments])}
                  className={`text-xs ${isDark ? 'text-amber-400 hover:text-amber-300' : 'text-amber-600 hover:text-amber-700'} font-medium`}>💾 Save for later</button>
              )}
            </div>
          </div>
          ...
          <button onClick={handleBuild} disabled={loading || (accomplishments.length === 0 && !currentEntry.trim())}
            ...
            {loading ? (<><span className="inline-block animate-spin text-lg">⏳</span> Transforming...</>) : (<><span className="text-lg">🏆</span> Build My Brag Sheet</>)}
          </button>
      ...
      {results && (
        <div className="space-y-5">
          ...
          {/* Add One More */}
          {addingMore && (
            <div className={`${c.card} rounded-xl shadow-lg p-5 border-l-4 ${isDark ? 'border-amber-500' : 'border-amber-400'}`}>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={handleAddSingle} disabled={addSingleLoading || !newEntry.trim()}
                  ...
                  {addSingleLoading ? <span className="inline-block animate-spin">⏳</span> : '🏆'}</button>
              ...
              {transforms.map((t, idx) => (
                <div key={idx} className={`${c.card} rounded-xl shadow-lg p-5 space-y-3`}>
                  ...
                  <div className="flex items-center gap-2 px-2">
                    ...
                    <span className={`text-[10px] font-bold ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>{t.what_changed}</span>
                    {t._refined && <span className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded ${isDark ? 'bg-green-900/40 text-green-300' : 'bg-green-100 text-green-700'}`}>Upgraded</span>}
                    {t._tweaked && <span className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded ${isDark ? 'bg-blue-900/40 text-blue-300' : 'bg-blue-100 text-blue-700'}`}>Tweaked</span>}
                  </div>
                  {t._refined && preUpgradeVersions[idx] && (
                    <div className={`px-3 py-2 rounded-lg ${isDark ? 'bg-zinc-700/30' : 'bg-gray-100'}`}>
                  ...
                  <div className="flex items-start gap-2">
                    <div className={`flex-1 ${c.afterBg} border rounded-lg p-3`}>
                      <p className={`text-[10px] font-bold ${isDark ? 'text-amber-400' : 'text-amber-600'} uppercase mb-1`}>Power version:</p>
                      <p className={`text-sm font-semibold ${c.text}`}>{t.improved}</p>
                    </div>
                    <CopyBtn content={`${t.improved}${BRAND}`} label="Copy" />
                  </div>
                  {t.metric_highlight && <p className={`text-xs font-bold px-2 ${isDark ? 'text-green-400' : 'text-green-600'}`}>📊 {t.metric_highlight}</p>}
                    ...
                    <div className="flex flex-wrap gap-2 px-1">
                      {t.verb_upgrades.map((vu, i) => (
                        <span key={i} className={`text-[10px] px-2 py-0.5 rounded-full ${c.accentBg} border font-bold`}>
                          <span className={c.textMuted}>{vu.from}</span> → <span className={isDark ? 'text-amber-300' : 'text-amber-700'}>{vu.to}</span>
                        </span>
                      ))}
                    </div>
                  )}
                    ...
                    {tweakingIdx === idx && (
                      <div className="flex gap-2 w-full mt-1">
                        ...
                        <button onClick={() => handleTweak(idx, tweakInstruction)} disabled={tweakLoading || !tweakInstruction.trim()}
                          className={`${c.btnPrimary} disabled:opacity-40 px-3 rounded-lg text-xs font-bold`}>{tweakLoading ? '⏳' : '✓'}</button>
                    ...
                    <div className={`p-3 rounded-lg border ${c.border} space-y-2`}>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleGenerateStar(idx)} disabled={starLoading === idx}
                          ...
                          {starLoading === idx ? '⏳ Generating...' : 'Generate'}</button>
                ...
                <div className={`${c.accentBg} border-2 rounded-xl p-5`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      {r.confidence.imposter_killer && <p className={`text-xs ${isDark ? 'text-amber-300' : 'text-amber-700'} font-semibold`}>{r.confidence.imposter_killer}</p>}
                    </div>
                  </div>
                </div>
              )}
          ...
          {activeTab === 'metrics' && (
            <div className={`${c.card} rounded-xl shadow-lg p-6 space-y-4`}>
                ...
                {metrics.map((mq, i) => {
                  ...
                  return (
                    <div key={i} className={`p-4 rounded-lg border ${c.border}`}>
                      {src && <p className={`text-[10px] font-bold ${isDark ? 'text-amber-400' : 'text-amber-600'} mb-2`}>Re: #{mq.accomplishment_index + 1} — "{src.original?.substring(0, 60)}…"</p>}
                      <p className={`text-sm font-semibold ${c.text} mb-1`}>{mq.question}</p>
                      <p className={`text-xs ${c.textMuted} mb-2`}>Why: {mq.why}</p>
                      {mq.example && <p className={`text-xs ${isDark ? 'text-amber-400' : 'text-amber-600'} mb-3`}>e.g., "{mq.example}"</p>}
              ...
              <button onClick={handleRefine} disabled={refineLoading || Object.values(metricsAnswers).every(v => !v?.trim())}
                ...
                {refineLoading ? '⏳ Upgrading...' : '🚀 Upgrade My Bullets'}</button>
              ...
              {!radarData && (
                <div className={`${c.card} rounded-xl shadow-lg p-6 text-center`}>
                  ...
                  <button onClick={handleRadar} disabled={radarLoading}
                    ...
                    {radarLoading ? '⏳ Analyzing...' : '📡 Analyze My Sheet'}</button>
                </div>
              )}
              {radarData && (
                <div className={`${c.card} rounded-xl shadow-lg p-6 space-y-5`}>
                  ...
                  <div className="space-y-2">
                    {radarData.dimensions?.map((dim, i) => (
                      <div key={i} className="space-y-1">
                        ...
                        {dim.gap_suggestion && <p className={`text-[10px] ${isDark ? 'text-cyan-400' : 'text-cyan-600'} ml-10`}>💡 {dim.gap_suggestion}</p>}
                      </div>
                    ))}
                  </div>
            ...
            <div className="space-y-4">
              <div className={`${c.card} rounded-xl shadow-lg p-6 space-y-4`}>
                ...
                <button onClick={handleTailor} disabled={tailorLoading || !jdText.trim()}
                  className={`w-full py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 ${c.btnPrimary} disabled:opacity-40`}>
                  {tailorLoading ? '⏳ Tailoring...' : '🎯 Tailor My Sheet'}</button>
              </div>
              {tailorData && (
                <div className="space-y-4">
                  {/* Match score */}
                  <div className={`${c.card} rounded-xl shadow-lg p-5`}>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className={`text-xl font-bold ${tailorData.match_score >= 70 ? (isDark ? 'text-green-400' : 'text-green-600') : isDark ? 'text-amber-400' : 'text-amber-600'}`}>{tailorData.match_score}%</span>
                    </div>
                    ...
                    <div className={`${c.card} rounded-xl shadow-lg p-5`}>
                      ...
                      <div className="space-y-2">
                        {tailorData.gaps.map((gap, i) => (
                          <div key={i} className={`p-3 rounded-lg border ${gap.severity === 'critical' ? c.danger : gap.severity === 'moderate' ? c.warning : c.info}`}>
              ...
              {!matrixData && (
                <div className={`${c.card} rounded-xl shadow-lg p-6 text-center`}>
                  ...
                  <button onClick={handleMatrix} disabled={matrixLoading}
                    className={`${c.btnPrimary} disabled:opacity-40 px-6 py-3 rounded-xl font-semibold text-sm`}>
                    {matrixLoading ? '⏳ Building matrix...' : '🎤 Build My Interview Prep'}</button>
                </div>
              )}
              {matrixData && (
                <div className="space-y-4">
                  ...
                  <div className={`${c.card} rounded-xl shadow-lg p-5`}>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className={`text-sm font-bold ${matrixData.coverage_score >= 70 ? (isDark ? 'text-green-400' : 'text-green-600') : isDark ? 'text-amber-400' : 'text-amber-600'}`}>
                    ...
                    <div className="space-y-2">
                      {matrixData.questions?.filter(q => q.best_match?.type !== 'none').map((q, i) => (
                        <div key={i} className={`p-3 rounded-lg ${c.cardAlt}`}>
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                <span className={`text-[9px] px-1.5 py-0.5 rounded ${c.info} border font-bold`}>{q.category}</span>
                          ...
                          {q.best_match?.angle && <p className={`text-xs ${isDark ? 'text-amber-400' : 'text-amber-600'} mt-2`}>💡 {q.best_match.angle}</p>}
            ...
            <div className="space-y-4">
              <div className={`${c.card} rounded-xl shadow-lg p-6 space-y-4`}>
                ...
                <button onClick={handleVoiceMatch} disabled={voiceLoading || !voiceSample.trim()}
                  className={`w-full py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 ${c.btnPrimary} disabled:opacity-40`}>
                  {voiceLoading ? '⏳ Matching voice...' : '✍️ Rewrite In My Voice'}</button>
              </div>
              {voiceData && (
                <div className="space-y-4">
                  {/* Voice profile */}
                  {voiceData.voice_profile && (
                    <div className={`${c.card} rounded-xl shadow-lg p-5`}>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <span className={`text-[10px] px-2.5 py-1 rounded-full ${c.info} border font-bold`}>Sentences: {voiceData.voice_profile.sentence_length}</span>
                        <span className={`text-[10px] px-2.5 py-1 rounded-full ${c.info} border font-bold`}>Formality: {voiceData.voice_profile.formality}</span>
                        <span className={`text-[10px] px-2.5 py-1 rounded-full ${c.info} border font-bold`}>Perspective: {voiceData.voice_profile.perspective}</span>
                      </div>
                          ...
                          <div key={i} className="space-y-1.5">
                            ...
                            <div className="flex items-start gap-2">
                              <div className={`flex-1 ${c.afterBg} border rounded-lg p-2.5`}>
                                <p className={`text-[10px] font-bold ${isDark ? 'text-amber-400' : 'text-amber-600'} mb-0.5`}>Your voice:</p>
                        ...
                        <div key={idx} className="space-y-2">
                            ...
                            {[{ k: 'S', l: 'Situation', v: story.situation }, { k: 'T', l: 'Task', v: story.task },
                              ...
                              .map(s => s.v ? (
                                <div key={s.k} className={`flex items-start gap-2 ${c.cardAlt} rounded-lg p-2.5`}>
                                  <span className={`text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${isDark ? 'bg-amber-700 text-amber-100' : 'bg-amber-100 text-amber-700'}`}>{s.k}</span>
                          ...
                          {story.good_for_questions?.length > 0 && (
                            <div className="flex flex-wrap gap-1.5 px-1">
                              ...
                              {story.good_for_questions.map((q, qi) => <span key={qi} className={`text-[10px] px-2 py-0.5 rounded-full ${c.info} border`}>{q}</span>)}
                            </div>
                          )}
                        </div>
                      ))}
          ...
          {activeTab === 'raise' && raise.summary && (
            <div className="space-y-4">
              <div className={`${c.card} rounded-xl shadow-lg p-6`}>
                ...
                {raise.script && (
                  <div className={`p-4 rounded-xl ${c.accentBg} border`}>
              ...
              <p className={`text-xs text-center ${c.textMuted}`}>
                Need to practice the conversation? <a href="/DifficultTalkCoach" target="_blank" rel="noopener noreferrer"
                  className={`font-semibold ${isDark ? 'text-cyan-400 hover:text-cyan-300' : 'text-cyan-600 hover:text-cyan-700'}`}>Difficult Talk Coach</a> will rehearse it with you.
              </p>
            </div>
          )}
