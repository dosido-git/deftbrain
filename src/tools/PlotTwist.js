...
const PlotTwist = ({ tool }) => {
  ...
  }, [decision, options, context, values, deadline, stuckReason, callToolEndpoint, userLocale, userCurrency, userRegion, t]);
  ...
  const buildFullCopy = useCallback(() => {
    ...
    if (results?.the_real_question) lines.push(t('ptw_copy_real_q'), results?.the_real_question, '');
    if (results?.gut_check) lines.push(t('ptw_copy_gut'), results?.gut_check, '');
  ...
  const renderInput = () => (
    <div className="space-y-4">
      ...
      <button title={t('cmd_enter')} onClick={analyze}
        ...
        className={`relative w-full ${c.btnPrimary} disabled:opacity-40 py-4 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 transition-all min-h-[48px]`}>
        {loading
      ...
      <div data-copy-results ref={resultsRef} className="space-y-4 mt-4">
        ...
        {results?.stuck_pattern && (
          <div className={`p-5 rounded-2xl border ${c.stuckBg}`}>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-lg">🧠</span>
              <span className={`text-sm font-bold ${c.stuckText}`}>{t('ptw_stuck_why', { pattern: results?.stuck_pattern?.pattern })}</span>
            </div>
                ...
                <div key={idx} className="p-5 space-y-4">
                  ...
                  {opt.ten_ten_ten && (
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        { label: t('ptw_10min'), val: opt.ten_ten_ten.ten_minutes },
                        { label: t('ptw_10months'), val: opt.ten_ten_ten.ten_months },
                        { label: t('ptw_10years'), val: opt.ten_ten_ten.ten_years },
                      ].map((row, i) => (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {opt.reversibility && (
                      <div className={`p-3 rounded-lg ${c.inset}`}>
                        <p className={`text-[10px] font-bold ${c.textMuteded} uppercase mb-1`}>{t('ptw_reversibility', { score: opt.reversibility.score })}</p>
                        <p className={`text-xs ${c.text}`}>{opt.reversibility.assessment}</p>
                      </div>
                    )}
                    {opt.values_alignment && (
                      <div className={`p-3 rounded-lg ${c.inset}`}>
                        <p className={`text-[10px] font-bold ${c.textMuteded} uppercase mb-1`}>{t('ptw_values_fit', { score: opt.values_alignment.score })}</p>
            ...
            {sessionHistory.map(entry => (
              <div key={entry.id} className={`rounded-xl border ${c.histCard} p-3 flex items-center gap-3`}>
                ...
                <button onClick={() => { setResults(entry.results); setShowHistory(false); }}
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold ${c.btnSecondary}`}>{t('ptw_view')}</button>
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className={`text-base ${c.textSecondary}`}>
                  <span className="me-2 text-lg">{tool?.icon ?? '🔀'}</span>{tool?.tagline ?? t('ptw_tagline')}
                </p>
