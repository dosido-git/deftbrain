...
// "AI regulation in Congress" is a different question depending on whether an
// investor, a developer, a policymaker or a curious reader is asking it. The
// event alone does not say which parallels would be worth reading, so the tool
// asks. Optional: with nothing picked it behaves as it always has.
const MOTIVATIONS = [
  { value: 'understand', icon: '🧭', labelKey: 'ht_mot_understand' },
  { value: 'worried',    icon: '⚠️', labelKey: 'ht_mot_worried' },
  { value: 'decision',   icon: '💼', labelKey: 'ht_mot_decision' },
...
const HistoryToday = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    const ex = pickExample('HistoryToday', EXAMPLES);
    setEvent(ex.event);
