      ...
      {!results && (
        ...
        <div className={`${c.card} border ${c.border} rounded-xl p-5`}>
          ...
          <button onClick={handleSearch} disabled={loading || !event.trim()}
            className={`flex-1 py-4 px-6 rounded-xl font-semibold text-lg transition-all flex items-center justify-center gap-2 shadow-lg ${loading ? (isDark ? 'bg-zinc-700 text-zinc-400' : 'bg-gray-200 text-gray-400') : c.btnPrimary} disabled:opacity-40`}>
...
{!counterData && (
                <button onClick={handleCounter} disabled={counterLoading}
                  ...
                  {counterLoading ? <span className="inline-block animate-spin">{tool?.icon ?? '📰'}</span> : <span>🔄</span>} Counter-Example
                </button>
              )}
            </div>
          </div>
                ...
                <div className={`flex items-center gap-2 pt-2 border-t ${c.border}`}>
                  {!deeperData[idx] ? (
                    <button onClick={() => handleDeeper(idx)} disabled={deeperLoading === idx}
                      ...
                      {deeperLoading === idx ? (<><span className="inline-block animate-spin">{tool?.icon ?? '📰'}</span> Loading...</>) : (<><span>🔍</span> Dig Deeper</>)}
                    </button>
                  ) : (
