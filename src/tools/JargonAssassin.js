    ...
    <div className={`space-y-4 ${c.text}`}>
      ...
      {mode === 'input' && <div className={`${c.card} ${c.border} border ${c.border} rounded-xl p-5 space-y-5`}>
        ...
        <div className="flex gap-2">
          <button title={t('cmd_enter')} onClick={handleTranslate} disabled={loading || (!docText.trim() && !fileBase64)} className={`relative flex-1 py-3 rounded-xl font-bold text-sm ${((!docText.trim() && !fileBase64)) ? c.btnIdle : c.btnPrimary}`}>
            {loading ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '🗡️'}</span> {t('jarg_working')}</> : <><span className='me-1 text-xl'>{tool?.icon ?? '🗡️'}</span> {t('jarg_translate')}</>}
      ...
      {mode === 'dossier' && <div className="space-y-4">
        <div className={`${c.card} ${c.border} border ${c.border} rounded-xl p-5 space-y-4`}>
          ...
          <button onClick={handleDossier} disabled={loading} className={`w-full py-3 rounded-xl font-bold text-sm ${c.btnPrimary} disabled:opacity-40`}>{loading ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '🗡️'}</span> {t('jarg_working')}</> : <><span className='me-1 text-xl'>{tool?.icon ?? '🗡️'}</span> {t('jarg_cross_reference')}</>}</button>
        </div>
      ...
      {mode === 'letter' && <div className="space-y-4">
        <div className={`${c.card} ${c.border} border ${c.border} rounded-xl p-5 space-y-4`}>
          ...
          <button onClick={handleLetter} disabled={loading} className={`w-full py-3 rounded-xl font-bold text-sm ${c.btnPrimary} disabled:opacity-40`}>{loading ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '🗡️'}</span> {t('jarg_working')}</> : <><span className='me-1 text-xl'>{tool?.icon ?? '🗡️'}</span> {t('jarg_generate_letter')}</>}</button>
        </div>
