...
const DOC_TYPES = [
  { id: 'unsure', icon: '❓', labelKey: 'jarg_dt_unsure' }, { id: 'legal', icon: '⚖️', labelKey: 'jarg_dt_legal' },
  { id: 'medical', icon: '🏥', labelKey: 'jarg_dt_medical' }, { id: 'insurance', icon: '🛡️', labelKey: 'jarg_dt_insurance' },
  { id: 'financial', icon: '💰', labelKey: 'jarg_dt_financial' }, { id: 'government', icon: '🏛️', labelKey: 'jarg_dt_government' },
  { id: 'technical', icon: '🔧', labelKey: 'jarg_dt_technical' }, { id: 'academic', icon: '🎓', labelKey: 'jarg_dt_academic' },
  { id: 'other', icon: '📄', labelKey: 'jarg_dt_other' },
];

const LEVELS = [
  { id: 'eli5', icon: '👶', labelKey: 'jarg_style_simple' },
  { id: '5th-grade', icon: '💬', labelKey: 'jarg_style_everyday' },
  { id: 'professional', icon: '📖', labelKey: 'jarg_style_detail' },
];
...
const JargonAssassin = ({ tool }) => {
  ...
  const [docType, setDocType] = useState('unsure');
  const [readLevel, setReadLevel] = useState('5th-grade');
  const [userGoal, setUserGoal] = useState('');
  const [showExplanationStyle, setShowExplanationStyle] = useState(false);
  ...
  // Which section's deep-dive is running. `loading` alone dims every card's
  // button at once, so the one you pressed looks like the ones you didn't.
  const [pendingSection, runSection] = usePendingKey();
  ...
  // Dossier
  const [dossDocs, setDossDocs] = useState([{ title: '', text: '', type: 'unsure' }, { title: '', text: '', type: 'unsure' }]);
  ...
  const handleTranslate = async () => {
    ...
    try {
      const data = await callToolEndpoint('jargon-assassin', { documentText: docText, documentType: docType, readingLevel: readLevel, userGoal: userGoal.trim(), imageBase64: fileBase64 || null, mediaType: fileMediaType || null, userLocale, userCurrency, userRegion });
      setResults(data); setActiveTab('translation'); setMode('results'); setTimeout(() => revealSection(resultsRef.current), 100);
  ...
  const loadExample = () => {
    ...
    setTplData(null); setApData(null); setPersData(null); setUserGoal(''); setError('');
  };
    ...
    <div className={`space-y-4 ${c.text}`}>
      <div className={`${c.card} border ${c.border} rounded-xl shadow-sm`}>
          ...
          <div className="flex items-start justify-between pb-3 border-b border-zinc-500">
            <div>
              ...
              <p className={`text-base ${c.textSecondary}`}>
                <span className="me-2 text-lg">{tool?.icon ?? '🗡️'}</span>{tool?.tagline ?? t('jarg_tagline')}
              </p>
        ...
        <p className={`text-xs ${c.textMuteded} px-5 pt-1`}>
          {t('jarg_xref_phrase_q')} <a href="/SayWhat" className={linkStyle}>💬 {t('jarg_xref_phrase')}</a>.
        </p>
      ...
      {mode === 'input' && <div className={`${c.card} ${c.border} border ${c.border} rounded-xl p-5 space-y-5`}>
        <div>
          <h3 className={`font-bold text-lg ${c.text}`}>🗡️ {t('jarg_input_heading')} <span className={c.required}>*</span></h3>
          <p className={`text-sm ${c.textMuteded} mt-1`}>{t('jarg_input_sub')}</p>
        </div>
        ...
        <textarea
          value={docText}
          onChange={e => { setDocText(e.target.value); if (e.target.value) { setFileBase64(null); setFileMediaType(null); setFileName(''); } }}
          placeholder={fileBase64 ? t('jarg_ph_file') : t('jarg_ph_paste')}
          rows={fileBase64 ? 3 : 8}
        ...
        <div>
          <p className={`text-xs font-bold ${c.text} mb-2`}>{t('jarg_doc_type')}</p>
          <div className="flex flex-wrap gap-1.5">
            {DOC_TYPES.map(dt => <button key={dt.id} onClick={() => setDocType(dt.id)} className={`px-3 py-1.5 rounded-lg text-xs font-medium border ${docType === dt.id ? c.pillOn : `${c.pillOff} ${c.border}`}`}>{dt.icon} {t(dt.labelKey)}</button>)}
          </div>
        </div>
        ...
        <div className={`border-t ${c.border} pt-3`}>
          <button type="button" onClick={() => setShowExplanationStyle(v => !v)} className={`text-xs font-semibold ${c.textSecondary}`}>
            {showExplanationStyle ? '▾' : '▸'} {t('jarg_explanation_style')}
          </button>
          {showExplanationStyle && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {LEVELS.map(rl => <button key={rl.id} onClick={() => setReadLevel(rl.id)} className={`px-3 py-1.5 rounded-lg text-xs font-medium border ${readLevel === rl.id ? c.pillOn : `${c.pillOff} ${c.border}`}`}>{rl.icon} {t(rl.labelKey)}</button>)}
            </div>
          )}
        </div>

        <div className="flex gap-2">
          <button title={t('cmd_enter')} onClick={handleTranslate} disabled={loading || (!docText.trim() && !fileBase64)} className={`relative flex-1 py-3 rounded-xl font-bold text-sm ${((!docText.trim() && !fileBase64)) ? c.btnIdle : c.btnPrimary}`}>
            {loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '🗡️'}</span> {t('jarg_working')}</> : <><span className='me-1'>{tool?.icon ?? '🗡️'}</span> {t('jarg_translate')}</>}
      ...
      {/* ═══ RESULTS ═══ */}
      {mode === 'results' && results && <div data-copy-results ref={resultsRef} className="scroll-mt-24 space-y-4">
        {/* Summary + Danger */}
        ...
        {activeTab === 'personalize' && <div className="space-y-4">
          <div className={`${c.card} ${c.border} border ${c.border} rounded-xl p-4 space-y-3`}>
            ...
            <button onClick={handlePersonalize} disabled={loading || !situation.trim()} className={`w-full py-2.5 rounded-xl font-bold text-sm ${c.btnPrimary} disabled:opacity-40`}>{loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '🗡️'}</span> {t('jarg_working')}</> : <><span className='me-1'>{persData ? '✅' : '🎯'}</span> {t('jarg_personalize_btn')}</>}</button>
          </div>
        ...
        {activeTab === 'highlights' && <div className="space-y-3">{!results.key_sections?.length ? <p className={`text-sm ${c.textMuteded} text-center py-4`}>{t('jarg_no_sections')}</p> : results.key_sections.map((s, i) => {
          ...
          return <div key={i} className={`${bg} border rounded-xl p-4 space-y-2`}>
            ...
            <PendingBtn itemKey={s.original_text} pending={pendingSection} icon={pendingSection === s.original_text ? (tool?.icon ?? '🗡️') : '🔍'} onClick={() => runSection(s.original_text, () => handleSection(s.original_text))} disabled={loading} className={`text-xs font-bold ${c.textSecondary}`}>{t('jarg_deep_dive')}</PendingBtn>
          </div>;
        })}</div>}
        ...
        {activeTab === 'explain' && <div className="space-y-4">
          <div className={`${c.card} ${c.border} border ${c.border} rounded-xl p-4 space-y-3`}>
            ...
            <button onClick={handleExplainTo} disabled={loading} className={`w-full py-2.5 rounded-xl font-bold text-sm ${c.btnPrimary} disabled:opacity-40`}>{loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '🗡️'}</span> {t('jarg_working')}</> : <><span className='me-1'>{tool?.icon ?? '🗡️'}</span> {t('jarg_reframe')}</>}</button>
          </div>
      ...
      {mode === 'dossier' && <div className="space-y-4">
        <div className={`${c.card} ${c.border} border ${c.border} rounded-xl p-5 space-y-4`}>
          ...
          <button onClick={() => { shouldFocusNewDossDocsRef.current = true; setDossDocs(prev => [...prev, { title: '', text: '', type: 'unsure' }]); }} className={`text-xs ${c.textSecondary} font-bold`}>➕ {t('jarg_add_document')}</button>
          <button onClick={handleDossier} disabled={loading} className={`w-full py-3 rounded-xl font-bold text-sm ${c.btnPrimary} disabled:opacity-40`}>{loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '🗡️'}</span> {t('jarg_working')}</> : <><span className='me-1'>{tool?.icon ?? '🗡️'}</span> {t('jarg_cross_reference')}</>}</button>
        </div>
      ...
      {mode === 'letter' && <div className="space-y-4">
        <div className={`${c.card} ${c.border} border ${c.border} rounded-xl p-5 space-y-4`}>
          ...
          <button onClick={handleLetter} disabled={loading} className={`w-full py-3 rounded-xl font-bold text-sm ${c.btnPrimary} disabled:opacity-40`}>{loading ? <><span className="inline-block animate-spin">{tool?.icon ?? '🗡️'}</span> {t('jarg_working')}</> : <><span className='me-1'>{tool?.icon ?? '🗡️'}</span> {t('jarg_generate_letter')}</>}</button>
        </div>
