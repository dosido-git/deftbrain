    ...
    <div className={`space-y-4 ${c.text}`}>
      ...
      <div className={`${c.highlight} border rounded-xl p-5 flex items-start gap-3`}>
        ...
        <div className="flex-1">
          <p className={`text-sm ${c.textSecondary}`}>Paste reviews or import from a URL. Real stats computed instantly, then AI scores each review, detects manipulation patterns, fingerprints authors, and teaches you to spot fakes.</p>
      ...
      <div className={`${c.card} border rounded-xl p-5`}>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={extractFromUrl} disabled={!productUrl.trim() || isRunning} className={`${c.btnSecondary} disabled:opacity-40 font-semibold px-4 py-2.5 rounded-lg text-sm whitespace-nowrap`}>{phase === 'extracting' ? <><span className="animate-spin inline-block">{tool?.icon ?? '🔍'}</span> ...</> : <><span>{tool?.icon ?? '🔍'}</span> Extract</>}</button>
        </div>
      ...
      <div className={`${c.card} border rounded-xl p-6`}>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {SOURCE_PRESETS.map(s => <button key={s} onClick={() => setCurrentSource(s)} className={`px-3 py-1 rounded-lg text-xs font-semibold border transition-colors ${currentSource === s ? (isDark ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-emerald-600 border-emerald-600 text-white') : `${c.btnSecondary} border-transparent`}`}>{s}</button>)}
      ...
      {reviewScores?.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setSortSuspicious(p => !p)} className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold ${c.btnSecondary}`}>↕️ {sortSuspicious ? 'Suspicious first' : 'Original'}</button>
          </div>
          ...
          {sorted.length > 5 && !showAllReviews && <button onClick={() => setShowAllReviews(true)} className={`w-full mt-3 py-3 rounded-lg text-sm font-semibold ${c.btnSecondary}`}>▼ Show all {sorted.length}</button>}
        </div>
      )}
