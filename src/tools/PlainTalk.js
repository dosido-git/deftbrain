...
import {
  Search, FileText, Upload, Loader2, CheckCircle, AlertCircle, AlertTriangle,
  X, RefreshCw, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, Copy, Check,
  BookOpen, Layers, Eye, Columns, ArrowRight, Lightbulb, Scale, Clock, List,
  MessageSquare, Sparkles, Zap, Printer
} from 'lucide-react';
...
const PlainTalk = () => {
  ...
  // ─── CopyButton helper ───
  const CopyBtn = ({ content, field, label = 'Copy', className = '' }) => (
    <button
        ...
        <div className="space-y-6">
          ...
          <div className={`${c.card} rounded-xl shadow-lg p-6`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                <FileText className={`w-8 h-8 mx-auto mb-3 ${inputMethod === 'text' ? 'text-sky-500' : c.textMuted}`} />
              ...
              >
                <Upload className={`w-8 h-8 mx-auto mb-3 ${inputMethod === 'file' ? 'text-sky-500' : c.textMuted}`} />
          ...
          >
            {loading ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> {loadingMessages[textType] || loadingMessages.auto}</>
            ) : (
              <><Search className="w-5 h-5" /> Analyze This Text</>
            )}
          </button>
            ...
            <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button onClick={reset} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium ${isDark ? 'bg-sky-900/40 text-sky-300 hover:bg-sky-800/50' : 'bg-sky-100 text-sky-700 hover:bg-sky-200'}`}>
                  <RefreshCw className="w-3.5 h-3.5" /> Analyze Another Text
                </button>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className={`${c.card} rounded-xl shadow-lg p-6`}>
                <h3 className={`font-bold ${c.text} mb-3 flex items-center gap-2`}>
                  <BookOpen className={`w-5 h-5 ${c.accent}`} /> Summary
                </h3>
              ...
              {results.what_matters_most && results.what_matters_most.length > 0 && (
                <div className={`${c.card} rounded-xl shadow-lg p-6`}>
                  <h3 className={`font-bold ${c.text} mb-4 flex items-center gap-2`}>
                    <Zap className={`w-5 h-5 ${isDark ? 'text-amber-400' : 'text-amber-500'}`} /> What Matters Most
                  </h3>
              ...
              {results.obligations_and_commitments?.applicable && (
                <div className={`${c.card} rounded-xl shadow-lg p-6`}>
                  <h3 className={`font-bold ${c.text} mb-4 flex items-center gap-2`}>
                    <Scale className={`w-5 h-5 ${c.accent}`} /> Obligations & Commitments
                  </h3>
              ...
              {results.narrative_analysis?.applicable && (
                <div className={`${c.card} rounded-xl shadow-lg p-6`}>
                  <h3 className={`font-bold ${c.text} mb-4 flex items-center gap-2`}>
                    <Sparkles className={`w-5 h-5 ${isDark ? 'text-purple-400' : 'text-purple-600'}`} /> Narrative Analysis
                  </h3>
              ...
              {results.argument_analysis?.applicable && (
                <div className={`${c.card} rounded-xl shadow-lg p-6`}>
                  <h3 className={`font-bold ${c.text} mb-4 flex items-center gap-2`}>
                    <MessageSquare className={`w-5 h-5 ${isDark ? 'text-teal-400' : 'text-teal-600'}`} /> Argument Analysis
                  </h3>
              ...
              {results.internal_contradictions?.length > 0 && (
                <div className={`${c.card} rounded-xl shadow-lg p-6 border-l-4 border-red-500`}>
                  <h3 className={`font-bold ${isDark ? 'text-red-400' : 'text-red-600'} mb-4 flex items-center gap-2`}>
                    <AlertTriangle className="w-5 h-5" /> Internal Contradictions ({results.internal_contradictions.length})
                  </h3>
                ...
                <div className={`${c.card} rounded-xl shadow-lg p-6`}>
                  <button onClick={() => toggleSection('glossary')} className={`w-full flex items-center justify-between ${c.text}`}>
                    <h3 className="font-bold flex items-center gap-2">
                      <List className={`w-5 h-5 ${c.accent}`} /> Glossary ({results.glossary.length} terms)
                    </h3>
          ...
          {activeTab === 'translation' && (
            <div className={`${c.card} rounded-xl shadow-lg p-6`}>
              <div className="flex items-center justify-between mb-4">
                ...
                <CopyBtn content={results.translation} field="translation" label="Copy Translation" />
              </div>
          ...
          {activeTab === 'xray' && results.structural_xray && (
            <div className="space-y-4">
              ...
              {results.structural_xray.architecture_summary && (
                <div className={`${c.card} rounded-xl shadow-lg p-6`}>
                  <h3 className={`font-bold ${c.text} mb-3 flex items-center gap-2`}>
                    <Layers className={`w-5 h-5 ${c.accent}`} /> How This Text Is Built
                  </h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className={`${c.card} rounded-xl shadow-lg p-5`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <CopyBtn content={results.translation} field="translation-side" label="Copy" />
                </div>
          ...
          {results.tool_suggestion?.suggested && results.tool_suggestion?.tool_name && (
            <div className={`rounded-xl p-5 border-2 border-dashed ${isDark ? 'border-sky-700 bg-sky-900/10' : 'border-sky-300 bg-sky-50/50'}`}>
              <div className="flex items-start gap-3">
                <Lightbulb className={`w-6 h-6 flex-shrink-0 mt-0.5 ${isDark ? 'text-sky-400' : 'text-sky-500'}`} />
                <div className="flex-1">
                  ...
                  <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold cursor-pointer transition-colors ${isDark ? 'bg-sky-800/50 text-sky-300 hover:bg-sky-700/50' : 'bg-sky-100 text-sky-700 hover:bg-sky-200'}`}>
                    Try {results.tool_suggestion.tool_name} <ArrowRight className="w-4 h-4" />
                  </span>
                </div>
              </div>
            </div>
