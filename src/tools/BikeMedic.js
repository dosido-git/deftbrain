...
const Toast = ({ message, onClose }) => {
  ...
  return (
    <div className="fixed top-4 end-4 z-50 bg-green-600 text-white px-4 py-2.5 rounded-xl shadow-lg text-sm font-semibold flex items-center gap-2 animate-fade-in">
      <span>✅</span> {message}
    </div>
  );
};
...
const BikeMedic = ({ tool }) => {
  ...
  const loadExample = () => {
    const ex = pickExample('BikeMedic', EXAMPLES);
    setAiDiagnosis(null);
  ...
  // Scroll to (and focus) each of the three result cards when it arrives —
  // none of them had this before, so focus stayed on whichever button
  // `loading` had just disabled (the browser blurs a disabled element to
  // <body>, with nothing to bring it back). Three separate refs because
  // the three results render in three different sections of the tool, not
  // one shared container.
  const aiDiagnosisRef = useRef(null);
  const seasonalRef = useRef(null);
  const customCheckRef = useRef(null);
  useScrollToSection(aiDiagnosisRef, aiDiagnosis);
  useScrollToSection(seasonalRef, seasonalResult);
  useScrollToSection(customCheckRef, customCheckResult);
    ...
    return (
      <div className={`flex items-center gap-2 mt-2 mb-1 px-3 py-1.5 rounded-lg text-xs ${c.textSecondary} border-2 ${c.activeBikeBorder}`}>
        ...
        <span className="font-semibold flex-shrink-0">
          {multi && <span className={`${c.amberText} me-1`}>{t('bmd_assessing')}</span>}
        ...
        <select value={activeBikeId || ''} onChange={e => setActiveBikeId(e.target.value)}
          ...
          className={`ms-auto px-2 py-0.5 rounded-md text-xs font-semibold ${c.input} border`}>
  ...
  const renderAIDiagnosis = (data) => (
    <div ref={aiDiagnosisRef} className="scroll-mt-24 space-y-4 mt-6">
          ...
          {ALL_TOOLS.map(itm => {
            ...
            return (
              <button key={itm.value} onClick={() => setMyTools(prev => owned ? prev.filter(v => v !== itm.value) : [...prev, itm.value])}
                className={`w-full flex items-center gap-3 p-3 rounded-xl border-2 text-start text-sm transition-all ${owned ? c.greenBanner : `${c.border} ${c.card}`}`}>
  ...
  const renderSeasonalSection = () => {
    ...
    <div className={`${c.card} rounded-xl border-2 ${c.border} p-6 mb-6`}>
        ...
        {seasonalResult && !loading && (
          <div ref={seasonalRef} className={`scroll-mt-24 ${c.card} rounded-xl border-2 ${c.border} p-6`}>
      ...
      <div className={c.text}>
              ...
              {favorites.map(fId => {
                ...
                return (
                  <div key={fId} className={`flex items-center gap-3 p-3 rounded-xl border-2 ${c.border} ${c.card}`}>
                    <button onClick={() => startFixDirect(fId)} className={`flex-1 text-start text-sm font-semibold ${c.text} hover:underline`}>{t(f.titleKey)}</button>
        ...
        <div>
          <h4 className={`text-sm font-bold ${c.textSecondary} uppercase tracking-wide mb-3`}>
            {t('bmd_hub_repair_history')}{activeBikeId && bikeProfile && <span className={`${c.textMuteded} font-normal normal-case ms-1`}>· {bikeProfile.name || bikeProfile.bikeType}</span>}
          </h4>
          {bikeRepairHistory.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {PROBLEMS.map(p => (
                <button key={p.id} onClick={() => startProblem(p.id)}
                  ...
                  <div className="absolute bottom-0 start-1/2 -translate-x-1/2 w-0 group-hover:w-3/4 h-[3px] rounded-full transition-all duration-200" style={{ backgroundColor: p.color }} />
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-6">
              <div className="md:col-span-2 relative">
                ...
                <button onClick={() => setAnimPaused(!animPaused)}
                  className="absolute top-2 end-2 p-1.5 rounded-lg bg-black/40 text-white/80 hover:text-white transition-colors text-sm">
                  {animPaused ? '▶️' : '⏸️'}
                </button>
              </div>
              <div className="md:col-span-3 flex flex-col justify-center">
                ...
                <div className="flex flex-wrap gap-1.5">
                  <span className={`text-xs font-bold ${c.textMuteded} uppercase self-center me-1`}>{t('bmd_fix_tools_label')}</span>
          ...
          <div className="space-y-3">
            {currentNode.options.map((opt, idx) => (
              <button key={idx} onClick={() => selectOption(opt)}
                className={`w-full flex items-center gap-4 p-4 rounded-xl border-2 ${c.border} ${c.borderHover} ${c.card} ${c.cardAltHover} transition-all text-start group`}>
