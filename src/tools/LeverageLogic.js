        ...
        <div className="space-y-5">
          <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5 space-y-5`}>
            ...
            <p className={`text-xs ${c.textMuted} text-center`}>
              Not sure which path to take? <a href="/DecisionCoach" className={linkStyle}>🧭 Decision Coach</a> can help you think it through first.
            </p>
          </div>
        )}
          ...
          <div className="flex gap-2 flex-wrap">
                ...
                <div className="space-y-1.5 mb-3">
                  {timeline.map((t, i) => (
                    <div key={t.id} className={`flex items-start gap-2 p-2 rounded-lg ${t.who === 'you' ? (isDark ? 'bg-emerald-900/15' : 'bg-emerald-50') : (isDark ? 'bg-red-900/15' : 'bg-red-50')}`}>
                      ...
                      <button onClick={() => removeTimelineEntry(t.id)} className={`text-[9px] ${c.deleteHover} shrink-0`}>✕</button>
                    </div>
                  ))}
                </div>
              )}
            ...
            {results.situation_read?.power_balance === 'they_have_leverage' && (
              <p className={`text-xs ${c.textMuted} text-center pt-2`}>
                Feeling outmatched? <a href="/PreMortem" className={linkStyle}>💀 Pre-Mortem</a> can help you stress-test your position before you go in.
              </p>
            )}
            <p className={`text-xs ${c.textMuted} text-center pt-1`}>
              Weighing multiple options? <a href="/ContrastReport" className={linkStyle}>📊 Contrast Report</a> lays them out side by side.
            </p>
          </div>
        )}
