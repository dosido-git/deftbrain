          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-base ${c.textSecondary}`}>
                <span className="me-2 text-xl">{tool?.icon ?? '🧯'}</span>{tool?.tagline ?? t('cc_tagline')}
              </p>
        ...
        {/* Mandatory delay — below header so tool name always renders first */}
          ...
          <div className="flex gap-3">
            <button title={t('cmd_enter')} onClick={handleAnalyze} disabled={loading}
              ...
              {loading
                ? <><span className="animate-spin inline-block text-xl">{tool?.icon ?? '🧯'}</span> {t('cc_analyzing')}</>
                : <><span className="text-xl">{tool?.icon ?? '🧯'}</span> {t('cc_analyze')}</>}
            {!loading && (
                        ...
                        {selectedStrategyIdx === idx && (
                          <div className={`mt-2 p-3 rounded ${c.toneSlider}`}>
                            ...
                            <button onClick={handleAdjustTone} disabled={toneLoading}
                              ...
                              {toneLoading ? <span className="animate-spin inline-block text-xl">{tool?.icon ?? '🧯'}</span> : t('cc_regenerate', { level: toneLevel })}
                            </button>
                            {adjustedResponse && (
                              <div className="mt-2">
          ...
          <div className={`${c.card} border ${c.border} rounded-xl p-5`}>
            ...
            <div className="scroll-mt-24 flex gap-2" ref={followupRef}>
              ...
              <button onClick={handleFollowup} disabled={followupLoading || !followupQuestion.trim()}
                ...
                {followupLoading ? <span className="animate-spin inline-block text-xl">{tool?.icon ?? '🧯'}</span> : '➤'}
              </button>
            </div>
          </div>
