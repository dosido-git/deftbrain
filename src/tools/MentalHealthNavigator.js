...
const EXAMPLES = [
  ...
  {
  situationAreas: ['grief', 'mood', 'relationship'],
  freeform: "My mother died last year and I moved countries three months later for my partner's job. I do not have a doctor here yet and my Dutch is not good enough for a conversation like this. Most days are fine. Then something small happens and I cannot get out of bed, and I do not know whether that is grief doing what grief does or something I should be getting help for.",
  triedBefore: ['nothing'],
...
function MentalHealthNavigator({ tool }) {
  ...
  const loadExample = useCallback(() => {
    const ex = pickExample('MentalHealthNavigator', EXAMPLES);
    setSituationAreas(ex.situationAreas);
  ...
  const renderInput = () => (
    <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5 space-y-5`}>
      ...
      {!results && (
        <p className={`text-sm ${c.textSecondary} px-1`}>{t('mhn_front_door')}</p>
      )}
      ...
      {/* Revealed once there is something to work with. Someone arriving
          overwhelmed meets one question, not twenty-five choices. */}
      {canSubmit && (
      <>
      {/* What you've tried */}
      <div>
      ...
      <button title={t('cmd_enter')} onClick={handleFind} disabled={!canSubmit || loading}
        ...
        {loading
          ? <><span className="inline-block animate-spin text-xl">{tool?.icon ?? '🧭'}</span> {t('mhn_finding')}</>
          : <><span className="me-1 text-xl">{tool?.icon ?? '🧭'}</span> {t('mhn_find')}</>}
      {!loading && (
  ...
  const renderResults = () => {
    ...
    return (
      <div className="scroll-mt-24 space-y-4" ref={resultsRef}>
        ...
        {/* Which DeftBrain tool fits what they described. Ids are resolved
            against the real catalog before rendering — the model can name a
            tool that does not exist, and links that 404 have shipped before. */}
        {Array.isArray(results?.deftbrain_next) && (() => {
          const found = results.deftbrain_next
            .map(n => ({ ...n, tool: getToolById(String(n.tool || '').trim().split(/[\s(|,]/)[0]) }))
              ...
              <div className="space-y-2">
                {found.map((n, i) => (
                  <p key={i} className="text-sm">
                    <a href={`/${n.tool.id}`} className={linkStyle}>{n.tool.icon} {n.tool.title}</a>
                    <span className={`${c.textSecondary}`}> — {n.why}</span>
                  </p>
                ))}
              </div>
            </div>
  ...
  const hasInput = !!(results || barriers.length || country.trim() || freeform.trim() || situationAreas.length || triedBefore.length);
