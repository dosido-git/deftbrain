  ...
  return (
    <div className={`space-y-4 ${c.text}`}>
      ...
      <p className={`text-xs text-center ${c.textMuteded}`}>
        Thoughts spiraling before you dive in?{' '}
        <a href="/SpiralStopper" className={linkStyle}>🌀 Spiral Stopper</a>{' '}
        can help you reset first.
      </p>
          ...
          <div className="space-y-4">
            <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5`}>
              <div className="flex items-center justify-between mb-3">
                <span className={`text-sm font-semibold ${c.textSecondary}`}>Latest snapshot</span>
            ...
            {logs.length >= 2 && <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-4`}><p className={`text-xs font-bold ${c.text} mb-2`}>7-Day Trend</p><TrendChart compact /></div>}

            {analysis?.preventive_interventions?.length > 0 && (
              <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-4`}><p className={`text-xs font-bold ${c.text} mb-2`}>🛡️ Top Priority</p>
        ...
        {mode === 'analysis' && results && (
          <div ref={analysisRef} className="space-y-4">
            ...
            {analysis.menstrual_cycle_correlation&&analysis.menstrual_cycle_correlation.current_phase!=='not tracked'&&<Section id="menstrual" title="Cycle Correlation" icon="🌙" expanded={false} onToggle={toggleSection} c={c}>
              <div className="space-y-2">
                {analysis.menstrual_cycle_correlation.current_phase&&<div className={`${c.cardAlt} border ${c.border} rounded-lg p-3`}><p className="text-xs font-bold mb-1">Current phase</p><p className={`text-sm ${c.textSecondary} capitalize`}>{analysis.menstrual_cycle_correlation.current_phase}</p></div>}
                {analysis.menstrual_cycle_correlation.energy_pattern&&<div className={`${c.cardAlt} border ${c.border} rounded-lg p-3`}><p className="text-xs font-bold mb-1">Energy pattern</p><p className={`text-sm ${c.textSecondary}`}>{analysis.menstrual_cycle_correlation.energy_pattern}</p></div>}
                {analysis.menstrual_cycle_correlation.crash_risk_adjustment&&<div className={`${c.high} border rounded-lg p-3`}><p className="text-xs font-bold mb-1">Crash risk adjustment</p><p className={`text-sm ${c.textSecondary}`}>{analysis.menstrual_cycle_correlation.crash_risk_adjustment}</p></div>}
                {analysis.menstrual_cycle_correlation.cycle_optimized_interventions&&<div className={`${c.low} border rounded-lg p-3`}><p className="text-xs font-bold mb-1">💡 Phase-specific advice</p><p className={`text-sm ${c.textSecondary}`}>{analysis.menstrual_cycle_correlation.cycle_optimized_interventions}</p></div>}
              </div>
            </Section>}

            {analysis.weather_sensitivity_analysis&&<Section id="weather" title="Weather Sensitivity" icon="🌦️" expanded={false} onToggle={toggleSection} c={c}>
              <div className="space-y-2">
                {analysis.weather_sensitivity_analysis.seasonal_pattern&&<div className={`${c.cardAlt} border ${c.border} rounded-lg p-3`}><p className="text-xs font-bold mb-1">Seasonal pattern</p><p className={`text-sm ${c.textSecondary}`}>{analysis.weather_sensitivity_analysis.seasonal_pattern}</p></div>}
                {analysis.weather_sensitivity_analysis.barometric_pressure_correlation&&<div className={`${c.cardAlt} border ${c.border} rounded-lg p-3`}><p className="text-xs font-bold mb-1">Pressure correlation</p><p className={`text-sm ${c.textSecondary}`}>{analysis.weather_sensitivity_analysis.barometric_pressure_correlation}</p></div>}
                {analysis.weather_sensitivity_analysis.weather_triggered_crashes&&<div className={`${c.high} border rounded-lg p-3`}><p className="text-xs font-bold mb-1">Weather-triggered crashes</p><p className={`text-sm ${c.textSecondary}`}>{analysis.weather_sensitivity_analysis.weather_triggered_crashes}</p></div>}
                {analysis.weather_sensitivity_analysis.recommendation&&<div className={`${c.low} border rounded-lg p-3`}><p className="text-xs font-bold mb-1">💡 Advice</p><p className={`text-sm ${c.textSecondary}`}>{analysis.weather_sensitivity_analysis.recommendation}</p></div>}
              </div>
            </Section>}
            ...
            {analysis.capacity_reality_check&&<Section id="capacity" title="Capacity" icon="🔋" expanded={expandedSections.capacity} onToggle={toggleSection} c={c}>
              <div className="space-y-2">
                ...
                {analysis.capacity_reality_check.rest_day_scheduling&&<div className={`${c.cardAlt} border ${c.border} rounded-lg p-3`}><p className="text-xs font-bold mb-1">📅 Rest scheduling</p><p className={`text-sm ${c.textSecondary}`}>{analysis.capacity_reality_check.rest_day_scheduling}</p></div>}
                {analysis.capacity_reality_check.future_crash_prevention&&<div className={`${c.low} border rounded-lg p-3`}><p className="text-xs font-bold mb-1">🛡️ Prevention</p><p className={`text-sm ${c.textSecondary}`}>{analysis.capacity_reality_check.future_crash_prevention}</p></div>}
              </div>
            </Section>}
            ...
            {analysis.if_youre_already_crashed&&<Section id="crashed" title="If Crashed" icon="💥" expanded={expandedSections.crashed} onToggle={toggleSection} c={c}>
              <div className="space-y-3">
                ...
                {(analysis.if_youre_already_crashed.crash_severity||analysis.if_youre_already_crashed.likelihood)&&<div className="flex gap-2">{analysis.if_youre_already_crashed.crash_severity&&<span className={`text-xs px-2 py-0.5 rounded font-bold ${c.urgent} border`}>Severity: {analysis.if_youre_already_crashed.crash_severity}</span>}{analysis.if_youre_already_crashed.likelihood&&<span className={`text-xs px-2 py-0.5 rounded font-bold ${c.high} border`}>Likelihood: {analysis.if_youre_already_crashed.likelihood}</span>}</div>}
                {analysis.if_youre_already_crashed.immediate_actions?.length>0&&<div className={`${c.urgent} border rounded-lg p-3`}><p className="text-xs font-bold mb-1">⚡ Immediate actions</p>{analysis.if_youre_already_crashed.immediate_actions.map((a,i)=><p key={i} className="text-sm mb-1">• {a}</p>)}</div>}
                ...
                {analysis.if_youre_already_crashed.recovery_timeline&&<div className={`${c.cardAlt} border ${c.border} rounded-lg p-3`}><p className="text-xs font-bold mb-1">🗓️ Recovery timeline</p><p className={`text-sm ${c.textSecondary}`}>{analysis.if_youre_already_crashed.recovery_timeline}</p></div>}
            ...
            {analysis.recovery_protocol&&<Section id="recovery" title="Recovery Protocol" icon="🛟" expanded={expandedSections.recovery} onToggle={toggleSection} c={c}>
              <div className="space-y-3">
                ...
                {analysis.recovery_protocol.relapse_prevention&&<div className={`${c.high} border rounded-lg p-3`}><h4 className="font-semibold text-sm mb-2">🔄 Relapse:</h4>{analysis.recovery_protocol.relapse_prevention.warning_signs_of_relapse&&<p className="text-xs mb-1"><strong>Watch:</strong> {analysis.recovery_protocol.relapse_prevention.warning_signs_of_relapse}</p>}{analysis.recovery_protocol.relapse_prevention.how_to_catch_early&&<p className="text-xs mb-1"><strong>Catch early:</strong> {analysis.recovery_protocol.relapse_prevention.how_to_catch_early}</p>}{analysis.recovery_protocol.relapse_prevention.if_relapse_starting&&<p className="text-xs font-bold"><strong>Act:</strong> {analysis.recovery_protocol.relapse_prevention.if_relapse_starting}</p>}</div>}
              </div>
            </Section>}
        ...
        {mode === 'history' && (
          <div className="space-y-4">
            <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5`}><h3 className={`text-sm font-bold ${c.text} mb-3`}>📈 Trends</h3><TrendChart/></div>

            {analysisHistory.length>0&&<div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5`}><h3 className={`text-sm font-bold ${c.text} mb-3`}>📊 Risk History</h3><AnalysisHistoryChart/><div className="flex gap-3 justify-center mt-1 text-xs"><span><span style={{color:'#22c55e'}}>●</span> Low</span><span><span style={{color:'#f59e0b'}}>●</span> Moderate</span><span><span style={{color:'rgb(239,68,68)'}}>●</span> High</span></div></div>}
            ...
            <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5`}>
              ...
              {patterns?(
                <div className="space-y-3">
                  ...
                  {patterns.crash_sequences?.length>0&&<div><p className={`text-xs font-bold ${c.text} mt-2 mb-2`}>⚡ Crash Sequences</p>{patterns.crash_sequences.map((seq,i)=><div key={i} className={`${c.cardAlt} border ${c.border} rounded-lg p-3 mb-2`}><p className="text-sm font-bold">{seq.trigger}</p>{seq.frequency&&<p className={`text-xs ${c.textMuteded}`}>Occurs: {seq.frequency}</p>}{seq.sequence?.map((s,j)=><p key={j} className={`text-xs ${c.textSecondary} ml-2`}>→ {s}</p>)}<p className="text-xs font-semibold mt-1">🎯 Intervene: {seq.early_warning_day}</p></div>)}</div>}
                  {patterns.what_helps?.length>0&&<div><p className={`text-xs font-bold ${c.text} mt-2 mb-2`}>✅ What helps you</p>{patterns.what_helps.map((w,i)=><div key={i} className={`${c.low} border rounded-lg p-3 mb-2`}><p className="text-sm font-bold">{w.intervention}</p>{w.effect&&<p className={`text-xs ${c.textSecondary}`}>Effect: {w.effect}</p>}{w.evidence&&<p className={`text-xs ${c.textMuteded}`}>{w.evidence}</p>}</div>)}</div>}
                  {patterns.biggest_risks?.length>0&&<div><p className={`text-xs font-bold ${c.text} mt-2 mb-2`}>🚨 Biggest risks</p>{patterns.biggest_risks.map((r,i)=><p key={i} className={`text-xs ${c.textSecondary} mb-1`}>• {r}</p>)}</div>}
                </div>
            ...
            <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5`}>
              ...
              <div className="flex gap-2 mb-3"><button onClick={()=>setShowPartnerPreview(!showPartnerPreview)} className={`${c.btnSecondary} px-3 py-1.5 rounded-lg text-sm`}>{showPartnerPreview?'Hide':'👁️ Preview'}</button></div>
            ...
            {/* LOG ENTRIES */}
            <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5`}>
              <div className="flex items-center justify-between mb-3"><h3 className={`text-sm font-bold ${c.text}`}>📋 Entries ({logs.length})</h3>{logs.length>0&&<button onClick={()=>{if(window.confirm('Clear all?'))setLogs([])}} className={`text-xs ${c.deleteTxt}`}>Clear</button>}</div>
