              ...
              <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5`}>
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex gap-3">
                    ...
                    {logs.length>=3&&<button onClick={handleAnalyze} disabled={loading} className={`${c.btnSecondary} py-3 px-4 rounded-lg`}>{loading?<span className="animate-spin inline-block">{tool?.icon ?? '⚠️'}</span>:<span>{tool?.icon ?? '⚠️'}</span>}</button>}
                  </div>
