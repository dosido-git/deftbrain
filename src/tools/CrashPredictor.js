...
const Slider = ({ label, emoji, value, onChange, min = 1, max = 10, lowLabel, highLabel, c }) => (
  <div>
    <label className={`block text-sm font-medium ${c.textSecondaryondary} mb-2 flex items-center justify-between`}>
              ...
              <div className={`${c.card} border ${c.border} rounded-xl shadow-lg p-5`}>
                ...
                {[{ key:'energy',label:'Energy',faces:['🪫','😓','😐','😊','🔋'] },
                  ...
                  <div key={key} className="mb-5">
                    <p className={`text-sm font-medium ${c.textSecondaryondary} mb-2 text-center`}>{label}</p>
            ...
            {!quickMode && (
              <div className={`${c.card} border ${c.border} rounded-xl shadow-lg p-5`}>
                ...
                <div className="space-y-5">
                  ...
                  <div><label className={`block text-sm font-medium ${c.textSecondaryondary} mb-2`}>💼 Activities</label>
                  ...
                  <div><label className={`block text-sm font-medium ${c.textSecondaryondary} mb-2`}>🤕 Physical symptoms</label>
                  ...
                  <div><label className={`block text-sm font-medium ${c.textSecondaryondary} mb-2`}>⚠️ Warning signs</label>
                  ...
                  {customSymptoms.length>0&&<div><label className={`block text-sm font-medium ${c.textSecondaryondary} mb-2`}>🏷️ Your signals</label>
                  ...
                  <div className={`${c.cardAlt} border ${c.border} rounded-lg p-4`}>
                    <label className={`block text-sm font-medium ${c.textSecondaryondary} mb-3`}>☕ Substances & Medications</label>
                  ...
                  <div><label className={`block text-sm font-medium ${c.textSecondaryondary} mb-2`}>🩸 Cycle Phase</label>
                  ...
                  <button onClick={()=>setShowBiometrics(!showBiometrics)} className={`text-sm font-medium ${c.textSecondaryondary} flex items-center gap-1`}><span>📟 Biometrics</span><span className={c.textMuteded}>(opt.)</span><span className="ml-1">{showBiometrics?'▲':'▼'}</span></button>
                  ...
                  <button onClick={()=>setShowWeather(!showWeather)} className={`text-sm font-medium ${c.textSecondaryondary} flex items-center gap-1`}><span>🌤️ Weather</span><span className={c.textMuteded}>(opt.)</span><span className="ml-1">{showWeather?'▲':'▼'}</span></button>
                  ...
                  <button onClick={()=>setShowCalendar(!showCalendar)} className={`text-sm font-medium ${c.textSecondaryondary} flex items-center gap-1`}><span>📅 Commitments</span><span className={c.textMuteded}>(opt.)</span><span className="ml-1">{showCalendar?'▲':'▼'}</span></button>
                  ...
                  <div><label className={`block text-sm font-medium ${c.textSecondaryondary} mb-2`}>📝 Notes</label><textarea value={currentEntry.notes} onChange={e=>updateEntry('notes',e.target.value)} placeholder="Big deadline... fight with partner..." rows={2} className={`${c.input} border rounded-lg px-3 py-2 text-sm w-full`}/></div>
          ...
          <div ref={analysisRef} className="space-y-4">
            ...
            {analysis.intervention_escalation && (
              <div className={`${c.card} border ${c.border} rounded-xl shadow-lg p-5`}>
                ...
                {analysis.intervention_escalation.why_this_level&&<div className={`${c.cardAlt} border ${c.border} rounded-lg p-3 mb-2`}><p className={`text-sm ${c.textSecondaryondary}`}><strong>Why:</strong> {analysis.intervention_escalation.why_this_level}</p></div>}
            ...
            {analysis.your_crash_pattern&&<Section id="pattern" title="Your Crash Pattern" icon="📉" expanded={expandedSections.pattern} onToggle={toggleSection} c={c}>
              <div className="space-y-3">
                ...
                {analysis.your_crash_pattern.identified_indicators?.length>0&&<div className={`${c.cardAlt} border ${c.border} rounded-lg p-3`}>{analysis.your_crash_pattern.identified_indicators.map((ind,i)=><p key={i} className={`text-sm ${c.textSecondaryondary} mb-1`}>⚠️ {ind}</p>)}</div>}
                {analysis.your_crash_pattern.current_status&&<div className={`${c.cardAlt} border ${c.border} rounded-lg p-3`}>{Object.entries(analysis.your_crash_pattern.current_status).map(([k,v])=><div key={k} className="flex justify-between text-sm mb-1"><span className={`${c.textSecondaryondary} capitalize`}>{k.replace(/_/g,' ')}</span><span className={`font-semibold ${String(v).match(/critical|below|deficit|depleted/)?'text-red-500':''}`}>{v}</span></div>)}</div>}
              </div>
            </Section>}
            ...
            {analysis.medication_correlation&&<Section id="medCorrelation" title="Substance Correlation" icon="💊" expanded={expandedSections.medCorrelation} onToggle={toggleSection} c={c}>
              <div className="space-y-2">{['caffeine_impact','alcohol_impact','medication_notes','substance_recommendation'].map(k=>{const v=analysis.medication_correlation[k];if(!v)return null;const labels={caffeine_impact:'☕ Caffeine',alcohol_impact:'🍷 Alcohol',medication_notes:'💊 Meds',substance_recommendation:'💡 Advice'};return<div key={k} className={`${k==='substance_recommendation'?c.low:c.cardAlt} border rounded-lg p-3`}><p className="text-xs font-bold mb-1">{labels[k]}</p><p className={`text-sm ${c.textSecondaryondary}`}>{v}</p></div>})}</div>
            </Section>}
            ...
            {analysis.biometric_analysis&&<Section id="biometricAnalysis" title="Biometrics" icon="📟" expanded={expandedSections.biometricAnalysis} onToggle={toggleSection} c={c}>
              <div className="space-y-2">{Object.entries(analysis.biometric_analysis).map(([k,v])=>v?<div key={k} className={`${c.cardAlt} border ${c.border} rounded-lg p-3`}><p className="text-xs font-bold mb-1 capitalize">{k.replace(/_/g,' ')}</p><p className={`text-sm ${c.textSecondaryondary}`}>{v}</p></div>:null)}</div>
            </Section>}
            ...
            {analysis.capacity_reality_check&&<Section id="capacity" title="Capacity" icon="🔋" expanded={expandedSections.capacity} onToggle={toggleSection} c={c}>
              <div className="space-y-2">
                ...
                {analysis.capacity_reality_check.what_this_means&&<div className={`${c.cardAlt} border ${c.border} rounded-lg p-3`}><p className={`text-sm ${c.textSecondaryondary}`}>{analysis.capacity_reality_check.what_this_means}</p></div>}
            ...
            {analysis.if_youre_already_crashed&&<Section id="crashed" title="If Crashed" icon="💥" expanded={expandedSections.crashed} onToggle={toggleSection} c={c}>
              <div className="space-y-3">
                ...
                {analysis.if_youre_already_crashed.recovery_stages&&<div className={`${c.cardAlt} border ${c.border} rounded-lg p-3`}>{Object.entries(analysis.if_youre_already_crashed.recovery_stages).map(([k,v])=><div key={k} className="mb-2"><p className="text-xs font-bold capitalize">{k.replace(/_/g,' ')}</p><p className={`text-sm ${c.textSecondaryondary}`}>{v}</p></div>)}</div>}
            ...
            <div className={`${c.card} border-2 ${c.border} border-dashed rounded-xl p-4 text-center`}>
              <p className={`text-sm ${c.textSecondaryondary} mb-2`}>Took action? Track recovery.</p>
        ...
        {mode === 'history' && (
          <div className="space-y-4">
            ...
            <div className={`${c.card} border ${c.border} rounded-xl shadow-lg p-5`}>
              ...
              {patterns?(
                <div className="space-y-3">
                  {patterns.summary&&<p className={`text-sm ${c.textSecondaryondary}`}>{patterns.summary}</p>}
