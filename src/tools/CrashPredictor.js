...
const EXAMPLES = [
  ...
  // The crash that follows a run of GOOD days — the pattern people cannot
  // see, because nothing in it looks like a warning at the time.
  {
  mode: 'dashboard',
  logs: [
    { date: new Date(Date.now() - 6*86400000).toISOString().split('T')[0],
      energy: 8, sleep: 8, stress: 2, mood: 8,
      activities: { work: false, social: true, exercise: false, rest: true, obligations: false },
      physicalSymptoms: { headache: false, fatigue: false, tension: false, appetiteChanges: false, sleepIssues: false },
      warningSigns: { irritability: false, brainFog: false, difficultyDeciding: false, cryingEasily: false, withdrawing: false },
      customSymptoms: [], caffeine: 1, alcohol: 1, medications: '', menstrualPhase: 'na',
      notes: 'Quiet weekend. Read most of Saturday. Felt like myself.',
      biometrics: {}, weather: {}, calendarContext: '' },
    { date: new Date(Date.now() - 5*86400000).toISOString().split('T')[0],
      energy: 7, sleep: 7, stress: 3, mood: 7,
      activities: { work: true, social: false, exercise: false, rest: true, obligations: false },
      physicalSymptoms: { headache: false, fatigue: false, tension: false, appetiteChanges: false, sleepIssues: false },
      warningSigns: { irritability: false, brainFog: false, difficultyDeciding: false, cryingEasily: false, withdrawing: false },
      customSymptoms: [], caffeine: 2, alcohol: 0, medications: '', menstrualPhase: 'na',
      notes: 'Normal Monday. Nothing notable.',
      biometrics: {}, weather: {}, calendarContext: '' },
    { date: new Date(Date.now() - 4*86400000).toISOString().split('T')[0],
      energy: 7, sleep: 7, stress: 4, mood: 7,
      ...
      warningSigns: { irritability: false, brainFog: false, difficultyDeciding: false, cryingEasily: false, withdrawing: false },
      customSymptoms: [], caffeine: 2, alcohol: 0, medications: '', menstrualPhase: 'na',
      notes: 'Good day. Gym at lunch, drinks after work.',
      biometrics: {}, weather: {}, calendarContext: '' },
    { date: new Date(Date.now() - 3*86400000).toISOString().split('T')[0],
      energy: 6, sleep: 6, stress: 5, mood: 6,
      activities: { work: true, social: true, exercise: false, rest: false, obligations: true },
      physicalSymptoms: { headache: false, fatigue: false, tension: true, appetiteChanges: false, sleepIssues: false },
      warningSigns: { irritability: false, brainFog: false, difficultyDeciding: false, cryingEasily: false, withdrawing: false },
      customSymptoms: [], caffeine: 3, alcohol: 2, medications: '', menstrualPhase: 'na',
      notes: 'Another evening out. Fine at the time.',
      biometrics: {}, weather: {}, calendarContext: '' },
    { date: new Date(Date.now() - 2*86400000).toISOString().split('T')[0],
      energy: 5, sleep: 5, stress: 6, mood: 5,
      activities: { work: true, social: true, exercise: false, rest: false, obligations: true },
      physicalSymptoms: { headache: false, fatigue: true, tension: true, appetiteChanges: false, sleepIssues: true },
      warningSigns: { irritability: true, brainFog: false, difficultyDeciding: false, cryingEasily: false, withdrawing: false },
      customSymptoms: [], caffeine: 4, alcohol: 1, medications: '', menstrualPhase: 'na',
      notes: 'Third night in a row of plans. Snapped at someone on the way home.',
      biometrics: {}, weather: {}, calendarContext: '' },
    { date: new Date(Date.now() - 1*86400000).toISOString().split('T')[0],
      energy: 3, sleep: 5, stress: 7, mood: 4,
      activities: { work: true, social: false, exercise: false, rest: false, obligations: true },
      physicalSymptoms: { headache: true, fatigue: true, tension: true, appetiteChanges: false, sleepIssues: true },
      warningSigns: { irritability: true, brainFog: true, difficultyDeciding: false, cryingEasily: false, withdrawing: true },
      customSymptoms: [], caffeine: 4, alcohol: 0, medications: '', menstrualPhase: 'na',
      notes: 'Ran on empty. Said yes to two more things I do not want to do.',
      biometrics: {}, weather: {}, calendarContext: '' },
    { date: new Date().toISOString().split('T')[0],
      energy: 2, sleep: 5, stress: 7, mood: 3,
      activities: { work: false, social: false, exercise: false, rest: true, obligations: false },
      physicalSymptoms: { headache: true, fatigue: true, tension: true, appetiteChanges: true, sleepIssues: true },
      warningSigns: { irritability: true, brainFog: true, difficultyDeciding: true, cryingEasily: true, withdrawing: true },
      customSymptoms: [], caffeine: 1, alcohol: 0, medications: '', menstrualPhase: 'na',
      notes: 'Nothing left. Nothing bad happened — that is the confusing part. It was four good days in a row.',
      biometrics: {}, weather: {}, calendarContext: '' },
  ],
}
];;
...
const Section = ({ id, title, icon, expanded, onToggle, children, c, badge }) => (
  <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5`}>
    <button onClick={() => onToggle(id)} className={`flex items-center gap-2 w-full text-start ${expanded ? 'mb-4' : ''}`}>
...
const CrashPredictor = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    ...
    const ex = pickExample('CrashPredictor', EXAMPLES);
    setMode(ex.mode);
    // Example logs are oldest-first; the store is newest-first.
    setLogs([...ex.logs].reverse());
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className={`text-base ${c.textSecondary}`}>
                  <span className="me-2 text-lg">{tool?.icon ?? '⚠️'}</span>{tool?.tagline ?? t('cpr_tagline')}
                </p>
          ...
          <div ref={analysisRef} className="space-y-4">
            ...
            {analysis.preventive_interventions?.length>0&&<Section id="interventions" title={t('cpr_sec_do_now')} icon="🛡️" expanded={expandedSections.interventions} onToggle={toggleSection} c={c}>
              <div className="space-y-3">{analysis.preventive_interventions.map((int,i)=><div key={i} className={`${getPriorityColor(int.priority)} border-2 rounded-lg p-4`}>
                <div className="flex items-start justify-between mb-2"><h4 className="font-bold text-sm">{int.action}</h4><span className="text-xs px-2 py-0.5 rounded bg-black/10 uppercase font-semibold ms-2">{int.priority}</span></div>
            ...
            {analysis.recovery_protocol&&<Section id="recovery" title={t('cpr_sec_recovery_protocol')} icon="🛟" expanded={expandedSections.recovery} onToggle={toggleSection} c={c}>
              <div className="space-y-3">
                ...
                {analysis.recovery_protocol.self_compassion_scripts?.length>0&&<div className={`${c.low} border rounded-lg p-3`}><h4 className="font-semibold text-sm mb-2">💚 {t('cpr_self_compassion')}</h4>{analysis.recovery_protocol.self_compassion_scripts.map((s,i)=><p key={i} className="text-sm mb-2 ps-2 border-s-2 border-green-400">{s}</p>)}</div>}
