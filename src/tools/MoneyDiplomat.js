...
const EXAMPLE = {
  activeType: 'salary',
  situation: "Mid-level marketing manager at a 200-person SaaS company, 4 years in role. Just got recruited for a senior position at a 50-person startup with broader ownership but less brand polish. They asked for my expectations. Current base is $115K + 10% bonus + token equity. The new role is fully remote, the current is hybrid 3 days/week downtown. I want to grow into director-track, and I think I'm underpaid for my market.",
  salaryFields: {
    currentSalary: '115000',
    targetRole: 'Senior Marketing Manager',
    location: 'Austin, TX (target role is remote)',
    experience: '6 years in B2B SaaS marketing, 4 in current company. Led the rebrand and the demand-gen program that 3x\'d pipeline last year.',
  },
};
const SITUATIONS = [
...
const MoneyDiplomat = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    ...
    setSalaryFields(EXAMPLE.salaryFields);
    setResults(null);
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <p className={`text-xs ${c.textMuted} mb-3`}>
                Need to negotiate the bill itself? <a href="/BillRescue" className={linkStyle}>🧾 Bill Rescue</a> helps draft the conversation.
              </p>
      ...
      {showSimulator && (
        <div className={`${c.card} border ${c.border} rounded-xl shadow-sm overflow-hidden border-2 ${isDark ? 'border-red-600' : 'border-red-400'}`}>
          ...
          <div className="p-4 space-y-3">
            ...
            {simLoading && <p className={`text-xs ${c.textMuteded} flex items-center gap-2`}><span className="inline-block animate-spin">{tool?.icon ?? '💵'}</span> Thinking in character...</p>}
          </div>
        </div>
      )}
          ...
          {activeType === 'cultural' && results.culture_clash_risk && (
            <div className={`${c.card} border ${c.border} rounded-xl shadow-sm p-5 space-y-4`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {results.their_norms && (
                  <div className={`p-3 rounded-lg ${isDark ? 'bg-red-900/15' : 'bg-red-50'}`}>
                    <p className={`text-[10px] font-bold ${isDark ? 'text-red-300' : 'text-red-700'}`}>THEIR NORMS</p>
