...
//
// One example per situation (20 across 18 types — family and cultural each
// keep two) so the single "Try an example" button can actually surface all
// eighteen situations, not just the four it used to rotate through. Enum
// fields (country, serviceType, relationship, region, etc.) are pinned to
// the exact English option values; free text goes through a `*Key` and is
// resolved via t() in loadExample, same convention as salaryFields below.
const EXAMPLES = [
  {
    activeType: 'salary',
  ...
  },
  {
    ...
    fields: { amount: '500', relationship: 'Close friend of ten years', sessionHistory: 'Second time this year' },
  },
  {
  ...
  },
  {
    ...
    familyFields: { familyDynamicKey: 'md_example4_dynamic' },
  },
  {
  ...
  {
    activeType: 'tip',
    situationKey: 'md_ex_tip_situation',
    fields: { country: 'Japan', serviceType: 'restaurant', billAmount: '112', partySize: '2' },
  },
  {
    activeType: 'split',
    situationKey: 'md_ex_split_situation',
    fields: { people: '4', totalBill: '186' },
  },
  {
    activeType: 'venmo',
    situationKey: 'md_ex_venmo_situation',
    fields: { amount: '62', relationship: 'Friend', timePassed: '2 months' },
  },
  {
    activeType: 'gift',
    situationKey: 'md_ex_gift_situation',
    fields: { occasionKey: 'md_ex_gift_occasion', relationshipKey: 'md_ex_gift_relationship', theirSpend: '', yourBudget: '', region: 'USA' },
  },
  {
    activeType: 'roommate',
    situationKey: 'md_ex_roommate_situation',
    fields: { people: '3', totalCost: '2850' },
  },
  {
    activeType: 'dining',
    situationKey: 'md_ex_dining_situation',
    fields: { context: 'Business meal', yourBudget: '45' },
  },
  {
    activeType: 'group',
    situationKey: 'md_ex_group_situation',
    fields: { eventType: 'Group trip', people: '6', expensesKey: 'md_ex_group_expenses' },
  },
  {
    activeType: 'work',
    situationKey: 'md_ex_work_situation',
    fields: { roleKey: 'md_ex_work_role', companySize: 'Startup (<20)' },
  },
  {
    activeType: 'travel',
    situationKey: 'md_ex_travel_situation',
    fields: { destinationKey: 'md_ex_travel_destination' },
  },
  {
    activeType: 'date',
    situationKey: 'md_ex_date_situation',
    fields: { dateNumber: '3rd date', dynamicKey: 'md_ex_date_dynamic', culturalContextKey: 'md_ex_date_cultural' },
  },
  {
    activeType: 'subs',
    situationKey: 'md_ex_subs_situation',
    fields: { service: 'Netflix', people: '3', monthlyCost: '19.99' },
  },
  {
    activeType: 'afford',
    situationKey: 'md_ex_afford_situation',
    fields: { cost: '2400', income: '5200' },
  },
  {
    activeType: 'inheritance',
    situationKey: 'md_ex_inheritance_situation',
    fields: { familyDynamicKey: 'md_ex_inh_dynamic', culturalContextKey: 'md_ex_inh_cultural' },
  },
  {
    activeType: 'charity',
    situationKey: 'md_ex_charity_situation',
    fields: { askType: 'Office collection', relationship: 'Coworker', amount: '25' },
  },
];
...
const MoneyDiplomat = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    ...
    // The other fourteen situations (previously zero examples between them)
    // share one generic path: an enum/number passes through as-is, and any
    // field named `*Key` resolves through t() — same convention as
    // salaryFields/familyFields/culturalFields above, applied once instead
    // of once per situation.
    if (ex.fields) {
      const setters = {
        tip: setTipFields, split: setSplitFields, venmo: setVenmoFields, gift: setGiftFields,
        roommate: setRoommateFields, dining: setDiningFields, group: setGroupFields,
        lend: setLendFields, work: setWorkFields, travel: setTravelFields, date: setDateFields,
        subs: setSubsFields, afford: setAffordFields, inheritance: setInheritanceFields,
        charity: setCharityFields,
      };
      const setter = setters[ex.activeType];
      if (setter) {
        const resolved = {};
        Object.entries(ex.fields).forEach(([k, v]) => {
          if (k.endsWith('Key')) resolved[k.slice(0, -3)] = v ? t(v) : '';
          else resolved[k] = v;
        });
        setter(resolved);
      }
    ...
    setResults(null);
      ...
      setTipFields, setSplitFields, setVenmoFields, setGiftFields, setRoommateFields, setDiningFields,
      setGroupFields, setLendFields, setWorkFields, setTravelFields, setDateFields, setSubsFields,
      setAffordFields, setInheritanceFields, setCharityFields]);
