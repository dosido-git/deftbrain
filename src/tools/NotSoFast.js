      ...
      {!results && !loading && (
        <div className={`${c.card} border ${c.border} rounded-xl shadow-sm px-5 pt-2.5 pb-5 space-y-4`}>
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className={`text-base ${c.textSecondary}`}>
                  <span className="me-2 text-lg">{tool?.icon ?? '🚪'}</span>{t('rb_tagline')}
                </p>
          ...
          <button title={t('cmd_enter')} onClick={handleSubmit} disabled={loading || !system.trim() || !problem.trim()}
            className={`relative w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 min-h-[48px] ${(!system.trim() || !problem.trim()) ? c.btnIdle : c.btnPrimary}`}>
            {loading
