      ...
      {/* ── HEADER ── */} <div className={`${c.card} border ${c.border} rounded-xl p-6`}>
        ...
        {/* ── QUICK PICKS ── */} {!results && (<div className="mb-5">
          ...
          <button title={t('cmd_enter')}
            onClick={() => findTools()} disabled={loading || !problem.trim()} className={`relative flex-1 ${(!problem.trim()) ? c.btnIdle : c.btnPrimary} disabled:cursor-not-allowed font-bold py-3 px-6 rounded-xl flex items-center justify-center gap-2 min-h-[48px] shadow-lg transition-all`} >
            {loading
