...
const EXAMPLE = {
  plan: 'Launch a paid newsletter on systems thinking aimed at mid-career product managers, with a free tier and $12/mo paid tier targeting 1,000 paid subscribers in year one.',
  planType: 'side_project',
  stakes: "12 hours a week of evenings and weekends, ~$3K in tooling and design costs, and the opportunity cost of not finishing the book draft I've been working on.",
  assumptions: "PMs will pay for a niche newsletter. I can write consistently every week without burning out. Twitter/LinkedIn distribution will compound. Free-to-paid conversion will be ~3-5%.",
};
const PLAN_TYPES = [
...
const PreMortem = ({ tool }) => {
  ...
  useRegisterActions(buildFullText(), tool?.title || 'Pre-Mortem');
        ...
        <div className="px-5 pt-5">
          <div className="pb-3 border-b border-zinc-500">
            <h2 className={`text-xl font-bold ${c.text}`}>
              <span className="mr-2">{tool?.icon ?? '💀'}</span>{tool?.title ?? 'Pre-Mortem'}
            </h2>
            <p className={`text-sm ${c.textSecondary}`}>{tool?.tagline ?? 'The obituary for your plan — written before you execute it.'}</p>
        ...
        {!results && (
          <div className="p-5 space-y-4">
            ...
            <p className={`text-xs ${c.textMuted}`}>
              Still deciding? <a href="/DecisionCoach" className={linkStyle}>🎯 Decision Coach</a> frames the choice before you stress-test it.
            </p>
            ...
            <div>
              <label className={`block text-sm font-semibold mb-2 ${c.text}`}>
                Type <span className={`font-normal ${c.textMuted}`}>(optional)</span>
              </label>
              <div className="flex flex-wrap gap-2">
                {PLAN_TYPES.map(t => (
                  <button key={t.id} onClick={() => setPlanType(planType === t.id ? '' : t.id)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${
                      planType === t.id ? c.pillActive : c.pillInactive
                    }`}>
                    {t.label}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className={`block text-sm font-semibold mb-1.5 ${c.text}`}>
                  What's at stake <span className={`font-normal ${c.textMuted}`}>(optional)</span>
                </label>
              ...
              <div>
                <label className={`block text-sm font-semibold mb-1.5 ${c.text}`}>
                  Key assumptions you're making <span className={`font-normal ${c.textMuted}`}>(optional)</span>
                </label>
          ...
          <button onClick={handleSubmit} disabled={loading || !plan.trim()}
            ...
            {loading
              ? <><span className="inline-block animate-spin">{tool?.icon ?? '💀'}</span> Writing the failure…</>
              : <><span>{tool?.icon ?? '💀'}</span> Write the Pre-Mortem</>}
            </button>

            <p className={`text-xs text-center ${c.textMuted}`}>
              This doesn't mean your plan will fail. It means you'll know where to look.
            </p>
          </div>
        )}
      </div>
        ...
        <div className="space-y-4" ref={resultsRef}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {pm?.the_fatal_assumption && (
              <div className={`rounded-2xl border p-4 ${isDark ? 'bg-red-600/10 border-red-600/40' : 'bg-red-50 border-red-200'}`}>
                <p className={`text-xs font-bold uppercase tracking-wider mb-2 ${isDark ? 'text-red-400' : 'text-red-600'}`}>💣 Fatal Assumption</p>
          ...
          {results.the_most_likely && (
            <div className={`rounded-2xl border-2 p-5 ${isDark ? 'border-cyan-800 bg-cyan-800/10' : 'border-cyan-200 bg-cyan-50'}`}>
              <p className={`text-xs font-black uppercase tracking-widest mb-3 ${isDark ? 'text-cyan-400' : 'text-cyan-800'}`}>🎯 Most Likely · One Prevention</p>
              ...
              <div className={`mt-3 p-3 rounded-xl ${isDark ? 'bg-zinc-900/60' : 'bg-white/80'}`}>
                <p className={`text-xs font-bold uppercase tracking-wide mb-1 ${c.textMuted}`}>✅ The Prevention</p>
          ...
          {results.the_one_thing && (
            <div className={`${c.cardAlt} border ${c.border} rounded-2xl p-5`}>
              <p className={`text-xs font-black uppercase tracking-widest mb-2 ${c.accentTxt}`}>⚡ The One Thing — Before You Launch</p>
          ...
          <div className={`${c.cardAlt} border ${c.border} rounded-xl p-4`}>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <a href="/EgoKiller"      className={`text-xs ${linkStyle}`}>🪦 Ego Killer</a>
