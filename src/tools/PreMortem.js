...
const PreMortem = ({ tool }) => {
  ...
  const loadExample = useCallback(() => {
    const ex = pickExample('PreMortem', EXAMPLES);
    setPlan(ex.plan);
    setPlanType(ex.planType);
    setStakes(ex.stakes);
    ...
    <div className={`space-y-4 ${c.text}`}>
      ...
      <div className={`${c.card} border ${c.border} rounded-xl shadow-sm overflow-hidden`}>
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className={`text-base ${c.textSecondary}`}>
                  <span className="me-2 text-lg">{tool?.icon ?? '💀'}</span>{tool?.tagline ?? t('pm_tagline')}
                </p>
        ...
        {!results && (
          ...
          <button title={t('cmd_enter')} onClick={handleSubmit} disabled={loading || !plan.trim()}
            className={`relative w-full ${(!plan.trim()) ? c.btnIdle : c.btnPrimary} font-bold py-3 rounded-xl flex items-center justify-center gap-2 min-h-[48px]`}>
            {loading
