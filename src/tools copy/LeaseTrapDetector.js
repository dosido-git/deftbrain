...
import { Home, AlertTriangle, Shield, FileText, Upload, Loader2, CheckCircle, AlertCircle, Info, X, RefreshCw, Scale, MessageSquare, Building, ChevronDown, ChevronUp, ChevronLeft, DollarSign, Gavel, Users } from 'lucide-react';
...
const LeaseTrapDetector = () => {
  ...
  // Theme colors
  const c = {
    gradient: isDark ? 'from-orange-900/20 to-red-900/20' : 'from-orange-50 to-red-50',
    card: isDark ? 'bg-gray-800' : 'bg-white',
    cardAlt: isDark ? 'bg-gray-700/50' : 'bg-orange-50/50',
  ...
  // Handle file upload — read PDF as base64 on the client
  ...
  const getResourceBadge = (type) => {
    if (!type) return { color: isDark ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600' };
    ...
    return { color: isDark ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600' };
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            <Home className="w-12 h-12 text-orange-600 dark:text-orange-400" />
            <h1 className={`text-4xl font-bold ${c.text}`}>Lease Trap Detector</h1>
          </div>
          <p className={`text-lg ${c.textSecondary}`}>
            Analyze rental agreements for predatory clauses and illegal provisions
          </p>
        </div>
          ...
          <div className="space-y-6">
                  ...
                  {!uploadedFile ? (
                    <div
                      ...
                      className={`border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition-all ${
                        isDark ? 'border-gray-600 hover:border-orange-500 hover:bg-gray-700/50' : 'border-gray-300 hover:border-orange-400 hover:bg-orange-50'
                      }`}
                    >
                    ...
                    <div className={`flex items-center justify-between p-4 rounded-xl border ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-green-50 border-green-200'}`}>
            ...
            <button
              ...
              className={`w-full py-4 px-6 rounded-xl font-semibold text-lg transition-all flex items-center justify-center gap-2 shadow-lg ${
                loading ? (isDark ? 'bg-gray-700 text-gray-400' : 'bg-gray-200 text-gray-400') : c.btnPrimary
              }`}
            >
              {loading ? (
            ...
            <div className={`p-4 rounded-xl text-center ${isDark ? 'bg-gray-800/50' : 'bg-gray-50'}`}>
              <p className={`text-xs ${c.textMuted}`}>
                ⚖️ This tool provides general guidance, not legal advice. Consult a local tenant rights attorney for specific legal questions.
        ...
        {results && (
          <div className="space-y-6">
            ...
            <div className={`${c.card} rounded-xl shadow-lg p-4 flex items-center justify-between`}>
              <button
                onClick={reset}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${isDark ? 'text-orange-400 hover:bg-gray-700' : 'text-orange-600 hover:bg-orange-50'}`}
              >
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h2 className={`text-2xl font-bold ${c.text} mb-1`}>
                      {results.overall_assessment.risk_level === 'high' ? '🚨 High Risk' :
                       results.overall_assessment.risk_level === 'medium' ? '⚠️ Medium Risk' : '✅ Low Risk'}
                    </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div className={`p-4 rounded-lg ${results.security_deposit_analysis.is_over_limit
                    ...
                    {results.security_deposit_analysis.is_over_limit && (
                      <p className="text-xs font-bold text-red-600 dark:text-red-400 mt-1">⚠️ OVER LEGAL LIMIT</p>
                    )}
                  </div>
                </div>
                ...
                {results.security_deposit_analysis.issues_found && results.security_deposit_analysis.issues_found.length > 0 && (
                  <div className={`p-3 rounded-lg ${isDark ? 'bg-red-900/20 border border-red-800' : 'bg-red-50 border border-red-200'}`}>
                    ...
                    {results.security_deposit_analysis.issues_found.map((issue, i) => (
                      <p key={i} className={`text-sm ${isDark ? 'text-red-300' : 'text-red-800'} mb-1`}>• {issue}</p>
                    ))}
                  </div>
                )}
              </div>
                    ...
                    <div key={idx} className="border-l-4 border-red-500 pl-4">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-start justify-between">
                          <p className={`font-bold text-red-700 dark:text-red-400 mb-1`}>⚠️ {flag.concern}</p>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {flag.what_lease_says && (
                              <div className={`p-3 rounded-lg ${isDark ? 'bg-red-900/20' : 'bg-red-50'}`}>
                                <p className={`text-xs font-bold text-red-700 dark:text-red-400 mb-1`}>📄 LEASE SAYS:</p>
                            ...
                            {flag.what_law_says && (
                              <div className={`p-3 rounded-lg ${isDark ? 'bg-green-900/20' : 'bg-green-50'}`}>
                                <p className={`text-xs font-bold text-green-700 dark:text-green-400 mb-1`}>⚖️ LAW SAYS:</p>
                        ...
                        {flag.landlord_likely_response && (
                          <div className={`p-3 rounded-lg ${isDark ? 'bg-amber-900/20' : 'bg-amber-50'}`}>
                            <p className={`text-xs font-bold text-amber-700 dark:text-amber-400 mb-1`}>🏠 LANDLORD WILL LIKELY SAY:</p>
                        ...
                        {flag.negotiation_script && (
                          <div className={`p-3 rounded-lg ${isDark ? 'bg-green-900/20' : 'bg-green-50'}`}>
                            <p className={`text-xs font-bold text-green-700 dark:text-green-400 mb-1`}>💬 YOUR SCRIPT:</p>
                ...
                <div className="space-y-4">
                  {results.yellow_flags.map((flag, idx) => (
                    <div key={idx} className="border-l-4 border-yellow-500 pl-4">
                      ...
                      <div className={`p-3 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'} mb-2`}>
                        <p className={`text-xs font-bold ${c.textMuted} mb-2`}>QUESTIONS TO ASK:</p>
                        {flag.questions_to_ask?.map((q, qidx) => (
                          <p key={qidx} className={`text-sm ${c.textSecondary} mb-1`}>• {q}</p>
                        ))}
                      </div>
                      {flag.landlord_likely_response && (
                        <div className={`p-3 rounded-lg ${isDark ? 'bg-amber-900/20' : 'bg-amber-50'}`}>
                          <p className={`text-xs font-bold text-amber-700 dark:text-amber-400 mb-1`}>🏠 EXPECTED RESPONSE:</p>
                          <p className={`text-sm ${c.textSecondary} italic`}>{flag.landlord_likely_response}</p>
                        </div>
                      )}
                      {flag.what_to_watch_for && (
                        <p className={`text-xs mt-2 ${c.textMuted}`}>👁️ Watch for: {flag.what_to_watch_for}</p>
                      )}
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  {results.green_flags.map((flag, idx) => (
                    <div key={idx} className={`${c.cardAlt} rounded-lg p-4 border-l-4 border-green-500`}>
                      ...
                      <p className={`text-sm ${c.text}`}>✓ {flag.why_good}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {results.missing_disclosures && results.missing_disclosures.length > 0 && (
              <div className={`${c.card} rounded-xl shadow-lg p-6`}>
                <h2 className={`text-2xl font-bold ${c.text} mb-4`}>⚠️ Missing Required Disclosures</h2>
                <div className="space-y-3">
              ...
              <div className={`${c.card} rounded-xl shadow-lg p-6`}>
                ...
                <div className="space-y-3">
                  {results.unusual_fees.map((fee, idx) => (
                    <div key={idx} className={`${c.cardAlt} rounded-lg p-4`}>
                      {fee.lease_reference && (
                        <span className={`inline-block px-2 py-0.5 rounded text-xs font-mono font-bold mb-2 ${isDark ? 'bg-gray-600 text-gray-300' : 'bg-gray-200 text-gray-700'}`}>
                          📌 {fee.lease_reference}
                        </span>
                      )}
                      <div className="flex items-start justify-between mb-2">
              ...
              <div className={`${c.card} rounded-xl shadow-lg p-6 border-l-4 border-blue-500`}>
                ...
                {results.negotiation_strategy.market_context && (
                  <div className={`p-3 rounded-lg mb-4 ${isDark ? 'bg-blue-900/20' : 'bg-blue-50'}`}>
                    <p className={`text-xs font-bold text-blue-700 dark:text-blue-400 mb-1`}>📊 MARKET CONTEXT:</p>
                ...
                {results.negotiation_strategy.leverage_points && results.negotiation_strategy.leverage_points.length > 0 && (
                  <div className={`${c.cardAlt} rounded-lg p-4 mb-4`}>
                    <p className={`text-xs font-bold ${c.textMuted} mb-2`}>💪 YOUR LEVERAGE POINTS:</p>
                    {results.negotiation_strategy.leverage_points.map((pt, idx) => (
                      <p key={idx} className={`text-sm ${c.textSecondary} mb-1`}>• {pt}</p>
                    ))}
                  </div>
                )}

                <div className={`${c.cardAlt} rounded-lg p-4 mb-4`}>
                  <p className={`text-xs font-bold ${c.textMuted} mb-2`}>📧 OPENING EMAIL:</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                  {results.negotiation_strategy.stand_firm_on && results.negotiation_strategy.stand_firm_on.length > 0 && (
                    <div className={`p-4 rounded-lg ${isDark ? 'bg-red-900/20 border border-red-800' : 'bg-red-50 border border-red-200'}`}>
                      <p className={`text-xs font-bold text-red-700 dark:text-red-400 mb-2`}>🛑 STAND FIRM ON:</p>
                      {results.negotiation_strategy.stand_firm_on.map((pt, idx) => (
                        <p key={idx} className={`text-sm ${isDark ? 'text-red-300' : 'text-red-800'} mb-1`}>• {pt}</p>
                      ))}
                    </div>
                  )}
                  {results.negotiation_strategy.compromise_positions && results.negotiation_strategy.compromise_positions.length > 0 && (
                    <div className={`p-4 rounded-lg ${isDark ? 'bg-green-900/20 border border-green-800' : 'bg-green-50 border border-green-200'}`}>
                      <p className={`text-xs font-bold text-green-700 dark:text-green-400 mb-2`}>🤝 OK TO COMPROMISE:</p>
                      {results.negotiation_strategy.compromise_positions.map((pt, idx) => (
                        <p key={idx} className={`text-sm ${isDark ? 'text-green-300' : 'text-green-800'} mb-1`}>• {pt}</p>
                      ))}
                    </div>
                  )}
                </div>
                    ...
                    {expandedSections.scripts && (
                      <div className="space-y-3">
                        {results.negotiation_strategy.if_they_say_scripts.map((script, idx) => (
                          <div key={idx} className={`rounded-lg overflow-hidden border ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
                            <div className={`p-3 ${isDark ? 'bg-red-900/20' : 'bg-red-50'}`}>
                              <p className={`text-xs font-bold text-red-700 dark:text-red-400 mb-1`}>🏠 IF LANDLORD SAYS:</p>
                              <p className={`text-sm ${c.textSecondary} italic`}>"{script.landlord_says}"</p>
                            </div>
                            <div className={`p-3 ${isDark ? 'bg-green-900/20' : 'bg-green-50'}`}>
                              <p className={`text-xs font-bold text-green-700 dark:text-green-400 mb-1`}>💬 YOU RESPOND:</p>
                ...
                <div className="space-y-3">
                  {results.resources.map((resource, idx) => (
                    <div key={idx} className={`${c.cardAlt} rounded-lg p-4`}>
                      ...
                      {resource.contact && <p className={`text-sm ${c.textMuted}`}>📞 {resource.contact}</p>}
