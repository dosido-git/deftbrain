...
export default function HomeIntro({ allTools=[], onBrowse, setSearchTerm }) {
  ...
  return <div className="w-full">
    ...
    <section className="my-7 rounded-2xl border overflow-hidden" style={{borderColor:BORDER,background:'linear-gradient(105deg,#fff0cf 0%,#f8ddd7 35%,#e9e1f5 68%,#d7ebf7 100%)'}}><div className="p-5 sm:p-6"><div className="flex items-end justify-between mb-4"><div><div className="text-[8px] uppercase tracking-[.16em] font-bold text-slate-600">Popular tools</div><h2 className="mt-1 text-[24px] font-bold" style={{fontFamily:SERIF,color:NAVY}}>Good places to start.</h2></div><button onClick={onBrowse} className="text-[10px] font-semibold underline underline-offset-4" style={{color:NAVY}}>See all tools →</button></div><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2.5">{POPULAR.map(id=><PopularCard key={id} tool={byId.get(id)}/>)}</div></div></section>
