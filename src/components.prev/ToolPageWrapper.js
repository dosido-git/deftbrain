      ...
      <div className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <main className="lg:col-span-8 space-y-6">
          <header className="space-y-2">
            <div className="flex items-center justify-between mb-4">
              <button 
                onClick={() => navigate('/')}
                className={`flex items-center gap-2 ${colors.textSecondary} ${colors.hoverAccent} transition-colors group`}
              >
                <ChevronLeft className="h-4 w-4 group-hover:-translate-x-1 transition-transform" />
                <span className="text-xs font-semibold uppercase tracking-wide">Back to Dashboard</span>
              </button>
              
              {/* Theme Toggle Button */}
              <button
                onClick={toggleTheme}
                className={`flex items-center gap-2 px-4 py-2 rounded-full ${colors.toggleBg} ${colors.toggleText} transition-all font-medium text-sm`}
        ...
        <aside className="lg:col-span-4 space-y-6">
          <div className={`${colors.surfaceAlt} border ${colors.border} rounded-2xl p-6 sticky top-8 max-h-[calc(100vh-4rem)] overflow-y-auto transition-colors duration-200`}>
            ...
            <div className={`flex items-center justify-center gap-2 mb-6 pb-4 border-b ${colors.border}`}>
