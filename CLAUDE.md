...
5. **Three-way sync.** A tool's frontend tab, backend route, and output renderer must stay in sync — change one, check the other two. Gate 8 (`scripts/three-way-sync-audit.js`) now enforces half of this: it fails on a call to an endpoint no route serves, and on a NEW field sent to a route that never reads it. The renderer half is still by eye.
...
**Generated per-language bundles.** The runtime does not import the per-tool
files any more — `scripts/build-locales.js` pivots them into one flat file per
language under `src/i18n/locales/generated/` (gitignored, rebuilt by `prebuild`
and `predev:frontend`), and only English ships in the main bundle. **After
editing any locale file mid-session, run `npm run build:locales`** or the new
keys render as their own names until the dev server restarts.

**Catalog structure:** base chrome keys live in the per-language objects; each fully-localized tool adds its own namespaced block (e.g. `const md = { en: {...}, es: {...}, ... }`) merged into `RESOURCES` via spread. **Reference implementation:** `src/tools/MarkupDetective.js` + its `mkd_*` block — copy that pattern for new tools. **The prefix must be unique across the whole catalog** — every tool's block spreads into one flat namespace per language, so two tools sharing a prefix means one silently renders the other's string for any key they both define. Gate 5 now fails on a collision. (The original reference, SubscriptionGuiltTrip, was retired/merged into SubSweep on 2026-07-06.) The `t()` interpolation syntax is `{{var}}`, e.g. `t('mkd_ph_price', { sym })`. Gate 5 (`scripts/localization-audit.js`) verifies a tool is fully localized; add a finished tool to its `LOCALIZED_TOOLS` allowlist.
...
guide, because `.claude/` is gitignored — a rule that only exists on one
