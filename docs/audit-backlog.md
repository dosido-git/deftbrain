...
- [ ] `audit_v2-3-2.py` PF-16 regex matches only literal `handleReset` — misses tools using `reset`, `handleClear`, `handleNew`, etc. Discovered in LeaseTrapDetector where `reset` function escaped detection entirely. **Re-confirmed in NameAudit (Session ~110) — two `<button onClick={reset}>` instances in results blocks slipped past the script entirely; only caught by manual read-through.** Bump to next-script-revision priority. Fix: broaden to `/on(?:Click|Reset)=\{(?:handleReset|reset|handleClear|handleNew)\}/` plus a `setResults(null)` heuristic.
- [ ] `audit_v2-3-2.py` S1.1i false-positive on dot-notation substrings — `uc.explanation` substring-matched `c.explanation` and flagged a nonexistent key. Fix: add negative lookbehind to ensure the `c` isn't part of a longer identifier (e.g., `(?<![a-zA-Z_])c\.`).
- [ ] `audit_v2-3-2.py` PF-2 ordering check — currently assumes c block comes before linkStyle. When order is reversed (as in LeaseTrapDetector pre-fix), the `jsx_area` slice cuts mid-c-block and spurious "hardcoded in JSX" flags fire for legitimate c-block values. Fix: explicitly detect `const c =` and `const linkStyle =` positions; fail early with a clear message if they're in the wrong order instead of letting it cascade into misleading downstream flags.
- [ ] `audit_v2-3-2.py` PF-15 dot-nested fallback line numbers — when path is dot-nested (e.g. `!form.field.trim()`), the script bails out of line tracking and reports the disabled-expression line as a uniform line number for all violations in that file. NameAudit (Session ~110) collapsed three distinct PF-15 violations onto "line 1060" — the actual labels were elsewhere. Fix: when dot-nested, fall back to reporting the line of the matching `value={path}` input rather than the disabled-expression line.
- [ ] `audit_v2-3-2.py` PF-3 nested-card scope — script only checks the first/header `c.card` for `border ${c.border} rounded-xl shadow-sm`. Misses nested `${c.card} rounded-xl shadow-lg` (no border, wrong shadow) deeper in the results tree. NameAudit had 15+ instances. Fix: count all `${c.card} rounded-xl shadow-lg` patterns in the file and flag any beyond the header card. Likely catalog-wide aftermath of an earlier shadow-lg → shadow-sm sweep that only touched header cards.
...
- [ ] PF-15 secondary-button required fields missing asterisks — surfaced by patched audit script after its fixes landed. Five fields have `!variable.trim()` disabled guards but no `c.required` asterisk on their labels:
  ...
  - `practiceInput` (Practice mode: chat-style send button) — see Bucket 3
  - `note` (Repairs mode: inline follow-up quick-add) — see Bucket 3
...
**Backend (name-audit.js) — open / candidate exception:**
- [ ] **Architectural deviation:** all 6 routes use `callClaudeWithRetry` instead of the project-memory backend convention (`anthropic.messages.create` → `cleanJsonResponse` → `JSON.parse`). The retry wrapper has real value (transient failure handling) and is exported from the same `lib/claude` module. Resolution path: either (a) update project memory to sanction `callClaudeWithRetry` as an approved alternative for routes that benefit from retry semantics, or (b) refactor all 6 routes to the canonical pattern. Bruce's design call. See Bucket 3.
...
- [ ] **Catalog-wide rate-limiting audit** — `grep -L "rateLimit()" backend/routes/*.js` will list every route file with zero rate-limit calls. NameAudit backend (Session ~110) had zero rate limiting across 6 routes — `rateLimiter` wasn't even imported. If NameAudit slipped through, others probably did too. **Highest-priority sweep before launch.**
...
- [ ] ApologyCalibrator `practiceInput` missing asterisk — **chat-style send button**. In chat UIs, the message input isn't traditionally marked as required with an asterisk even when the send button is disabled while empty. Design call: amend PF-15 to carve out a chat-input exception, or add the asterisk for strict consistency. If exception: document in CONVENTIONS.md under PF-15 as "chat-style inputs exempt."
- [ ] ApologyCalibrator `note` missing asterisk — **inline quick-add input** in a repairs list. Same shape as chat: disabled when empty, visually lightweight, not a primary form field. Likely covered by the same exception as `practiceInput` if that's the call.
- [ ] BatchFlow `dumpText` missing label — if the paste-list-as-`<p>`-heading pattern is approved as a valid alternative to `<label>`, document it. Otherwise resolves as the Bucket 2 fix above.
- [ ] **`callClaudeWithRetry` vs `anthropic.messages.create` (NameAudit backend, Session ~110)** — project memory states the canonical backend pattern is `anthropic.messages.create` → `cleanJsonResponse` → `JSON.parse`, with explicit "(not `callClaudeWithRetry`)" guidance. NameAudit backend uses `callClaudeWithRetry` exclusively across all 6 routes. The wrapper provides transient-failure retry semantics that the canonical pattern lacks. Two resolution paths: (a) sanction `callClaudeWithRetry` as the preferred pattern for routes where transient retries matter (rename project-memory rule to "use `callClaudeWithRetry` for production routes; raw `anthropic.messages.create` only when bypass needed"), or (b) refactor NameAudit's 6 routes to canonical pattern and lose the retry semantics. Cost of (b) is real; benefit unclear. Recommend (a) and document in `backend-audit-section7.md` as the preferred S7.4 pattern.
- [ ] PF-14 single-mode vs multi-mode — already resolved (multi-mode exception added in CONVENTIONS.md with ApologyCalibrator as reference case). Left here as closed record.
  - [-] Closed as exception — documented in CONVENTIONS.md PF-14.
...
(Empty — items move here when checked off and retained as record. Periodic prune: delete entries older than 60 days, keep decision records forever.)
