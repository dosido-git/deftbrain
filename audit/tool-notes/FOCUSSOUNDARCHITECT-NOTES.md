...
## 2026-09-16 (later still) — fsa_intro reworded, all 13 locales

Owner reworded the intro line (`fsa_intro`): "Tell me... I will build..."
(first person) → "Tell us... DeftBrain will build..." (brand-voice, third
person). Translated all 13 languages by hand rather than leaving English
change and letting the other twelve go stale — same trap
REWRITE-INSTALL-KIT.md §3 warns about ("Keys whose ENGLISH changed leave
the other twelve holding a translation of the old wording"). "DeftBrain"
kept untranslated (proper noun) in every language, matching the existing
"Generated by DeftBrain" branding convention. Ran `npm run build:locales`
after (source file isn't what the dev server reads). Verified zero
duplicate keys per language block, localization-audit and
i18n-convention-audit both clean, live-checked English and Spanish render.
