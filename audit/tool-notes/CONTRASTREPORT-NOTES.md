...
Two-path life-decision contrast: frames the decision, narrates a vivid "day in each life" (path_a / path_b with best moment + honest cost), then what-you're-trading reflection. **Display name "Which Life?"** (renamed from What If?); **route stays `/contrast-report`**, frontend file `src/tools/WhichLife.js`. **Backend:** `backend/routes/which-life.js` (2 endpoints). **Golden:** `audit/contrast-report-golden-sample.json` (2 cases). Verify: `npm run check:golden contrast-report` (~28–35s/case).
