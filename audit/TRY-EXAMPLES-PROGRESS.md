...
| 106 | TheCrux (i18n file: recall.js) | i18n | 4 (distill/study_guide/test_prep/connect — example only demos distill, the shared-transcript entry point; established precedent, not restructured) | 2 lecture transcripts | 5 (+ Computer Science: Big-O complexity, Economics: sunk cost/opportunity cost, Art History: Dutch Golden Age painting — full lecture-length transcripts, 9 keys ×13) | ✅ 2026-09-11 |
| 107 | TheDebrief | i18n | 8 meeting types × 3 tones | 2 (both auto/professional) | 5 (+ retro/casual sprint-delay retro, one_on_one/formal career-growth conversation, client/professional tense renewal call; loadExample now varies meetingType and tone per example, not just content; 9 keys ×13; fixed 10 i18n-convention findings — ja pronoun, ko 당신, hi तुम-level, ru ты, fr tutoiement, th particle, de formal-address) | ✅ 2026-09-11 |
| 108 | TheFinalWord | inline | 4 (question/dispute/factcheck/trivia — trivia excluded, it's a scored quiz flow, not a text-input example) | 2 (dispute only) | 5 per mode across question/dispute/factcheck (15 total): restructured EXAMPLES from a flat dispute-only list into per-mode arrays and loadExample to branch by mode, so 'try example' now actually demonstrates all 3 free-text modes instead of only ever loading a dispute. Verified in-browser across all 3 modes. | ✅ 2026-09-11 |
